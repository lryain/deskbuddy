...
namespace {
  constexpr Lrya::AudioMetaData::GameObjectType kSdkGameObject = Lrya::AudioMetaData::GameObjectType::TextToSpeech;
  constexpr Lrya::AudioEngine::PlugIns::StreamingWavePortalPlugIn::PluginId_t kSdkPluginId = 100;
}
...
{
  if (!LRYA_VERIFY(volume <= 1.0f && volume >= 0.0f, 
...
bool SdkAudioComponent::PrepareAudioEngine(const RobotInterface::ExternalAudioPrepare& msg )
{
  ...
  if (!LRYA_VERIFY(msg.audio_rate >= 8000 && msg.audio_rate <= 16025,
...
bool SdkAudioComponent::AddAudioChunk(const RobotInterface::ExternalAudioChunk& msg) {
  ...
  Lrya::AudioEngine::StandardWaveDataContainer waveContainer(_audioRate, 1, wave_data_size); 
