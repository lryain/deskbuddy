/**
 * File: textToSpeechProvider_mateos.cpp
 *
...
// TODO: Fix this, find another tts provider
// #include "ic_babile.h"
// #include "dblsman.ch"
// #include "error.ch"
...
Result TextToSpeechProviderImpl::Initialize(const std::string & locale)
{
  ...
  std::string language = Lrya::Util::Locale::LocaleFromString(locale).GetLanguageString();
  if (language.empty()) {
...
//   const char * defaultText = NULL;
  LOG_WARNING("TextToSpeechProvider.Initialize.LoadIniFile", "Load ini file %s", iniFile.c_str());
//   BB_S32 synthAvail = 0;
//   short synthModule = 0;
//   short nlpModule = 0;
//   BB_DbLs * nlpeLS = NULL;
                                                   ...
                                                   bool & done)
{
  LOG_INFO("TextToSpeechProvider.GetFirstAudioData", "text=%s duration=%.2f pitch=%.2f",
            Lrya::Util::HidePersonallyIdentifiableInfo(text.c_str()),
            durationScalar,
            pitchScalar);
...
//   }
  ...
  const auto speed = Lrya::Util::numeric_cast<int>(std::round(adjustedSpeed));
  ...
  const auto pitch = Lrya::Util::numeric_cast<int>(std::round(adjustedPitch));

  // Get shaping
  const auto shaping = _tts_config->GetShaping();
  LOG_WARNING("TextToSpeechProvider.GetFirstAudioData", "speed: %d pitch: %d shaping: %d", speed, pitch, shaping);
