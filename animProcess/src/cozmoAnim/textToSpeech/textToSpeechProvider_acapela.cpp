...
float AcapelaTTS::GetSpeechRate(int speed, float durationScalar)
{
  ...
  durationScalar = Lrya::Util::Clamp(durationScalar, kDurationScalarMin, kDurationScalarMax);

  // Adjust base rate (100%) by duration scalar
  const float speechRate = speed / durationScalar;

  // Clamp adjusted rate to allowable range
  return Lrya::Util::Clamp(speechRate, kSpeechRateMin, kSpeechRateMax);
...
float AcapelaTTS::GetAdjustedPitch(int basePitch, float pitchScalar)
{
  ...
  pitchScalar = Lrya::Util::Clamp(pitchScalar, kPitchScalarMin, kPitchScalarMax);
  ...
  return Lrya::Util::Clamp(pitch, kPitchMin, kPitchMax);
}
