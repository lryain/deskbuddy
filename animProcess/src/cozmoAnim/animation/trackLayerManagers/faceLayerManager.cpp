...
// Only used for setting console var ranges below, so not used in shipping builds (thus LRYA_DEV_CHEATS)
...
int GetNumEyeDartInterpFrames(const Point2f& dartVector)
{
  ...
  if(LRYA_DEV_CHEATS && (kKeepAliveEyeDart_MediumDistanceThresh_pix >= kKeepAliveEyeDart_LongDistanceThresh_pix))
  {
