...
void AlexaImpl::InitThread()
{
  ...
  if( LRYA_VERIFY(_settingsCallbacks != nullptr, "AlexaImpl.InitThead.NoCallbacks","Could not create settings callbacks") ) {
  ...
  if( LRYA_DEV_CHEATS && kDumpAlexaTriggerAudio ) {
...
void AlexaImpl::AttemptToFixStuckInSpeakingBug()
{
  ...
  const bool recovered = LRYA_VERIFY(_uxState == AlexaUXState::Idle,
...
{
  if( LRYA_VERIFY( _client != nullptr,
...
void AlexaImpl::OnSourcePlaybackChange( SourceId id, bool playing )
{
  ...
  if( playing ) {
    ...
    if( LRYA_VERIFY( res.second, "AlexaImpl.OnSourcePlaybackChange.Exists", "Source %llu was already playing", id ) ) {
      CheckForUXStateChange();
    }
  } else {
  ...
  if( _client != nullptr ) {
    if( !_isTapOccurring ) {
      ...
      if( !_client->IsAVSConnected() ) {
        ...
        if( !_alertActive ) {
          LRYA_VERIFY( _client->NotifyOfTapToTalk( *_tapToTalkAudioProvider ).get(),
