 ...
 *              DefaultClient that works with other Lrya analogs like speakers/mics.
 *
 *
                        ...
                        avsCommon::sdkInterfaces::softwareInfo::FirmwareVersion firmwareVersion)
{
  ...
  // VIC-11806: notifications cleared when deskmate was turned off never recieve the SetIndicator(OFF) callback so
...
{
  if( LRYA_VERIFY(_connectionManager != nullptr, "AlexaClient.AddConnectionObserver.Null", "Connection manager is null") ) {
...
{
  if( LRYA_VERIFY(_connectionManager != nullptr, "AlexaClient.RemoveConnectionObserver.Null", "Connection manager is null") ) {
                                             ...
                                             CapabilitiesObserverInterface::Error newError )
{
  if( CapabilitiesObserverInterface::State::SUCCESS == newState ) {
    if( LRYA_VERIFY(_connectionManager != nullptr,
...
{
  if( LRYA_VERIFY(_audioFocusManager != nullptr,
...
{
  if( LRYA_VERIFY(_alertsCapabilityAgent != nullptr,
...
{
  if( LRYA_VERIFY(_speakerManager != nullptr,
...
{
  if( LRYA_VERIFY(_dialogUXStateAggregator != nullptr,
...
{
  if( LRYA_VERIFY(_internetConnectionMonitor != nullptr,
...
{
  if( LRYA_VERIFY(_notificationsCapabilityAgent != nullptr,
...
{
  if( LRYA_VERIFY( _audioPlayer != nullptr,
