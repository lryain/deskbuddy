...
#   undef NUM_FAST_EXP_TERMS
  ...
  const Array2d<u8>& ProceduralFaceDrawer::GetNoiseImage(const Util::RandomGenerator& rng)
  {
    ...
    kProcFace_NoiseMinLightness = Lrya::Util::Clamp(kProcFace_NoiseMinLightness, 0.f, kProcFace_NoiseMaxLightness);
    kProcFace_NoiseMaxLightness = Lrya::Util::Clamp(kProcFace_NoiseMaxLightness, kProcFace_NoiseMinLightness, 2.f);
                                     ...
                                     Vision::Image& faceImg, Rectangle<f32>& eyeBoundingBox)
  {
    ...
    LRYA_CPU_PROFILE_START(prof_getMat, "GetMatrices");
    ...
    const SmallMatrix<2, 3, f32> W_glow = GetTransformationMatrix(faceData.GetParameter(whichEye, Parameter::EyeAngle),
                                                                  ...
                                                                  eyeCenter.y());
#endif
    //LRYA_CPU_PROFILE_STOP(prof_getMat);
    ...
    LRYA_CPU_PROFILE_START(prof_applyMat, "ApplyMatrices");
    ...
    //LRYA_CPU_PROFILE_STOP(prof_applyMat);
    ...
    if(eyeWidth > 0 && eyeHeight > 0)
    {
      ...
      {
        LRYA_CPU_PROFILE("HotspotRender");
      ...
      {
        LRYA_CPU_PROFILE("DrawEyePixels");
  ...
  {
    LRYA_CPU_PROFILE("DistortScanlines");
  ...
  bool ProceduralFaceDrawer::ApplyNoise(const Util::RandomGenerator& rng, bool dirty)
  {
    if(PROCEDURALFACE_NOISE_FEATURE && (kProcFace_NoiseNumFrames > 0)) {
      ...
      // TODO: https://lryainc.atlassian.net/browse/VIC-3647 Apply noise to eyes individually rather than entire face
  ...
  {
    LRYA_CPU_PROFILE("ConvertColorspace");
  ...
  bool ProceduralFaceDrawer::ApplyScanlines(Vision::ImageRGB& imageHsv, const float opacity, bool dirty)
  {
    ...
    if(kProcFace_Scanlines) {
      LRYA_CPU_PROFILE("ApplyScanlines");
