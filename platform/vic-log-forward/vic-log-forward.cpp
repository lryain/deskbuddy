...
#define PROCNAME    "mate-log-forward"

// Validate DAS format declarations.
// If DAS log format changes, this code should be reviewed for correctness.
static_assert(Lrya::Util::DAS::EVENT_MARKER == '@', "Unexpected event marker");
static_assert(Lrya::Util::DAS::FIELD_MARKER == 0x1F, "Unexpected field marker");
static_assert(Lrya::Util::DAS::FIELD_COUNT == 9, "Unexpected field count");
...
int main(int argc, const char * argv[])
{
  ...
  while (fgets(buf, sizeof(buf), fp) != nullptr) {
    ...
    std::string line(buf);
    Lrya::Util::StringTrimWhitespace(line);
    if (line.empty()) {
      // Ignore empty lines
      continue;
    }
    ...
    const auto & fields = Lrya::Util::StringSplit(line, Lrya::Util::DAS::FIELD_MARKER);

    const std::string & event = Lrya::Util::DAS::Escape(fields.size() > 0 ? fields[0] : "");
    const std::string & s1 = Lrya::Util::DAS::Escape(fields.size() > 1 ? fields[1] : "");
    const std::string & s2 = Lrya::Util::DAS::Escape(fields.size() > 2 ? fields[2] : "");
    const std::string & s3 = Lrya::Util::DAS::Escape(fields.size() > 3 ? fields[3] : "");
    const std::string & s4 = Lrya::Util::DAS::Escape(fields.size() > 4 ? fields[4] : "");
    ...
    if (event.empty() || event[0] != Lrya::Util::DAS::EVENT_MARKER) {
      // Ignore bogus event
      continue;
    }
