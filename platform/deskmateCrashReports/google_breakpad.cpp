...
#if (defined(MATEOS) && defined(USE_GOOGLE_BREAKPAD))
...
constexpr const char* kRobotVersionFile = "/lrya/etc/version";
...
{
  static const std::string dump_directory = "/data/data/com.lrya.deskmate/cache/crashDumps";
...
{
  // If activation socket exists, activate lrya-crash-log.service.
  // This allows unprivileged processes (mate-cloud, mate-gateway)
  // to fetch log messages with reading /var/log/messages directly.
  //
  // Lrya-crash-log.service is only available on developer builds.
  // Crash reports from a production build will not include log messages.
  //
  static const char * socket = "/run/lrya-crash-log";
  if (!Lrya::Util::FileUtils::PathExists(socket)) {
...
void InstallGoogleBreakpad(const char* filenamePrefix)
{
  ...
  Lrya::Util::FileUtils::CreateDirectory(dump_directory);
