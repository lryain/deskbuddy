...
* Date:   02/27/2019
*
* Description: Entry point for mate-rescue. This program establishes
*              connection with lryabluetoothd and enters pairing
...
using namespace Lrya::Switchboard;
...
void RescueDaemon::SetAdvertisement()
{
  ...
  mdata.push_back(Lrya::kVictorProductIdentifier); // distinguish from future Lrya products
  ...
  _taskExecutor->Wake([rc, this] {
    ...
    if(rc != 0) {
      if(_securePairing == nullptr) {
        ...
        _rescueEngineClient->ShowPairingStatus(Lrya::Vector::SwitchboardInterface::ConnectionStatus::END_PAIRING);
      }
    }
  });
}
...
{
  Log::Write("Disconnected from lryabluetoothd.");
}

// ============================================================================================

void RescueDaemon::OnPairingStatus(Lrya::Vector::ExternalInterface::MessageEngineToGame message)
{
  ...
  switch(tag)
  {
    case Lrya::Vector::ExternalInterface::MessageEngineToGameTag::EnterPairing:
    {
      _rescueEngineClient->ShowPairingStatus(
        Lrya::Vector::SwitchboardInterface::ConnectionStatus::SHOW_PRE_PIN
      );
      break;
    }
...
void RescueDaemon::OnReceivedPin(std::string pin)
{
  ...
  _rescueEngineClient->ShowPairingStatus(
    Lrya::Vector::SwitchboardInterface::ConnectionStatus::SHOW_PIN
  );
  Log::Blue((" " + pin + " ").c_str());
}
...
{ 
  ...
  static std::unique_ptr<Lrya::Switchboard::RescueDaemon> sDaemon;
...
bool parse_args(int argc, char** argv, int* faultCode, int* timeout)
{
  ...
  while((opt = getopt(argc, argv, "ht:c:")) != -1)
  {
    switch(opt)
    {
      case 'h':
        printf("Options:\n");
        printf("  -t N   [N is positive integer] Set the timeout (sec) for mate-rescue to wait for BLE connection before exiting.\n\n");
        printf("  -c N   [N is positive integer] Set the fault code to draw on screen while mate-rescue is connected over BLE.\n\n");
...
int main(int argc, char** argv)
{
  ...
  setAndroidLoggingTag("mate-rescue");
  Log::Write("Loading up mate-rescue");
  ...
  DASMSG(rescue_hello, "mate-rescue.hello", "mate-rescue started");
  DASMSG_SEND();
  ...
  // initialize daemon
  sDaemon = std::make_unique<Lrya::Switchboard::RescueDaemon>(sLoop, fault, timeout_s);
  sDaemon->Start();
