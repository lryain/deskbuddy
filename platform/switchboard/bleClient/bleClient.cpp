 ...
 * Created: 2/20/2018
 *
 * Description: ble Client for lryabluetoothd
 *
...
void BleClient::OnReceiveMessage(const int connection_id,
  ...
  if(characteristic_uuid == kAppWriteCharacteristicUUID) {
    ...
    ((Lrya::Switchboard::INetworkStream*)_stream)->ReceivePlainText((uint8_t*)&value[0], value.size());
  } else if(characteristic_uuid == kAppWriteCharacteristicUUID) {
    // We are receiving input to read stream
    ((Lrya::Switchboard::INetworkStream*)_stream)->ReceiveEncrypted((uint8_t*)&value[0], value.size());
  }
}
