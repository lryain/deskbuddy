...
#include "lrya-ble/common/log.h"
#include "lrya-ble/common/lrya_ble_uuids.h"
#include "lrya-ble/common/ble_advertise_settings.h"
#include "lrya-wifi/wifi.h"
#include "lrya-wifi/exec_command.h"
...
#define LOG_PROCNAME "mate-switchboard"
...
void Daemon::Start() {
  setAndroidLoggingTag("mate-switchboard");
  Log::Write("Loading up Switchboard Daemon");

  _loop = ev_default_loop(0);

  _taskExecutor = std::make_shared<Lrya::TaskExecutor>(_loop);
  ...
  ev_timer_init(&_lryabtdTimer, HandleLryabtdTimer, kRetryInterval_s, kRetryInterval_s);
  ...
  Lrya::Wifi::Initialize(_taskExecutor);
  _wifiWatcher = std::make_shared<WifiWatcher>(_loop);
  _wifiChangedHandle = Lrya::Wifi::GetWifiChangedSignal().ScopedSubscribe(std::bind(&Daemon::OnWifiChanged, this, std::placeholders::_1, std::placeholders::_2));
...
void Daemon::Stop() {
  ...
  if(_engineMessagingClient != nullptr) {
    ...
    _engineMessagingClient->ShowPairingStatus(Lrya::Vector::SwitchboardInterface::ConnectionStatus::END_PAIRING);
  }
...
void Daemon::LogWifiState() {
  ...
  bool connected = (wifiState.connState == Lrya::Wifi::WiFiConnState::CONNECTED) ||
  ...
  uint8_t apMac[6];
  bool hasMac = Lrya::Wifi::GetApMacAddress(apMac);
...
bool Daemon::IsVectorConnectedToEscapePod() {
  std::string jsonContents = Lrya::Util::FileUtils::ReadFile(kServerConfigFilePath);
  Json::Reader reader;
  Json::Value config;
...
bool Daemon::TryConnectToLryaBluetoothDaemon() {
  ...
  if(_bleClient->IsConnected()) {
    ...
    if(++_connectionFailureCounter >= kFailureCountToLog) {
      Log::Write("Failed to connect to lryabluetoothd ... trying again.");
      _connectionFailureCounter = 0;
    }
  }
...
bool Daemon::TryConnectToTokenServer() {
  ...
  if (connected) {
    ...
    (void)_tokenClient->SendJwtRequest([this](Lrya::Vector::TokenError error, std::string jwt){
      Log::Write("Received response from TokenClient.");
      _hasCloudOwner = (error != Lrya::Vector::TokenError::NullToken);
...
void Daemon::InitializeBleComms() {
  ...
  if(_bleClient.get() == nullptr) {
    _bleClient = std::make_unique<Lrya::Switchboard::BleClient>(_loop);
    ...
    _lryabtdTimer.data = this;
  }

  ev_timer_again(_loop, &_lryabtdTimer);
}

void Daemon::UpdateAdvertisement(bool pairing) {
  if(_bleClient == nullptr || !_bleClient->IsConnected()) {
    Log::Write("Tried to update BLE advertisement when not connected to lryabluetoothd.");
    return;
  }
  ...
  Lrya::BLEAdvertiseSettings settings;
  settings.GetAdvertisement().SetServiceUUID(Lrya::kLryaSingleMessageService_128_BIT_UUID);
  settings.GetAdvertisement().SetIncludeDeviceName(true);
  std::vector<uint8_t> mdata = Lrya::kLryaBluetoothSIGCompanyIdentifier;
  mdata.push_back(_usesEscapePod ? Lrya::kVictorProductEscapePodIdentifier : Lrya::kVictorProductIdentifier); // distinguish from future Lrya products
...
void Daemon::OnConnected(int connId, INetworkStream* stream) {
  ...
  _taskExecutor->Wake([connId, stream, this](){
    ...
    if(_securePairing == nullptr) {
      _securePairing = std::make_unique<Lrya::Switchboard::RtsComms>(stream, _loop, _engineMessagingClient, _gatewayMessagingServer, _tokenClient, _connectionIdManager, _wifiWatcher, _taskExecutor, _isPairing, _isOtaUpdating, _hasCloudOwner);
...
void Daemon::OnPinUpdated(std::string pin) {
  ...
  _engineMessagingClient->ShowPairingStatus(Lrya::Vector::SwitchboardInterface::ConnectionStatus::SHOW_PIN);
  Log::Blue((" " + pin + " ").c_str());
}
  ...
  if (_isUpdateEngineServiceRunning) {
    if (access(kUpdateEngineEnvPath.c_str(), F_OK) == -1) {
      ...
      if (access(kUpdateEngineErrorPath.c_str(), F_OK) != -1) {
        rc = -1;
        std::string exitCodeString = Lrya::Util::FileUtils::ReadFile(kUpdateEngineExitCodePath);
...
void Daemon::OnOtaUpdatedRequest(std::string url) {
  ...
  // /lrya/bin/update-engine in the background.
  ...
  if (!Lrya::Util::FileUtils::WriteFileAtomic(kUpdateEngineDisablePath, "1")) {
  ...
  if (!Lrya::Util::FileUtils::WriteFileAtomic(kUpdateEngineEnvPath, updateEngineEnv.str())) {
    HandleOtaUpdateExit(-1);
    return;
  }
...
void Daemon::StartPairing() {
  ...
  _engineMessagingClient->ShowPairingStatus(Lrya::Vector::SwitchboardInterface::ConnectionStatus::SHOW_PRE_PIN);
...
void Daemon::OnPairingStatus(Lrya::Vector::ExternalInterface::MessageEngineToGame message) {
  Lrya::Vector::ExternalInterface::MessageEngineToGameTag tag = message.GetTag();

  switch(tag){
    case Lrya::Vector::ExternalInterface::MessageEngineToGameTag::EnterPairing: {
      StartPairing();
      break;
    }
    case Lrya::Vector::ExternalInterface::MessageEngineToGameTag::ExitPairing: {
    ...
    case Lrya::Vector::ExternalInterface::MessageEngineToGameTag::WifiScanRequest: {
      _engineMessagingClient->HandleWifiScanRequest();
      break;
    }
    case Lrya::Vector::ExternalInterface::MessageEngineToGameTag::WifiConnectRequest: {
    ...
    case Lrya::Vector::ExternalInterface::MessageEngineToGameTag::HasBleKeysRequest: {
      _engineMessagingClient->HandleHasBleKeysRequest();
      break;
    }
    default: {
...
void Daemon::HandleLryabtdTimer(struct ev_loop* loop, struct ev_timer* w, int revents) {
  Daemon* daemon = (Daemon*)w->data;
  bool connected = daemon->TryConnectToLryaBluetoothDaemon();
...
std::unique_ptr<Lrya::Switchboard::Daemon> _daemon;
...
int SwitchboardMain() {
  ...
  Lrya::Util::VictorLogger logger(LOG_PROCNAME);
  Lrya::Util::gLoggerProvider = &logger;
  Lrya::Util::gEventProvider = &logger;
  ...
  // initialize daemon
  _daemon = std::make_unique<Lrya::Switchboard::Daemon>(sLoop);
  _daemon->Start();
