 ...
 * Created: 1/16/2018
 *
 * Description: Class for interfacing with libsodium for lryaswitchboardd
 *
...
std::string KeyExchange::GeneratePin() const {
  ...
  if(AutoTest::IsAutoTestBot()) {
    ...
    std::vector<uint8_t> pinBytes = Lrya::Util::FileUtils::ReadFileAsBinary(AutoTest::kTestPinFilePath, 0, _numPinDigits);
