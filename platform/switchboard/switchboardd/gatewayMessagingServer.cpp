...
bool GatewayMessagingServer::Init() {
  ...
  _server.StartListening(Lrya::Vector::SWITCH_GATEWAY_SERVER_PATH);
...
void GatewayMessagingServer::HandleAuthRequest(const SwitchboardRequest& message) {
  ...
  tokenClient->SendJwtRequest(
    [this, sessionToken, clientName, appId, tokenClient](Lrya::Vector::TokenError error, std::string jwtToken) {
      ...
      switch(error) {
        ...
        default: {
          ...
          SendMessage(SwitchboardResponse(Lrya::Vector::AuthResponse("", "", error)));
        }
        break;
      }
  });
...
void GatewayMessagingServer::ProcessCloudAuthResponse(bool isPrimary, Lrya::Vector::TokenError authError, std::string appToken, std::string authJwtToken) {
  // Send SwitchboardResponse
  SwitchboardResponse res = SwitchboardResponse(Lrya::Vector::AuthResponse(appToken, "", authError));
  SendMessage(res);
}
...
void GatewayMessagingServer::sEvGatewayMessageHandler(struct ev_loop* loop, struct ev_timer* w, int revents) {
  ...
  while((recvSize = wData->server->Recv((char*)sMessageData, sizeof(sMessageData))) > kMessageHeaderLength) {
    ...
    if(msgSize > (sizeof(sMessageData) - sizeof(msgSize))) {
      ...
      Log::Write("GatewayMessagingServer received message from mate-gateway that didn't fit into our buffer.");
      continue;
    }
