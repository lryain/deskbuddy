...
const std::string SavedSessionManager::kRtsKeyDataFile = "/data/data/com.lrya.deskmate/persistent/switchboard/sessions";
...
const char* SavedSessionManager::kPrefix = "LRYABITS";
...
bool SavedSessionManager::IsValidRtsKeysData(const std::vector<uint8_t>& data) {
  ...
  // Must start with LRYABITS magic
...
RtsKeys SavedSessionManager::LoadRtsKeysFromFile(const std::string& fileName, size_t length) {
  ...
  std::vector<uint8_t> data = Lrya::Util::FileUtils::ReadFileAsBinary(fileName, 0, length);
...
int SavedSessionManager::SaveRtsKeysToFile(RtsKeys& saveData,
  ...
  bool success = Lrya::Util::FileUtils::WriteFile(fileName, data);
  if (!success) {
