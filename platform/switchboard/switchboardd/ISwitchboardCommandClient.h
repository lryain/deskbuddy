...
class ISwitchboardCommandClient {
  ...
  virtual void SendMessage(const Lrya::Vector::ExternalInterface::MessageGameToEngine& message) = 0;
  virtual void SetPairingPin(std::string pin) = 0 ;
  virtual void SendBLEConnectionStatus(bool connected) = 0;
  virtual void ShowPairingStatus(Lrya::Vector::SwitchboardInterface::ConnectionStatus status) = 0;
  ...
  using EngineMessageSignal = Signal::Signal<void (Lrya::Vector::ExternalInterface::MessageEngineToGame)>;
