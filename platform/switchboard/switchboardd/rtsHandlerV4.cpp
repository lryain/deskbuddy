...
void RtsHandlerV4::HandleRtsConnResponse(const Lrya::Vector::ExternalComms::RtsConnection_4& msg) {
...
void RtsHandlerV4::ProcessCloudAuthResponse(bool isPrimary, Lrya::Vector::TokenError authError, std::string appToken, std::string authJwtToken) {
...
void RtsHandlerV4::HandleRtsCloudSessionRequest(const Vector::ExternalComms::RtsConnection_4& msg) {
  ...
  Lrya::Vector::ExternalComms::RtsCloudSessionRequest cloudReq =
  ...
  std::weak_ptr<TokenResponseHandle> tokenHandle = _tokenClient->SendJwtRequest(
    [this, sessionToken](Lrya::Vector::TokenError error, std::string jwtToken) {
...
void RtsHandlerV4::HandleRtsAck(const Vector::ExternalComms::RtsConnection_4& msg) {
  ...
  if(_state == RtsPairingPhase::AwaitingNonceAck &&
    ack.rtsConnectionTag == (uint8_t)Lrya::Vector::ExternalComms::RtsConnection_4Tag::RtsNonceMessage) {
    HandleNonceAck();
  } else {
    // ignore msg
    IncrementAbnormalityCount();
...
void RtsHandlerV4::UpdateFace(Lrya::Vector::SwitchboardInterface::ConnectionStatus state) {
  if(_engineClient == nullptr) {
