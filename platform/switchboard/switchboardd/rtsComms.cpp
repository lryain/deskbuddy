...
void RtsComms::SendHandshake() {
  ...
  // ANY deskmate needs to be able to communicate with
...
void RtsComms::UpdateFace(Lrya::Vector::SwitchboardInterface::ConnectionStatus state) {
...
void RtsComms::HandleReset(bool forced) {
  ...
  _taskExecutor->Wake([this, weakSafeHandle, forced]() {    
    ...
    if(forced) {
      ...
      if(_isPairing) {
        UpdateFace(Lrya::Vector::SwitchboardInterface::ConnectionStatus::SHOW_PRE_PIN);
      } else {
        UpdateFace(Lrya::Vector::SwitchboardInterface::ConnectionStatus::END_PAIRING);
      }
    } else {
