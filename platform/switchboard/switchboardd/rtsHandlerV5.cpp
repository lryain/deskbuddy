...
using namespace Lrya::Vector::ExternalComms;
...
bool RtsHandlerV5::IsAuthenticated() {
  ...
  #if LRYA_SWITCHBOARD_CLOUD_AUTH
...
void RtsHandlerV5::HandleRtsConnResponse(const Lrya::Vector::ExternalComms::RtsConnection_5& msg) {
  if(!HasState(RtsCommsType::Unencrypted)) {
    return;
  }

  if(_state == RtsPairingPhase::AwaitingPublicKey) {
    Lrya::Vector::ExternalComms::RtsConnResponse connResponse = msg.Get_RtsConnResponse();

    if(connResponse.connectionType == Lrya::Vector::ExternalComms::RtsConnType::FirstTimePair) {
...
void RtsHandlerV5::HandleRtsChallengeMessage(const Vector::ExternalComms::RtsConnection_5& msg) {
  ...
  if(_state == RtsPairingPhase::AwaitingChallengeResponse) {
    Lrya::Vector::ExternalComms::RtsChallengeMessage challengeMessage = msg.Get_RtsChallengeMessage();
...
void RtsHandlerV5::HandleRtsWifiConnectRequest(const Vector::ExternalComms::RtsConnection_5& msg) {
  ...
  if(_state == RtsPairingPhase::ConfirmedSharedSecret) {
    Lrya::Vector::ExternalComms::RtsWifiConnectRequest wifiConnectMessage = msg.Get_RtsWifiConnectRequest();
    ...
    UpdateFace(Lrya::Vector::SwitchboardInterface::ConnectionStatus::SETTING_WIFI);
...
void RtsHandlerV5::HandleRtsWifiForgetRequest(const Vector::ExternalComms::RtsConnection_5& msg) {
  ...
  if(_state == RtsPairingPhase::ConfirmedSharedSecret) {
    // Get message
    Lrya::Vector::ExternalComms::RtsWifiForgetRequest forgetMsg = msg.Get_RtsWifiForgetRequest();
...
void RtsHandlerV5::HandleRtsOtaUpdateRequest(const Vector::ExternalComms::RtsConnection_5& msg) {
  ...
  if(_state == RtsPairingPhase::ConfirmedSharedSecret && !_isOtaUpdating) {
    Lrya::Vector::ExternalComms::RtsOtaUpdateRequest otaMessage = msg.Get_RtsOtaUpdateRequest();
...
void RtsHandlerV5::HandleRtsWifiAccessPointRequest(const Vector::ExternalComms::RtsConnection_5& msg) {
  ...
  if(_state == RtsPairingPhase::ConfirmedSharedSecret) {
    Lrya::Vector::ExternalComms::RtsWifiAccessPointRequest accessPointMessage = msg.Get_RtsWifiAccessPointRequest();
    if(accessPointMessage.enable) {
      // enable access point mode on Victor
      char vicName[PROPERTY_VALUE_MAX] = {0};
      (void)property_get("lrya.robot.name", vicName, "");
...
void RtsHandlerV5::ProcessCloudAuthResponse(bool isPrimary, Lrya::Vector::TokenError authError, std::string appToken, std::string authJwtToken) {
  RtsCloudStatus status;
  switch(authError) {
    case Lrya::Vector::TokenError::NoError:
      Log::Write("CloudAuth - Successfully authorized account with mate-cloud.");
      ...
      if (_engineClient != nullptr) {
        ...
        const auto msg = Lrya::Vector::ExternalInterface::MessageGameToEngine::CreateUserLoggedIn({});
        _engineClient->SendMessage(msg);
      }
      break;
    case Lrya::Vector::TokenError::InvalidToken:
      Log::Error("CloudAuth - mate-cloud received invalid token.");
      status = RtsCloudStatus::InvalidSessionToken;
      break;
    case Lrya::Vector::TokenError::Connection:
      Log::Error("CloudAuth - mate-cloud could not connect to server.");
      status = RtsCloudStatus::ConnectionError;
      break;
    case Lrya::Vector::TokenError::WrongAccount:
      Log::Error("CloudAuth - Tried to authorize with wrong Lrya account.");
      status = RtsCloudStatus::WrongAccount;
      break;
    case Lrya::Vector::TokenError::NullToken:
      Log::Error("CloudAuth - mate-cloud has null token.");
      status = RtsCloudStatus::UnknownError;
      break;
    default:
      Log::Error("CloudAuth - mate-cloud unknown error.");
      appToken = "";
      break;
  }
  ...
  if(_gatewayServer != nullptr) {
    std::shared_ptr<SafeHandle> handle = _gatewayServer->SendClientGuidRefreshRequest([this, authError, status, appToken] (bool success) {
      ...
      SendRtsMessage<RtsCloudSessionResponse>(
        authError==Lrya::Vector::TokenError::NoError, 
        status,
        appToken);
    });
...
void RtsHandlerV5::HandleRtsCloudSessionRequest(const Vector::ExternalComms::RtsConnection_5& msg) {
  ...
  Lrya::Vector::ExternalComms::RtsCloudSessionRequest_5 cloudReq =
  ...
  std::weak_ptr<TokenResponseHandle> tokenHandle = _tokenClient->SendJwtRequest(
    [this, sessionToken, clientName, appId](Lrya::Vector::TokenError error, std::string jwtToken) {
      ...
      switch(error) {
        case Lrya::Vector::TokenError::NullToken: {
          ...
          std::weak_ptr<TokenResponseHandle> authHandle =
            ...
            [this, isPrimary](Lrya::Vector::TokenError authError, std::string appToken, std::string authJwtToken) {
...
void RtsHandlerV5::HandleRtsAppConnectionIdRequest(const Vector::ExternalComms::RtsConnection_5& msg) {
  ...
  Lrya::Vector::ExternalComms::RtsAppConnectionIdRequest appConnIdMsg = 
...
void RtsHandlerV5::HandleRtsSdkProxyRequest(const Vector::ExternalComms::RtsConnection_5& msg) {
  ...
  if(_gatewayServer != nullptr) {
    Lrya::Vector::ExternalComms::RtsSdkProxyRequest sdkMsg = 
...
void RtsHandlerV5::HandleRtsLogRequest(const Vector::ExternalComms::RtsConnection_5& msg) {
  ...
  int exitCode = ExecCommand({"sudo", "/lrya/bin/diagnostics-logger"});

  std::vector<uint8_t> logBytes
    = Lrya::Util::FileUtils::ReadFileAsBinary("/data/diagnostics/logs.tar.bz2");
...
void RtsHandlerV5::HandleRtsAck(const Vector::ExternalComms::RtsConnection_5& msg) {
  Lrya::Vector::ExternalComms::RtsAck ack = msg.Get_RtsAck();
  if(_state == RtsPairingPhase::AwaitingNonceAck &&
    ack.rtsConnectionTag == (uint8_t)Lrya::Vector::ExternalComms::RtsConnection_5Tag::RtsNonceMessage) {
    HandleNonceAck();
  } else {
    // ignore msg
    IncrementAbnormalityCount();
...
void RtsHandlerV5::SendChallengeSuccess() {
  ...
  UpdateFace(Lrya::Vector::SwitchboardInterface::ConnectionStatus::END_PAIRING);
...
void RtsHandlerV5::SendWifiScanResult() {
  ...
  std::vector<Lrya::Vector::ExternalComms::RtsWifiScanResult_3> wifiScanResults;
  ...
  for(int i = 0; i < wifiResults.size(); i++) {
    ...
    Lrya::Vector::ExternalComms::RtsWifiScanResult_3 result = Lrya::Vector::ExternalComms::RtsWifiScanResult_3(wifiResults[i].auth,
...
void RtsHandlerV5::UpdateFace(Lrya::Vector::SwitchboardInterface::ConnectionStatus state) {
  if(_engineClient == nullptr) {
    // no engine client -- probably testing
    return;
  }

  if((state == Lrya::Vector::SwitchboardInterface::ConnectionStatus::UPDATING_OS) ||
    (state == Lrya::Vector::SwitchboardInterface::ConnectionStatus::SETTING_WIFI)) {
    return;
  }
