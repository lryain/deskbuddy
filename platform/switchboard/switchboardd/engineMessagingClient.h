...
public:
  ...
  bool Connect();
  bool Disconnect();
  void SendMessage(const Lrya::Vector::ExternalInterface::MessageGameToEngine& message);
  void SetPairingPin(std::string pin);
  void SendBLEConnectionStatus(bool connected);
  void ShowPairingStatus(Lrya::Vector::SwitchboardInterface::ConnectionStatus status);
