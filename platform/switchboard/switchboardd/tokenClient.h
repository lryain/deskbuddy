...
class TokenClient {
  typedef std::function<void(Lrya::Vector::TokenError, std::string, std::string)> AuthRequestCallback;
  typedef std::function<void(Lrya::Vector::TokenError, std::string)> JwtRequestCallback;
  using TokenMessageSignal = Signal::Signal<void (Lrya::Vector::TokenResponse)>;
...
private:
  ...
  void SendMessage(const Lrya::Vector::TokenRequest& message);
  void HandleTokenResponse(Lrya::Vector::TokenResponse response);
