...
private:
  ...
  void HandleCloudSessionAuthResponse(Lrya::Vector::TokenError error, std::string appToken, std::string jwtToken);

  void ProcessCloudAuthResponse(bool isPrimary, Lrya::Vector::TokenError authError, std::string appToken, std::string authJwtToken);

  void IncrementAbnormalityCount();
  void IncrementChallengeCount();

  void SubscribeToCladMessages();
  void UpdateFace(Lrya::Vector::SwitchboardInterface::ConnectionStatus state);
  ...
  int SendRtsMessage(Args&&... args) {
    Lrya::Vector::ExternalComms::ExternalComms msg = Lrya::Vector::ExternalComms::ExternalComms(
      Lrya::Vector::ExternalComms::RtsConnection(Lrya::Vector::ExternalComms::RtsConnection_4(T(std::forward<Args>(args)...))));
