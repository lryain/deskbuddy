...
int main(int argc, const char * argv[])
{
  ...
  static_assert(Lrya::Util::DAS::FIELD_COUNT == 9, "Unexpected DAS field count");

  // Set up the logger
  Lrya::Util::VictorLogger logger(source);
  Lrya::Util::gLoggerProvider = &logger;
  Lrya::Util::gEventProvider = &logger;

  // Log the event
  DASMSG(mate_log_event, event, "Scripted event");
