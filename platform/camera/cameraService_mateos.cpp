...
#include "camera/mateos/camera_client/camera_client.h"
...
#ifdef SIMULATOR
#error SIMULATOR should NOT be defined by any target using cameraService_mateos.cpp
#endif
...
namespace Lrya {
  namespace Vector {
    ...
    namespace { // "Private members"
      struct lrya_camera_handle* _camera = nullptr;
...
#pragma mark --- Hardware Method Implementations ---
    ...
    Result CameraService::InitCamera()
    {
      ...
      lrya_camera_status_t status = camera_status(_camera);
      if(status == LRYA_CAMERA_STATUS_RUNNING &&
         _powerState == CameraPowerState::Running)
      {
      ...
      else if(status != LRYA_CAMERA_STATUS_OFFLINE ||
    ...
    Result CameraService::Update()
    {
      ...
      if (_isRestartingCamera && (status == LRYA_CAMERA_STATUS_RUNNING))
      {
      ...
      if (status != LRYA_CAMERA_STATUS_RUNNING)
      {
        ...
        if (status == LRYA_CAMERA_STATUS_OFFLINE)
        {
        ...
        if((rc == 0) && (status == LRYA_CAMERA_STATUS_IDLE))
        {
    ...
    void CameraService::CameraSetCaptureFormat(Vision::ImageEncoding format)
    {
      ...
      lrya_camera_pixel_format_t cameraFormat;
      switch(format)
      {
        case Vision::ImageEncoding::YUV420sp:
          cameraFormat = megapixels() == 1 ? LRYA_CAM_FORMAT_YUV :  LRYA_CAM_FORMAT_YUV_2MP;
          break;
        case Vision::ImageEncoding::RawRGB:
          cameraFormat =  megapixels() == 1 ? LRYA_CAM_FORMAT_RGB888 : LRYA_CAM_FORMAT_RGB888_2MP;
          break;
        case Vision::ImageEncoding::BAYER:
          cameraFormat =  megapixels() == 1 ? LRYA_CAM_FORMAT_BAYER_MIPI_BGGR10 :  LRYA_CAM_FORMAT_BAYER_MIPI_BGGR10_2MP;
          break;
        default:
    ...
    bool CameraService::CameraGetFrame(u32 atTimestamp_ms, Vision::ImageBuffer& buffer)
    {
      ...
      std::lock_guard<std::mutex> lock(_lock);
      lrya_camera_frame_t* capture_frame = NULL;
      ...
      {
        case LRYA_CAM_FORMAT_BAYER_MIPI_BGGR10:
        case LRYA_CAM_FORMAT_BAYER_MIPI_BGGR10_2MP:
          format = Vision::ImageEncoding::BAYER;
          break;
        case LRYA_CAM_FORMAT_RGB888:
        case LRYA_CAM_FORMAT_RGB888_2MP:
          format = Vision::ImageEncoding::RawRGB;
          break;
        case LRYA_CAM_FORMAT_YUV:
        case LRYA_CAM_FORMAT_YUV_2MP:
