...
static int sMinLogLevel = LryaCameraLogLevelError;
...
void __log_write(int prio, const char *msg) {
  if (prio >= sMinLogLevel) {
    if (!sAndroidLoggingTag[0]) {
      setAndroidLoggingTag("lrya-camera");
    }
...
void logv(const char* fmt, ...) {
  ...
  va_start(ap, fmt);
  __log_vprint(LryaCameraLogLevelVerbose, fmt, ap);
  va_end(ap);
}
...
void logd(const char* fmt, ...) {
  ...
  va_start(ap, fmt);
  __log_vprint(LryaCameraLogLevelDebug, fmt, ap);
  va_end(ap);
}
...
void logi(const char* fmt, ...) {
  ...
  va_start(ap, fmt);
  __log_vprint(LryaCameraLogLevelInfo, fmt, ap);
  va_end(ap);
}
...
void logw(const char* fmt, ...) {
  ...
  va_start(ap, fmt);
  __log_vprint(LryaCameraLogLevelWarn, fmt, ap);
  va_end(ap);
...
void loge(const char* fmt, ...) {
  ...
  va_start(ap, fmt);
  __log_vprint(LryaCameraLogLevelError, fmt, ap);
  va_end(ap);
}
