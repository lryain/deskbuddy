...
static char *cli_socket_path = "/dev/socket/mate-engine-cam_client0";
static char *srv_socket_path = "/var/run/mm-lrya-camera/camera-server";
...
#define LRYA_CAMERA_MAX_PACKETS 12
#define LRYA_CAMERA_MAX_FRAME_COUNT 6
...
typedef struct {
  ...
  _Atomic uint32_t frame_locks[LRYA_CAMERA_MAX_FRAME_COUNT];
} lrya_camera_buf_lock_t;

typedef struct {
  uint8_t magic[4];
  lrya_camera_buf_lock_t locks;
  uint32_t frame_count;
  uint32_t frame_size;
  uint32_t frame_offsets[LRYA_CAMERA_MAX_FRAME_COUNT];
  uint8_t data[0];
} lrya_camera_buf_header_t;
...
struct client_ctx {
  ...
  int request_close;
  int request_start;
  lrya_camera_status_t status;

  struct camera_capture_mem_info camera_buf;
  uint64_t locked_slots[LRYA_CAMERA_MAX_FRAME_COUNT];

  uint32_t rx_cursor;
  struct lrya_camera_msg rx_packets[LRYA_CAMERA_MAX_PACKETS];

  uint32_t tx_cursor;
  struct lrya_camera_msg tx_packets[LRYA_CAMERA_MAX_PACKETS];
};

//
// Internal extended version of public struct
//
struct lrya_camera_handle_private {
  int client_handle;
...
static int send_message(struct client_ctx *ctx, struct lrya_camera_msg *msg)
{
...
{
  assert(slot < LRYA_CAMERA_MAX_FRAME_COUNT);
  int rc = -1;
  if (slot < LRYA_CAMERA_MAX_FRAME_COUNT) {
...
static void lock_all_slots(struct client_ctx *ctx)
{
  ...
  lrya_camera_buf_header_t *header = (lrya_camera_buf_header_t *)data;

  for (uint32_t slot = 0; slot < LRYA_CAMERA_MAX_FRAME_COUNT; ++slot) {
    // lock slot
...
static void unlock_slots_except(struct client_ctx *ctx, uint32_t slot)
{
  ...
  for(int i = 0; i < LRYA_CAMERA_MAX_FRAME_COUNT; i++)
  {
...
{
  unlock_slots_except(ctx, LRYA_CAMERA_MAX_FRAME_COUNT+1);
}
...
static int write_outgoing_data(struct client_ctx *ctx)
{
  ...
  for (i = 0; i < msg_count; ++i) {
    struct lrya_camera_msg *msg = &(ctx->tx_packets[i]);
...
static int enqueue_message(struct client_ctx *ctx, lrya_camera_msg_id_t msg_id)
{
  ...
  struct lrya_camera_msg *msg = &ctx->tx_packets[cursor];
  msg->msg_id = msg_id;
...
static int enqueue_message_with_payload(struct client_ctx *ctx, lrya_camera_msg_id_t msg_id, void* buf, size_t len)
{
  pthread_mutex_lock(&ctx->ipc_mutex);
  uint32_t cursor = ctx->tx_cursor;
...
static int process_one_message(struct client_ctx *ctx, struct lrya_camera_msg *msg)
{
  int rc = 0;
  const lrya_camera_msg_id_t msg_id = msg->msg_id;
  switch (msg_id) {
  case LRYA_CAMERA_MSG_S2C_STATUS: {
    lrya_camera_msg_id_t ack_msg_id = msg->payload[0];
    logv("%s: received STATUS ack: %d\n", __func__, ack_msg_id);
    switch (ack_msg_id) {
    case LRYA_CAMERA_MSG_C2S_CLIENT_REGISTER: {
      ctx->status = LRYA_CAMERA_STATUS_IDLE;
    }
    break;
    case LRYA_CAMERA_MSG_C2S_CLIENT_UNREGISTER: {
      ctx->status = LRYA_CAMERA_STATUS_OFFLINE;
    }
    break;
    case LRYA_CAMERA_MSG_C2S_START: {
      ctx->status = LRYA_CAMERA_STATUS_RUNNING;
    }
    break;
    case LRYA_CAMERA_MSG_C2S_STOP: {
      ctx->status = LRYA_CAMERA_STATUS_IDLE;
    }
    break;
    default:
  ...
  break;
  case LRYA_CAMERA_MSG_S2C_BUFFER: {
    ...
    if(ctx->camera_buf.camera_capture_fd > 0)
    {
      ...
      if(rc < 0)
      {
        loge("%s: LRYA_CAMERA_MSG_S2C_BUFFER unmap failed %d", __func__, rc);
      }
    }
    ...
    logv("%s: received LRYA_CAMERA_MSG_S2C_BUFFER :: fd=%d size=%u\n",
  ...
  break;
  case LRYA_CAMERA_MSG_S2C_HEARTBEAT: {
    break;
  }
  break;
  default: {
...
static int process_incoming_messages(struct client_ctx *ctx)
{
  ...
  for (i = 0; i < msg_count; ++i) {
    struct lrya_camera_msg *msg = &(ctx->rx_packets[i]);
...
static int read_incoming_data(struct client_ctx *ctx)
{
  ...
  do {
    ...
    if (ctx->rx_cursor == LRYA_CAMERA_MAX_PACKETS) {
    ...
    // Prepare rx buffer
    struct lrya_camera_msg *msg = &(ctx->rx_packets[ctx->rx_cursor]);
...
static int event_loop(struct client_ctx *ctx)
{
  ...
  if(ctx->status == LRYA_CAMERA_STATUS_OFFLINE)
  {
    return -1;
  }
...
static void *camera_client_thread(void *camera_handle_ptr)
{
  ...
  struct lrya_camera_handle_private *handle = (struct lrya_camera_handle_private *)camera_handle_ptr;
  struct client_ctx *client = &handle->camera_client;

  client->status = LRYA_CAMERA_STATUS_IDLE;
  enqueue_message(client, LRYA_CAMERA_MSG_C2S_CLIENT_REGISTER);

  struct timespec lastHeartbeatTs = {0, 0};

  int rc = 0;
  while (client->status != LRYA_CAMERA_STATUS_OFFLINE) {
    ...
    if (client->status == LRYA_CAMERA_STATUS_IDLE && !client->waiting_for_delete) {
      if (client->request_start) {
        client->status = LRYA_CAMERA_STATUS_STARTING;
        enqueue_message(client, LRYA_CAMERA_MSG_C2S_START);
    ...
    if (elapsedNs > HEARTBEAT_INTERVAL_NS) {
      enqueue_message(client, LRYA_CAMERA_MSG_C2S_HEARTBEAT);
      lastHeartbeatTs = now;
    }
  }

  client->status = LRYA_CAMERA_STATUS_OFFLINE;
...
#define CAMERA_HANDLE_P(camera) ((struct lrya_camera_handle_private *)(camera))
...
//   }
  ...
  s_camera_handle.current_frame_id = UINT32_MAX;
  s_camera_handle.last_frame_slot = UINT32_MAX;
  *camera = (struct lrya_camera_handle *)&s_camera_handle;
...
{
  // Stop
  enqueue_message(&CAMERA_HANDLE_P(camera)->camera_client, LRYA_CAMERA_MSG_C2S_STOP);
  return 0;
}
...
//   }
  ...
  if(pause)
  {
    ...
    for(uint32_t slot = 0; slot < LRYA_CAMERA_MAX_FRAME_COUNT; slot++)
    {
      ...
      lrya_camera_frame_t *frame = (lrya_camera_frame_t *)&data[frame_offset];
      if(frame != NULL)
      {
        frame->timestamp = 0;
      }
...
int camera_release(struct lrya_camera_handle *camera)
{
  ...
  enqueue_message(&CAMERA_HANDLE_P(camera)->camera_client, LRYA_CAMERA_MSG_C2S_CLIENT_UNREGISTER);
  return 0;
}
                         ...
                         lrya_camera_frame_t **out_frame)
{
  ...
  if(wSlot >= LRYA_CAMERA_MAX_FRAME_COUNT)
  {
...
lrya_camera_status_t camera_status(struct lrya_camera_handle *camera)
{
  if(camera == NULL)
  {
    return LRYA_CAMERA_STATUS_OFFLINE;
  }
...
int camera_set_capture_format(struct lrya_camera_handle* camera, lrya_camera_pixel_format_t format)
{
  ...
  lrya_camera_msg_params_payload_t payload;
  payload.id = LRYA_CAMERA_MSG_C2S_PARAMS_ID_FORMAT;
...
{
  lrya_camera_msg_params_payload_t payload;
  payload.id = LRYA_CAMERA_MSG_C2S_PARAMS_ID_SNAPSHOT;
