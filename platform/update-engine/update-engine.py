...
"""
Example implementation of Lrya Victor Update engine.
"""
__author__ = "Daniel Casner <daniel@lrya.com>"
...
OTA_PUB_KEY = "/lrya/etc/ota.pub"
OTA_ENC_PASSWORD = "/lrya/etc/ota.pas"
...
def das_event(name, parameters = []):
    "Log a DAS event"
    args = ["/lrya/bin/mate-log-event", "update-engine", name]
    for p in parameters:
...
def die(code, text):
    ...
    das_event("robot.ota_download_end", ["fail", get_prop("ro.lrya.version"), str(text), "", str(code)])
    if DEBUG:
...
def open_url_stream(url):
    ...
    try:
        ...
        os_version = get_prop("ro.lrya.version")
        deskmate_version = get_prop("ro.lrya.deskmate.version")
        deskmate_target = get_prop("ro.build.target")
        if '?' in url:  # Already has a query string
            if not url.endswith('?'):
                url += '&'
        else:
            url += '?'
        url += "emresn={0:s}&lryaversion={1:s}&deskmateversion={2:s}&deskmatetarget={3:s}".format(
...
def handle_delta(current_slot, target_slot, manifest, tar_stream):
    ...
    current_version = get_prop("ro.lrya.version")
...
def handle_lryaRCM(manifest, tar_stream):
    ...
    try:
        ...
        lrya_ti = tar_stream.next()
        src_file = tar_stream.extractfile(lrya_ti)
        sha_fh = ShaFile(src_file)
        lrya_tar = make_tar_stream(sha_fh, "r|" + manifest.get("LRYA", "compression"))
        lrya_tar.extractall(lrya_path)
...
def update_from_url(url):
    ...
    current_os_version = get_prop("ro.lrya.version")
    ...
    with make_tar_stream(stream) as tar_stream:
        ...
        if is_dev_robot(cmdline):
            if not manifest.getint("META", "lryadev"):
                die(214, "Lryadev OS can't install non-lryadev OTA file")
        elif manifest.getint("META", "lryadev"):
            die(214, "Non-lryadev OS can't install lryadev OTA file")

        num_images = manifest.getint("META", "num_images")

        #RCM 2020-8-20 don't nuke the boot or system if its a small update
        if num_images == 1:
            if manifest.has_section("LRYA"):
                pass
        ...
        if num_images == 2:
            ...
            #RCM 2020-8-20 Allow the whole lrya folder to be updated
            elif manifest.has_section("LRYA"):
                # RCM 2020-8-20 this is set since we are only updating the filesystem
                skip_bootctl = True
                handle_lryaRCM(manifest, tar_stream)
            else:
                die(201, "One image specified but not SYSTEM or DELTA or LRYA")
        elif num_images == 3:
...
def construct_update_url(os_version, cmdline):
    ...
    if is_dev_robot(cmdline):
        base_url = os.getenv("UPDATE_ENGINE_LRYADEV_BASE_URL", base_url)
    if not base_url:
        return None
...
if __name__ == '__main__':
    ...
    if url == "auto":
        ...
        url = construct_update_url(get_prop("ro.lrya.version"), get_cmdline())
        if not url:
