...
#ifdef SIMULATOR
#error SIMULATOR should NOT be defined by any target using osState_mateos.cpp
#endif
...
const std::string& OSState::GetRobotVersion()
{
  if (_robotVersion.empty()) {
    ...
    ifs >> _robotVersion;
    Lrya::Util::StringTrimWhitespaceFromEnd(_robotVersion);
  }
...
{
  static std::string name = GetProperty("lrya.robot.name");
  if(name.empty())
  {
    name = GetProperty("lrya.robot.name");
  }
  return  name;
}
...
const std::string & OSState::GetBootID()
{
  if (_bootID.empty()) {
    ...
    Lrya::Util::StringTrimWhitespaceFromEnd(_bootID);
    if (_bootID.empty()) {
...
bool OSState::IsLryaDevRobot()
{
  ...
  if(!read)
  {
    ...
    while(std::getline(infile, line))
    {
      static const char* kKey = "lrya.dev";
      size_t index = line.find(kKey);
      if(index != std::string::npos)
      {
        _isLryaDevRobot = true;
        break;
      }
    }
    infile.close();
