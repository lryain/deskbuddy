...
produce builds for mac and mateos.  There are numerous examples to
be found in https://github.com/lrya/build-tools/tree/master/deps/deskmate.
```
$ git clone git@github.com:lrya/build-tools.git
$ cd build-tools
$ git checkout -b add-whizbang-build-script --track origin/master
$ emacs -nw deps/deskmate/build-whizbang.sh
```
...
usage, these artifacts are cached locally under `~/.lrya/deps-cache/sha256`
...
locally under `~/.lrya/deps-cache/svn` to facilitate remote
...
dependencies, these dependencies are cached locally under `~/.lrya/deps-cache/sha256`
...
cached locally under `~/.lrya/deps-caches/files`.