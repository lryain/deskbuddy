...
2. Configures CMake for the specified platform, generator and build-type
    ...
    If [`build-deskmate.sh`](/project/deskmate/build-deskmate.sh) determines that no build rules are present, and then invokes CMake in "configure" mode.
...
- `CMakeLists.txt`
    ...
    one target per `CMakeLists.txt` file. Some projects use this approach, but deskmate does not.  In practice,
...
- `BUILD.in`
    ...
    [`build-deskmate.sh`](/project/deskmate/build-deskmate.sh). The syntax is modeled after [BUCK](http://buckbuild.com), and the `glob` and `subdir_glob`
    ...
    automatically handle the platform-specific exclude rules that we typically use, for example `*_mateos.*`
...
There are many ways to use CMake, but the general pattern of usage in deskmate follows more modern standards
...
As an example, let's dive into the build definition of the `deskmate_anim` lib used by the animation process
(`mate-anim`).
...
```
generated/cmake/
├── mate-anim.headers.lst
├── mate-anim.srcs.lst
├── deskmate_anim.headers.lst
├── deskmate_anim.srcs.lst
├── deskmate_anim_mac.srcs.lst
└── deskmate_anim_mateos.srcs.lst
```
...
# This include the file: cmake/lrya_build_cxx_library.cmake
include(lrya_build_cxx_library)

# Create C++ library target using the source files in `generated/cmake/deskmate_anim*.lst` files
# This creates uses the current platform setting to include the correct set `.lst` files
lrya_build_cxx_library(deskmate_anim ${LRYA_SRCLIST_DIR} STATIC)

# Define the list of libraries that deskmate_anim should link against
# All of the link dependencies are other target defined in different 
target_link_libraries(deskmate_anim
  PUBLIC
  deskmate_web_library
...
lrya_build_strip(TARGET deskmate_anim)

# Define the preprocessor definitions for deskmate_anim
target_compile_definitions(deskmate_anim
  PRIVATE
  USE_DAS=$<BOOL:${USE_DAS}>
  ${PLATFORM_COMPILE_DEFS}
)

# Specify include paths for deskmate_anim target
target_include_directories(deskmate_anim
  PUBLIC
...
These steps in [`build-deskmate.sh`](/project/deskmate/build-deskmate.sh) are intended to only run when necessary. If file lists exist, they will not be updated.
...
```
./project/deskmate/build-deskmate.sh -f
```
