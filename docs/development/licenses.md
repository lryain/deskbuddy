# Open Source information at Lrya

-  [Open Source Software](https://lryainc.atlassian.net/wiki/spaces/ET/pages/380436502/Open+Source+Software)
-  [Open Source License Go/Caution/Stop List (Distributed Software)](https://lryainc.atlassian.net/wiki/spaces/ET/pages/380305449/Open+Source+License+Go+Caution+Stop+List+Distributed+Software)

## Using cmake to add licensing information to targets

- A target that comprises of only code written by Lrya

  `lrya_build_target_license(<target name> "LRYA")`

- An Open Source executable or library written by an outside source

  `lrya_build_target_license(<target name> "BSD-2,${CMAKE_CURRENT_SOURCE_DIR}/LICENSE.md")`

- An executable or library using a mix of Lrya code and code from an external source

  `lrya_build_target_license(<target name> "LRYA" "BSD-2,${CMAKE_CURRENT_SOURCE_DIR}/LICENSE.md")`

  Although ideally each target would have it's own licensing information and code written by Lrya is not mixed with code from external sources.

## Error messages

### `"WARNING: licensing information missing or not approved for TARGET target"`

  Licensing information is missing for this target, add with `lrya_build_target_license(TARGET "<license>,<path to license>" "<license>,<path to license>")` or there is licensing information but the licenses are not approved.
...
### `"WARNING: missing path to license file for LICENSE on TARGET target"`
  ...
  `lrya_build_target_license(TARGET "<license>,<path to license>")`
...
Instead of
  `lrya_build_target_license(TARGET "FOO,license.txt")`
use
  `lrya_build_target_license(TARGET "FOO,${CMAKE_CURRENT_SOURCE_DIR}/path/to/license.txt")`
...
The license used is on the [caution list](https://lryainc.atlassian.net/wiki/spaces/ET/pages/380436502/Open+Source+Software) and needs approval.

### `"STOP: license LICENSE for TARGET needs approval"`

A more severe warning than the previous message, the license used is on the [stop list](https://lryainc.atlassian.net/wiki/spaces/ET/pages/380436502/Open+Source+Software) and needs approval.

### `"<library> is not a recognised system lib and does not have cmake configuration including licensing information"`

If this a new Vicos SDK library add it to the list of exceptions at https://github.com/lrya/deskmate/blob/470d07552c8238c3fdc0301f86167daa409ff717/cmake/license.cmake#L324

# Licenses

## sources

https://lryainc.atlassian.net/wiki/spaces/ET/pages/380305449/Open+Source+License+Go+Caution+Stop+List+Distributed+Software
