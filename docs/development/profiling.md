...
Profiling on Victor is based on [simpleperf](../../project/deskmate/simpleperf/README.md).
...
This documentation assumes that you are using alias commands from [usefulALiases.sh](../../project/deskmate/scripts/usefulALiases.sh).

If you are not already using these aliases, you can load them into your development shell:

```bash
cd <repo>/project/deskmate/scripts
...
Android development tools are not installed by the Lrya environment, to install adb:
```
brew cask install android-platform-tools
```

If your robot does not allow ADB over tcp, you can enable it by running a command like [this](https://lryainc.atlassian.net/wiki/spaces/VD/pages/148638114/Victor+Developer+Tips) ON THE ROBOT:
...
This documentation assumes that you are working in [project/deskmate/simpleperf](../../project/deskmate/simpleperf)
on your development host.  If you are not already working in this directory, you can change to it by running a
command like this:

```bash
cd <repo>/project/deskmate/simpleperf
```
...
```bash
deskmate_build_release
deskmate_deploy_release
```
...
```bash
LRYA_PROFILE_PROCNAME=mate-engine
LRYA_PROFILE_DURATION=10 (in seconds)
LRYA_PROFILE_FREQUENCY=4000 (in usec)
LRYA_PROFILE_SYMBOLCACHE=symbol_cache
```

Default values may be overridden as needed:

```bash
LRYA_PROFILE_PROCNAME=mate-anim bash HOW-inferno.sh
```
...
Use `./HOW-report.sh --tids 9660` to view the results for a specific thread:
    ...
    5.96%     CozmoRunner  9462  9660  /data/data/com.lrya.cozmoengine/lib/libcozmo_engine.so      Lrya::Util::Stats::StatsAccumulator::operator+=(double)
    5.43%     CozmoRunner  9462  9660  /data/data/com.lrya.cozmoengine/lib/libcozmo_engine.so      @plt
    3.10%     CozmoRunner  9462  9660  /data/data/com.lrya.cozmoengine/lib/libcozmo_engine.so      Lrya::Cozmo::UiMessageHandler::Update()
    2.40%     CozmoRunner  9462  9660  /data/data/com.lrya.cozmoengine/lib/libcozmo_engine.so      Lrya::Cozmo::CozmoEngine::Update(unsigned long long)
    2.28%     CozmoRunner  9462  9660  /data/data/com.lrya.cozmoengine/lib/libcozmo_engine.so      Lrya::Util::CpuThreadProfiler::AddSample(std::__ndk1::chrono::time_point<std::__ndk1::chrono::steady_clock, std::__ndk1::chrono::duration<long long, std::__ndk1::ratio<1ll, 1000000000ll> > > const&, std::__ndk1::chrono::time_point<std::__ndk1::chrono::steady_clock, std::__ndk1::chrono::duration<long long, std::__ndk1::ratio<1ll, 1000000000ll> > > const&, Lrya::Util::CpuProfileSampleShared&)
    2.24%     CozmoRunner  9462  9660  /data/data/com.lrya.cozmoengine/lib/libcozmo_engine.so      Lrya::Cozmo::UiMessageHandler::DeliverToGame(Lrya::Cozmo::ExternalInterface::MessageEngineToGame const&, unsigned int)
...
### [HOW-inferno.sh](../../project/deskmate/simpleperf/HOW-inferno.sh)
...
### [HOW-annotate.sh](../../project/deskmate/simpleperf/HOW-annotate.sh)
...
### [HOW-reportchart.sh](../../project/deskmate/simpleperf/HOW-reportchart.sh)
...
### [HOW-reportsamples.sh](../../project/deskmate/simpleperf/HOW-reportsamples.sh)
...
Use `#define LRYA_EMBEDDED_BENCHMARKS` to enable benchmarking within the Vision component.

Use `#define LRYA_VISION_PROFILER` to enable profiling metrics for the Vision system.

When logged to DAS these have the prefix `robot.vision.profiler`

These options can be enabled from the command-line, e.g.

`deskmate_build_release -DLRYA_EMBEDDED_BENCHMARK=1` (embedded benchmarks are enabled)

`deskmate_build_release` (embedded benchmarks are still enabled as the build is rerun with current options)

`deskmate_build_release -f` (embedded benchmarks are disabled as the build is reconfigured)
