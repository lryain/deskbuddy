...
The underlying build system for deskmate is currently `CMake`.  The appropriate version of CMake and other dependencies required for building deskmate will be fetched automatically.
...
To build for mateos (the default):

```
./project/deskmate/build-deskmate.sh

# you can also explicitly pass the platform
./project/deskmate/build-deskmate.sh -p mateos
```

To build for mac:

```
./project/deskmate/build-deskmate.sh -p mac

# To generate a `cozmo.xcodeproj` Xcode project without building
./project/deskmate/build-deskmate.sh -p mac -g Xcode -C
```
...
```
./project/deskmate/build-deskmate.sh -c Release
```

For the complete list of build options:

```
./project/deskmate/build-deskmate.sh -h
```
...
./project/deskmate/build-deskmate.sh -- --target mate-anim
```
...
./project/deskmate/build-deskmate.sh -- --target help

# list targets through ninja
./project/deskmate/build-deskmate.sh -- -- -t targets all
```
...
```
./project/deskmate/build-deskmate.sh -- --target clean
```
