...
```bash
$ export LD_LIBRARY_PATH=/usr/lib:/lrya/lib
```
...
(lldb) platform connect connect://${LRYA_ROBOT_HOST}:55001
(lldb) settings set target.exec-search-paths ./_build/mateos/Debug/bin ./_build/mateos/Debug/lib
```

You will have to fill in the IP address for your robot.

If you are debugging a release build, set search paths to use `_build/mateos/Release` instead of `_build/mateos/Debug`.

### 4. Attach to a process

```lldb
(lldb) process attach -name mate-robot
```
...
```json
 "lldb.dbgconfig": {
        "host": "LRYA-ROBOT-HOST-IP",
        "bin": "${workspaceRoot}/_build/mateos/Debug/bin",
        "lib": "${workspaceRoot}/_build/mateos/Debug/lib"
    },
```
...
```bash
systemctl stop deskmate.target
systemctl restart mate-robot
/lrya/bin/mate-anim -c /lrya/etc/config/platform_config.json
```
...
systemctl stop deskmate.target
systemctl restart mate-robot
ASAN_OPTIONS=detect_container_overflow=0 /lrya/bin/mate-anim -c /lrya/etc/config/platform_config.json
ASAN_OPTIONS=detect_container_overflow=0:replace_intrin=0 /lrya/bin/mate-anim -c /lrya/etc/config/platform_config.json
...
systemctl stop deskmate.target
systemctl restart mate-robot
ASAN_OPTIONS=detect_container_overflow=0:replace_intrin=0 VIC_ANIM_CONFIG=/lrya/etc/config/platform_config.json gdb /lrya/bin/mate-anim
```

Note: setting arguments within `gdb` does not work, for example:

```bash
(gdb) set args "-c" "/lrya/etc/config/platform_config.json"
(gdb) set args -c /lrya/etc/config/platform_config.json
(gdb) run "-c /lrya/etc/config/platform_config.json"
```
...
(but first, ssh into robot and get a list of process ids with `ps | grep mate-`)
...
```lldb
(lldb) bt all
warning: (arm) /Users/richard/projects/deskmate/_build/mateos/Debug/lib/libcubeBleClient.so 0x91f00010526: DW_AT_specification(0x000009a9) has no decl

warning: (arm) /Users/richard/projects/deskmate/_build/mateos/Debug/lib/libcozmo_engine.so 0x11164f000217ea: DW_AT_specification(0x00001321) has no decl

* thread #1, name = 'mate-engine', stop reason = signal SIGSTOP
    frame #0: 0xb6be3814 libc.so.6`nanosleep + 68
    frame #1: 0xb6c172e0 libc.so.6`usleep + 76
  * frame #2: 0x7f5632d6 mate-engine
...
```lldb
thread select 17
* thread #17, name = 'mate-anim', stop reason = signal SIGSTOP
