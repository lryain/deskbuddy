...
What is recorded for each mate-engine tick:
...
What is recorded for each mate-anim tick:
...
```
 ...
 Summary:  (RELEASE build; MATEOS; 20 engine ticks; 1.204 seconds total)
         Engine   Engine    Sleep    Sleep     Over      RtE   EtR   GtE   EtG   Viz  Battery
...
```-DLRYA_PERF_METRIC_ENABLED=1```

(or 0 to disable)

### Use from command line
The interface to PerfMetric is through Victor's embedded web server. From the command line, this will start a recording session for mate-engine, assuming LRYA_ROBOT_HOST is set to your robot IP (use port 8889 for mate-anim):

```curl ${LRYA_ROBOT_HOST}':8888/perfmetric?start'```
...
```curl ${LRYA_ROBOT_HOST}':8888/perfmetric?stop&dumplogall'```
...
```curl ${LRYA_ROBOT_HOST}':8888/perfmetric?start&waitseconds30&stop&dumplogall'```
...
* "dumpfiles" writes all info to two files on the robot: One is a formatted txt file, and the other a csv file. These go in /data/data/com.lrya.deskmate/cache/perfMetricLogs. The filename has the time of the file write baked in, as well as "R" or "D" to indicate Release or Debug build, and "Eng" or "Anim" to indicate mate-engine or mate-anim. An example: "perfMetric_2018-11-29_17-41-05_R_Eng.txt"
