...
## Common Uses
  ...
  * Data defined behaviors often use BEI Conditions as WantsToBeActivated conditions. By adding the condition type to a behavior instance's JSON file the behavior will only want to be activated if the condition is true. This supports the seperation of what the robot is "reacting to" from how the robot "reacts".  "Reacting" to being picked up can be implemented as a "Play Animation" behavior instance with a "Picked Up" WantsToBeActivated condition. A more detailed overview of this practice is available [on confluence](https://lryainc.atlassian.net/wiki/spaces/CBS/pages/199917569/Leveraging+BEI+Conditions+for+Data+Driven+Behaviors)
