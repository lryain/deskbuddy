...
* Forward inference through neural nets runs asynchronously within the VisionSystem, via a `std::future`
  ...
  - We have our own ["private fork" of TensorFlow](https://github.com/lrya/tensorflow), which has an `lrya` branch with an `lrya` subdirectory containing our special build scripts (e.g. for building a `mateos`-compatible version of TensorFlow / TFLite).
