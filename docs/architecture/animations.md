...
An Animation is an open-loop sequence of highly coordinated movements, faces (eyes), lights, and sounds used to demonstrate an emotion or reaction. They can be created dynamically at runtime ("procedural" animations) or they are more commonly authored by Lrya's animation team in Maya and loaded from files ("canned" animations). 
...
Animation files and tools are stored in a separate subversion repository, setup instructions are [here](https://lryainc.atlassian.net/wiki/spaces/COZMO/pages/73498743/Setting+up+Cornerstone+for+the+first+time).
...
`cozmo-animation/trunk/tools/other/export_maya_anim.sh` is the main entry point that initializes the Maya python API and calls into the Lrya Python library (`cozmo-animation/trunk/tools/pylibs/lryamaya`) that does the work.
...
Exported animation files are pulled into the repo by [`project/deskmate/build-deskmate.sh`](../../project/deskmate/build-deskmate.sh) that issues svn commands to update from the animation repo.
...
As a behavior engineer, you're likely to be called upon to make use of new animations produced by Lrya's awesome animators.
...
- In `deskmate/DEPS`, set the "version" number (in the "deskmate-animation-assets" entry under "svn", line 65 at the time of this writing) to the DEPS number provided by the animator (if it is not already equal to or greater than that number). Note that this line is prone to merge conflicts: it's likely that someone else working on a feature in parallel has also changed this line and merged to master while you were working on your feature. Pay attention when merging.
- Create new animation triggers in `deskmate/clad/src/clad/types/animationTrigger.clad`. (It's just a big ol' enum. Try to keep it in alphabetical order.)
- Edit `deskmate/resources/assets/cladToFileMaps/AnimationTriggerMap.json` to map between the animation triggers you just created and the animation groups created by the animator.
