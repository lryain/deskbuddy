...
Additional troubleshooting can be found on [Confluence](https://lryainc.atlassian.net/wiki/spaces/ATT/pages/362807304/Victor+Build+Troubleshooting).

### How do I see what's happening on my robot?

* Run `deskmate_log` from a terminal. If you would like to simultaneously save the log output to file, run `deskmate_log | tee log.txt`.
* Try using the webserver on the robot. In a browser, type `<my_deskmate_ip_address>:8888` in the address bar, where `<my_deskmate_ip_address>` is the IP address of your robot. Change the `8888` to an `8889` to see anim process information. You can open webviz, an interactive status page, by clicking the `WebViz` link on the web server landing page. More information [here](/docs/development/web-server.md).
...
### I can't connect or ping Victor, he doesn't seem to be connected to wifi
  ...
  - Use the mac client to troubleshoot wifi. Information [here](https://lryainc.atlassian.net/wiki/spaces/ATT/pages/323321886/Victor+OTA+update+using+Mac-Client+tool#VictorOSand%5COTAupdateusingMac-Clienttool-OTA) and [here](/docs/mac-client-setup.md).
...
### `deskmate_stop`/`deskmate_restart` hangs with `stopping <number>...` when trying to kill one of the processes
  - Manually kill the process with `ssh root@<robot-ip> kill -9 <number>`
  - Run `deskmate_stop` to ensure the rest of the processes are stopped
  
### One of the processes crashed/aborted and the stack trace has no symbols, how do I get symbols for the trace?

  - Manually deploy the full libraries (with symbols) to the robot; they are located in `_build/mateos/<Debug/Release>/lib/*.so.full`
  - `scp _build/mateos/<Debug/Release>/lib/<one_or_more_of_the_libraries>.so.full /lrya/lib/<one_or_more_of_the_libraries>.so`
  - Reproduce the crash and you should now see symbols
  - [Follow these detailed instructions for using the Tombstone file (with stack trace)](https://lryainc.atlassian.net/wiki/spaces/VD/pages/404652111/Victor+Crash+Reports)
  
### How do I do performance analysis/benchmarking on robot?
  - Use simpleperf to generate list of highest overhead functions in a process by running [`project/deskmate/simpleperf/HOW-simpleperf.sh`](/project/deskmate/simpleperf/HOW-simpleperf.sh)
  - If the script fails with the error: `[native_lib_dir] "./project/deskmate/simpleperf/symbol_cache" is not a dir`, cd into `./project/deskmate/simpleperf/` and run `make_symbol_cache.sh`
  - Use inferno to generate a flame graph of call hierarchies by running [`project/deskmate/simpleperf/HOW-inferno.sh`](/project/deskmate/simpleperf/HOW-inferno.sh)
  - By default both scripts run on the engine process, `cozmoengined`. Change this by prepending `LRYA_PROFILE_PROCNAME="<name_of_process>"` to the command to run the script. The other two process names are `deskmate_animator` and `robot_supervisor`
  - To see overall cpu load per process run `top -m 10`
  
### `Sending Mode Change ...` or `spine_header ...` errors continually spam in the logs
  - Ensure you have deployed correctly built code otherwise Syscon and robot process are likely incompatible, give to [Al](https://lryainc.atlassian.net/wiki/display/~Al.Chaussee) or [Kevin](https://lryainc.atlassian.net/wiki/display/~kevin) to have firmware flashed
  
### Deploying to the robot fails (possibly complains about out of disk space?)
  - You can safely remove the cache and persistent folders with `ssh <robot-ip> rm -rf /data/data/com.lrya.deskmate/cache /data/data/com.lrya.deskmate/persistent`
  ...
  - You can then inspect space usage with `du -h /lrya` which will list disk usage of all directories in `/lrya`. Most things that get deployed live in `/lrya`. Files that get generated at run-time may be stored in `/data/data/com.lrya.deskmate`.
  - You can also delete the `/lrya` folder with `rm -rf /lrya`, but in general you should never have to do. It may be useful when the you get an error like this.
...
### How do I run unit tests?
  - https://lryainc.atlassian.net/wiki/spaces/VD/pages/149363555/Victor+Unit+Tests
...
2:  
2: (t:01) [Error] TFLiteModel.LoadModelInternal.FailedToBuildFromFile /Users/arjun/Code/deskmate/_build/mac/Debug/test/engine/resources/config/engine/vision/dnn_models/dfp_deskmate_6x6_tiny_128x128_36b906234ae4405dbf479d42d87787da.tflite 
...
### I get permission denied during build `error: can't exec 'deskmate/_build/mac/Debug-Xcode/launch-c' (Permission denied)`
  - `chmod u=rwx deskmate/_build/mac/Debug-Xcode/launch-c`
  - `chmod u=rwx deskmate/_build/mac/Debug-Xcode/launch-cxx`
...
### Can't generate Xcode project using `build-deskmate.sh -p mac -g Xcode -C` and getting error output like this
```
...
### Can't generate Xcode project using `build-deskmate.sh -g Xcode -C` and getting error output like this
```
...
-- Configuring done
-- Generating done
-- Build files have been written to: /my_deskmate_dir/_build/mateos/Release
[0/1] Re-running CMake...
-- The C compiler identification is Clang 5.0.1
-- The CXX compiler identification is Clang 5.0.1
-- CMAKE_C_COMPILER=/Users/username/.lrya/mateos-sdk/dist/0.9-r03/prebuilt/bin/arm-oe-linux-gnueabi-clang
-- CMAKE_CXX_COMPILER=/Users/username/.lrya/mateos-sdk/dist/0.9-r03/prebuilt/bin/arm-oe-linux-gnueabi-clang++
```
...
### How do I decipher this crash in the deskmate log
...
12-31 16:22:17.466  2884  2884 F DEBUG   : pid: 2534, tid: 2733, name: CozmoRunner  >>> /data/data/com.lrya.cozmoengine/bin/cozmoengined <<<
...
12-31 16:22:17.512  2884  2884 F DEBUG   :     #00 pc 00287960  /data/data/com.lrya.cozmoengine/lib/libcozmo_engine.so
12-31 16:22:17.513  2884  2884 F DEBUG   :     #01 pc 002c34a7  /data/data/com.lrya.cozmoengine/lib/libcozmo_engine.so
12-31 16:22:17.513  2884  2884 F DEBUG   :     #02 pc 00273793  /data/data/com.lrya.cozmoengine/lib/libcozmo_engine.so
12-31 16:22:17.513  2884  2884 F DEBUG   :     #03 pc 002751ed  /data/data/com.lrya.cozmoengine/lib/libcozmo_engine.so
```
...
Looking at the first four lines after `backtrace:` you can see the top four addresses on the stack and the shared library they reside.
  ...
  - `deskmate_addr2line libcozmo_engine.so 00287960 002c34a7 00273793 002751ed`

to get:

```
0x00287960: .lrya/mateos-sdk/dist/0.9-r03/sysroot/usr/include/c++/v1/memorymemory:4041
0x002c34a7: projects/deskmate/_build/mateos/Release/../../../engine/aiComponent/behaviorComponent/behaviors/iCozmoBehavior.cpp:714
0x00273793: projects/deskmate/_build/mateos/Release/../../../engine/aiComponent/behaviorComponent/behaviorStack.cpp:123
0x002751ed: projects/deskmate/_build/mateos/Release/../../../engine/aiComponent/behaviorComponent/behaviorSystemManager.cpp:154
```
...
### Is this Open Source license ok for use?
  - https://lryainc.atlassian.net/wiki/spaces/ET/pages/380436502/Open+Source+Software
...
### Why did I get a 915 error?
  ...
  - There is a client/server connection between `mate-engine` and `mate-anim` through [`LocalUdpServer.cpp`](/coretech/messaging/shared/LocalUdpServer.cpp) and [`LocalUdpClient.cpp`](/coretech/messaging/shared/LocalUdpClient.cpp) and sockets `/dev/socket/_engine_anim_server_0` and `/dev/socket/_engine_anim_client_0`
  - `mate-engine` sends enough data to `mate-anim` that it fills the queue (256KB) which causes function `LocalUdpClient.Send` to get a `Resource temporarily unavailable` error from the socket which in turn disconnects. All future attempts to send from `mate-engine` fail.
  - `mate-anim` tries to send to `mate-engine` but the function `LocalUdpServer.Send` gets a `Connection refused` because it was disconnected, `mate-anim` also disconnects and issues a 915 error.
