...
def get_sdk_version_from_buckconfig(path):
    return get_property_from_buckconfig(path, 'lrya', 'android_sdk_version')
...
def get_sdk_version_from_sdk_dir(path):
    if not path:
        return None
    source_properties_path = os.path.join(path, 'lrya', 'source.properties')
    if os.path.isfile(source_properties_path):
        return get_pkg_revision_from_source_properties(source_properties_path)
    return None
...
def get_lrya_android_directory():
    lrya_android_dir = os.path.join(os.path.expanduser("~"), ".lrya", "android")
    util.File.mkdir_p(lrya_android_dir)
    return lrya_android_dir

def get_lrya_android_ndk_repository_directory():
    ndk_repo_directory_path = os.path.join(get_lrya_android_directory(), 'ndk-repository')
    util.File.mkdir_p(ndk_repo_directory_path)
    return ndk_repo_directory_path

def get_lrya_android_ndk_downloads_directory():
    ndk_download_directory_path = os.path.join(get_lrya_android_directory(), 'ndk-downloads')
    util.File.mkdir_p(ndk_download_directory_path)
    return ndk_download_directory_path

def get_lrya_android_sdk_repository_directory():
    sdk_repo_directory_path = os.path.join(get_lrya_android_directory(), 'sdk-repository')
    util.File.mkdir_p(sdk_repo_directory_path)
    return sdk_repo_directory_path

def get_lrya_android_sdk_downloads_directory():
    sdk_download_directory_path = os.path.join(get_lrya_android_directory(), 'sdk-downloads')
...
def find_ndk_root_for_ndk_version(version_arg):
    ...
    if not ndk_repo_path:
        ndk_repo_path = get_lrya_android_ndk_repository_directory()
...
def find_android_home_for_sdk_version(version):
    ...
    if not sdk_repo_path:
        sdk_repo_path = get_lrya_android_sdk_repository_directory()
...
def install_ndk(revision):
    ...
    ndk_downloads_path = get_lrya_android_ndk_downloads_directory()
...
def install_sdk(version):
    ...
    sdk_downloads_path = get_lrya_android_sdk_downloads_directory()
    ...
    sdk_base_name = "android-sdk-{0}-lrya-{1}".format(os_name, version)
