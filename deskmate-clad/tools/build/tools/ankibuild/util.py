...
class Module(object):
    ...
    def _find_module_config(config_file=None):
        ...
        env_key = 'LRYA_BUILD_MODULE_CONFIG'
        if env_key in os.environ:
            return os.environ[env_key] 

        repo_config = os.path.join(Git.repo_root(), '.lryamodules')
        if repo_config:
            return repo_config
...
class Git(object):
    ...
    def repo_root(cls):
        ...
        retcode = p.poll()
        if retcode != 0:
            env_repo_root = os.environ['LRYA_BUILD_REPO_ROOT']
...
class Profile(object):
    ...
    @staticmethod
    def begin(file, args):
        LRYA_PROFILE = os.environ.get('LRYA_PROFILE')
        if LRYA_PROFILE:
            basename = os.path.basename(file)
            if not os.path.exists(LRYA_PROFILE):
                os.makedirs(LRYA_PROFILE)
            with open(os.path.join(LRYA_PROFILE, basename+".log"), "w") \
              as profile_file:
    ...
    def end(file, args):
        ...
        if LRYA_PROFILE:
            basename = os.path.basename(file)
            with open(os.path.join(LRYA_PROFILE, basename+".log"), "a") \
              as profile_file:
