...
class FindSrc(object):
    ...
    LRYA_BUILD_PLATFORMS = ('android', 'ios', 'mac', 'linux', 'mateos', 'windows')
    LRYA_CXX_SRC_EXTS = ( '.c', '.cpp', '.cc' )
    LRYA_GO_SRC_EXTS = ( '.go', )
    LRYA_CXX_INCLUDE_EXTS = ( '.h', '.hpp', '.inl', '.def' )
    LRYA_PLATFORM_SRC_GLOBS = {
        'ios': [ '*.m', '*.mm' ],
        'mac': [ '*.m', '*.mm', '*_osx*' ]
    }
    ...
    def platform_src_globs(self, platform):
        if platform in self.LRYA_PLATFORM_SRC_GLOBS:
            return self.LRYA_PLATFORM_SRC_GLOBS[platform]
        else:
            return []
    ...
    def find_src(self, search_base_path, project_root, includes, excludes, platforms):
        ...
        for p in self.LRYA_BUILD_PLATFORMS:
...
def asset_project(name, project_root, srcs, platform_srcs=[]):
    ...
    for entry in platform_srcs:
        ...
        lst = entry[1]
        for p in FindSrc.LRYA_BUILD_PLATFORMS:
            if regex.match(p):
                p_srcs = []
                p_dsts = []
...
def cxx_header_glob(search_base, include_paths, includes=[], excludes=[], platform=None):
    return cxx_glob(search_base,
                    include_paths,
                    FindSrc.LRYA_CXX_INCLUDE_EXTS,
                    includes,
                    excludes,
                    [platform] if platform else [])

def all_glob(search_base, include_paths, includes=[], excludes=[], platform=None):
    exts = FindSrc.LRYA_CXX_INCLUDE_EXTS + FindSrc.LRYA_CXX_SRC_EXTS + FindSrc.LRYA_GO_SRC_EXTS
    return cxx_glob(search_base,
                    include_paths,
                    exts,
                    includes,
