...
def build(output_dir, useCpp, useCsharp, usePython):
    global verbose
    process = ["cmake", "-G", "Ninja", "-DCLAD_DESKMATE_SKIP_LICENSE=ON"]
    process.append("-DCLAD_DESKMATE_EMIT_CPP={}".format("ON" if useCpp else "OFF"))
    process.append("-DCLAD_DESKMATE_EMIT_CSHARP={}".format("ON" if useCsharp else "OFF"))
    process.append("-DCLAD_DESKMATE_EMIT_PYTHON={}".format("ON" if usePython else "OFF"))
    process.append("..")
    if verbose:
        print("Running {}".format(process))
    return call(process)
