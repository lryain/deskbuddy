...
def _modify_path():
    message_buffers_path = os.path.join(os.path.dirname(__file__), '..', '..', 'deskmate-clad', 'tools', 'message-buffers')
...
class SendHelperHeaderEmitter(ast.NodeVisitor):
    ...
    def visit_UnionDecl(self, node):
        for member in node.members():
            self.output.write('inline bool SendMessage(const {member_type}& msg) {{ return Lrya::Vector::HAL::RadioSendMessage(msg.GetBuffer(), msg.Size(), {member_tag}); }}\n'.format(
