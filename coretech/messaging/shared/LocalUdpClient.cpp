...
#ifdef MATEOS
// todo: restore logging when mateos toolchain is available -PRA (approved by BRC)
#undef LOG_CHANNEL
#endif
...
{
  if (_socket >= 0) {
    return Lrya::Messaging::GetIncomingSize(_socket);
  }
  return -1;
}
...
{
  if (_socket >= 0) {
    return Lrya::Messaging::GetOutgoingSize(_socket);
  }
  return -1;
}
