...
namespace Vision {
  ...
  Result FaceTracker::Impl::SetFacePoseFromParts(const s32 nrows, const s32 ncols, TrackedFace& face, f32& intraEyeDist)
  {
    ...
    if(!LRYA_VERIFY(_camera.IsCalibrated(), "FaceTrackerImpl.SetFacePoseFromParts.CameraNotCalibrated", ""))
    {
      return RESULT_FAIL;
    }
    ...
    if(!IsFirstPointOffsetSet(kFirstPointOffset))
    {
      ...
      if(LRYA_DEV_CHEATS && IsFirstPointOffsetSet(kFirstPointOffset))
      {
        ...
        {
          if(!LRYA_VERIFY((*pt == _facialParts[i].x) && (*(pt+1) == _facialParts[i].y),
    ...
    if(!LRYA_VERIFY(IsFirstPointOffsetSet(kFirstPointOffset),
    ...
    // and is different than the lrya coordindate system. Specifically the x-axis points
    ...
    // to the lrya coordindate system.
