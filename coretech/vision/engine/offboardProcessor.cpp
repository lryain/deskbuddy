...
#ifdef MATEOS
    // Right now we only support mateos for offboard vision per the implementation
    // in mate-cloud, if you're running on mac ... there isn't really a use case for
    // offboard ... yet.
...
Result OffboardProcessor::DetectWithCLAD(const ImageRGB& img, std::list<SalientPoint>& salientPoints)
{
  ...
  if(!LRYA_VERIFY(packedSize == expectedSize, "OffboardProcessor.DetectWithCLAD.MessageSizeMismatch",
  ...
  if(!LRYA_VERIFY(sentSize == packedSize,
...
{
  if(LRYA_VERIFY(nullptr != _udpClient, "OffboardProcessor.IsConnected.NoUdpClient", "UDP client should be instantiated"))
  {
