...
bool FaceRecognizer::HasName(TrackingID_t forTrackingID) const
{
  ...
  if(!LRYA_VERIFY(faceID != UnknownFaceID, "FaceRecognizer.HasName.TrackedFaceWithUnknownID", ""))
  {
    return false;
  }
  ...
  if(!LRYA_VERIFY(enrollIter != _enrollmentData.end(), "FaceRecognizer.HasName.TrackedFaceWithNoEnrollData", ""))
  {
    return false;
  }
...
{
  Lrya::Util::SetThreadName(pthread_self(), "FaceRecognizer");
...
void FaceRecognizer::SetAllowedEnrollments(s32 N, FaceID_t forFaceID, bool forceNewID)
{
  ...
  if(forceNewID)
  {
    ...
    if(LRYA_VERIFY(_enrollmentTrackID != UnknownFaceID,
                                     ...
                                     DebugImageList<CompressedImage>& debugImages)
{
     ...
     _detectionInfo.nID == _enrollmentTrackID)
  {
    ...
    else
    {
      ...
      auto enrollDataIter = _enrollmentData.find(_enrollmentID);
      if(LRYA_VERIFY(enrollDataIter != _enrollmentData.end(),
