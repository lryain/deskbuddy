...
Result IAsyncRunner::Init(const std::string& cachePath, const Json::Value& config)
{
  ...
  if(LRYA_VERIFY(RESULT_OK == result, "IAsyncRunner.Init.InitInternalFailed", ""))
  {
    _isInitialized = true;
  }
...
bool IAsyncRunner::StartProcessingIfIdle(const Vision::ImageRGB& img)
{
  if(!_isInitialized)
  {
    ...
    //  2. Git LFS has failed you. See: https://lryainc.atlassian.net/browse/VIC-13455
