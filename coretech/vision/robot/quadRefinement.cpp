...
static Lrya::Embedded::Matlab matlab(false);
#endif
...
namespace Lrya {
  namespace Embedded {
      ...
      MemoryStack scratch)
    {
      ...
      LryaConditionalErrorAndReturnValue(AreEqualSize(3, 3, refinedHomography, initialHomography),
        RESULT_FAIL_INVALID_SIZE,
        "RefineQuadrilateral",
        "Input and Output refined homography array must be valid and 3x3.");

      LryaConditionalErrorAndReturnValue(NotAliased(initialHomography, refinedHomography),
      ...
      LryaConditionalErrorAndReturnValue(xSquare.IsValid(), RESULT_FAIL_MEMORY,
        "RefineQuadrilateral",
        "Failed to allocate xSquare Array.");

      LryaConditionalErrorAndReturnValue(ySquare.IsValid(), RESULT_FAIL_MEMORY,
        "RefineQuadrilateral",
        "Failed to allocate ySquare Array.");

      LryaConditionalErrorAndReturnValue(Tx.IsValid(), RESULT_FAIL_MEMORY,
        "RefineQuadrilateral",
        "Failed to allocate Tx Array.");

      LryaConditionalErrorAndReturnValue(Ty.IsValid(), RESULT_FAIL_MEMORY,
        "RefineQuadrilateral",
      ...
      */
      ...
      LryaConditionalErrorAndReturnValue(A.IsValid(), RESULT_FAIL_MEMORY,
        "RefineQuadrilateral",
...
#     endif
      ...
      LryaConditionalErrorAndReturnValue(AWAt.IsValid(), RESULT_FAIL_MEMORY,
        "RefineQuadrilateral",
        "Failed to allocate AWAt matrix.");

      LryaConditionalErrorAndReturnValue(b.IsValid(), RESULT_FAIL_MEMORY,
        "RefineQuadrilateral",
        "Failed to allocate b vector.");

      LryaConditionalErrorAndReturnValue(homographyUpdate.IsValid(), RESULT_FAIL_MEMORY,
        "RefineQuadrilateral",
        "Failed to allocate homographyUpdate matrix.");

      LryaConditionalErrorAndReturnValue(newHomography.IsValid(), RESULT_FAIL_MEMORY,
        "RefineQuadrilateral",
...
#         endif // #if ACCELERATION_TYPE == ACCELERATION_NONE ... #else
        ...
        if(numericalFailure){
          LryaWarn("RefineQuadrilateral", "numericalFailure");
        ...
        if(currentCornerChange < minCornerChange) {
          ...
          //LryaWarn("QuadRefinement.CornersConverged",
...
#     endif
      ...
      */
      ...
      if(restoreOriginal) {
#       if DEBUG_QUAD_REFINEMENT
        LryaWarn("RefineQuadrilateral", "Restoring original quad");
#       endif
...
#         if DEBUG_QUAD_REFINEMENT
          LryaWarn("RefineQuadrilateral", "Quad changed too much");
#         endif
