  ...
  namespace Embedded
  {
    namespace TemplateTracker
    {
        ...
        : baseImageWidth(templateImage.get_size(1)), baseImageHeight(templateImage.get_size(0)), numPyramidLevels(numPyramidLevels), templateImageHeight(templateImage.get_size(0)), templateImageWidth(templateImage.get_size(1)), ridgeWeight(ridgeWeight), isValid(false), isInitialized(false)
      {
        ...
        LryaConditionalErrorAndReturn(templateImageHeight > 0 && templateImageWidth > 0,
          "LucasKanadeTracker_Slow::LucasKanadeTracker_Slow", "template widths and heights must be greater than zero, and multiples of %d", LRYA_VISION_IMAGE_WIDTH_MULTIPLE);

        LryaConditionalErrorAndReturn(numPyramidLevels > 0,
          "LucasKanadeTracker_Slow::LucasKanadeTracker_Slow", "numPyramidLevels must be greater than zero");

        LryaConditionalErrorAndReturn(transformType==Transformations::TRANSFORM_TRANSLATION || transformType == Transformations::TRANSFORM_AFFINE || transformType==Transformations::TRANSFORM_PROJECTIVE,
        ...
        LryaConditionalErrorAndReturn(ridgeWeight >= 0.0f,
        ...
        LryaConditionalErrorAndReturn(((1<<initialImagePowerS32)*templateImage.get_size(1)) == baseImageWidth,
        ...
        for(s32 i=1; i<(numPyramidLevels-1); i++) {
          ...
          LryaConditionalErrorAndReturn(!IsOdd(curTemplateHeight) && !IsOdd(curTemplateWidth),
        ...
        LryaConditionalErrorAndReturn(A_full.IsValid(),
        ...
        LryaConditionalErrorAndReturn(templateCoordinates.IsValid(),
        ...
        LryaConditionalErrorAndReturn(templateImagePyramid.IsValid(),
        ...
        LryaConditionalErrorAndReturn(templateWeights.IsValid(),
      ...
      Result LucasKanadeTracker_Slow::InitializeTemplate(const Array<u8> &templateImage, MemoryStack &memory)
      {
        ...
        LryaConditionalErrorAndReturnValue(this->isInitialized == false,
          RESULT_FAIL, "LucasKanadeTracker_Slow::InitializeTemplate", "This object has already been initialized");

        LryaConditionalErrorAndReturnValue(templateImageHeight == templateImage.get_size(0) && templateImageWidth == templateImage.get_size(1),
          RESULT_FAIL_INVALID_SIZE, "LucasKanadeTracker_Slow::InitializeTemplate", "template size doesn't match constructor");

        LryaConditionalErrorAndReturnValue(templateRegion.left < templateRegion.right && templateRegion.left >=0 && templateRegion.right < templateImage.get_size(1) &&
        ...
        for(s32 iScale=0; iScale<this->numPyramidLevels; iScale++) {
          ...
          LryaConditionalErrorAndReturnValue(this->A_full[iScale].IsValid(),
          ...
          LryaConditionalErrorAndReturnValue(this->templateImagePyramid[iScale].IsValid(),
          ...
          LryaConditionalErrorAndReturnValue(this->templateWeights[iScale].IsValid(),
        ...
        // Everything below here is temporary
        {
          ...
          for(s32 iScale=0; iScale<this->numPyramidLevels; iScale++) {
            ...
            LryaAssert(AreEqualSize(xIn, yIn));
      ...
      Result LucasKanadeTracker_Slow::IterativelyRefineTrack(const Array<u8> &nextImage, const s32 maxIterations, const s32 whichScale, const f32 convergenceTolerance, const Transformations::TransformType curTransformType, const bool useWeights, bool &verify_converged, MemoryStack scratch)
      {
        ...
        LryaConditionalErrorAndReturnValue(this->isInitialized == true,
        ...
        for(s32 iteration=0; iteration<maxIterations; iteration++) {
          ...
          if(numInBounds < 16) {
            LryaWarn("LucasKanadeTracker_Slow::IterativelyRefineTrack", "Template drifted too far out of image.");
            break;
          }
          ...
          if(numericalFailure){
            LryaWarn("LucasKanadeTracker_Slow::IterativelyRefineTrack", "numericalFailure");
            return RESULT_OK;
          }
