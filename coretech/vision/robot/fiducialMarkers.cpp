...
#   endif
    ...
    FiducialMarkerParserBit::FiducialMarkerParserBit(const FiducialMarkerParserBit& bit2)
    {
      ...
      LryaAssert(bit2.probeLocations.get_size() == bit2.probeWeights.get_size());
    ...
    {
      LryaAssert(numProbes <= MAX_FIDUCIAL_MARKER_BIT_PROBE_LOCATIONS);
    ...
    Result FiducialMarkerParserBit::ExtractMeanValue(const Array<u8> &image, const Quadrilateral<s16> &quad, const Array<f32> &homography, s16 &meanValue) const
    {
      ...
      for(s32 probe=0; probe<numProbeLocations; probe++) {
        ...
        LryaAssert(warpedY >= 0  && warpedX >= 0 && warpedY < image.get_size(0) && warpedX < image.get_size(1)); // Verify y against image height and x against width
    ...
    Result FiducialMarkerParser::InitializeAsDefaultParser(MemoryStack &memory)
    {
      if(INITIALIZE_WITH_DEFINITION_TYPE == 0) {
        LryaAssert(NUM_BITS_TYPE_0 <= MAX_FIDUCIAL_MARKER_BITS);
        LryaAssert(NUM_PROBES_PER_BIT_TYPE_0 <= MAX_FIDUCIAL_MARKER_BIT_PROBE_LOCATIONS);
      ...
      LryaAssert(upBitIndex >= 0 && downBitIndex >= 0 && leftBitIndex >= 0 && rightBitIndex >= 0);
    ...
    {
      LryaConditionalErrorAndReturnValue(meanValues.IsValid(),
        RESULT_FAIL_INVALID_OBJECT, "FiducialMarkerParser::DetermineOrientation", "meanValues is not valid");

      LryaConditionalErrorAndReturnValue(binarizedBits.IsValid(),
      ...
      LryaAssert(meanValues.get_size() == NUM_BITS);
    ...
    {
      LryaConditionalErrorAndReturnValue(startIndex >= 0,
    ...
    , markerType(Lrya::Vision::MARKER_UNKNOWN)
...
#   elif RECOGNITION_METHOD == RECOGNITION_METHOD_CNN
    ...
    Vision::ConvolutionalNeuralNet& VisionMarker::GetCNN()
    {
      ...
      if(!cnn.IsLoaded()) {
        LryaConditionalErrorAndReturnValue(_dataPath.empty() == false, cnn,
        ...
        LryaConditionalError(loadResult == RESULT_OK,
...
#     endif // #if RECOGNITION_METHOD == ?
    ...
    {
      LryaConditionalErrorAndReturnValue(AreValid(image, homography),
      ...
      LryaAssert(VisionMarker::NUM_PROBE_POINTS <= 9);
      ...
      LryaConditionalErrorAndReturnValue(probeValues.rows*probeValues.cols == VisionMarker::NUM_PROBES,
      ...
      for(s32 iProbe=0; iProbe<numProbes; ++iProbe)
      {
        ...
        for(s32 iOffset=0; iOffset<numProbeOffsets; iOffset++) {
          ...
          // This should only fail if there's a bug in the quad extraction
          LryaAssert(warpedY >= 0  && warpedX >= 0 && warpedY < image.get_size(0) && warpedX < image.get_size(1));
      ...
      /*
       ...
       LryaConditionalErrorAndReturnValue(maxValue > minValue, RESULT_FAIL,
...
#     endif
      ...
      LryaAssert(VisionMarker::NUM_FRACTIONAL_BITS >= 0);
      ...
      for(s32 i_probe=0; i_probe<NUM_THRESHOLD_PROBES; ++i_probe) {
        ...
        for(s32 i_pt=0; i_pt<NUM_PROBE_POINTS; i_pt++) {
          { // Dark points
            ...
            // This should only fail if there's a bug in the quad extraction
            LryaAssert(warpedY >= 0  && warpedX >= 0 && warpedY < imageHeight && warpedX < imageWidth);
      ...
      if(enoughContrast) {
        ...
        if(refine_quadRefinementIterations > 0) {
            ...
            scratch)) != RESULT_OK)
          {
            ...
            LryaConditionalErrorAndReturnValue(lastResult == RESULT_OK,
        ...
        LryaWarn("VisionMarker::Extract", "Poor contrast marker detected");
        #endif
        */
...
#     endif
      ...
      //LryaConditionalErrorAndReturnValue(this->validity == VisionMarker::UNKNOWN,
...
#     elif RECOGNITION_METHOD == RECOGNITION_METHOD_DECISION_TREES
      ...
      LryaAssert(NUM_TREES <= std::numeric_limits<u8>::max());
      ...
      for(s32 iTree=0; iTree < NUM_TREES; ++iTree) {
        ...
        LryaAssert(tempLabel < NUM_MARKER_LABELS_ORIENTED);
        LryaAssert(tempLabel >= 0);
      ...
      LryaConditionalErrorAndReturnValue(maxVotes>0, RESULT_FAIL, "VisionMarker.Extract.NoVotes", "No votes given to any marker label")
      LryaConditionalErrorAndReturnValue(selectedLabel>=0, RESULT_FAIL, "VisionMarker.Extract.NoBestLabel", "No label with max votes selected");
...
#       ifdef OUTPUT_FAILED_MARKER_STEPS
        LryaWarn("VisionMarker::Extract", "Verify failed or UNKNOWN/INVALID marker detected");
#       endif
         */
    ...
    Lrya::Vision::MarkerType LookupMarkerType(const char * name)
    {
#if LRYACORETECH_EMBEDDED_USE_OPENCV
      ...
      if(nameLength >= MAX_NAME_LENGTH) {
        return Lrya::Vision::MARKER_UNKNOWN;
      }
      ...
      for(s32 i=0; i<Lrya::Vision::NUM_MARKER_TYPES; i++) {
        const char * withoutPrefixName = Lrya::Vision::MarkerTypeStrings[i] + 7;
        if(strcmp(withoutPrefixName, upperCaseName) == 0) {
          return static_cast<Lrya::Vision::MarkerType>(i);
        }
      }
      ...
      : isValid(false)
    {
...

      {
        ...
        LryaConditionalErrorAndReturn(databaseImageWidth == databaseImageHeight,
      ...
      databaseLabelIndexes = FixedLengthList<Lrya::Vision::MarkerType>(numDatabaseImages, memory, Flags::Buffer(false, false, true));
      ...
      for(s32 iFile=0; iFile<numDatabaseImages; iFile++) {
        ...
        LryaConditionalErrorAndReturn(image.rows == databaseImageHeight && image.rows == databaseImageWidth,
          "VisionMarkerImages::VisionMarkerImages", "All images must be equal size and square");
        ...
        for(s32 y=0; y<image.rows; y++) {
          ...
          if(image.step.buf[1] == 1) {
            LryaAssert(false); // TODO: implement
...
#else
      LryaError("VisionMarkerImages::VisionMarkerImages", "OpenCV is required to load files");
#endif // #if LRYACORETECH_EMBEDDED_USE_OPENCV ... #else

      this->isValid = true;
    }

    VisionMarkerImages::VisionMarkerImages(const s32 numDatabaseImages, const s32 databaseImageHeight, const s32 databaseImageWidth, u8 * pDatabaseImages, Lrya::Vision::MarkerType * pDatabaseLabelIndexes)
      : numDatabaseImages(numDatabaseImages), databaseImageHeight(databaseImageHeight), databaseImageWidth(databaseImageWidth), isValid(true)
    {
      ...
      this->databaseLabelIndexes = FixedLengthList<Lrya::Vision::MarkerType>(numDatabaseImages, pDatabaseLabelIndexes, 2*numDatabaseImages*sizeof(Lrya::Vision::MarkerType), Flags::Buffer(false, false, true));
    }
    ...
    Result VisionMarkerImages::MatchExhaustive(const Array<u8> &image, const Quadrilateral<f32> &quad, VisionMarker &extractedMarker, f32 &matchQuality, MemoryStack fastScratch, MemoryStack slowScratch) const
    {
      ...
      LryaAssert(databaseImageWidth == databaseImageHeight);

      LryaConditionalErrorAndReturnValue(NotAliased(fastScratch, slowScratch),
      ...
      LryaConditionalErrorAndReturnValue(homography.IsValid(),
      ...
      LryaConditionalErrorAndReturnValue(toShowImages.IsValid(),
        RESULT_FAIL_OUT_OF_MEMORY, "VisionMarkerImages::MatchExhaustive", "Out of memory");
      ...
      for(s32 iRotation=0; iRotation<4; iRotation++) {
        for(s32 iDatabase=0; iDatabase<numDatabaseImages; iDatabase++) {
          ...
          LryaConditionalErrorAndReturnValue(toShowImages[iRotation][iDatabase].IsValid(),
            RESULT_FAIL_OUT_OF_MEMORY, "VisionMarkerImages::MatchExhaustive", "Out of memory");
      ...
      LryaAssert(totalDifferences != NULL);
      ...
      //LryaAssert(pDatabaseImages != NULL);
      ...
      LryaAssert(fastScratch.IsValid());
    ...
    const FixedLengthList<Lrya::Vision::MarkerType>& VisionMarkerImages::get_databaseLabelIndexes()
    {
