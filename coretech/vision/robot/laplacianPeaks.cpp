...
#     endif
    ...
    Result ExtractLineFitsPeaks(const FixedLengthList<Point<s16>> &boundary, 
                                ...
                                MemoryStack scratch)
    {
      LryaConditionalErrorAndReturnValue(boundary.IsValid(),
        RESULT_FAIL_INVALID_OBJECT, "ComputeQuadrilateralsFromConnectedComponents", "boundary is not valid");

      LryaConditionalErrorAndReturnValue(peaks.IsValid(),
      ...
      LryaConditionalErrorAndReturnValue(peaks.get_maximumSize() == 4,
