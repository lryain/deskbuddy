...
namespace Lrya
{
  namespace Embedded
  {
                                            ...
                                            cv::Mat_<u8>& cvImageROI_orig)  // stores a copy of the original data inside the ROI
    {
      ...
      LryaConditionalErrorAndReturnValue(cvImageROI.empty() == false, RESULT_FAIL_INVALID_SIZE,
    ...
    {
      LryaAssert(params.fiducialThicknessFraction.x > 0 && params.fiducialThicknessFraction.y > 0);
      ...
      LryaConditionalErrorAndReturnValue(image.IsValid() && markers.IsValid() && scratchOffChip.IsValid() && scratchOnchip.IsValid() && scratchCcm.IsValid(),
      ...
      if(scratchOffChip.get_totalBytes() < 1000000 && scratchOnchip.get_totalBytes() < 1000000 && scratchCcm.get_totalBytes() < 1000000) {
        LryaConditionalErrorAndReturnValue(imageHeight <= 240 && imageWidth <= 320,
          RESULT_FAIL_INVALID_SIZE, "DetectFiducialMarkers", "The image is too large to test");

        LryaConditionalErrorAndReturnValue(params.scaleImage_numPyramidLevels <= 3,
      ...
      LryaConditionalErrorAndReturnValue(extractedComponents.IsValid(),
      ...
      if(params.useIntegralImageFiltering) {
        ...
        LryaConditionalErrorAndReturnValue(filterHalfWidths.IsValid(),
      ...
      {
        Lrya::Embedded::Matlab matlab(false);
        ...
        for(s32 iQuad=0; iQuad<extractedQuads.get_size(); iQuad++) {
          ...
          if(numericalFailure) {
            ...
            if(currentHomography[2][0] > maxProjectiveTermValue || currentHomography[2][1] > maxProjectiveTermValue) {
              LryaWarn("DetectFiducialMarkers", "Homography projective terms are unreasonably large");
              currentMarker.validity = VisionMarker::NUMERICAL_FAILURE;
            }
          }
      ...
      for(s32 iMarker=0; iMarker<markers.get_size(); iMarker++)
      {
        ...
        if(params.useIlluminationNormalization)
        {
          ...
          if(lastResult != RESULT_OK) {
            LryaWarn("DetectFiducialMarkers", "Illumination normalization failed, skipping marker %d", iMarker);
            continue;
          }
        }
...
#                                                endif
              ...
              LryaConditionalWarn(lastResult == RESULT_OK, "DetectFiducialMarkers",
