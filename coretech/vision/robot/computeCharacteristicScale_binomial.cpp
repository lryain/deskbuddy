...
namespace Lrya
{
  namespace Embedded
  {
      ...
      MemoryStack slowestScratch)
    {
      ...
      LryaConditionalErrorAndReturnValue(AreValid(image, components),
      ...
      LryaConditionalErrorAndReturnValue(AreValid(imagePyramid, blurredImagePyramid, dogPyramid, dogMax, scaleImage),
      ...
      for(s32 iLevel=0; iLevel<numPyramidLevels; iLevel++) {
        ...
        LryaConditionalErrorAndReturnValue(AreValid(imagePyramid[iLevel], blurredImagePyramid[iLevel], dogPyramid[iLevel]),
          RESULT_FAIL_OUT_OF_MEMORY, "ExtractComponentsViaCharacteristicScale_binomial", "Out of memory");
      }

      Array<u8> binaryImageRow(1, imageWidth, fastScratch);

      LryaConditionalErrorAndReturnValue(AreValid(binaryImageRow),
