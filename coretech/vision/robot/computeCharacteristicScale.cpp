...
Lrya::Embedded::Array<u8> g_binaryImage;
...
namespace Lrya
{
  namespace Embedded
  {
    ...
    NO_INLINE void ecvcs_filterRows(const ScrollingIntegralImage_u8_s32 &integralImage, const FixedLengthList<s32> &filterHalfWidths, const s32 imageY, FixedLengthList<Array<u8> > &filteredRows)
    {
      ...
      LryaAssert(numFilterHalfWidths == filteredRows.get_size());
    ...
    NO_INLINE void ecvcs_computeBinaryImage_numFilters3(const Array<u8> &image, FixedLengthList<Array<u8> > &filteredRows, 
                                                        ...
                                                        const bool isDarkOnLight)
    {
      LryaAssert(filteredRows.get_size() == 3);
      ...
      LryaAssert(filteredRows.get_size() <= MAX_FILTER_HALF_WIDTH);
    ...
    {
      LryaAssert(filteredRows.get_size() == 5);
    ...
    {
      LryaAssert(filteredRows.get_size() == 5);
      LryaAssert(scaleImage_thresholdMultiplier == 65536);
      ...
      MemoryStack slowestScratch)
    {
      ...
      LryaConditionalErrorAndReturnValue(AreValid(fastScratch, slowerScratch, slowestScratch),
        RESULT_FAIL_INVALID_OBJECT, "ExtractComponentsViaCharacteristicScale", "scratch is not valid");

      LryaConditionalErrorAndReturnValue(NotAliased(fastScratch, slowerScratch, slowestScratch),
      ...
      LryaConditionalErrorAndReturnValue(AreValid(image, filterHalfWidths, components),
        RESULT_FAIL_INVALID_OBJECT, "ExtractComponentsViaCharacteristicScale", "Invalid objects");

      LryaConditionalErrorAndReturnValue(numFilterHalfWidths > 0 && numFilterHalfWidths <= MAX_FILTER_HALF_WIDTH,
      ...
      for(s32 i=0; i<numFilterHalfWidths; i++) {
        LryaAssert(pFilterHalfWidths[i] >= 0);
        LryaAssert(pFilterHalfWidths[i] <= MAX_FILTER_HALF_WIDTH);
      ...
      LryaConditionalErrorAndReturnValue(filteredRows.IsValid(),
      ...
      for(s32 i=0; i<numFilterHalfWidths; i++) {
        ...
        LryaConditionalErrorAndReturnValue(filteredRows[i].IsValid(),
          RESULT_FAIL_OUT_OF_MEMORY, "ExtractComponentsViaCharacteristicScale", "filteredRows is not valid");
      }

      Array<u8> binaryImageRow(1, imageWidth, fastScratch);

      LryaConditionalErrorAndReturnValue(binaryImageRow.IsValid(),
      ...
      if(!g_saveBinaryInitialized) {
        ...
        g_binaryImage = Lrya::Embedded::Array<u8>(image.get_size(0), image.get_size(1), tmpMemory);
