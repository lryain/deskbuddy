...
namespace Lrya
{
  namespace Embedded
  {
    ...
    {
      LryaConditionalErrorAndReturnValue(AreValid(image, edgeLists.xDecreasing, edgeLists.xIncreasing, edgeLists.yDecreasing, edgeLists.yIncreasing),
        RESULT_FAIL_INVALID_OBJECT, "DetectBlurredEdges", "Invalid objects");

      LryaConditionalErrorAndReturnValue(minComponentWidth > 0,
        RESULT_FAIL_INVALID_SIZE, "DetectBlurredEdges", "minComponentWidth is too small");

      LryaConditionalErrorAndReturnValue(edgeLists.xDecreasing.get_maximumSize() == edgeLists.xIncreasing.get_maximumSize() &&
    ...
    Result DetectBlurredEdges_DerivativeThreshold(const Array<u8> &image, const Rectangle<s32> &imageRegionOfInterest, const s32 combHalfWidth, const s32 combResponseThreshold, const s32 everyNLines, EdgeLists &edgeLists)
    {
      ...
      LryaConditionalErrorAndReturnValue(combHalfWidth > 0 && combResponseThreshold >= 0,
    ...
    {
      LryaConditionalErrorAndReturnValue(this->imageHeight > 16 && this->imageHeight < 2000 && this->imageWidth > 16 && this->imageWidth < 2000,
