...
#ifndef _LRYACORETECHEMBEDDED_VISION_IMAGE_PROCESSING_H_
#define _LRYACORETECHEMBEDDED_VISION_IMAGE_PROCESSING_H_
  ...
  namespace Embedded
  {
    namespace ImageProcessing
    {
      ...
      template<typename InType, typename IntermediateType, typename OutType> Result ComputeXGradient(const Array<InType> &in, Array<OutType> &out)
      {
        ...
        LryaConditionalErrorAndReturnValue(AreValid(in, out),
          RESULT_FAIL_INVALID_OBJECT, "ComputeXGradient", "Invalid objects");

        LryaConditionalErrorAndReturnValue(AreEqualSize(in, out),
      ...
      template<typename InType, typename IntermediateType, typename OutType> Result BinomialFilter(const Array<InType> &image, Array<OutType> &imageFiltered, MemoryStack scratch)
      {
        ...
        LryaConditionalErrorAndReturnValue(AreValid(image, imageFiltered, scratch),
          RESULT_FAIL_INVALID_OBJECT, "BinomialFilter", "Invalid objects");

        LryaConditionalErrorAndReturnValue(imageHeight == imageFiltered.get_size(0) && imageWidth == imageFiltered.get_size(1),
        ...
        LryaConditionalErrorAndReturnValue(NotAliased(image, imageFiltered),
        ...
        LryaAssert(imageFilteredTmp.get_stride() % sizeof(IntermediateType) == 0);
      ...
      {
        LryaConditionalErrorAndReturnValue(AreValid(image, integralImage),
        ...
        LryaConditionalErrorAndReturnValue(AreEqualSize(image, integralImage),
      ...
      {
        LryaConditionalErrorAndReturnValue(AreValid(image, filtered),
          RESULT_FAIL_INVALID_OBJECT, "BoxFilter", "Image is invalid");

        LryaConditionalErrorAndReturnValue(NotAliased(image, filtered),
        ...
        LryaConditionalErrorAndReturnValue(AreEqualSize(image, filtered),
          RESULT_FAIL_INVALID_SIZE, "BoxFilter", "Output normalized image must match input image's size.");

        LryaConditionalErrorAndReturnValue(boxHeight > 2 && boxWidth > 2 && IsOdd(boxWidth) && IsOdd(boxHeight),
      ...
      template<typename InType, typename IntermediateType, typename OutType> Result DownsampleByTwo(const Array<InType> &in, Array<OutType> &out)
      {
        ...
        LryaConditionalErrorAndReturnValue(AreEqualSize(inHeight / 2, inWidth / 2, out),
      ...
      template<typename InType, typename IntermediateType, typename OutType> Result DownsampleByPowerOfTwo(const Array<InType> &in, const s32 downsamplePower, Array<OutType> &out, MemoryStack scratch)
      {
        ...
        LryaConditionalErrorAndReturnValue(AreValid(in, out , scratch),
          RESULT_FAIL_INVALID_OBJECT, "DownsampleByFactor", "Invalid objects");

        LryaConditionalErrorAndReturnValue(AreEqualSize(smallHeight, smallWidth, out),
          RESULT_FAIL_INVALID_SIZE, "DownsampleByFactor", "size(out) is not equal to size(in) >> downsampleFactor");

        LryaConditionalErrorAndReturnValue(largeWidth % 4 == 0,
        ...
        for(s32 ySmall=0; ySmall<smallHeight; ySmall++) {
          ...
          for(s32 yp=0; yp<downsampleFactor; yp++) {
            ...
            for(s32 i=0; i<numWordsToCopy; i++) {
              LryaAssert(reinterpret_cast<size_t>(pIn) % 4 == 0);
        ...
        MemoryStack &memory)  //< Memory for the output will be allocated by this function
      {
        ...
        LryaConditionalErrorAndReturnValue(image.IsValid() && numPyramidLevels >= 0,
        ...
        for(s32 iLevel=1; iLevel<numPyramidLevels; iLevel++) {
          ...
          LryaConditionalErrorAndReturnValue(scaledHeight%2 == 0 && scaledWidth%2 == 0,
        ...
        LryaConditionalErrorAndReturnValue(pyramid.IsValid(),
        ...
        for(s32 iLevel=1; iLevel<=numPyramidLevels; iLevel++) {
          ...
          LryaConditionalErrorAndReturnValue(pyramid[iLevel].IsValid(),
            FixedLengthList<Array<OutType> >(), "BuildPyramid", "Out of memory");
      ...
      template<typename InType, typename IntermediateType, typename OutType> NO_INLINE Result Correlate1d(const FixedPointArray<InType> &in1, const FixedPointArray<InType> &in2, FixedPointArray<OutType> &out)
      {
        ...
        LryaConditionalErrorAndReturnValue(AreValid(in1, in2, out),
          RESULT_FAIL_INVALID_OBJECT, "Correlate1d", "Invalid objects");

        LryaConditionalErrorAndReturnValue(in1.get_size(0) == 1 && in2.get_size(0) == 1 && out.get_size(0) == 1,
          RESULT_FAIL_INVALID_SIZE, "Correlate1d", "Arrays must be 1d and horizontal");

        LryaConditionalErrorAndReturnValue(out.get_size(1) == outputLength,
          RESULT_FAIL_INVALID_SIZE, "Correlate1d", "Out must be the size of in1 + in2 - 1");

        LryaConditionalErrorAndReturnValue(NotAliased(in1, in2, out),
      ...
      {
        LryaConditionalErrorAndReturnValue(AreValid(in, points),
          RESULT_FAIL_INVALID_OBJECT, "LocalMaxima", "Invalid inputs");

        points.Clear();

        if(values) {
          LryaConditionalErrorAndReturnValue(values->IsValid(),
            RESULT_FAIL_INVALID_OBJECT, "LocalMaxima", "Invalid inputs");
        ...
        LryaConditionalErrorAndReturnValue(searchHeight == 3 && searchWidth == 3,
      ...
      template<int upsamplePower> NO_INLINE Result UpsampleByPowerOfTwoBilinear(const Array<u8> &in, Array<u8> &out, MemoryStack scratch)
      {
        ...
        LryaConditionalErrorAndReturnValue(AreEqualSize(smallHeight, smallWidth, in),
        ...
        LryaConditionalErrorAndReturnValue(upsamplePower > 0 && upsamplePower < 8,
...
#endif // _LRYACORETECHEMBEDDED_VISION_IMAGE_PROCESSING_H_
