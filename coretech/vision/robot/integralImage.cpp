...
namespace Lrya
{
  namespace Embedded
  {
    ...
    {
      LryaConditionalErrorAndReturn(this->IsValid(),
    ...
    {
      //LryaAssert(imageWidth%LRYA_VISION_IMAGE_WIDTH_SHIFT == 0);

      if(numBorderPixels < 0 || numBorderPixels > bufferHeight || numBorderPixels > imageWidth) {
        LryaError("Lrya.ScrollingIntegralImage_u8_s32.ScrollingIntegralImage_u8_s32", "numBorderPixels must be greater than or equal to zero, and less than the size of this ScrollingIntegralImage.");
        InvalidateArray();
        return;
      }
    }
    ...
    Result ScrollingIntegralImage_u8_s32::ScrollDown(const Array<u8> &image, s32 numRowsToScroll, MemoryStack scratch)
    {
      ...
      //LryaAssert(imageWidth%LRYA_VISION_IMAGE_WIDTH_SHIFT == 0);
      LryaAssert(this->imageWidth == image.get_size(1));

      LryaConditionalErrorAndReturnValue(numRowsToScroll > 0 && numRowsToScroll <= integralImageHeight,
    ...
    Result ScrollingIntegralImage_u8_s32::PadImageRow(const Array<u8> &image, const s32 whichRow, Array<u8> &paddedRow)
    {
      ...
      LryaAssert(image.IsValid());
      LryaAssert(paddedRow.IsValid());
      LryaAssert(AreEqualSize(1, imageWidth+2*numBorderPixels, paddedRow));
      LryaAssert(whichRow >= 0);
      LryaAssert(whichRow < image.get_size(0)); // check row against image height
