...
#ifndef _LRYACORETECHEMBEDDED_VISION_VISIONKERNELS_CONNECTEDCOMPONENTS_H_
#define _LRYACORETECHEMBEDDED_VISION_VISIONKERNELS_CONNECTEDCOMPONENTS_H_
...
namespace Lrya
{
  namespace Embedded
  {
    ...
    {
      //LryaConditionalErrorAndReturn(maxComponentSegments > 0 && maxComponentSegments <= Type_MAX,
      //  "ConnectedComponentsTemplate<Type>::ConnectedComponents", "maxComponentSegments must be greater than zero and less than 0xFFFF");

      LryaConditionalErrorAndReturn(maxImageWidth > 0 && maxImageWidth <= std::numeric_limits<s16>::max(),
    ...
    template<typename Type> Result ConnectedComponentsTemplate<Type>::Extract2dComponents_PerRow_Initialize(MemoryStack &fastMemory, MemoryStack &slowerMemory, MemoryStack &slowestMemory)
    {
      ...
      LryaConditionalErrorAndReturnValue(this->curState == STATE_CONSTRUCTED,
    ...
    template<typename Type> Result ConnectedComponentsTemplate<Type>::Extract2dComponents_PerRow_NextRow(const u8 * restrict binaryImageRow, const s32 imageWidth, const s16 whichRow, const s16 minComponentWidth, const s16 maxSkipDistance)
    {
      ...
      LryaAssert(imageWidth <= maxImageWidth);

      LryaConditionalErrorAndReturnValue(this->curState == STATE_INITIALIZED,
      ...
      for(s32 iCurrent=0; iCurrent<numCurrentComponents1d; iCurrent++) {
          ...
          if(pPreviousComponents1d[iPrevious].xStart <= pCurrentComponents1d[iCurrent].xEnd &&
            pPreviousComponents1d[iPrevious].xEnd >= pCurrentComponents1d[iCurrent].xStart) {
              if(!foundMatch) {
                ...
                LryaConditionalErrorAndReturnValue(result == RESULT_OK, result, "extract2dComponents", "Extracted maximum number of 2d components");
        ...
        if(!foundMatch) {
          ...
          LryaConditionalWarnAndReturnValue(result == RESULT_OK, result, "extract2dComponents", "Extracted maximum number of 2d components");
        } // if(!foundMatch)
      } // for(s32 iCurrent=0; iCurrent<numCurrentComponents1d; iCurrent++)
      ...
      for(s32 iEquivalent=0; iEquivalent<MAX_EQUIVALENT_ITERATIONS; iEquivalent++) {
        ...
        for(s32 iComponent=0; iComponent<numComponents; iComponent++) {
          ...
          if(pEquivalentComponents[minNeighbor] != minNeighbor) {
            ...
            LryaConditionalWarnAndReturnValue(recursionLevel < MAX_RECURSION_LEVEL, RESULT_FAIL, "extract2dComponents", "Issue with equivalentComponents minimum search (%d)", recursionLevel);
    ...
    {
      LryaConditionalErrorAndReturnValue(componentSizes.IsValid(), RESULT_FAIL_INVALID_OBJECT, "ComputeComponentSizes", "componentSizes is not valid");
      LryaConditionalErrorAndReturnValue(components.IsValid(), RESULT_FAIL_INVALID_OBJECT, "ComputeComponentSizes", "components is not valid");
    ...
    template<typename Type> Result ConnectedComponentsTemplate<Type>::ComputeComponentCentroids(FixedLengthList<Point<s16> > &componentCentroids, MemoryStack scratch)
    {
      ...
      LryaConditionalErrorAndReturnValue(componentCentroids.IsValid(), RESULT_FAIL_INVALID_OBJECT, "ComputeComponentSizes", "componentCentroids is not valid");
      LryaConditionalErrorAndReturnValue(components.IsValid(), RESULT_FAIL_INVALID_OBJECT, "ComputeComponentSizes", "components is not valid");
    ...
    {
      LryaConditionalErrorAndReturnValue(componentBoundingBoxes.IsValid(), RESULT_FAIL_INVALID_OBJECT, "ComputeComponentSizes", "componentBoundingBoxes is not valid");
      LryaConditionalErrorAndReturnValue(components.IsValid(), RESULT_FAIL_INVALID_OBJECT, "ComputeComponentSizes", "components is not valid");
    ...
    {
      LryaConditionalErrorAndReturnValue(numComponentSegments.IsValid(), RESULT_FAIL_INVALID_OBJECT, "ComputeComponentSizes", "numComponentSegments is not valid");
      LryaConditionalErrorAndReturnValue(components.IsValid(), RESULT_FAIL_INVALID_OBJECT, "ComputeComponentSizes", "components is not valid");
    ...
    template<typename Type> Result ConnectedComponentsTemplate<Type>::CompressConnectedComponentSegmentIds(MemoryStack scratch)
    {
      ...
      LryaConditionalErrorAndReturnValue(usedIds, RESULT_FAIL_OUT_OF_MEMORY, "CompressConnectedComponentSegmentIds", "Couldn't allocate usedIds");
      LryaConditionalErrorAndReturnValue(idLookupTable, RESULT_FAIL_OUT_OF_MEMORY, "CompressConnectedComponentSegmentIds", "Couldn't allocate idLookupTable");
    ...
    template<typename Type> Result ConnectedComponentsTemplate<Type>::InvalidateSmallOrLargeComponents(const s32 minimumNumPixels, const s32 maximumNumPixels, MemoryStack scratch)
    {
      ...
      LryaConditionalErrorAndReturnValue(componentSizes, RESULT_FAIL_OUT_OF_MEMORY, "InvalidateSmallOrLargeComponents", "Couldn't allocate componentSizes");
    ...
    template<typename Type> Result ConnectedComponentsTemplate<Type>::InvalidateSolidOrSparseComponents(const s32 sparseMultiplyThreshold, const s32 solidMultiplyThreshold, MemoryStack scratch)
    {
      ...
      LryaConditionalErrorAndReturnValue(minX && maxX && minY && maxY && componentSizes,
...
#endif // _LRYACORETECHEMBEDDED_VISION_VISIONKERNELS_CONNECTEDCOMPONENTS_H_
