...
// imageMarker = rgb2gray(imread('C:\Lrya\blockImages\fiducial105_6ContrastReduced.png'));
// image = 255*ones(480,640,'uint8');
// image(101:(100+size(imageMarker,1)-2), 101:(100+size(imageMarker,2)-2)) = imageMarker(2:(end-1), 2:(end-1));
// imageSize = [480,640];

// image = imresize(rgb2gray(imread('C:\Lrya\blockImages\testTrainedCodes.png')), [240,320]);
...
# endif
  ...
  Lrya::Embedded::FiducialDetectionParameters params;
  ...
  LryaConditionalErrorAndReturn(nrhs >= 21 && nrhs <= 22 && nlhs >= 2 && nlhs <= 4, "mexDetectFiducialMarkers", "Call this function as following: [quads, markerTypes, <markerNames>, <markerValidity>] = mexDetectFiducialMarkers(uint8(image), useIntegralImageFiltering, scaleImage_numPyramidLevels, scaleImage_thresholdMultiplier, component1d_minComponentWidth, component1d_maxSkipDistance, component_minimumNumPixels, component_maximumNumPixels, component_sparseMultiplyThreshold, component_solidMultiplyThreshold, component_minHollowRatio, minLaplacianPeakRatio, quads_minQuadArea, quads_quadSymmetryThreshold, quads_minDistanceFromImageEdge, decode_minContrastRatio, quadRefinementIterations, numRefinementSamples, quadRefinementMaxCornerChange, quadRefinementMinCornerChange, returnInvalidMarkers, <cornerMethod>);");

  MemoryStack memory(mxMalloc(bufferSize), bufferSize);
  LryaConditionalErrorAndReturn(memory.IsValid(), "mexDetectFiducialMarkers", "Memory could not be allocated");
  ...
  LryaConditionalErrorAndReturn(image.IsValid(), "mexDetectFiducialMarkers", "Could not allocate image");

  MemoryStack scratch0(mxCalloc(bufferSize,1), bufferSize);
  LryaConditionalErrorAndReturn(scratch0.IsValid(), "mexDetectFiducialMarkers", "Scratch0 could not be allocated");

  MemoryStack scratch1(mxCalloc(bufferSize,1), bufferSize);
  LryaConditionalErrorAndReturn(scratch1.IsValid(), "mexDetectFiducialMarkers", "Scratch1 could not be allocated");

  MemoryStack scratch2(mxCalloc(bufferSize,1), bufferSize);
  LryaConditionalErrorAndReturn(scratch2.IsValid(), "mexDetectFiducialMarkers", "Scratch2 could not be allocated");

  MemoryStack scratch3(mxCalloc(bufferSize,1), bufferSize);
  LryaConditionalErrorAndReturn(scratch3.IsValid(), "mexDetectFiducialMarkers", "Scratch3 could not be allocated");
  ...
  {
    const Lrya::Result result = DetectFiducialMarkers(
      image,
      markers,
      params,
      scratch1,
    ...
    LryaConditionalErrorAndReturn(result == Lrya::RESULT_OK, "mexDetectFiducialMarkers", "mexDetectFiducialMarkers Failed");
  }
  ...
  if(numMarkers != 0) {
    ...
    if(nlhs >= 3) {
      ...
      for(s32 i=0; i<numMarkers; ++i) {
        LryaAssert(markers[i].markerType >= 0 && markers[i].markerType <= Lrya::Vision::NUM_MARKER_TYPES);
        mxSetCell(markerNamesMatlab, i, mxCreateString(Lrya::Vision::MarkerTypeStrings[markers[i].markerType]));
      }
