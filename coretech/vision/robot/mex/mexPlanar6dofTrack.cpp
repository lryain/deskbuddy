...
void NormalizeImage(Array<u8>& grayscaleImage, const Quadrilateral<f32>& currentQuad, const f32 filterWidthFraction, MemoryStack scratch)
{
  ...
  Lrya::Result lastResult = ImageProcessing::BoxFilterNormalize(grayscaleImage, filterWidth, static_cast<u8>(128),
    grayscaleImageNormalized, scratch);

  mxAssert(lastResult == Lrya::RESULT_OK, "BoxFilterNormalize failed.");
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if(nrhs == 14 && (nlhs ==0 || nlhs == 1 || nlhs == 7)) {
    ...
    */
    ...
    // Update tracker
    const Lrya::Result trackerResult = tracker.UpdateTrack(grayscaleImage,
      maxIterations,
    ...
    if(trackerResult != Lrya::RESULT_OK) {
      char buffer[128];
