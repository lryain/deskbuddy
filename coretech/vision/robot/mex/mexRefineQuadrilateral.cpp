...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  LryaConditionalErrorAndReturn(scratch.IsValid(), "mexRefineQuadrilateral", "Memory could not be allocated");
  
  s32 argIndex = 0;
  
  LryaConditionalErrorAndReturn(mxGetClassID(prhs[0]) == mxUINT8_CLASS, "mexRefineQuadrilateral", "prhs[0] should be UINT8");
  Array<u8> image = mxArrayToArray<u8>(prhs[argIndex++], scratch);
  
  LryaConditionalErrorAndReturn(image.IsValid(), "mexRefineQuadrilateral", "Could not create %dx%d image in %d-byte buffer.\n", mxGetM(prhs[0]), mxGetN(prhs[0]), bufferSize);
  LryaConditionalErrorAndReturn(mxGetClassID(prhs[1]) == mxINT16_CLASS, "mexRefineQuadrilateral", "prhs[1] should be INT16");
  LryaConditionalErrorAndReturn(mxGetM(prhs[1]) == 4 && mxGetN(prhs[1]) == 2, "mexRefineQuadrilateral", "prhs[1] should be 4x2 array");
  Array<s16> initQuadData = mxArrayToArray<s16>(prhs[argIndex++], scratch);
  LryaConditionalErrorAndReturn(initQuadData.get_size(0) == 4 && initQuadData.get_size(1) == 2, "mexRefineQuadrilateral", "initial quad should be 4x2 array");
  ...
  LryaConditionalErrorAndReturn(mxGetClassID(prhs[2]) == mxSINGLE_CLASS, "mexRefineQuadrilateral", "prhs[2] should be SINGLE");
  LryaConditionalErrorAndReturn(mxGetM(prhs[2]) == 3 && mxGetN(prhs[2]) == 3, "mexRefineQuadrilateral", "prhs[2] should be 3x3 array");
  Array<f32> initialHomography = mxArrayToArray<f32>(prhs[argIndex++], scratch);
  LryaConditionalErrorAndReturn(initialHomography.get_size(0) == 3 && initialHomography.get_size(1) == 3, "mexRefineQuadrilateral", "initial homography should be 3x3 array");
  
  const s32 iterations          = static_cast<s32>(mxGetScalar(prhs[argIndex++]));
  LryaConditionalErrorAndReturn(mxGetNumberOfElements(prhs[argIndex])==1, "mexRefineQuadrilateral", "Separate squareThicknesses in x/y not supported yet. prhs[%d] must be a scalar.", argIndex);
  ...
  LryaConditionalErrorAndReturn(mxGetNumberOfElements(prhs[argIndex])==1, "mexRefineQuadrilateral", "Separate roundedCornerFraction in x/y not supported yet. prhs[%d] must be a scalar.", argIndex);
  ...
  Lrya::Result lastResult = RefineQuadrilateral(initialQuadF32, initialHomography, image, Point<f32>(squareWidthFraction, squareWidthFraction), Point<f32>(roundedCornersFraction,roundedCornersFraction), iterations, darkGray, brightGray, numSamples, maxCornerChange, minCornerChange, refinedQuad, refinedHomography, scratch);
  ...
  LryaConditionalErrorAndReturn(lastResult == Lrya::RESULT_OK, "mexRefineQuadrilateral", "RefineQuadrilateral failed.");
