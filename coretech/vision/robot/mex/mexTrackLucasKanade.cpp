...
#include "lrya/common/robot/fixedLengthList.h"
...
// templateImage = uint8(rgb2gray(imresize(imread('C:\Lrya\blockImages\blockImages00315.png'), [480,640]/8)));
// nextImage = uint8(rgb2gray(imresize(imread('C:\Lrya\blockImages\blockImages00316.png'), [480,640]/8)));
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  LryaConditionalErrorAndReturn(templateImage.IsValid(), "mexTrackLucasKanade", "Could not allocate templateImage");
  LryaConditionalErrorAndReturn(templateRegionRectangle_array.IsValid(), "mexTrackLucasKanade", "Could not allocate templateRegionRectangle_array");
  LryaConditionalErrorAndReturn(nextImage.IsValid(), "mexTrackLucasKanade", "Could not allocate nextImage");
  LryaConditionalErrorAndReturn(homography_f64.IsValid(), "mexTrackLucasKanade", "Could not allocate homography_f64");
  ...
  LryaConditionalErrorAndReturn(tracker.IsValid(), "mexTrackLucasKanade", "Could not construct tracker");
  ...
  if(tracker.set_transformation(initialTransform) != Lrya::RESULT_OK) {
    LryaError("mexTrackLucasKanade", "set_transformation");
    return;
  }

  bool converged = false;
  if(tracker.UpdateTrack(nextImage, maxIterations, convergenceTolerance, true, converged, memory) != Lrya::RESULT_OK) {
    LryaError("mexTrackLucasKanade", "UpdateTrack");
    return;
  }
