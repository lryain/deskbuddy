...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  LryaConditionalErrorAndReturn(image.IsValid(), "mexDetectFiducialMarkers_quadInput", "Could not allocate image");

  LryaConditionalErrorAndReturn(quadsF64.get_size(0) == 1 || quadsF64.get_size(1) == 1,  "mexDetectFiducialMarkers_quadInput", "Quads must be 1 dimensional");
  ...
  for(s32 iQuad=0; iQuad<quadsS16.get_size(); iQuad++) {
    ...
    if((lastResult = Transformations::ComputeHomographyFromQuad(currentQuad, currentHomography, numericalFailure, memory)) != Lrya::RESULT_OK) {
      return;
    }
    ...
    if((lastResult = currentMarker.RefineCorners(image, currentHomography, decode_minContrastRatio,
                                                 ...
                                                 memory)) != Lrya::RESULT_OK)
    {
      return;
    }

    if(currentMarker.validity == VisionMarker::LOW_CONTRAST) {
      currentMarker.markerType = Lrya::Vision::MARKER_UNKNOWN;
    } else {
