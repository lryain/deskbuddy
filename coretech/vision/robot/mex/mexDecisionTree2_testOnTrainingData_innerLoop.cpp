                    ...
                    const s64 treeLength)
{
  for(s64 iImage=0; iImage<numImages; iImage++) {
    ...
    // LryaAssert(pFeatureValuesCur >= pFeatureValues);
    ...
    LryaAssert(curLeftChildIndex < 0);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  LryaConditionalErrorAndReturn(nrhs == 7 && nlhs == 3, "mexDecisionTree2_testOnTrainingData_innerLoop", "Call this function as follows: [numCorrect, numTotal, labelIds] = mexDecisionTree2_testOnTrainingData_innerLoop(featureValues, cTree_whichFeatures, cTree_u8Thresholds, cTree_leftChildIndexs, samplePositions, groundTruthLabels, numThreads);");

  const s64 numFeatures = mxGetM(prhs[0]);
  const s64 numImages = mxGetN(prhs[0]);
  const s64 treeLength = mxGetM(prhs[1]);
  
  LryaConditionalErrorAndReturn(mxGetN(prhs[1]) == 1 && mxGetN(prhs[2]) == 1 && mxGetN(prhs[3]) == 1 && mxGetN(prhs[4]) == 1 && mxGetN(prhs[5]) == 1, "mexDecisionTree2_testOnTrainingData_innerLoop", "Incorrect input size");
  ...
  LryaConditionalErrorAndReturn(mxGetM(prhs[0]) > 0, "mexDecisionTree2_testOnTrainingData_innerLoop", "Incorrect input size");

  // NOTE: Theoretically, this could be reasonable, and not due to a transpose mistake
  LryaConditionalErrorAndReturn(numFeatures < numImages, "mexDecisionTree2_testOnTrainingData_innerLoop", "Possible transpose mistake");
