...
#include "lrya/tools/threads/threadSafeCounter.h"
#include "lrya/tools/threads/threadSafeFixedLengthList.h"
#include "lrya/tools/threads/threadSafeQueue.h"
#include "lrya/tools/threads/threadSafeVector.h"
...
ThreadResult BuildTreeThread(void * voidBuildTreeParams)
{
  ...
  while(true) {
    ...
    LryaAssert(buildTreeParams->workQueue.Size_unsafe() > 0);
    ...
    LryaAssert(workItem.featuresUsed.size > 0 && workItem.featuresUsed.buffer);
    LryaAssert(workItem.remaining.size > 0 && workItem.remaining.buffer);
...
#ifdef PRINT_INTERMEDIATE
      Lrya::CoreTechPrint("Thread %d: MaxDepth LeafNode for label = %d, or \"%s\" at depth %d\n", buildTreeParams->threadId, mostCommonLabel, buildTreeParams->labelNames[mostCommonLabel], curNode.depth);
#endif
...
#ifdef PRINT_INTERMEDIATE
        Lrya::CoreTechPrint("Thread %d: Worker finished\n", buildTreeParams->threadId);
#endif
      }
    ...
    if(curNode.bestEntropy > 100000.0 || curNode.whichFeature < 0) {
      ...
      Lrya::CoreTechPrint("Thread %d: Could not split LeafNode at depth %d for labels = {", buildTreeParams->threadId, curNode.depth);

      const s32 numLabels = failure.labels.size();
      for(s32 i=0; i<numLabels; i++) {
        Lrya::CoreTechPrint("(%d, %s), ", failure.labels[i].value, buildTreeParams->labelNames[failure.labels[i].value]);
      }
    ...
    LryaAssert(curNode.numLeft > 0 && curNode.numLeft < numRemaining && curNode.numRight > 0 && curNode.numRight < numRemaining);
    ...
    LryaAssert(cLeft == curNode.numLeft);
    LryaAssert(cRight == curNode.numRight);
...
#ifdef PRINT_INTERMEDIATE
    Lrya::CoreTechPrint("Thread %d: Thread finished\n", buildTreeParams->threadId);
#endif
    ...
    LryaAssert(leftWorkItem.remaining.buffer);
    LryaAssert(leftWorkItem.featuresUsed.buffer);
    LryaAssert(rightWorkItem.remaining.buffer);
    LryaAssert(rightWorkItem.featuresUsed.buffer);
...
namespace Lrya
{
  namespace Embedded
  {
    ...
    ThreadResult BenchmarkingThread(void * voidBenchmarkingParams)
    {
      ...
      LryaAssert(benchmarkingParams->sampleEveryNSeconds >= 0.1);
      ...
      )
    {
      ...
      LryaConditionalErrorAndReturnValue(AreValid(labelNames, labels, featureValues, u8ThresholdsToUse),
