...
template<typename Type> FixedLengthList<Type> LoadIntoList_temporaryBuffer(const char * filenamePrefix, const char * filenameSuffix, MemoryStack scratch1, MemoryStack scratch2, void * allocatedBuffer, const s32 allocatedBufferLength)
{
  ...
  LryaConditionalErrorAndReturnValue(raw.get_size(0) == 1,
  ...
  LryaConditionalErrorAndReturnValue(AreValid(raw, out),
...
std::vector<U8Bool> LoadIntoList_grayvalueBool(const char * filenamePrefix, const char * filenameSuffix, MemoryStack scratch1, MemoryStack scratch2)
{
  ...
  LryaConditionalErrorAndReturnValue(raw.IsValid(),
    out, "LoadIntoList_grayvalueBool", "Could not allocate");

  LryaAssert(rawWidth == 256);
...
int main(int argc, const char* argv[])
{
  ...
  LryaConditionalErrorAndReturnValue(AreValid(featureValues),
  ...
  // Load all inputs
  {
    ...
    LryaAssert(numImages > 0);
    //LryaAssert((numImages * numFeatures) < 1e10);
  ...
  for(s32 i=0; i<numFeatures; i++) {
    LryaConditionalErrorAndReturnValue(labels.get_size() == featureValues[i].get_size(),
  ...
  // Save the output
  {
    ...
    LryaConditionalErrorAndReturnValue(AreValid(depths, bestEntropys, whichFeatures, u8Thresholds, leftChildIndexs, cpuUsageSamples),
      -7, "run_trainDecisionTree", "Out of memory for saving");
