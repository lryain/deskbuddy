...
namespace {
  ...
  Lrya::Util::PrintfLoggerProvider printfLogger;
}
                            ...
                            Vision::IFaceEval::FileList& backgrounds)
{
  ...
  else
  {
    ...
    std::string backgroundDir = "resources/test/markerDetectionTests/NoMarkers"; // assumes running from deskmate base dir
    argParser.GetArg("--backgroundDir", backgroundDir);
    if(!backgroundDir.empty())
    {
      if(LRYA_VERIFY(Util::FileUtils::DirectoryExists(backgroundDir),
