                                          ...
                                          float maxPenalty) const
{
  ...
  const size_t numPointsAngle
    = std::ceil(Lrya::Util::Abs(Radians(segmentDef.turn.targetAngle - start_theta).ToFloat()) * kPrimAngleStepSizeRecip);
  // number of points, including start and end
  const size_t numIntermediatePoints = Lrya::Util::Max((size_t)2, Lrya::Util::Max(numPointsAngle, numPointsPosition));
...
bool xythetaEnvironment::PathIsSafe(const Planning::Path& path, float startAngle, Planning::Path& validPath) const
{
  ...
  const float maxPenalty = Lrya::Util::FLOATING_POINT_COMPARISON_TOLERANCE;
