...
GTEST_TEST(TestEnvironment, DumpAndInit)
{
  ...
  env.AddObstacleAllThetas(Lrya::RotatedRectangle(50.0, -10.0, 80.0, -10.0, 20.0));
  env.AddObstacleAllThetas(Lrya::RotatedRectangle(200.0, -10.0, 230.0, -10.0, 20.0));

  ASSERT_EQ(env.GetNumObstacles(), 2);

  // uncomment to see the output in a file
  // Lrya::Util::JsonWriter tmpWriter("/tmp/env.json");
  // env.Dump(tmpWriter);
  // tmpWriter.Close();
...
GTEST_TEST(TestEnvironment, ReverseSuccessorsMatch_WithObstacle)
{
  ...
  env.AddObstacleAllThetas(Lrya::RotatedRectangle(-20.0, 100.0, 20.0, 100.0, 20.0), 1.0f);
