...
#pragma warning(disable: 4100) // Unreference formal parameter
  ...
  inline static bool ShouldLog(int logLevel)
  {
    ...
    {
      case LRYA_LOG_LEVEL_DEBUG:
      case LRYA_LOG_LEVEL_INFO:
      case LRYA_LOG_LEVEL_EVENT:
        return (LRYA_DEBUG_LEVEL <= LRYA_DEBUG_ALL);
        
      case LRYA_LOG_LEVEL_WARN:
        return (LRYA_DEBUG_LEVEL <= LRYA_DEBUG_ERRORS_AND_WARNS);
        
      case LRYA_LOG_LEVEL_ASSERT:
        return (LRYA_DEBUG_LEVEL <= LRYA_DEBUG_ERRORS_AND_WARNS_AND_ASSERTS);
        
      case LRYA_LOG_LEVEL_ERROR:
      case LRYA_LOG_LEVEL_NUMLEVELS:
        return (LRYA_DEBUG_LEVEL <= LRYA_DEBUG_ERRORS);
    }
  }
  ...
  void _Lrya_Log(int logLevel, const char* eventName, const char* eventValue, const char* file, const char* funct, int line, ...)
  {
    if(ShouldLog(logLevel)) {
      ...
      Lrya::CoreTechPrint("LOG[%d] - %s@%d - %s - ", logLevel, file, line, eventName);

      va_start(argList, line);
      Lrya::CoreTechPrint(eventValue, argList);
      va_end(argList);
