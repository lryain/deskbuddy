...
namespace Lrya
{
  namespace Embedded
  {
    ...
    {
#if defined(__EDG__)
      LryaError("Array<Type>::LoadBinaryArray_Generic", "Cannot load files on embedded");
      return Array<u8>();
#else     
      ...
      bool useMalloc;
      if(!scratch) {
#if LRYACORETECH_EMBEDDED_USE_MALLOC
        LryaAssert(!memory && allocatedBuffer && allocatedBufferLength > 0);
        useMalloc = true;
#else
        LryaError("Array<Type>::LoadBinaryArray_Generic", "malloc is not enabled");
        return newArray;
#endif
      } else {
        LryaAssert(memory && !allocatedBuffer && allocatedBufferLength < 0);
        ...
        LryaConditionalErrorAndReturnValue(NotAliased(*scratch, *memory) && AreValid(*scratch, *memory),
          newArray, "Array<Type>::LoadBinaryArray_Generic", "scratch and memory must be different");

        scratch_local = MemoryStack(*scratch);
      }

      LryaConditionalErrorAndReturnValue(filename,
      ...
      LryaConditionalErrorAndReturnValue(bytesRead1 == ARRAY_FILE_HEADER_LENGTH && strncmp(reinterpret_cast<const char*>(tmpBuffer), ARRAY_FILE_HEADER, ARRAY_FILE_HEADER_VALID_LENGTH) == 0,
        newArray, "Array<Type>::LoadBinaryArray_Generic", "File is not an Lrya Embedded Array");
      ...
      if(reinterpret_cast<const char*>(tmpBuffer)[ARRAY_FILE_HEADER_VALID_LENGTH+1] == 'z') {
        ...
        isCompressed = true;
#else
        LryaError("Array<Type>::LoadBinaryArray_Generic", "zlib is not enabled");
        return newArray;
#endif
      }
      ...
      LryaConditionalErrorAndReturnValue(bytesRead2 > 0,
        newArray, "Array<Type>::LoadBinaryArray_Generic", "File is not an Lrya Embedded Array");
      ...
      SerializedBuffer serializedBuffer(tmpBuffer, tmpBufferLength, Lrya::Embedded::Flags::Buffer(false, true, true));
      ...
      if(!nextItem && strcmp(typeName, "Array") != 0) {
        ...
        LryaError("Array<Type>::LoadBinaryArray_Generic", "Could not parse data");
        return newArray;
      }
      ...
      if(!(basicType_isBasicType || basicType_isString)) {
        ...
        LryaError("Load", "can only load a basic type or string");
        return newArray;
      }
