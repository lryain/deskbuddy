      ...
      for(s32 i=0; i<bytePatternLength; i++) {
        for(s32 j=0; j<bytePatternLength; j++) {
          ...
          if(bytePattern[i] == bytePattern[j]) {
            LryaError("FindBytePattern", "bytePattern is not unique");
            return -1;
          }
        }
      }
...
#     endif
      ...
      const f32 timeInSeconds = Lrya::Vector::HAL::GetMicroCounter() / 1000000.0f;
#else // Generic Unix
      timespec ts;
...
#elif defined(__EDG__)  // ARM-MDK
      const f64 timeInSeconds = Lrya::Vector::HAL::GetMicroCounter() * (1.0 / 1000000.0);
#else // Generic Unix
      timespec ts;
      clock_gettime(CLOCK_MONOTONIC, &ts);
