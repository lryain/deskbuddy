...
#ifndef _LRYACORETECHEMBEDDED_COMMON_ARRAYSLICES_H_
#define _LRYACORETECHEMBEDDED_COMMON_ARRAYSLICES_H_
...
namespace Lrya
{
  namespace Embedded
  {
    ...
    template<typename Type> s32 ArraySlice<Type>::Set(const Type * const values, const s32 numValues)
    {
      ...
      LryaConditionalErrorAndReturnValue(limits.rawIn1Limits.ySize * limits.rawIn1Limits.xSize == numValues,
        0, "ArraySlice<Type>::Set", "Limits is not valid");
      ...
      LryaAssert(ci == limits.rawIn1Limits.ySize * limits.rawIn1Limits.xSize);
    ...
    {
      LryaConditionalErrorAndReturnValue(AreValid(*this, input),
        0, "ArraySlice<Type>::Set", "Invalid objects");

      LryaConditionalErrorAndReturnValue(this->get_array().get_buffer() != input.get_array().get_buffer(),
      ...
      if(!limits.isValid) {
        if(automaticTranspose) {
          ...
          if(!limits.isValid) {
            LryaError("ArraySlice<Type>::Set", "Subscripted assignment dimension mismatch");
            return 0;
          }
        } else {
          LryaError("ArraySlice<Type>::Set", "Subscripted assignment dimension mismatch");
      ...
      rawIn1Limits(in1_ySlice, in1_xSlice), in1_isTransposed(in1_isTransposed)
    {
      ...
      if(!isValid) {
        LryaError("ArraySliceLimits_in1_out1", "Subscripted assignment dimension mismatch");
        return;
      }
      ...
      rawIn2Limits(in2_ySlice, in2_xSlice), in2_isTransposed(in2_isTransposed)
    {
      ...
      if(!in1_isTransposed && !in2_isTransposed) {
        ...
        if(in1_isTransposed && in2_isTransposed) {
          ...
          LryaAssert(false); // should not be possible
        }

        if(!sizesMatch) {
          LryaError("ArraySliceLimits_in2_out1", "Subscripted assignment dimension mismatch");
          return;
        }
...
#endif // _LRYACORETECHEMBEDDED_COMMON_ARRAYSLICES_H_
