...
#ifndef _LRYACORETECHEMBEDDED_COMMON_ARRAY2D_H_
#define _LRYACORETECHEMBEDDED_COMMON_ARRAY2D_H_
...
#if LRYACORETECH_EMBEDDED_USE_OPENCV
#define LRYACORETECH_EMBEDDED_USE_MALLOC 1
#define LRYACORETECH_EMBEDDED_USE_ZLIB 1
#endif

#if LRYACORETECH_EMBEDDED_USE_ZLIB
#include "zlib.h"
#endif
...
namespace Lrya
{
  namespace Embedded
  {
    ...
    {
      LryaConditionalErrorAndReturnValue(numCols >= 0,
    ...
    {
      LryaConditionalErrorAndReturnValue(numCols >= 0 && numRows >= 0,
    ...
    template<typename Type> Array<Type>::Array(const s32 numRows, const s32 numCols, void * data, const s32 dataLength, const Flags::Buffer flags)
    {
      ...
      LryaConditionalErrorAndReturn(reinterpret_cast<size_t>(data)%MEMORY_ALIGNMENT == 0,
      ...
      LryaConditionalErrorAndReturn(numCols >= 0 && numRows >= 0 && dataLength >= numRows*this->stride,
      ...
      if(flags.get_isFullyAllocated()) {
        if(numRows == 1) {
          ...
          LryaConditionalErrorAndReturn(this->stride <= dataLength,
          ...
          LryaConditionalErrorAndReturn(this->stride == simpleStride,
            "Array<Type>::Array", "if the data buffer being passed in doesn't contain a raw buffer, the stride must be simple");

          LryaConditionalErrorAndReturn((numCols*sizeof(Type)) % MEMORY_ALIGNMENT == 0,
        ...
        LryaConditionalErrorAndReturn(flags.get_useBoundaryFillPatterns() == false,
    ...
    template<typename Type> Array<Type>::Array(const s32 numRows, const s32 numCols, MemoryStack &memory, const Flags::Buffer flags)
    {
      ...
      LryaConditionalErrorAndReturn(numCols >= 0 && numRows >= 0,
        "Array<Type>::Array", "Invalid size");
    ...
    template<typename Type> Array<Type> Array<Type>::LoadImage(const char * filename, MemoryStack &memory)
    {
      ...
      LryaConditionalErrorAndReturnValue(cvImage.cols > 0 && cvImage.rows > 0,
        newArray, "Array<Type>::LoadImage", "Could not load image");

      newArray = Array<Type>(cvImage.rows, cvImage.cols, memory);

      LryaConditionalErrorAndReturnValue(newArray.IsValid(),
...
#else
      LryaError("Array<Type>::Array", "OpenCV is required to load an image from an image file");
#endif
    ...
    {
      LryaConditionalErrorAndReturnValue(AreValid(*this, scratch) && filename,
        RESULT_FAIL_INVALID_OBJECT, "Array<Type>::SaveBinary", "Invalid inputs");

      LryaConditionalErrorAndReturnValue(compressionLevel >= 0 && compressionLevel <= 9,
      ...
      LryaConditionalErrorAndReturnValue(buffer,
      ...
      LryaConditionalErrorAndReturnValue(fp,
      ...
      if(compressionLevel > 0) {
        ...
        if(!uncompressed || !compressed) {
          ...
          LryaError("Array<Type>::SaveBinary", "Out of memory");
        ...
        if(compressionResult != Z_OK) {
          ...
          LryaError("Array<Type>::SaveBinary", "Zlib error");
        ...
        LryaConditionalErrorAndReturnValue(
...
#else
        LryaError("Array<Type>::SaveBinary", "Saving with compression requires zlib");
        return RESULT_FAIL;
#endif
      } else {
    ...
    template<typename Type> s32 Array<Type>::Set(const cv::Mat_<Type> &in)
    {
      ...
      LryaConditionalErrorAndReturnValue(inHeight != 0,
      ...
      if(isColor) {
        LryaConditionalErrorAndReturnValue(inHeight == this->size[0],
          0, "Array<Type>::Set", "input cv::Mat is the incorrect size.");
      } else {
        LryaConditionalErrorAndReturnValue(inHeight == this->size[0] && inWidth == this->size[1],
          0, "Array<Type>::Set", "input cv::Mat is the incorrect size.");
      }
    ...
    {
#if LRYACORETECH_EMBEDDED_USE_OPENCV
      LryaConditionalError(this->IsValid(), "Array<Type>::Show", "Array<Type> is not valid");
    ...
    {
      LryaConditionalErrorAndReturnValue(this->IsValid(),
    ...
    template<typename Type> template<typename InType> s32 Array<Type>::SetCast(const Array<InType> &in)
    {
      ...
      LryaConditionalErrorAndReturnValue(AreValid(*this, in),
    ...
    {
      LryaConditionalErrorAndReturnValue(dimension >= 0,
    ...
    {
      LryaConditionalError(numRows >= 0 && stride > 0,
    ...
    {
      if(!rawData) {
        LryaError("Lrya.Array2d.initialize", "input data buffer is NULL, numRows=%d, numCols=%d, dataLength=%d",
                  numRows, numCols, dataLength);
        InvalidateArray();
        return RESULT_FAIL_UNINITIALIZED_MEMORY;
      }

      LryaConditionalErrorAndReturnValue(numCols >= 0 && numRows >= 0 && dataLength >= MEMORY_ALIGNMENT,
        RESULT_FAIL_INVALID_SIZE, "Array<Type>::InitializeBuffer", "Negative dimension");

      LryaConditionalErrorAndReturnValue(!flags.get_useBoundaryFillPatterns(),
      ...
      if(requiredBytes > dataLength) {
        LryaError("Lrya.Array2d.initialize", "Input data buffer is not large enough. %d bytes is required.", requiredBytes);
        InvalidateArray();
        return RESULT_FAIL_OUT_OF_MEMORY;
      }
    ...
    {
      LryaConditionalError(numFractionalBits >= 0 && numFractionalBits <= (sizeof(Type)*8),  "FixedPointArray<Type>", "numFractionalBits number is invalid");
    }
    ...
    {
      LryaConditionalError(numFractionalBits >= 0 && numFractionalBits <= static_cast<s32>(sizeof(Type)*8),  "FixedPointArray<Type>", "numFractionalBits number is invalid");
    }
    ...
    {
      LryaConditionalErrorAndReturnValue(in.IsValid() && out,
...
#endif // _LRYACORETECHEMBEDDED_COMMON_ARRAY2D_H_
