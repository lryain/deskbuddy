...
#ifndef _LRYACORETECHEMBEDDED_COMMON_FIND_H_
#define _LRYACORETECHEMBEDDED_COMMON_FIND_H_
...
namespace Lrya
{
  namespace Embedded
  {
    ...
    {
      LryaConditionalErrorAndReturnValue(AreValid(*this, memory),
        RESULT_FAIL_INVALID_OBJECT, "Find.Evaluate", "Invalid objects");

      LryaConditionalErrorAndReturnValue(this->numOutputDimensions == 1,
      ...
      LryaAssert(array1.get_size(0) == 1); // Verify array height
      ...
      LryaConditionalErrorAndReturnValue(AreValid(indexes),
      ...
      if(this->compareWithValue) {
        ...
        LryaAssert(AreEqualSize(array1, array2));
    ...
    template<typename Type1, typename Operator, typename Type2> Result Find<Type1,Operator,Type2>::Evaluate(Array<s32> &yIndexes, Array<s32> &xIndexes, MemoryStack &memory) const
    {
      ...
      LryaConditionalErrorAndReturnValue(AreValid(yIndexes, xIndexes),
        RESULT_FAIL_OUT_OF_MEMORY, "Find.Evaluate", "Invalid objects");
    ...
    template<typename Type1, typename Operator, typename Type2> Result Find<Type1,Operator,Type2>::ComputeNumMatches() const
    {
      ...
      if(this->numMatchesComputed) {
        LryaAssert(newNumMatches == this->numMatches); // This should only happen if the data is changed, which it shouldn't be
      }
    ...
    template<typename Type1, typename Operator, typename Type2> Result Find<Type1,Operator,Type2>::ComputeLimits() const
    {
      ...
      if(this->limitsComputed) {
        LryaAssert(newLimits == this->limits);
      }
    ...
    template<typename Type1, typename Operator, typename Type2> template<typename ArrayType> Result Find<Type1,Operator,Type2>::SetArray(Array<ArrayType> &out, const ArrayType value) const
    {
      ...
      LryaConditionalErrorAndReturnValue(AreValid(*this, out),
        RESULT_FAIL_INVALID_OBJECT, "Find.SetArray", "Invalid objects");

      LryaConditionalErrorAndReturnValue(AreEqualSize(array1, out),
    ...
    {
      LryaConditionalErrorAndReturnValue(AreValid(*this, in, out),
        RESULT_FAIL_INVALID_OBJECT, "Find.SetArray", "Invalid objects");

      LryaConditionalErrorAndReturnValue(this->numOutputDimensions == 1,
        RESULT_FAIL_INVALID_SIZE, "Find.SetArray", "One-dimensional SetArray only works with one-dimensional Array input");

      LryaConditionalErrorAndReturnValue(findWhichDimension == 0 || findWhichDimension == 1,
      ...
      LryaAssert(array1Height == 1);

      if(findWhichDimension == 0) {
        LryaConditionalErrorAndReturnValue(outHeight == numMatches && outWidth == inWidth,
          RESULT_FAIL_INVALID_SIZE, "Find.SetArray", "out is not the correct size");

        LryaConditionalErrorAndReturnValue(inHeight == MAX(array1Height, array1Width),
          RESULT_FAIL_INVALID_SIZE, "Find.SetArray", "in is not the correct size");
      } else {
        LryaConditionalErrorAndReturnValue(outHeight == inHeight && outWidth == numMatches,
          RESULT_FAIL_INVALID_SIZE, "Find.SetArray", "out is not the correct size");

        LryaConditionalErrorAndReturnValue(inWidth == MAX(array1Height, array1Width),
          RESULT_FAIL_INVALID_SIZE, "Find.SetArray", "in is not the correct size");
      }
...
#endif // _LRYACORETECHEMBEDDED_COMMON_FIND_H_
