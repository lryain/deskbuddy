...
namespace Lrya
{
  namespace Embedded
  {
    ...
    {
      LryaAssert(flags.get_useBoundaryFillPatterns());

      if(flags.get_isFullyAllocated()) {
        LryaConditionalErrorAndReturn((reinterpret_cast<size_t>(buffer)+MemoryStack::HEADER_LENGTH)%MEMORY_ALIGNMENT == 0,
      ...
      LryaConditionalError(buffer, "Lrya.MemoryStack.MemoryStack", "Buffer must be allocated");
      LryaConditionalError(bufferLength <= 0x3FFFFFFF, "Lrya.MemoryStack.MemoryStack", "Maximum size of a MemoryStack is 2^30 - 1");
      LryaConditionalError(MEMORY_ALIGNMENT == 16, "Lrya.MemoryStack.MemoryStack", "Currently, only MEMORY_ALIGNMENT == 16 is supported");
    }
    ...
    {
      LryaConditionalWarn(ms.buffer, "Lrya.MemoryStack.MemoryStack", "Buffer must be allocated");
      LryaConditionalWarn(ms.totalBytes <= 0x3FFFFFFF, "Lrya.MemoryStack.MemoryStack", "Maximum size of a MemoryStack is 2^30 - 1");
      LryaConditionalWarn(MEMORY_ALIGNMENT == 16, "Lrya.MemoryStack.MemoryStack", "Currently, only MEMORY_ALIGNMENT == 16 is supported");
      LryaConditionalWarn(ms.totalBytes >= ms.usedBytes, "Lrya.MemoryStack.MemoryStack", "Buffer is using more bytes than it has. Try running IsValid() to test for memory corruption.");
    }
    ...
    void* MemoryStack::Allocate(const s32 numBytesRequested, const bool zeroAllocatedMemory, s32 &numBytesAllocated)
    {
      ...
      LryaConditionalErrorAndReturnValue(numBytesRequested > 0, NULL, "Lrya.MemoryStack.Allocate", "numBytesRequested > 0");
      LryaConditionalErrorAndReturnValue(numBytesRequested <= 0x3FFFFFFF, NULL, "Lrya.MemoryStack.Allocate", "numBytesRequested <= 0x3FFFFFFF");
      ...
      LryaConditionalErrorAndReturnValue((usedBytes+requestedBytes) <= totalBytes, NULL, "Lrya.MemoryStack.Allocate",
                                         "Ran out of scratch space, requesting %d bytes, with %d of %d used",
                                         requestedBytes, usedBytes, totalBytes);

      // Is this possible?
      LryaConditionalErrorAndReturnValue(static_cast<u32>(reinterpret_cast<size_t>(segmentFooter) - reinterpret_cast<size_t>(segmentMemory)) == numBytesRequestedRounded,
        NULL, "Lrya.MemoryStack.Allocate", "Odd error");
    ...
    void* MemoryStack::Reallocate(void* memoryLocation, s32 numBytesRequested, s32 &numBytesAllocated)
    {
      ...
      LryaConditionalErrorAndReturnValue(memoryLocation == lastAllocatedMemory, NULL, "Lrya.MemoryStack.Reallocate", "The requested memory is not at the end of the stack");
    ...
    {
      LryaConditionalErrorAndReturnValue(buffer != NULL, false, "Lrya.MemoryStack.IsValid", "buffer is not allocated");

      LryaConditionalWarnAndReturnValue(usedBytes <= totalBytes, false, "Lrya.MemoryStack.IsValid", "usedBytes is greater than totalBytes");
      LryaConditionalWarnAndReturnValue(usedBytes >= 0 && totalBytes >= 0, false, "Lrya.MemoryStack.IsValid", "usedBytes or totalBytes is less than zero");
      ...
      for(size_t i=0; (i<MIN(LOOP_MAX,totalBytes)) && (index<usedBytes); i++) {
        ...
        LryaConditionalWarnAndReturnValue(segmentLength == roundedSegmentLength, false, "Lrya.MemoryStack.IsValid", "The segmentLength is not a multiple of MEMORY_ALIGNMENT");
        ...
        LryaConditionalWarnAndReturnValue(segmentLength <= (usedBytes-index-HEADER_LENGTH-FOOTER_LENGTH), false, "Lrya.MemoryStack.IsValid", "The segment end is beyond the end of the buffer");

        const u32 segmentHeader = reinterpret_cast<const u32*>(bufferCharStar+index)[1];

        LryaConditionalWarnAndReturnValue(segmentHeader == FILL_PATTERN_START, false, "Lrya.MemoryStack.IsValid", "segmentHeader == FILL_PATTERN_START");

        const u32 segmentFooter = reinterpret_cast<const u32*>(bufferCharStar+index+HEADER_LENGTH+segmentLength)[0];

        LryaConditionalWarnAndReturnValue(segmentFooter == FILL_PATTERN_END, false, "Lrya.MemoryStack.IsValid", "segmentFooter == FILL_PATTERN_END");
      ...
      if(index == usedBytes) {
        ...
        LryaError("Lrya.MemoryStack.IsValid", "Infinite while loop");
        return false;
      } else {
        LryaError("Lrya.MemoryStack.IsValid", "Loop exited at an incorrect position, probably due to corruption");
        return false;
      }
#endif // #if LRYA_DEBUG_LEVEL >= LRYA_DEBUG_ERRORS_AND_WARNS
      return true;
    }
    ...
    const void * MemoryStackConstIterator::GetNext(s32 &segmentLength, const bool requireFillPatternMatch)
    {
      ...
      LryaConditionalErrorAndReturnValue(segmentLength == roundedSegmentLength, NULL, "Lrya.MemoryStackConstIterator.GetNext", "The segmentLength is not a multiple of MEMORY_ALIGNMENT (%x!=%x)", segmentLength, roundedSegmentLength);

      // Check if the segment end is beyond the end of the buffer (NOTE: this is not conservative enough, though errors should be caught later)
      LryaConditionalErrorAndReturnValue(segmentLength <= (memory.get_usedBytes()-this->index-MemoryStack::HEADER_LENGTH-MemoryStack::FOOTER_LENGTH), NULL, "Lrya.MemoryStackConstIterator.GetNext", "The segment end is beyond the end of the buffer. segmentLength=%d (0x%x) usedBytes=%d all=%d", segmentLength, segmentLength, memory.get_usedBytes(), (memory.get_usedBytes()-this->index-MemoryStack::HEADER_LENGTH-MemoryStack::FOOTER_LENGTH));

      if(requireFillPatternMatch) {
        const u32 segmentHeader = reinterpret_cast<const u32*>(bufferCharStar+this->index)[1];

        LryaConditionalErrorAndReturnValue(segmentHeader == MemoryStack::FILL_PATTERN_START, NULL, "Lrya.MemoryStackConstIterator.GetNext", "segmentHeader == FILL_PATTERN_START (%x!=%x)", segmentHeader, MemoryStack::FILL_PATTERN_START);

        const u32 segmentFooter = reinterpret_cast<const u32*>(bufferCharStar+this->index+MemoryStack::HEADER_LENGTH+segmentLength)[0];

        LryaConditionalErrorAndReturnValue(segmentFooter == MemoryStack::FILL_PATTERN_END, NULL, "Lrya.MemoryStackConstIterator.GetNext", "segmentFooter == FILL_PATTERN_END (%x != %x)", segmentFooter, MemoryStack::FILL_PATTERN_END);
      }
