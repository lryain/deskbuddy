 ...
 **/
#ifndef LRYA_CORETECH_COMMON_EMBEDDED_MATLAB_INTERFACE_H_
#define LRYA_CORETECH_COMMON_EMBEDDED_MATLAB_INTERFACE_H_

#if LRYACORETECH_EMBEDDED_USE_MATLAB
...
namespace Embedded {
  ...
  //These mex-only functions and #defines output to the Matlab command line, in a similar way to the LryaError #defines

#ifdef LRYA_MEX_BUILD

  extern char tmpMexBuffer1[1024];
  extern char tmpMexBuffer2[1024];
  
#define _LryaMatlab_Logf(logLevel, eventName, eventValue, file, funct, line, ...)\
{\
...
{ _LryaMatlab_Logf(LRYA_LOG_LEVEL_WARN, eventName, (eventValue_format), __FILE__, __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__); }
  
#undef LryaConditionalWarn
#define LryaConditionalWarn(expression, eventName, eventValue_format, ...) \
if(!(expression)) { _LryaMatlab_Logf(LRYA_LOG_LEVEL_WARN, eventName, (eventValue_format), __FILE__, __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__); }
  
#undef LryaConditionalWarnAndReturn
...
#undef LryaError
#define LryaError(eventName, eventValue_format, ...) _LryaMatlab_Logf(LRYA_LOG_LEVEL_ERROR, eventName, (eventValue_format), __FILE__, __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
  
#undef LryaConditionalError
#define LryaConditionalError(expression, eventName, eventValue_format, ...) \
if(!(expression)) { \
_LryaMatlab_Logf(LRYA_LOG_LEVEL_ERROR, eventName, (eventValue_format), __FILE__, __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__); \
}
  
#undef LryaConditionalErrorAndReturn
#define LryaConditionalErrorAndReturn(expression, eventName, eventValue_format, ...) \
if(!(expression)) {\
_LryaMatlab_Logf(LRYA_LOG_LEVEL_ERROR, eventName, (eventValue_format), __FILE__, __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__); \
return;\
}
#endif // #ifdef LRYA_MEX_BUILD
  ...
  Result Matlab::PutArray(const Array<Type> &matrix, const std::string name)
  {
    ...
    LryaConditionalErrorAndReturnValue(ep, RESULT_FAIL, "Lrya.PutArray<Type>", "Matlab engine is not started/connected");
    
    const mxClassID whichClass = GetMatlabClassID<Type>();
    LryaConditionalErrorAndReturnValue(whichClass != mxUNKNOWN_CLASS, RESULT_FAIL, "Lrya.PutArray<Type>", "Unknown type to convert to a mxClassID");
    ...
    LryaConditionalErrorAndReturnValue(matrixHeight <= std::numeric_limits<s32>::max(),
    ...
    /*
     const std::string tmpName = name + std::string("_LryaTMP");
     const std::string matlabTypeName = Lrya::Embedded::ConvertToMatlabTypeString(typeid(Type).name(), sizeof(Type));
  ...
  {
    LryaConditionalErrorAndReturnValue(ep, RESULT_FAIL, "Lrya.PutQuad<Type>", "Matlab engine is not started/connected");
  ...
  {
    LryaConditionalErrorAndReturnValue(ep, Array<Type>(0, 0, NULL, 0), "Lrya.GetArray<Type>", "Matlab engine is not started/connected");
    ...
    LryaConditionalErrorAndReturnValue(arrayTmp, Array<Type>(0, 0, NULL, 0), "Lrya.GetArray<Type>", "%s could not be got from Matlab", tmpName.data());
    
    LryaConditionalErrorAndReturnValue(mxGetClassID(arrayTmp) == GetMatlabClassID<Type>(), Array<Type>(0, 0, NULL, 0), "Lrya.GetArray<Type>", "matlabClassId != lryaVisionClassId");
    ...
    Array<Type> lryaArray(static_cast<s32>(numRows), static_cast<s32>(numCols), memory);
    
    Type *matlabArrayTmp = reinterpret_cast<Type*>(mxGetPr(arrayTmp));
    s32 matlabIndex = 0;
    
    s32 lryaArrayHeight = lryaArray.get_size(0);
    s32 lryaArrayWidth = lryaArray.get_size(1);
    
    for(s32 y=0; y<lryaArrayHeight; y++) {
      Type * pThisData = lryaArray.Pointer(y, 0);
      for(s32 x=0; x<lryaArrayWidth; x++) {
  ...
  {
    LryaConditionalErrorAndReturnValue(this->ep, Quadrilateral<Type>(),
                                       "Lrya.Get", "Matlab engine is not started/connected");
    
    const mxArray* mxQuad = SharedMatlabInterface::GetArray(name);
    
    LryaConditionalErrorAndReturnValue(mxQuad != NULL, Quadrilateral<Type>(),
                                       "Lrya.GetQuad", "No variable named '%s' found.", name.c_str());
    LryaConditionalErrorAndReturnValue(mxGetM(mxQuad)==4 && mxGetN(mxQuad)==2, Quadrilateral<Type>(),
                                       "Lrya.GetQuad", "Variable '%s' is not 4x2 in size.", name.c_str());
    LryaConditionalErrorAndReturnValue(mxGetClassID(mxQuad)==mxDOUBLE_CLASS, Quadrilateral<Type>(),
                                       "Lrya.GetQuad", "Variable '%s' must be DOUBLE to get as quad.", name.c_str());
...
#endif // #if LRYACORETECH_EMBEDDED_USE_MATLAB

#endif // LRYA_CORETECH_COMMON_EMBEDDED_MATLAB_INTERFACE_H_
