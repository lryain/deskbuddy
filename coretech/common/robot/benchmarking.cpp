...
OFFCHIP static BenchmarkEvent g_benchmarkEvents[Lrya::Embedded::MAX_BENCHMARK_EVENTS];

// The index of the next place to record a benchmark event
static s32 g_numBenchmarkEvents;

//static const s32 benchmarkPrintBufferLength = 512 + Lrya::Embedded::MAX_BENCHMARK_EVENTS*350;
...
staticInline u32 GetBenchmarkTime()
{
  ...
  return Lrya::Vector::HAL::GetMicroCounter();
#else
  timespec ts;
...
{
  LryaAssert(strlen(name) < Lrya::Embedded::BenchmarkElement::NAME_LENGTH);
  ...
  if(g_numBenchmarkEvents >= Lrya::Embedded::MAX_BENCHMARK_EVENTS)
    g_numBenchmarkEvents = Lrya::Embedded::MAX_BENCHMARK_EVENTS-1;
}
#endif // LRYA_EMBEDDED_BENCHMARK
  ...
  namespace Embedded
  {
    ...
    public:
      ...
      static FixedLengthList<BenchmarkElement> ComputeBenchmarkResults(const s32 numBenchmarkEvents, const BenchmarkEvent * benchmarkEvents, MemoryStack &memory)
      {
        ...
        LryaConditionalErrorAndReturnValue(numBenchmarkEvents > 0 && numBenchmarkEvents < MAX_BENCHMARK_EVENTS,
...

        {
          ...
          LryaConditionalErrorAndReturnValue(AreValid(outputResults, basicOutputResults, fullList, parseStack),
          ...
          for(s32 iEvent=0; iEvent<numBenchmarkEvents; iEvent++) {
            ...
            if(benchmarkEvents[iEvent].type == BENCHMARK_EVENT_BEGIN) {
              ...
              LryaConditionalErrorAndReturnValue(strcmp(startInstance.name, benchmarkEvents[iEvent].name) == 0 && startInstance.level == endLevel,
          ...
          for(s32 iInstance=0; iInstance<numFullList; iInstance++) {
            ...
            if(index < 0) {
              ...
              LryaAssert(index == CompileBenchmarkResults::GetNameIndex(curName, basicOutputResults));
            }
    ...
    Result PrintBenchmarkResults(const FixedLengthList<BenchmarkElement> &results, const bool verbose, const bool microseconds)
    {
      ...
      LryaConditionalErrorAndReturnValue(minCharacterToPrint.IsValid(),
        RESULT_FAIL_OUT_OF_MEMORY, "PrintBenchmarkResults", "Out of memory");
    ...
    Result ComputeAndPrintBenchmarkResults(const bool verbose, const bool microseconds, MemoryStack scratch)
    {
      ...
      LryaConditionalErrorAndReturnValue(AreValid(scratch, results),
      ...
      const s32 imageWidth)
    {
      ...
      LryaConditionalErrorAndReturnValue(totalTimeIndex >= 0,
        RESULT_FAIL, "ShowBenchmarkResults", "Requires a \"TotalTime\" benchmark event");

      LryaConditionalErrorAndReturnValue(namesToDisplay.get_size() > 0,
      ...
      return RESULT_OK;
#else // #if LRYACORETECH_EMBEDDED_USE_OPENCV
      return RESULT_FAIL;
    ...
    Result PrintPercentileBenchmark(const FixedLengthList<FixedLengthList<BenchmarkElement> > &benchmarkElements, const s32 numRuns, const f32 elementPercentile, MemoryStack scratch)
    {
      ...
      for(s32 iRun=1; iRun<numRuns; iRun++) {
        if(benchmarkElements[0].get_size() != benchmarkElements[iRun].get_size()) { LryaError("PrintPercentileBenchmark", "number failure %d!=%d", benchmarkElements[0].get_size(), benchmarkElements[iRun].get_size()); return RESULT_FAIL; }
        for(s32 i=0; i<numElements; i++) {
          if(benchmarkElements[0][i].numEvents != benchmarkElements[iRun][i].numEvents) { LryaError("PrintPercentileBenchmark", "number failure %d!=%d (%s,%s)", benchmarkElements[0][i].numEvents, benchmarkElements[iRun][i].numEvents, benchmarkElements[0][i].name, benchmarkElements[iRun][i].name); return RESULT_FAIL; }
          if(strcmp(benchmarkElements[0][i].name, benchmarkElements[iRun][i].name) != 0) { LryaError("PrintPercentileBenchmark", "name failure %s!=%s", benchmarkElements[0][i].name, benchmarkElements[iRun][i].name); return RESULT_FAIL; }
        }
      }
      ...
      for(s32 i=0; i<numElements; i++) {
        ...
        if(Matrix::InsertionSort<u32>(sortedElements, 1) != RESULT_OK) { LryaError("PrintPercentileBenchmark", "sort failure"); return RESULT_FAIL; }
