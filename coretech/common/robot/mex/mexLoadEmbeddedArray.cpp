...
{
  LryaConditionalErrorAndReturnValue(filename && allocatedBuffer,
  ...
  if(basicType_isString) {
    ...
    if(basicType_isFloat) {
      if(basicType_sizeOfType == 4) {
        ...
        LryaError("Load", "can only load a basic type");
        return NULL;
      }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  LryaConditionalErrorAndReturn(nrhs == 1 && nlhs == 1, "mexLoadEmbeddedArray", "Call this function as follows: array = mexLoadEmbeddedArray(filename);");
  ...
  LryaConditionalErrorAndReturn(allocatedBuffer, "mexLoadEmbeddedArray", "Memory could not be allocated");
