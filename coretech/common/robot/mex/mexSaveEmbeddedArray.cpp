...
#include "lrya/tools/threads/threadSafeUtilities.h"
...
template<typename Type> Result AllocateAndSave(const mxArray *matlabArray, const char *filename, const s32 compressionLevel, MemoryStack scratch1, MemoryStack scratch2)
{
  LryaAssert(!mxIsCell(matlabArray));

  Array<Type> lryaArray = mxArrayToArray<Type>(matlabArray, scratch1);

  if(!lryaArray.IsValid())
    return RESULT_FAIL;

  return lryaArray.SaveBinary(filename, compressionLevel, scratch2);
}
...
template<> Result AllocateAndSaveCell<char*>(const mxArray *matlabArray, const char *filename, const s32 compressionLevel, MemoryStack scratch1, MemoryStack scratch2)
{
  ...
  Array<char *> lryaArray = mxCellArrayToStringArray(matlabArray, scratch1);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  LryaConditionalErrorAndReturn((nrhs >= 2 && nrhs <= 4) && nlhs == 0, "mexSaveEmbeddedArray", "Call this function as follows: mexSaveEmbeddedArray(array, filename, <compressionLeve>, <numThreads>); % array and filename may be cell arrays.");
  ...
  if(mxIsCell(prhs[0])) {
    if(mxIsCell(prhs[1])) {
      ...
      LryaConditionalErrorAndReturn(mxIsChar(curMatlabArray), "mexSaveEmbeddedArray", "If one input is a cell both must be. The only exception is if the first input is a cell array of strings.");
    }
  }

  if(areCellArrays) {
    MemoryStack memory(mxMalloc(100000), 100000);
    LryaConditionalErrorAndReturn(memory.IsValid(), "mexSaveEmbeddedArray", "Memory could not be allocated");
    ...
    for(s32 iThread=0; iThread<numThreads; iThread++) {
      ...
      LryaConditionalErrorAndReturn(scratch1s[iThread].IsValid() && scratch2s[iThread].IsValid(), "mexSaveEmbeddedArray", "Memory could not be allocated");
    }
    ...
    LryaConditionalErrorAndReturn(!mxIsCell(prhs[1]), "mexSaveEmbeddedArray", "If one input is a cell, both must be");

    MemoryStack memory1(mxMalloc(bufferSize), bufferSize);
    MemoryStack memory2(mxMalloc(bufferSize), bufferSize);
    LryaConditionalErrorAndReturn(memory1.IsValid() && memory2.IsValid(), "mexSaveEmbeddedArray", "Memory could not be allocated");
