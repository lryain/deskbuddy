...
#ifndef _LRYACORETECH_COMMON_JSONTOOLS_H_
#define _LRYACORETECH_COMMON_JSONTOOLS_H_
...
namespace Lrya
{
  ...
  namespace JsonTools
  {
    ...
    // Get an Lrya::Point by name.  Returns true if the point is set successfully,
    // or false otherwise.
    template<typename T, size_t N>
    bool GetPointOptional(const Json::Value& node, const std::string& key, Lrya::Point<N,T>& pt);
    
    // Get an Lrya::Pose3d object by name (translation, rotation axis/angle).
    // Returns true if Pose is set successfully, false otherwise.
    bool GetPoseOptional(const Json::Value& node, const std::string& key, Lrya::Pose3d& pose);
    ...
    bool GetColorOptional(const Json::Value& node, const std::string& key, Lrya::ColorRGBA& color);
