...
#ifndef __Lrya_Common_Math_PoseTreeNode_H__
#define __Lrya_Common_Math_PoseTreeNode_H__
...
std::string PoseBase<PoseNd,TransformNd>::PoseTreeNode::GetNamedPathToRoot(bool showTranslations) const
{
  ...
  const PoseCPtrSet* validPoses = (LRYA_DEV_CHEATS ? &Dev_GetValidPoses() : nullptr);
  ...
  if ( DO_DEV_POSE_CHECKS )
  {
    ...
    if ( newParent )
    {
      ...
      if ( match != validPoses.end() )
      {
        ...
        LRYA_VERIFY(newParent->_devChildrenPtrs.find(child) == newParent->_devChildrenPtrs.end(),
      ...
      else
      {
        ...
        LRYA_VERIFY(false, "PoseBase.Dev_SwitchParent.NewParentIsNotAValidPose",
                    "New parent '%s'(%p) path to root: %s",
                                                                                ...
                                                                                const PoseTreeNode* child)
{
  ...
  {
    if(!LRYA_VERIFY(parent->_ownerCount > 0,
  ...
  if ( DO_DEV_POSE_CHECKS )
  {
    ...
    PoseCPtrSet& validPoses = Dev_GetValidPoses();
    if( !LRYA_VERIFY(validPoses.find(child) != validPoses.end(),
    ...
    if ( nullptr != parent )
    {
      ...
      if( !LRYA_VERIFY(validPoses.find(parent) != validPoses.end(),
      ...
      if( !LRYA_VERIFY(parent->_devChildrenPtrs.find(child) != parent->_devChildrenPtrs.end(),
...
void PoseBase<PoseNd,TransformNd>::PoseTreeNode::Dev_PoseDestroyed(const PoseTreeNode* nodePtr)
{
  if ( DO_DEV_POSE_CHECKS )
  {
    ...
    LRYA_VERIFY(removedCount == 1, "PoseBase.Dev_PoseDestroyed.DestroyingInvalidPose",
...
void PoseBase<PoseNd,TransformNd>::PoseTreeNode::Dev_PoseCreated(const PoseTreeNode* nodePtr)
{
  if ( DO_DEV_POSE_CHECKS )
  {
    ...
    LRYA_VERIFY(insertRetInfo.second, "PoseBase.Dev_PoseCreated.CreatingDuplicatedPointer",
                "Path from '%s'(%p) to root: %s",
                nodePtr->GetName().c_str(), nodePtr,
                nodePtr->GetNamedPathToRoot(true).c_str());
  }
...
{
  if(LRYA_VERIFY(_ownerCount > 0, "PoseBase.PoseTreeNode.RemoveOwner.ZeroOwners", ""))
  {
    --_ownerCount;
...
#endif /* __Lrya_Common_Math_PoseTreeNode_H__ */
