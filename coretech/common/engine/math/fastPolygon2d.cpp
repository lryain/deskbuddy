    ...
    for(size_t i = 0; i < numPts; ++i) {
      ...
      if (!IsNearlyEqual(_circleCenter - _points[pointIdx], {0.f, 0.f}) && 
        ...
        distanceToEdgeSquared = std::pow(
          Lrya::DotProduct( _perpendicularEdgeVectors[i].first, _circleCenter - _points[pointIdx] ),
          2);
      }
                                          ...
                                          bool dryRun)
{
  ...
  for( auto& testEntry : testPoints ) {
    ...
    const float dot = Lrya::DotProduct( _perpendicularEdgeVectors[edgeIdx].first, testEntry.second - _points[pointIdx] );
