...
PoseOriginID_t PoseOriginList::AddNewOrigin()
{
  ...
  LRYA_VERIFY(success, "PoseOriginList.AddNewOrigin.AddWithIDFailed", "ID:%d", ID);
...
bool PoseOriginList::AddOriginWithID(PoseOriginID_t ID)
{
  ...
  if(!LRYA_VERIFY(result.second, "PoseOriginList.AddOriginWithID.DuplicateID", "%d", ID))
  {
    if(LRYA_DEV_CHEATS)
    {
      Util::sAbort();
...
{
  auto iter = _origins.find(ID);
  if(!LRYA_VERIFY(iter != _origins.end(), "PoseOriginList.GetOriginByID.BadID", "%d", ID))
  {
...
Result PoseOriginList::Rejigger(const PoseOrigin& newOrigin, const Transform3d& transform)
{
  ...
  if(!LRYA_VERIFY(ContainsOriginID(newOriginID), "PoseOriginList.Rejigger.UnknownOrigin",
                  "Pose ID:%d (%s) not known to PoseOriginList", newOriginID, newOrigin.GetName().c_str()))
  {
    return RESULT_FAIL;
  }

  if(!LRYA_VERIFY(_current.originID != newOriginID, "PoseOriginList.Rejigger.NewOriginIsCurrent", "ID:%d", newOriginID))
  {
    return RESULT_FAIL;
  }
...
bool PoseOriginList::SanityCheckOwnership() const
{
  ...
  for(auto const& originAndIdPair : _origins)
  {
    ...
    allPosesOwned &= LRYA_VERIFY(origin->IsOwned(),
    ...
    if(origin->HasParent())
    {
      ...
      allPosesOwned &= LRYA_VERIFY(parent.IsOwned(),
