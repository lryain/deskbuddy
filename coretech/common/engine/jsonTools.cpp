...
uint8_t ParseUint8(const Json::Value& config, const char* key, const std::string& debugName) {
  ...
  return Lrya::Util::numeric_cast<uint8_t>(val.asInt());
}
...
int8_t ParseInt8(const Json::Value& config, const char* key, const std::string& debugName) {
  ...
  return Lrya::Util::numeric_cast<int8_t>(val.asInt());
}
...
// Specializations of GetValue for all types we care about
  ...
  bool GetPoseOptional(const Json::Value& config, const std::string& key, Lrya::Pose3d& pose)
  {
    bool retVal = false;
    
    const Json::Value& child = config[key];
    if(not child.isNull()) {
      Lrya::Point3f axis, translation;
      if(child.isMember("Angle") &&
         GetPointOptional(child, "Axis", axis) &&
         GetPointOptional(child, "Translation", translation))
      {
        const Lrya::Radians angle(child["Angle"].asFloat());
  ...
  bool GetColorOptional(const Json::Value& jsonRoot, const std::string& key, Lrya::ColorRGBA& color)
  {
    bool retVal = false;
