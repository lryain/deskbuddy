...
#include "lrya/common/robot/errorHandling.h"
...
{
#ifdef _MSC_VER
  LryaConditionalErrorAndReturnValue(isOpen == false,
  ...
  LryaConditionalErrorAndReturnValue(comPortHandle != NULL && comPortHandle != INVALID_HANDLE_VALUE,
  ...
  LryaConditionalErrorAndReturnValue(isPurged,
  ...
  return RESULT_OK;
#else // #ifdef _MSC_VER
  LryaError("Serial::Open", "Only MSVC supported");
  return RESULT_FAIL;
...
Result Serial::Read(void * buffer, s32 bufferLength, s32 &bytesRead)
{
  ...
  LryaConditionalErrorAndReturnValue(isOpen && comPortHandle!=NULL,
  ...
  LryaConditionalErrorAndReturnValue(fileRead,
