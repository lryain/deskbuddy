...
namespace Lrya
{
  namespace Embedded
  {
    ...
    DebugStreamClient::ObjectToSave::ObjectToSave(DebugStreamClient::Object &object, const char * filename)
    {
      ...
      snprintf(this->typeName, Lrya::Embedded::SerializedBuffer::DESCRIPTION_STRING_LENGTH, "%s", object.typeName);
      snprintf(this->objectName, Lrya::Embedded::SerializedBuffer::DESCRIPTION_STRING_LENGTH, "%s", object.objectName);
      snprintf(this->filename, Lrya::Embedded::DebugStreamClient::ObjectToSave::SAVE_FILENAME_PATTERN_LENGTH, "%s", filename);
    }
    ...
    void DebugStreamClient::ProcessRawBuffer(DebugStreamClient::RawBuffer &buffer, ThreadSafeQueue<DebugStreamClient::Object> &parsedObjectQueue, const bool requireMatchingSegmentLengths)
    {
      ...
      SerializedBuffer serializedBuffer(buffer.data, buffer.curDataLength, Lrya::Embedded::Flags::Buffer(false, true, true));
      ...
      while(iterator.HasNext()) {
        ...
        if(strcmp(typeName, "Basic Type Buffer") == 0) {
          ...
          LryaAssert(stringLength <= s32_MAX);
    ...
    ThreadResult DebugStreamClient::ConnectionThread(void *threadParameter)
    {
      ...
      if(!usbBuffer || !nextRawBuffer.data) {
        //LryaError("DebugStreamClient::ConnectionThread", "Could not allocate usbBuffer and nextRawBuffer.data");
#ifdef _MSC_VER
        return -1;
#else
        return NULL;
