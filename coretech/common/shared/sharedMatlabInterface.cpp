...
#if (defined(LRYACORETECH_EMBEDDED_USE_MATLAB) && LRYACORETECH_EMBEDDED_USE_MATLAB) || \
  (defined(LRYACORETECH_USE_MATLAB) && LRYACORETECH_USE_MATLAB)
...
#define TEXT_BUFFER_SIZE 1024
  ...
  */
  ...
  SharedMatlabInterface::SharedMatlabInterface(bool clearWorkspace)
  {
    ...
    EvalString("lastLryaCommandBuffer=cell(0, 1);");
  }

  std::string SharedMatlabInterface::EvalString(const char * const format, ...)
  {
    if(!ep) {
      //LryaConditionalErrorAndReturnValue(this->ep, "", "Lrya.", "Matlab engine is not started/connected");
      LOG_ERROR("Lrya.", "Matlab engine is not started/connected");
      return "";
    }
    va_list args;
    char *buffer;
  ...
  std::string SharedMatlabInterface::EvalStringEcho(const char * const format, ...)
  {
    ...
    std::string toReturn = std::string(buffer);
    PutString(buffer, (int)toReturn.size(), "lastLryaCommand");

    EvalString("if length(lastLryaCommandBuffer)==%d lastLryaCommandBuffer=lastLryaCommandBuffer(2:end); end; lastLryaCommandBuffer{end+1}=lastLryaCommand;", SharedMatlabInterface::COMMAND_BUFFER_SIZE);
  ...
  {
    if(!ep) {
      // LryaConditionalErrorAndReturnValue(this->ep, NULL, "Lrya.", "Matlab engine is not started/connected");
      LOG_ERROR("Lrya.", "Matlab engine is not started/connected");
      return NULL;
    }
  ...
  {
    if(!ep) {
      // LryaConditionalErrorAndReturnValue(this->ep, TYPE_UNKNOWN, "Lrya.", "Matlab engine is not started/connected");
      LOG_ERROR("Lrya.", "Matlab engine is not started/connected");
      return TYPE_UNKNOWN;
    }
...
#if defined(LRYA_USE_OPENCV)
  Result SharedMatlabInterface::GetCvMat(const CvMat *matrix, std::string name)
  {
    if(!ep) {
      // LryaConditionalErrorAndReturnValue(this->ep, RESULT_FAIL, "Lrya.", "Matlab engine is not started/connected");
      LOG_ERROR("Lrya.", "Matlab engine is not started/connected");
      return RESULT_FAIL;
    }

    if(matrix == NULL) {
      // LryaConditionalErrorAndReturnValue(matrix != NULL, RESULT_FAIL, "Error: CvMat is not initialized for %s\n", name.data());
      LOG_ERROR("Lrya.", "Error: CvMat is not initialized for %s\n", name.data());
      return RESULT_FAIL;
    }

    char tmpName[TEXT_BUFFER_SIZE];
    snprintf(tmpName, TEXT_BUFFER_SIZE, "%s_LryaTMP", name.data());
  ...
  {
    LryaConditionalErrorAndReturnValue(this->ep, RESULT_FAIL, "Lrya.SharedMatlabInterface.PutCvMat",
                                       "Matlab engine is not started/connected");

    LryaConditionalErrorAndReturnValue(matrix != NULL, RESULT_FAIL, "Lrya.SharedMatlabInterface.PutCvMat",
...
#endif // #if defined(LRYA_USE_OPENCV)
  ...
  {
    if(!ep) {
      //LryaConditionalErrorAndReturnValue(this->ep, -1, "Lrya.", "Matlab engine is not started/connected");
      LOG_ERROR("Lrya.", "Matlab engine is not started/connected");
      return -1;
    }
...
#endif // #if (defined(LRYACORETECH_EMBEDDED_USE_MATLAB) && LRYACORETECH_EMBEDDED_USE_MATLAB) || (defined(LRYACORETECH_USE_MATLAB) && LRYACORETECH_USE_MATLAB)
