...
#ifndef LRYA_CORETECH_COMMON_SHARED_MATLAB_INTERFACE_H_
#define LRYA_CORETECH_COMMON_SHARED_MATLAB_INTERFACE_H_
...
#if LRYACORETECH_EMBEDDED_USE_OPENCV || LRYACORETECH_USE_OPENCV
...
#define LOG_ERROR(name, format, ...) LryaError(name, format, ##__VA_ARGS__)
#define LOG_WARNING(name, format, ...) LryaWarn(name, format, ##__VA_ARGS__)
...
namespace Lrya
{
  ...
  template<typename Type> Result SharedMatlabInterface::PutOpencvMat(const cv::Mat_<Type> &matrix, const std::string name)
  {
    ...
    if(!ep) {
      ...
      //LryaConditionalErrorAndReturnValue(ep, RESULT_FAIL, "Lrya.PutOpencvMat<Type>", "Matlab engine is not started/connected");
      LOG_ERROR("Lrya.PutOpencvMat<Type>", "Matlab engine is not started/connected");
      return RESULT_FAIL;
    }
    ...
    if(whichClass == mxUNKNOWN_CLASS) {
      // TODO: How to print a message the right way for basestation vs. embedded??
      //LryaConditionalErrorAndReturnValue(whichClass != mxUNKNOWN_CLASS, RESULT_FAIL, "Lrya.PutOpencvMat<Type>", "Unknown type to convert to a mxClassID");
      return RESULT_FAIL;
    }
  ...
  Result SharedMatlabInterface::Put(const Type * values, s32 nValues, const std::string name)
  {
    if(!ep) {
      // TODO: How to print a message the right way for basestation vs. embedded??
      //LryaConditionalErrorAndReturnValue(this->ep, RESULT_FAIL, "Lrya.Put", "Matlab engine is not started/connected");
      return RESULT_FAIL;
    }
  ...
  Type* SharedMatlabInterface::Get(const std::string name)
  {
    if(!ep) {
      // TODO: How to print a message the right way for basestation vs. embedded??
      //LryaConditionalErrorAndReturnValue(this->ep, NULL, "Lrya.Get", "Matlab engine is not started/connected");
      return NULL;
    }
    ...
    if(arrayTmp) {
      ...
      // TODO: How to print a message the right way for basestation vs. embedded??
      //LryaError("Lrya.Get", "No variable named %s exists on the workspace", name.data());
      return NULL;
    }
...
#endif // LRYA_CORETECH_COMMON_SHARED_MATLAB_INTERFACE_H_
