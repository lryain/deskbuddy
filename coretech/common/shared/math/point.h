...
#ifndef _LRYACORETECH_COMMON_POINT_H_
#define _LRYACORETECH_COMMON_POINT_H_
...
template <typename T> 
struct std::hash<Lrya::Point2<T>> {
  std::enable_if_t<sizeof(T) <= 4, s64> 
  constexpr operator()(const Lrya::Point2<T>& p) const
  { 
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
template <Lrya::PointDimType N, typename T>
struct std::equal_to<Lrya::Point<N,T>>  {
  template <typename T1, typename = std::enable_if_t<std::is_floating_point<T1>::value> >
  constexpr bool operator()(const Lrya::Point<N,T1>& p, const Lrya::Point<N,T1>& q) const 
  { 
    return AllHelper(p, q, [] (T1 a, T1 b) { return FLT_NEAR(a, b); });
  }

  constexpr bool operator()(const Lrya::Point<N,T>& p, const Lrya::Point<N,T>& q) const 
  { 
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
template <Lrya::PointDimType N, typename T>
struct std::not_equal_to<Lrya::Point<N,T>>  {
  constexpr bool operator()(const Lrya::Point<N,T>& p, const Lrya::Point<N,T>& q) const 
  { 
    return !std::equal_to<Lrya::Point<N,T>>()(p,q);
  }
};

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
template <Lrya::PointDimType N, typename T>
struct std::less<Lrya::Point<N,T>>  {
  template <typename T1, typename = std::enable_if_t<std::is_floating_point<T1>::value> >
  constexpr bool operator()(const Lrya::Point<N,T1>& p, const Lrya::Point<N,T1>& q) const 
  { 
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
template <Lrya::PointDimType N, typename T>
struct std::less_equal<Lrya::Point<N,T>>  {
  template <typename T1, typename = std::enable_if_t<std::is_floating_point<T1>::value> >
  constexpr bool operator()(const Lrya::Point<N,T1>& p, const Lrya::Point<N,T1>& q) const 
  { 
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
template <Lrya::PointDimType N, typename T>
struct std::greater<Lrya::Point<N,T>>  {
  template <typename T1, typename = std::enable_if_t<std::is_floating_point<T1>::value> >
  constexpr bool operator()(const Lrya::Point<N,T1>& p, const Lrya::Point<N,T1>& q) const 
  { 
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
template <Lrya::PointDimType N, typename T>
struct std::greater_equal<Lrya::Point<N,T>>  {
  template <typename T1, typename = std::enable_if_t<std::is_floating_point<T1>::value> >
  constexpr bool operator()(const Lrya::Point<N,T1>& p, const Lrya::Point<N,T1>& q) const 
  { 
...
namespace {
  ...
  template <Lrya::PointDimType N, typename T, size_t... Is>
  constexpr Lrya::Point<N,T> fill_point(const T& init, std::index_sequence<Is...> const&) {
    // use `Is` to duplicate `init`, but cast `Is` to void to ignore it
    return Lrya::Point<N,T>{ ((void)Is, init)...}; 
  }
    
  template <Lrya::PointDimType N, typename T>
  constexpr Lrya::Point<N,T> fill_point(const T& init) {
    return fill_point<N,T>( init, std::make_index_sequence<N>{} );
  }

  template <Lrya::PointDimType N, typename T, typename Generator, size_t... Is>
  constexpr Lrya::Point<N,T> generate_point(const Generator& gen, std::index_sequence<Is...> const&) {
    return Lrya::Point<N,T>{ gen[Is]... };
  }
    
  template <Lrya::PointDimType N, typename T, typename Generator>
  constexpr Lrya::Point<N,T> generate_point(const Generator& gen) {
...
#endif // _LRYACORETECH_COMMON_POINT_IMPL_H_
