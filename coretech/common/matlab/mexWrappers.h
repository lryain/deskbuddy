...
#define USE_OPENCV 1
#define USE_LRYA_CORE_LIBRARIES 1
...
void mapCvMat2mxArray(const cv::Mat &mat, const int nbands, mxArray *array)
{
  ...
  if(numMatlabElements != numOpenCvElements) {
    Lrya::CoreTechPrint("Matlab array has a different number of elements than the OpenCV Mat(%d != %d)\n", numMatlabElements, numOpenCvElements);
    return;
  }
...
template <class T> std::vector<cv::Point_<T> > mxArray2CvPointVector(const mxArray *array)
{
  ...
  if(numMatlabElements%2 != 0) {
    Lrya::CoreTechPrint("Matlab array number of elements must be divisible by two\n");
    return points;
  }

  if(numDimensions != 2) {
    Lrya::CoreTechPrint("Matlab array must be 2D\n");
    return points;
  }

  if(dimensions[0]!=2 && dimensions[1]!=2) {
    Lrya::CoreTechPrint("Matlab array must be 2xN or Nx2\n");
    return points;
  }

  if(dimensions[0]==2 && dimensions[1]==2) {
    Lrya::CoreTechPrint("Warning: Matlab array is 2x2, which is ambiguous\n");
  }
