...
void app_gap_bond_tk_cfm(uint8_t tk_type)
{
    ...
    if ((tk_type == GAP_TK_KEY_ENTRY) || (tk_type == GAP_TK_OOB))
    {
        ...
        memcpy(req->data.tk.key, spota_options.tk, spota_options.tk_len);
    }
...
int parse_cmd_line_args(int argc, char** argv)
{
    ...
    if (argc == tk_arg_pos + 1) {
        ...
        if (tk_len <= 6)
        {
            ...
            spota_options.tk_len = sizeof(*passkey_val);
            spota_options.tk = malloc(spota_options.tk_len);

            // Store passkey
            passkey_val = (void *) spota_options.tk;
        ...
        else if (tk_len == 2 * KEY_LEN)
        {
            ...
            for (i = 0; i < KEY_LEN; i++)
            {
                ...
                sscanf(pos + (2 * i), "%02hhx", &val);
                spota_options.tk[i] = val;
            }
        }
        else
        {
