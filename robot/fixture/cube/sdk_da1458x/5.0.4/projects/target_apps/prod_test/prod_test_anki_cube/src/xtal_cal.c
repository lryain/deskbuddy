...
// Testing standalone of Xtal-trimming
// update: 20151216 {HM}

// connection for testing 852
// Tx = P0.1 -> terminal emulator
// Rx = P0.0 -> input for 500 msec puls
...
int Overall_calculation(uint8_t port_number)
{
    ...
    while( (Fr[c1] < (IDEAL_CNT - PPM_1)) && (c1 < 10));
...
int auto_trim(uint8_t port_number)
{
    ...
    // check boundaries
    if ((Q == 1 && (C[2] < (IDEAL_CNT - PPM_BOUNDARY))) | (Q == 4 && (C[4] > (IDEAL_CNT + PPM_BOUNDARY))))
    {
    ...
    if (ccc < (IDEAL_CNT - PPM_1))
    {
        Setting_Trim(0);                        // if no square pulse was detected, then TRIM = 0
        return (-1);                            // no square pulse detected
    }
