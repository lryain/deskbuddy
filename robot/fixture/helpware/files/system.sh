...
if [ "$3" == "" ]; then
  /lrya/menuman/system.sh $1 $2 async &
  exit 0
fi

# Try to stop everything - if it fails, restart the wlan module
systemctl stop lryabluetoothd.service
...
/lrya/menuman/btconfig --soc smd /dev/smd3 rawcmd 0x08 0x001F   # BLE off
...
if [ $1 == BLE ]; then
  ...
  /lrya/menuman/btconfig --soc smd /dev/smd3 rawcmd 0x08 0x001E $2 0x23 0x00  # See KBA-170629211501 - 0x23=60% duty cycle
  exit 0
fi
if [ $1 == RX ]; then
  echo BLE RX on BLE channel $2 - hex 0x0..0x27
  rmmod wlan # Must stop WLAN before BLE test can take over
  /lrya/menuman/btconfig --soc smd /dev/smd3 rawcmd 0x08 0x001D $2 0x23 0x00  # See KBA-170629211501
  exit 0
fi
