...
#pragma mark --- Simulated Hardware Method Implementations ---
    ...
    Result HAL::Init(const int * shutdownSignal)
    {
      ...
      if (backpackButtonPressedField_ == nullptr) {
        LryaError("sim_hal.Init.NoBackpackButtonPressedField", "");
        return RESULT_FAIL;
      }
      ...
      if (touchSensorTouchedField_ == nullptr) {
        LryaError("sim_hal.Init.NoTouchSensorTouchedField", "");
        return RESULT_FAIL;
      }

      if (InitRadio() != RESULT_OK) {
        LryaError("sim_hal.Init.InitRadioFailed", "");
        return RESULT_FAIL;
      }
...
#     endif
    ...
    u16 HAL::GetButtonState(const ButtonID button_id)
    {
      switch(button_id) {
        ...
        default:
        {
          LryaError( "sim_hal.GetButtonState.UnexpectedButtonType", "Button ID=%d does not have a sensible return value", button_id);
          return 0;
        }
      }
    ...
    {
      LryaWarn("HAL.PrintBodyData.NotSupportedInSim", "");
    }
    ...
    void HAL::PowerSetDesiredMode(const PowerState state)
    {
      ...
      if (powerState_ != POWER_MODE_ACTIVE) {
        LryaWarn("HAL.PowerSetDesiredMode.UnsupportedMode", 
