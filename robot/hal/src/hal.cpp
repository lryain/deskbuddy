...
bool check_spine_readable(spine_ctx_t spine)
{
  ...
  if (selectTimeoutCount >= SELECT_TIMEOUT_ATTEMPTS) {
    LryaError("HAL.check_spine_readable.timeoutCountReached","");
  ...
  if (nfds < 0) {
    LryaWarn("HAL.check_spine_readable.error", "select error %s", strerror(errno));
    return false;
  }

  if (nfds == 0) {
    selectTimeoutCount++;
    LryaWarn("HAL.check_spine_readable.timeout", "select timeout %u", selectTimeoutCount);
...
void das_log_version_info(const VersionInfo* versionInfo)
{
  ...
  DASMSG_SET(s3, LRYA_BUILD_SHA,           "Build SHA")
  DASMSG_SEND();
}
...
Result spine_wait_for_first_frame(spine_ctx_t spine, const int * shutdownSignal)
{
  ...
  while (!initialized && *shutdownSignal == 0) {
    ...
    {
      LryaError("spine_wait_for_first_frame.timeout","");
      break;
    }
    ...
    if (r < 0) {
      ...
      if (hdr->payload_type == PAYLOAD_DATA_FRAME) {
        ...
        LryaWarn("HAL.SpineWaitForFirstFrame.InvalidSyscon","");
...
Result HAL::Init(const int * shutdownSignal)
{
  ...
  // Set ID
  robotID_ = Lrya::Vector::DEFAULT_ROBOT_ID;
  ...
  if (InitRadio() != RESULT_OK) {
    LryaError("HAL.Init.InitRadioFailed", "");
    return RESULT_FAIL;
  }

#ifndef HAL_DUMMY_BODY
  {
    LryaInfo("HAL.Init.StartingSpineHAL", "");
    ...
    LryaDebug("HAL.Init.SettingRunMode", "");

    spine_set_mode(&spine_, RobotMode_RUN);

    LryaDebug("HAL.Init.WaitingForDataFrame", "");

    const Result res =  spine_wait_for_first_frame(&spine_, shutdownSignal);
    if(res != RESULT_OK)
    {
      LryaError("HAL.Init.NoFirstFrame", "");
      return res;
    }
    LryaDebug("HAL.Init.GotFirstFrame", "");
...
Result HAL::Step(void)
{
  ...
  if (now-last_packet_send >= MIN_CCC_XMIT_SPACING_US ) {
    ...
    if (currPowerMode != desiredPowerMode_) {
      if ( ((lastPowerSetModeTime_ms_ == 0) && reportUnexpectedPowerMode_) ||
        ...
        LryaWarn("HAL.Step.UnexpectedPowerMode",
  ...
  do {
    ...
    if (timeSinceStartOfGetFrame_ms > SPINE_GET_FRAME_TIMEOUT_MS) {
      LryaError("HAL.Step.SpineLoopTimeout", "");
      return result;
    }
    ...
    if (*shutdownSignal_ != 0 && timeSinceStartOfGetFrame_ms > ROBOT_TIME_STEP_MS) {
      LryaInfo("HAL.Step.ShuttingDown", "");
      return RESULT_OK;
    }
  ...
  if (bodyData_ != nullptr) {
    ...
    if (currTemp > 50 &&
      ...
      LryaWarn("HAL.Step.BodyTemp", "%dC", currTemp);
...
void ProcessMicError()
{
  ...
  if(!sentDAS && whichChannelsStuck > 0)
  {
    ...
    LryaError("HAL.ProcessMicError.StuckBitDetected", "0x%x", whichChannelsStuck);
...
inline RangeStatus ConvertToApiRangeStatus(const u8 deviceRangeStatus)
{
  ...
  switch (internal_device_range_status)
  {
    ...
    default:
      ...
      // LryaWarn("HAL.ConvertToApiRangeStatus.InvalidStatus", "0x%02x", deviceRangeStatus);
  ...
  if (HAL::PowerGetMode() == POWER_MODE_CALM) {
    ...
    switch(proxData_.rangeStatus) {
      ...
      case RangeStatus::NO_UPDATE:
        ...
        break;
      default:
        LryaWarn("HAL.ProcessProxData.UnhandledStatus", "%s", EnumToString(proxData_.rangeStatus));
        break;
    }
...
u8 HAL::BatteryGetTemperature_C()
{
  if (bodyData_->battery.temperature > 0xff) {
    LryaWarn("HAL.BatteryGetTemperature_C.InvalidTemp", "%u", bodyData_->battery.temperature);
    return 0;
  }
...
{
  LryaInfo("HAL.PowerSetDesiredMode", "%d", state);
