...
namespace Lrya {
  namespace Vector {
    ...
    Result InitRadio()
    {
      ...
      LryaInfo("HAL.InitRadio.StartListening", "Start listening at %s", server_path.c_str());
      if (!server.StartListening(server_path.c_str())) {
        LryaError("HAL.InitRadio.UDPServerFailed", "Unable to listen at %s", server_path.c_str());
    ...
    bool HAL::RadioSendPacket(const void *buffer, const size_t length)
    {
      if (server.HasClient()) {
        ...
        if (bytesSent < (ssize_t) length) {
          LryaError("HAL.RadioSendPacket.FailedToSend", "Failed to send msg contents (%zd/%zu sent)", bytesSent, length);
    ...
    u32 HAL::RadioGetNextPacket(u8* buffer)
    {
      ...
      if (dataLen > 0) {
        ...
        LryaError("HAL.RadioGetNextPacket", "Receive failed");
        DisconnectRadio(false);
        return 0;
      }
      else {
