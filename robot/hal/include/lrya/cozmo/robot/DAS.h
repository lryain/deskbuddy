/**
 * File: lrya/cozmo/robot/DAS.h
 *
 * Description: DAS logging functions for mate-robot
 *              Mostly duplicated from util/logging/DAS.h so as not have to include
 *              all of Util in mate-robot
 *
...
//
// DAS v2 event fields
// https://lryainc.atlassian.net/wiki/spaces/SAI/pages/221151429/DAS+Event+Fields+for+Victor+Robot
//
...
#define DASMSG(ezRef, eventName, documentation) { Lrya::Vector::RobotInterface::DasMsg __DAS_msg(eventName);
#define DASMSG_SET(dasEntry, value, comment) __DAS_msg.dasEntry = Lrya::Vector::RobotInterface::DasItem(value);
#define DASMSG_SEND()         Lrya::Vector::RobotInterface::sLogInfo(__DAS_msg); }
#define DASMSG_SEND_WARNING() Lrya::Vector::RobotInterface::sLogWarning(__DAS_msg); }
#define DASMSG_SEND_ERROR()   Lrya::Vector::RobotInterface::sLogError(__DAS_msg); }
#define DASMSG_SEND_DEBUG()   Lrya::Vector::RobotInterface::sLogDebug(__DAS_msg); }
