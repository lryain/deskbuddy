...
if [ -e ${SCRIPT_PATH}/deskmate_env.sh ]; then
    # This is so that this same dfu script can be used for 
    # standalone syscon update packages containing deskmate_env.sh
    # in the same folder.
    source ${SCRIPT_PATH}/deskmate_env.sh
else
    source ${SCRIPT_PATH}/../project/deskmate/scripts/deskmate_env.sh
fi
...
: ${VERBOSE:=0}
: ${STAGING_DIR:="/lrya/bin/dfu_staging"}
...
robot_sh systemctl stop lrya-robot.target
...
while [ $NUM_RETRIES -ge 0 ]; do
    status=$(/usr/bin/expect << EOF
        spawn ssh ${LRYA_ROBOT_USER}@${LRYA_ROBOT_HOST}
        expect "~ #"
