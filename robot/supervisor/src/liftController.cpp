  ...
  namespace Vector {
    namespace LiftController {
      ...
      void OnMotorCalibrated()
      {
        ...
        LryaInfo("LiftController.Calibrated",
            ...
            if (currentAngle_rad_ - lowLiftAngleDuringCalib_rad_ > UPWARDS_LIFT_MOTION_FOR_CALIB_ABORT_RAD) {
              ...
              if (liftAngleHigherThanCalibAbortAngleCount_ >= UPWARDS_LIFT_MOTION_FOR_CALIB_ABORT_CNT) {
                if (firstCalibration_) {
                  LryaWarn("LiftController.CalibrationUpdate.RestartingCalib",
                  ...
                  LryaInfo("LiftController.CalibrationUpdate.Abort",
                           "Someone is probably messing with lift (low: %fdeg, curr: %fdeg)",
                           RAD_TO_DEG(lowLiftAngleDuringCalib_rad_), RAD_TO_DEG(currentAngle_rad_));
      ...
      void PoseAndSpeedFilterUpdate()
      {
        ...
        LryaDebug( "LiftController", "LIFT FILT: speed %f, speedFilt %f, currentAngle %f, currHalPos %f, prevPos %f, pwr %f\n",
                                    ...
                                    bool useVPG)
      {
        ...
        if (inPosition_ &&
          ...
          #if(DEBUG_LIFT_CONTROLLER)
          LryaDebug( "LiftController", "Already at desired angle %f", newDesiredAngle);
          #endif
          return;
        }
        ...
        if (duration_seconds > 0) {
          ...
          if (!res) {
            LryaInfo("LiftController.SetDesiredAngle.VPGFixedDurationFailed", 
...
#if DEBUG_LIFT_CONTROLLER
        LryaDebug( "LiftController", "VPG (fixedDuration): startVel %f, startPos %f, acc_start_frac %f, acc_end_frac %f, endPos %f, duration %f\n",
      ...
      bool MotorBurnoutProtection() {
        ...
        if (potentialBurnoutStartTime_ms_ == 0) {
          ...
          if (IsInPosition() || IMUFilter::IsBeingHeld() || ProxSensors::IsAnyCliffDetected()) {
            ...
            LryaInfo("LiftController.MotorBurnoutProtection.GoingLimp", "");
            Messages::SendMotorAutoEnabledMsg(MotorID::MOTOR_LIFT, false);
            DisableInternal(true);
          } else {
            // Burnout protection triggered. Recalibrating.
            LryaInfo("LiftController.MotorBurnoutProtection.Recalibrating", "");
      ...
      void Unbrace() {
        LryaInfo("LiftController.Unbrace", "");
        SetPower(0.f);
      ...
      Result Update()
      {
        ...
        if (bracing_ || MotorBurnoutProtection()) {
          ...
          if ((unbracingStartTime_ms_ > 0) &&
            ...
            LryaInfo("LiftController.Update.UnbracingComplete", "");
            unbracingStartTime_ms_ = 0;
            ResetAnglePosition(currentAngle_rad_);
            prevAngleError_ = 0.f;
            angleErrorSum_ = 0.f;
        ...
        if (checkingForLoadStartTime_ > 0) {
          if (currTime > checkingForLoadStartTime_ + CHECKING_FOR_LOAD_TIMEOUT_MS) {
            LryaInfo( "LiftController.Update.NoLoadDetected", "");
            ...
            LryaInfo( "LiftController.Update.LoadDetected", "in %d ms", currTime - checkingForLoadStartTime_);
            checkForLoadWhenInPosition_ = false;
            checkingForLoadStartTime_ = 0;
            if (checkForLoadCallback_) {
        ...
        if((ABS(angleError) < LIFT_ANGLE_TOL) && (desiredAngle_rad_ == currDesiredAngle_rad_)) {
          ...
          if (ABS(power_) > maxPowerInPosition) {
            ...
            LryaInfo( "LiftController.Update.CheckingForLoad", "%d", checkingForLoadStartTime_);
            power_ = 0;
          }
          ...
          if (lastInPositionTime_ms_ == 0) {
            ...
            inPosition_ = true;
#if(DEBUG_LIFT_CONTROLLER)
            LryaDebug( "LiftController", " LIFT HEIGHT REACHED (%f mm)", GetHeightMM());
#endif
          }
        } else {
...
#if(DEBUG_LIFT_CONTROLLER)
        LryaDebugPeriodic(50, "LiftController.Update.Values", "LIFT: currA %f, curDesA %f, currVel %f, desA %f, err %f, errSum %f, inPos %d",
                          currentAngle_rad_,
                          ...
                          angleErrorSum_,
                          inPosition_ ? 1 : 0);
        LryaDebugPeriodic(50, "LiftController.Update.Power", "P: %f, I: %f, D: %f, total: %f",
                          (Kp_ * angleError),
                          (Ki_ * angleErrorSum_),
      ...
      void SetGains(const f32 kp, const f32 ki, const f32 kd, const f32 maxIntegralError)
      {
        ...
        LryaInfo( "LiftController.SetGains", "New lift gains: kp = %f, ki = %f, kd = %f, maxSum = %f",
