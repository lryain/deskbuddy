...
namespace Lrya
{
  namespace Vector
  {
    namespace PathFollower
    {
      ...
      {
        LryaAssert(path_.GetNumSegments() > 0, "");
        ...
        for (u8 i=0; i<path_.GetNumSegments(); ++i) {
          ...
          LryaDebug( "PathFollower.GetClosestSegment.PathDist", "%f  (res=%d)", distToSegment, res);
#endif
...
#if(DEBUG_PATH_FOLLOWER)
            LryaDebug( "PathFollower.GetClosestSegment", " New closest seg: %d, distToSegment %f (res=%d)", i, distToSegment, res);
#endif
          }
        }
      ...
      bool StartPathTraversal(u16 path_id)
      {
        ...
        if (path_.GetNumSegments() > 0) {
          ...
          LryaConditionalErrorAndReturnValue(path_.CheckContinuity(CONTINUITY_TOL_MM2), false, "PathFollower.StartPathTraversal.PathIsDiscontinuous", "");
          ...
          LryaDebug( "PathFollower.StartPathTraversal", "Start segment %d, speed = %f, accel = %f, decel = %f",
                currPathSegment_,
                targetSpeed,
      ...
      Planning::SegmentRangeStatus ProcessPathSegment(f32 &shortestDistanceToPath_mm, f32 &radDiff)
      {
        ...
        LryaAssert(Planning::PST_LINE == path_[currPathSegment_].GetType() || Planning::PST_ARC == path_[currPathSegment_].GetType(), "");
        ...
        if ((currPathSegment_ == path_.GetNumSegments() - 1) || (path_[currPathSegment_+1].GetType() == Planning::PST_POINT_TURN)) {
          ...
          if (!startedDecelOnSegment_) {
            ...
            if (distToStopIfDecelNow >= distToEnd) {
              ...
              //LryaDebug( "PathFollower", "PathFollower: Decel to end of segment %d (of %d) at %d mm/s^2 from speed of %d mm/s (meas %d mm/s) over %f mm\n",
...
#         if(DEBUG_PATH_FOLLOWER)
            ...
            LryaDebug( "PathFollower.ProcessPathSegment.Decel", "currCmdSpeed %d mm/s, currSpeed %d mm/s)",
...
#         endif
      ...
      Planning::SegmentRangeStatus ProcessPathSegmentPointTurn(f32 &shortestDistanceToPath_mm, f32 &radDiff)
      {
        ...
        LryaDebug( "PathFollower.ProcessPathSegmentPointTurn", "currPathSeg: %d, TURN currAngle: %f, targetAngle: %f",
...
#if(DEBUG_PATH_FOLLOWER)
          LryaDebug( "PathFollower.ProcessPathSegmentPointTurn.ExecutePointTurn", "");
#endif
      ...
      void PathComplete()
      {
        ...
        LryaInfo("PathFollower.PathComplete", "");
      }
      ...
      Result Update()
      {
        ...
        switch (path_[currPathSegment_].GetType()) {
          ...
          case Planning::PST_POINT_TURN:
            ...
            break;
          default:
            LryaWarn( "PathFollower.Update.InvalidSegmentType", "Segment %d has invalid type %d", currPathSegment_, path_[currPathSegment_].GetType());
            break;
        }

#if(DEBUG_PATH_FOLLOWER)
        LryaDebug( "PathFollower.Update.DistToPath", "%f mm, %f deg, segRes %d, segType %d, currSeg %d", distToPath_mm_, RAD_TO_DEG_F32(radToPath_), segRes, path_[currPathSegment_].GetType(), currPathSegment_);
#endif
...
#if(DEBUG_PATH_FOLLOWER)
          LryaDebug( "PathFollower.Update.SegmentSpeed", "Segment %d, speed = %f, accel = %f, decel = %f",
                currPathSegment_,
                targetSpeed,
                path_[currPathSegment_].GetAccel(),
                path_[currPathSegment_].GetDecel());
        ...
        if (!DockingController::IsBusy()) {
          ...
          if (ABS(distToPath_mm_) > TOO_FAR_FROM_PATH_DIST_MM) {
            LryaWarn( "PathFollower.Update.StartingErrorTooLarge", "%f mm", distToPath_mm_);
            ClearPath();
            return RESULT_FAIL;
          }
        }
      ...
      bool DriveStraight(f32 dist_mm, f32 acc_start_frac, f32 acc_end_frac, f32 duration_sec)
      {
        ...
        if (acc_start_frac < 0 || acc_end_frac < 0) {
          LryaWarn( "PathFollower.DriveStraight.NegativeFraction", "start: %f, end: %f", acc_start_frac, acc_end_frac);
          return false;
        }
        ...
        if (!vpg.StartProfile_fixedDuration(0, currSpeed, acc_start_frac * duration_sec,
          ...
          LryaWarn( "PathFollower.DriveStraight.VPGFail", "");
          return false;
        }
        ...
        LryaInfo( "PathFollower.DriveStraight.Params", "total dist %f, startDist %f, endDist %f", dist_mm, startAccelDist, endAccelDist);
        ...
        LryaDebug( "PathFollower.DriveStraight.Accels", "start %f, end %f, vel %f\n", startAccel, endAccel, maxReachableVel);
        LryaDebug( "PathFollower.DriveStraight.Points", "(%f, %f) to (%f, %f) to (%f, %f) to (%f, %f)\n", curr_x, curr_y, int_x1, int_y1, int_x2, int_y2, dest_x, dest_y);
        ClearPath();
      ...
      bool DriveArc(f32 sweep_rad, f32 radius_mm, f32 acc_start_frac, f32 acc_end_frac, f32 duration_sec)
      {
        ...
        // Check for valid fractions
        if (acc_start_frac < 0 || acc_end_frac < 0) {
          LryaWarn( "PathFollower.DriveArc.NegativeFraction", "start: %f, end: %f", acc_start_frac, acc_end_frac);
          return false;
        }
        ...
        if (!vpg.StartProfile_fixedDuration(0, currAngSpeed, acc_start_frac * duration_sec,
                                            ...
                                            duration_sec, CONTROL_DT) ) {
          LryaWarn( "PathFollower.DriveArc.VPGFail", "");
          return false;
        }
        ...
        LryaDebug( "PathFollower.DriveArc", "curr_x,y  (%f, %f), center x,y (%f, %f), radius %f", curr_x, curr_y, x_center, y_center, radius_mm);
        LryaDebug( "PathFollower.DriveArc", "start + sweep1 = ang1 (%f + %f = %f), end + sweep2 = ang2 ang2 (%f - %f = %f)", startRad, startAccelSweep, int_ang1, startRad + sweep_rad, endAccelSweep, int_ang2);
        LryaDebug( "PathFollower.DriveArc", "targetSpeed %f, startAccel %f, endAccel %f", targetSpeed, startAccel, endAccel);
      ...
      bool DrivePointTurn(f32 sweep_rad, f32 acc_start_frac, f32 acc_end_frac, f32 angleTolerance, f32 duration_sec)
      {
        ...
        // Check for valid fractions
        if (acc_start_frac < 0 || acc_end_frac < 0) {
          LryaWarn( "PathFollower.DrivePointTurn.NegativeFraction", "start: %f, end: %f", acc_start_frac, acc_end_frac);
          return false;
        }
        ...
        if (!vpg.StartProfile_fixedDuration(0, 0, acc_start_frac * duration_sec,
                                            ...
                                            MAX_BODY_ROTATION_SPEED_RAD_PER_SEC, MAX_BODY_ROTATION_ACCEL_RAD_PER_SEC2,
                                            duration_sec, CONTROL_DT)) {
          LryaWarn( "PathFollower.DrivePointTurn.VPGFail", "sweep_rad: %f, acc_start_frac %f, acc_end_frac %f, duration_sec %f",
        ...
        LryaDebug( "PathFollower.DrivePointTurn", "start %f, int_ang1 %f, int_ang2 %f, dest %f", curr_angle.ToFloat(), int_ang1, int_ang2, dest_ang);
        LryaDebug( "PathFollower.DrivePointTurn", "targetRotSpeed %f, startRotAccel %f, endRotAccel %f", targetRotVel, startAngAccel, endAngAccel);
