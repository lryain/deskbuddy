...
namespace Lrya {
  ...
  namespace SteeringController {
    ...
    {
      LryaInfo( "SteeringController.SetGains", "New gains: k1 %f, k2 %f, dist_cap %f mm, ang_cap %f rad",
    ...
    void SetPointTurnGains(f32 kp, f32 ki, f32 kd, f32 maxIntegralError)
    {
      ...
      */
      LryaInfo( "SteeringController.SetPointTurnGains", "New gains: kp %f, ki %f, kd %f, maxSum %f", kp, ki, kd, maxIntegralError);
    }
    ...
    void Manage()
    {
      ...
      LryaDebug( "SteeringController.Manage.Mode", "%d", currSteerMode_);
#endif
...
//          LryaInfo( "SteeringController", "  Wheel speed adjust: (%f, %f), adjustment %f", *higherWheelSpeed, *lowerWheelSpeed, speedAdjust);
        }
      }
     ...
     * @copyright This original code is provided to Lrya, Inc under royalty-free non-exclusive license per terms of consulting agreement.
...
#if(DEBUG_STEERING_CONTROLLER)
      LryaDebug( "SteeringController.RunLineFollowControllerNL.Errors", "offsetError_mm: %f, headingError_rad: %f, curvature: %f, currSpeed: %d", offsetError_mm, headingError_rad, curvature, currspeed);
#endif
...
#if(DEBUG_STEERING_CONTROLLER)
      LryaDebug( "SteeringController.RunLineFollowControllerNL.Speeds", "%d (L), %d (R)", wleft, wright);
#endif
    ...
    {
      if (!enable_) {
        LryaDebug("SteeringController.ExecuteDirectDrive.Disabled", "lspeed: %f mm/s, rspeed: %f mm/s", left_vel, right_vel);
        return;
      }

      //LryaDebug( "DIRECT DRIVE", "%d: %f   %f  (%f %f)", HAL::GetTimeStamp(), left_vel, right_vel, left_accel, right_accel);
...
//      PRINT("CURR: %f %f\n", targetLeftVel_, targetRightVel_);
    ...
    void ExecutePointTurn_Internal(f32 maxAngularVel, f32 angularAccel)
    {
      ...
      if (ABS(maxAngularVel_) < POINT_TURN_TERMINAL_VEL_RAD_PER_S) {
        ...
        LryaWarn( "SteeringController.ExecutePointTurn_Internal.PointTurnTooSlow", "Speeding up commanded point turn of %f rad/s to %f rad/s",  maxAngularVel, maxAngularVel_);
      }
    ...
    {
      if (!enable_) {
        LryaDebug("SteeringController.ExecutePointTurn.Disabled", "Speed: %f rad/s", angularVel);
        return;
      }

      LryaDebug( "SteeringController.ExecutePointTurn_2.Params", "%d: vel %f, accel %f",
      ...
      if (isnan(angularVel) || isnan(angularAccel)) {
        LryaWarn( "SteeringController.ExecutePointTurn_2.NanParamFound", "%f, %f", angularVel, angularAccel);
        return;
      }
      
      if (fabsf(angularVel) > MAX_BODY_ROTATION_SPEED_RAD_PER_SEC) {
        LryaWarn( "SteeringController.ExecutePointTurn_2.PointTurnTooFast", "Speed of %f deg/s exceeds limit of %f deg/s. Clamping.",
    ...
    void ExitPointTurn()
    {
      LryaDebug( "SteeringController.ExitPointTurn", "");
    ...
    {
      if (!enable_) {
        LryaDebug("SteeringController.ExecutePointTurn.Disabled", "targetAngle: %f rad, Speed: %f rad/s", targetAngle, maxAngularVel);
        return;
      }

      LryaDebug( "SteeringController.ExecutePointTurn.Params", "%d: target %f, vel %f, accel %f, decel %f, tol %f, shortestDir %d, numHalfRevs %d",
                HAL::GetTimeStamp(),
      ...
      if (isnan(targetAngle)   ||
        ...
        LryaWarn( "SteeringController.ExecutePointTurn.NanParamFound", "%f, %f, %f, %f, %f",
      ...
      if (fabsf(maxAngularVel) > MAX_BODY_ROTATION_SPEED_RAD_PER_SEC) {
        LryaWarn( "SteeringController.ExecutePointTurn.PointTurnTooFast", "Speed of %f deg/s exceeds limit of %f deg/s. Clamping.",
      ...
      if ((fabsf(angularDistExpected_) < pointTurnAngTol_) && !WheelController::AreWheelsMoving()) {
        ExitPointTurn();
        LryaDebug( "SteeringController.ExecutePointTurn.AlreadyAtDest", "");
        return;
      }
    ...
    void ManagePointTurn()
    {
      ...
      if (currPitch.getAbsoluteVal() > MAX_PITCH_POINT_TURN_RAD && !isBeingHeld) {
        ExitPointTurn();
        LryaInfo( "SteeringController.ManagePointTurn.StoppingDueToPitch",
      ...
      if (isPointTurnWithTarget_) {
        ...
        if (absAngularDistToTarget < pointTurnAngTol_) {
          if (inPositionStartTime_ == 0) {
            LryaDebug( "SteeringController.ManagePointTurn.InRange", "distToTarget %f, currAngle %f, currDesired %f (currTime %d, inPosTime %d)", RAD_TO_DEG_F32(angularDistToTarget), currAngle.getDegrees(), RAD_TO_DEG_F32(currDesiredAngle), HAL::GetTimeStamp(), inPositionStartTime_);
...
#           if(DEBUG_STEERING_CONTROLLER)
            ...
            LryaDebug( "SteeringController.ManagePointTurn.Stopping", "currAngle %f, currDesired %f, currVel %f, distTraversed %f, distExpected %f,  wheelSpeeds %f %f, desSpeeds %f %f",
...
#           endif
          ...
          LryaDebugPeriodic(100, "SteeringController.ManagePointTurn.OOR", "distToTarget %f, currAngle %f, currDesired %f (currTime %d, inPosTime %d)", RAD_TO_DEG_F32(angularDistToTarget), currAngle.getDegrees(), RAD_TO_DEG_F32(currDesiredAngle), HAL::GetTimeStamp(), inPositionStartTime_);
          ...
          if (fabsf(pointTurnAngleErrorSum_) == pointTurnMaxIntegralError_) {
            if (pointTurnIntegralPowerMaxedStartTime_ == 0 || fabsf((pointTurnIntegralPowerMaxedStartAngle_ - currAngle).ToFloat()) < POINT_TURN_STUCK_THRESHOLD_RAD) {
              ...
              LryaInfo( "SteeringController.ManagePointTurn.StoppingCuzStuck", "currAngle %f, currDesired %f", currAngle.getDegrees(), RAD_TO_DEG_F32(currDesiredAngle));
              ExitPointTurn();
              return;
            }
          }
...
//        LryaDebug( "SteeringController.ManagePointTurn.Controller", "timestamp %d, currAngle %.1f, desAngle %.1f, currSpeed %.1f, desSpeed %.1f, arcVel %d, ff %.1f, p %.1f, i %.1f, d %.1f, errSum %.1f",
//                  HAL::GetTimeStamp(),
...
//                  pointTurnAngleErrorSum_);
        ...
        //LryaDebug( "PointTurnSpeed", "des %f deg/s, meas: %f deg/s, arcVel %d mm/s, errorSum %f", RAD_TO_DEG_F32(currDesiredAngularVel), RAD_TO_DEG_F32(currAngularSpeed), arcVel, pointTurnAngleErrorSum_ );
      }


#if(DEBUG_STEERING_CONTROLLER)
      LryaDebug( "SteeringController.ManagePointTurn.DistToTarget", "angularDistToTarget: %f radians, arcVel: %d mm/s",  angularDistToTarget, arcVel);
#endif
      ...
      if (!isPointTurnWithTarget_) {
        if (arcVel == 0 && maxAngularVel_ == 0) {
          ExitPointTurn();
          LryaDebug( "SteeringController.ManagePointTurn.StoppingBecause0Vel", "");
          return;
        }
      }
    }
    ...
    {
      if (!enable_) {
        LryaDebug("SteeringController.ExecuteDriveCurvature.Disabled", "Speed: %f mm/s, curvature: %f mm", speed, curvatureRadius_mm);
        return;
      }
