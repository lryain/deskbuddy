...
namespace Lrya {
  namespace Vector {
    namespace TestModeController {
      ...
      Result Reset()
      {
        LryaInfo( "TestModeController.Reset", "");
      ...
      {
        LryaInfo( "TestModeController.PlaceOnGroundTestInit", "xOffset %d mm, yOffset %d mm, angleOffset %d degrees",
      ...
      Result PlaceOnGroundTestUpdate()
      {
        if (!PickAndPlaceController::IsBusy()) {
          LryaInfo( "TestModeController.PlaceOnGroundTestUpdate.Complete", "");
          Reset();
        }
        return RESULT_OK;
      }
      ...
      {
        LryaInfo( "TestModeController.DockPathTestInit", "");
        ticCnt_ = 0;
      ...
      Result DockPathTestUpdate()
      {
        ...
        if (ticCnt_++ >= DOCK_PATH_TOGGLE_TIME_S * ONE_OVER_CONTROL_DT ) {
          ...
          LryaInfo( "TestModeController.DockPathTestUpdate", "Test state: %d", dockPathState_);
          ticCnt_ = 0;
        }
      ...
      {
        LryaInfo( "TestModeController.PathFollowTestInit", "");
        pathStarted_ = false;
      ...
      Result PathFollowTestUpdate()
      {
        ...
        if (!pathStarted_ && HAL::GetTimeStamp() > startDriveTime_ms) {
          ...
          */
          ...
          LryaInfo( "TestModeController.PathFollowTestUpdate.Complete", "");
          Reset();
        }
      ...
      {
        LryaInfo( "TestModeController.PathFollowConvenienceFuncTestInit", "");
        PathFollower::ClearPath();
        ...
        if (!PathFollower::IsTraversingPath()) {
          ...
          switch(pfcfState_) {
            case PFCF_DRIVE_STRAIGHT:
              LryaInfo( "TestModeController.PathFollowConvenienceFuncTestUpdate.DriveStraight", "Started");
              if (!PathFollower::DriveStraight(100, 0.25, 0.25, 2)) {
                 LryaInfo( "TestModeController.PathFollowConvenienceFuncTestUpdate.DriveStraight", "FAILED");
                return RESULT_FAIL;
              }
              break;

            case PFCF_DRIVE_ARC:
              LryaInfo( "TestModeController.PathFollowConvenienceFuncTestUpdate.DriveArc", "Started");
              if (!PathFollower::DriveArc(-M_PI_2_F, 40, 0.25, 0.25, 1)) {
                LryaInfo( "TestModeController.PathFollowConvenienceFuncTestUpdate.DriveArc", "FAILED");
                return RESULT_FAIL;
              }
            ...
            case PFCF_DRIVE_POINT_TURN:
              LryaInfo( "TestModeController.PathFollowConvenienceFuncTestUpdate.DrivePointTurn", "Started");
              if (!PathFollower::DrivePointTurn(-M_PI_2_F, 0.25, 0.25, DEG_TO_RAD_F32(1.f), 1)) {
              LryaInfo( "TestModeController.PathFollowConvenienceFuncTestUpdate.DrivePointTurn", "FAILED");
                return RESULT_FAIL;
              }
              break;
            default:
      ...
      {
        LryaInfo( "TestModeController.DriveTestInit", "%x, %d, %d", flags, powerStepPercent, wheelSpeedOrPower);
      ...
      Result DriveTestUpdate()
      {
        ...
        if (ticCnt_++ >= WHEEL_TOGGLE_DIRECTION_PERIOD_MS / ROBOT_TIME_STEP_MS) {
          ...
          if (enableDirectHALTest_) {
            LryaInfo( "TestModeController.DriveTestUpdate", "Applying %.3f power (currSpeed %.2f %.2f, filtSpeed %.2f %.2f)",
                  wheelTargetPower_*0.01f,
                  lSpeed, rSpeed, lSpeed_filt, rSpeed_filt);
          } else {
            LryaInfo( "TestModeController.DriveTestUpdate", "Going %.3f mm/s (currSpeed %.2f %.2f, filtSpeed %.2f %.2f)",
                  wheelSpeed_mmps_,
                  lSpeed, rSpeed, lSpeed_filt, rSpeed_filt);
          }
          ...
          // To prevent compiler error if LryaInfo compiled out 
          (void)lSpeed;
          (void)rSpeed;
        }
      ...
      {
        LryaInfo( "TestModeController.LiftTestInit", "flags = %d, powerPercent = %d", mode, powerPercent);
        ticCnt_ = 0;
        ticCnt2_ = 0;
      ...
      Result LiftTestUpdate()
      {
        ...
        switch (liftTestMode_) {
          ...
          case LiftTestFlags::LiftTF_TEST_HEIGHTS:
          {
            ...
            if (ticCnt_++ >= 1000 / ROBOT_TIME_STEP_MS) {
              ...
              if (liftTestMode_ == LiftTestFlags::LiftTF_TEST_HEIGHTS) {
                up = !up;
                if (up) {
                  LryaInfo( "TestModeController.LiftTestUpdate", "Lift HIGH %f mm (maxSpeed %f rad/s)", LIFT_DES_HIGH_HEIGHT, maxLiftSpeed_radPerSec_);
                  LiftController::SetDesiredHeight(LIFT_DES_HIGH_HEIGHT);
                } else {
                  LryaInfo( "TestModeController.LiftTestUpdate", "Lift LOW %f mm (maxSpeed %f rad/s)", LIFT_DES_LOW_HEIGHT, maxLiftSpeed_radPerSec_);
                ...
                up = !up;
                if (up) {
                  LryaInfo( "TestModeController.LiftTestUpdate", "Lift UP %f power (maxSpeed %f rad/s)", liftPower_, maxLiftSpeed_radPerSec_);
                  HAL::MotorSetPower(MotorID::MOTOR_LIFT, liftPower_);
                } else {
                  LryaInfo( "TestModeController.LiftTestUpdate", "Lift DOWN %f power (maxSpeed %f rad/s)", -liftPower_, maxLiftSpeed_radPerSec_);
                  HAL::MotorSetPower(MotorID::MOTOR_LIFT, -liftPower_);
                }
          ...
          default:
          {
            LryaWarn( "TestModeController.LiftTestUpdate", "WARN: Unknown lift test mode %hhu", liftTestMode_);
            Reset();
          }
        }
        ...
        if (ABS(lSpeed) > 0.001f) {
          ...
          LryaInfo( "TestModeController.LiftTestUpdate", "Avg lift speed %f rad/s, height change %f mm", avgLiftSpeed_, LiftController::GetHeightMM() - startLiftHeightMM_);
          avgLiftSpeed_ = 0;
          ticCnt2_ = 0;
        }
      ...
      {
        LryaInfo( "TestModeController.LiftToggleTestInit", "");
      ...
      Result LiftToggleTestUpdate()
      {
        ...
        if (LiftController::IsInPosition()) {
          if (inPositionCycles++ > 500) {
            ...
            LryaInfo( "TestModeController.LiftToggleTestUpdate", "SET LIFT TO %f mm", liftHeight);
        ...
        if (ticCnt2_++ >= printCyclePeriod_) {
          ...
          LryaInfo( "TestModeController.LiftToggleTestUpdate", "Lift speed %f rad/s, height %f mm", lSpeed, lPos);
          ticCnt2_ = 0;
      ...
      {
        LryaInfo( "TestModeController.HeadTestInit", "flags = %d, powerPercent = %d", mode, powerPercent);
        ticCnt_ = 0;
        ticCnt2_ = 0;
        printCyclePeriod_ = 250 / ROBOT_TIME_STEP_MS;
      ...
      Result HeadTestUpdate()
      {
        ...
        switch(headTestMode_) {
          ...
          case HeadTestFlags::HTF_TEST_ANGLES:
          {
            ...
            if (ticCnt_++ >= headNodCycleTime_ms_ / ROBOT_TIME_STEP_MS) {
              ...
              if (headTestMode_ == HeadTestFlags::HTF_TEST_ANGLES) {
                up = !up;
                if (up) {
                  LryaInfo( "TestModeController.HeadTestUpdate", "Head HIGH %f rad (maxSpeed %f rad/s)", HEAD_DES_HIGH_ANGLE, maxHeadSpeed_radPerSec_);
                  HeadController::SetDesiredAngle(HEAD_DES_HIGH_ANGLE);
                } else {
                  LryaInfo( "TestModeController.HeadTestUpdate", "Head LOW %f rad (maxSpeed %f rad/s)", HEAD_DES_LOW_ANGLE, maxHeadSpeed_radPerSec_);
                  HeadController::SetDesiredAngle(HEAD_DES_LOW_ANGLE);
                }
              } else {
                up = !up;
                if (up) {
                  LryaInfo( "TestModeController.HeadTestUpdate", "Head UP %f power (maxSpeed %f rad/s)", headPower_, maxHeadSpeed_radPerSec_);
                  HAL::MotorSetPower(MotorID::MOTOR_HEAD, headPower_);
                } else {
                  LryaInfo( "TestModeController.HeadTestUpdate", "Head DOWN %f power (maxSpeed %f rad/s)", -headPower_, maxHeadSpeed_radPerSec_);
                  HAL::MotorSetPower(MotorID::MOTOR_HEAD, -headPower_);
                }
          ...
          default:
            LryaWarn( "TestModeController.HeadTestUpdate", "Unknown head test mode %hhu", headTestMode_);
            break;
        }
        ...
        if (ticCnt2_++ >= printCyclePeriod_) {
          ...
          if (ABS(hSpeed) > 0 || ABS(hSpeed_filt) > 0) {
            LryaInfo( "TestModeController.HeadTestUpdate", "Head speed %f rad/s (filt %f rad/s), angle %f rad", hSpeed, hSpeed_filt, hPos);
          }
          ticCnt2_ = 0;
      ...
      {
        LryaInfo( "TestModeController.IMUTestInit", "");
      ...
      Result IMUTestUpdate()
      {
        ...
        if (++ticCnt_ >= 200 / ROBOT_TIME_STEP_MS) {
          ...
          LryaInfo( "TestModeController.IMUTestUpdate", "Gyro (%f,%f,%f) rad/s, (%f,%f,%f) mm/s^2",
          ...
          LryaInfo( "TestModeController.IMUTestUpdate", "Rot(IMU): %f deg",
      ...
      {
        LryaInfo( "TestModeController.LightTestInit", "flags = %x, ledID = %d, color = %x", flags, ledID, color);
      ...
      Result LightTestUpdate()
      {
        ...
        if (ticCnt_++ > 2000 / ROBOT_TIME_STEP_MS) {
          LryaInfo( "TestModeController.LightTestUpdate", "LED channel %hhu, color 0x%x", ledID_, LEDColorList_[ledColorIdx_]);
      ...
      {
        LryaInfo( "TestModeController.StopTestInit", "");
        ticCnt_ = 0;
        ST_go = false;
        ST_speed = 0.f;
      ...
      Result StopTestUpdate()
      {
        if (ticCnt_++ > ST_period_ms / ROBOT_TIME_STEP_MS) {
          ...
          if(ST_go) {
            ...
            LryaInfo( "TestModeController.StopTestUpdate", "GO: %f mm/s", ST_speed);
        ...
        if(ST_slowingDown) {
          if (HAL::MotorGetPosition(MotorID::MOTOR_LEFT_WHEEL) == ST_prevLeftPos &&
            ...
            LryaInfo( "TestModeController.StopTestUpdate", "STOPPED: (%f, %f) mm in %d tics", ST_prevLeftPos, ST_prevRightPos, ST_slowDownTics);
            ST_slowingDown = false;
          }
      ...
      {
        LryaInfo( "TestModeController.MaxPowerTestInit", "");
        ticCnt_ = 0;
      ...
      Result MaxPowerTestUpdate()
      {
        ...
        if (ticCnt_++ > 5000 / ROBOT_TIME_STEP_MS) {
          LryaInfo( "TestModeController.MaxPowerTestUpdate", "SWITCHING POWER: %f", pwr);
      ...
      Result Start(const TestMode mode, s32 p1, s32 p2, s32 p3)
      {
        ...
        switch(testMode_) {
          ...
          case TestMode::TM_MAX_POWER_TEST:
            ...
            break;
          default:
            LryaWarn( "TestModeController.Start", "Undefined test mode %hhu\n", testMode_);
            Reset();
            ret = RESULT_FAIL;
            break;
        }
