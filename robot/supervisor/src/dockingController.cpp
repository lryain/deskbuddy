  ...
  namespace Vector {
    namespace DockingController {
      ...
      namespace {
        ...
        Lrya::Embedded::Pose2d approachStartPose_;

        // The pose of the block as we're docking (note: this is actually the _marker_ pose)
        Lrya::Embedded::Pose2d blockPose_;

        // The docking pose
        Lrya::Embedded::Pose2d dockPose_;
        ...
        Lrya::Embedded::Pose2d lastMarkerPoseObservedInValidFOV_;
        ...
        Lrya::Embedded::Pose2d backupPose_; // Pose of robot when it started to backup
      ...
      void SetDockingMethod(DockingMethod method)
      {
        if(!IsBusy())
        {
          //LryaDebug( "DockingController", "Setting docking method: %d", method);
      ...
      f32 GetVerticalFOV() {
        LryaConditionalError(headCamFOV_ver_ != 0.f, "DockingController.GetVerticalFOV.ZeroFOV", "");
        return headCamFOV_ver_;
      }

      f32 GetHorizontalFOV() {
        LryaConditionalError(headCamFOV_hor_ != 0.f, "DockingController.GetHorizontalFOV.ZeroFOV", "");
      ...
      bool IsMarkerInFOV(const Lrya::Embedded::Pose2d &markerPose, const f32 markerWidth)
      {
      ...
      Result SendGoalPoseMessage(const Lrya::Embedded::Pose2d &p)
      {
        GoalPose msg;
        msg.pose.x = p.GetX();
        msg.pose.y = p.GetY();
      ...
      {
        LryaDebug( "DockingController.SetCameraFieldOfView.Values", "H: %f, V: %f", horizontalFOV, verticalFOV);
      ...
      Result Update()
      {
        ...
        if(failureMode_ == BACKING_UP)
        {
          ...
          if(Localization::GetDistTo(backupPose_.x(), backupPose_.y()) > DIST_TO_BACKUP_ON_FAILURE_MM)
          {
            ...
            if(IsHybridDocking())
            {
              LryaDebug( "DockingController.Update.BackingUpAndStopping", "");
        ...
        while( dockingErrSignalMsgReady_ )
        {
          ...
          if (lastTime != 0) {
            ...
            LryaDebug( "DockingController.Update.TrackerPeriod", "%d ms", period);
          }
          lastTime = currTime;
#endif
...
//              LryaInfo( "DockingController.Update.NoLongerPastPointOfNoReturn", "(%f > %d)",
...
//          }
...
#if(DEBUG_DOCK_CONTROLLER)
          LryaInfo( "DockingController.Update.ReceivedErrorSignal", "time=%d, x_distErr=%f, y_horErr=%f, z_height=%f, angleErr=%fdeg",
        ...
        if (markerOutOfFOV_) {
          ...
          if (IsMarkerInFOV(lastMarkerPoseObservedInValidFOV_, MARKER_WIDTH)) {
            LryaDebug( "DockingController.Update.MarkerExpectedInsideFOV", "");
          ...
          if (lastMarkerPoseObservedIsSet_) {
            ...
            if (PathFollower::IsTraversingPath() &&
              ...
              LryaDebug( "DockingController.Update.MarkerExpectedOutsideFOV", "");
...
#if(DEBUG_DOCK_CONTROLLER)
          LryaInfo( "DockingController.Update.PointOfNoReturn", "(%f < %d) mode:%i", distToMarker, pointOfNoReturnDistMM_, mode_);
#endif
        ...
        switch(mode_)
        {
          ...
          case LOOKING_FOR_BLOCK:
            ...
            if ((!pastPointOfNoReturn_)
              ...
              LryaInfo( "DockingController.LOOKING_FOR_BLOCK.TooLongWithoutErrorSignal", "currTime %d, lastErrSignal %d, Giving up.", HAL::GetTimeStamp(), lastDockingErrorSignalRecvdTime_);
            }
            break;
          case APPROACH_FOR_DOCK:
          {
            ...
            if(dockingMethod_ != DockingMethod::EVEN_BLINDER_DOCKING)
            {
              ...
              if (!markerlessDocking_
                ...
                SpeedController::SetUserCommandedDesiredVehicleSpeed(0);
                mode_ = LOOKING_FOR_BLOCK;
                LryaInfo( "DockingController.APPROACH_FOR_DOCK.TooLongWithoutErrorSignal", "currTime %d, lastErrSignal %d, Looking for block...", HAL::GetTimeStamp(), lastDockingErrorSignalRecvdTime_);
                break;
              }
            }
                ...
                failureMode_ == NO_FAILURE)
            {
              ...
              Lrya::Embedded::Pose2d curPose = Localization::GetCurrPose();
              Localization::ConvertToDriveCenterPose(curPose, curPose);
              Lrya::Embedded::Pose2d relPose = dockPose_.GetWithRespectTo(curPose);
              ...
              if(dockingMethod_ != DockingMethod::BLIND_DOCKING && dockingMethod_ != DockingMethod::EVEN_BLINDER_DOCKING)
              {
                ...
                if(!markerlessDocking_ && !isBeelineDocking)
                {
                  ...
                  {
                    LryaInfo("DockingController.Update.FinishedPathButErrorSignalTooLarge_1",
                             "x: %f, y: %f, angle: %f",
                    ...
                    {
                      LryaInfo( "DockingController.Update.AbleToDoHannsManeuver_1", "");
  #ifndef SIMULATOR
                  ...
                  {
                    LryaInfo("DockingController.Update.FinishedPathButErrorSignalTooLarge_2",
                             "ver: %f, hor: %f, angle: %f",
                    ...
                    {
                      LryaInfo( "DockingController.Update.AbleToDoHannsManeuver_2", "");
  #ifndef SIMULATOR
                      doHannsManeuver = true;
  #endif
                    }
              ...
              if(!markerlessDocking_ && !inPosition)
              {
                if(dockingMethod_ != DockingMethod::BLIND_DOCKING && dockingMethod_ != DockingMethod::EVEN_BLINDER_DOCKING)
                {
                     ...
                     !isAligning)
                  {
                    ...
                    LryaInfo( "DockingController.Update.ExecutingHannsManeuver", "");
                  ...
                  else
                  {
                    ...
                    LryaInfo( "DockingController.Update.BackingUp", "");
                    
                    // If we have failed too many times just give up
                    if(numDockingFails_++ >= maxDockRetries_)
                    {
                      StopDocking(DockingResult::DOCK_FAILURE_RETRY);
                      LryaInfo( "DockingController.Update.TooManyDockingFails", "MaxRetries %d", maxDockRetries_);
                      break;
                    }
              ...
              {
                LryaInfo( "DockingController.Update.Success", "vert:%f horz:%f rel_x:%f rel_y:%f t:%d",  rel_vert_dist_block, ABS(rel_horz_dist_block), dockingErrSignalMsg_.x_distErr, dockingErrSignalMsg_.y_horErr, (int)(HAL::GetTimeStamp()-dockingErrSignalMsg_.timestamp));
          ...
          default:
            mode_ = IDLE;
            LryaError( "DockingController.Update.InvalidMode", "%d", mode_);
            break;
        }
      ...
      void SetRelDockPose(f32 rel_x, f32 rel_y, f32 rel_rad, TimeStamp_t t)
      {
        ...
        if (rel_x < 0.f || ABS(rel_rad) > 0.75f*M_PI_2_F) {
          LryaWarn( "DockingController.SetRelDockPose.OORErrorSignal", "x: %f, y: %f, rad: %f", rel_x, rel_y, rel_rad);
          return;
        }
...
#if(DEBUG_DOCK_CONTROLLER)
          LryaInfo( "DockingController.SetRelDockPose.DockPoseReached", "dockOffsetDistX = %f", dockOffsetDistX_+DOCK_OFFSET_DIST_X_OFFSET);
#endif
          return;
        }
...
#if(DEBUG_DOCK_CONTROLLER)
        Lrya::Embedded::Pose2d currPose = Localization::GetCurrPose();
        LryaInfo( "DockingController.SetRelDockPose", "HistPose %f %f %f (t=%d), currPose %f %f %f (t=%d)",
        ...
        if(IsHybridDocking() && prev_blockPose_x_ != -1)
        {
          ...
          {
            LryaInfo( "DockingController.SetRelDockPose.AcquireNewSignal", "%f %f %f",
        ...
        if (markerOutOfFOV_) {
          ...
          if (IsMarkerInFOV(blockPose_, MARKER_WIDTH)) {
            LryaInfo( "DockingController.SetRelDockPose.MarkerInsideFOV", "");
            markerOutOfFOV_ = false;
          } else {
            LryaInfo( "DockingController", "Marker is expected to be out of FOV. Ignoring error signal\n");
            return;
          }
        }
        ...
        do {
          ...
          if (++endRadiiIdx == NUM_END_RADII) {
            LryaInfo( "DockingController.SetRelDockPose.DubinsPathFailed", "");
        ...
        if (followingBlockNormalPath_) {
...
#endif
          {
            ...
            LryaInfo( "DockingController", "Computing straight line path (%f, %f) to (%f, %f)\n",x_start_mm, y_start_mm, x_end_mm, y_end_mm);
          }
#if ALLOW_TRACKER_DOCKING
          else
          {
        ...
        // Debug
        if (!createdValidPath_) {
          LryaError( "DockingController.SetRelDockPose.FailedToCreatePath", "");
      ...
      const Lrya::Embedded::Pose2d& GetLastMarkerAbsPose()
      {
        return blockPose_;
      }
