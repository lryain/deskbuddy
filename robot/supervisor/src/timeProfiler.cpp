...
namespace Vector {
  ...
  void TimeProfiler::MarkNextProfile_Internal() {
    LryaAssert(timeProfIdx_ < MAX_NUM_PROFILES, "");
    timeProfiles_[timeProfIdx_] = HAL::GetMicroCounter();
  ...
  {
    LryaConditionalWarnAndReturnValue((!isProfiling_), NULL, "timeprofiler.getprofname_while_busy", "GetProfName called in middle of profile. Ignoring.");
  ...
  u32 TimeProfiler::ComputeStats(u32 *avgTimes[], u32 *maxTimes[]) {
    LryaConditionalWarnAndReturnValue((!isProfiling_), 0, "timeprofiler.computestats_while_busy", "ComputeStats called in middle of profile. Ignoring.");
  ...
  void TimeProfiler::PrintStats() {
    ...
    for (u32 i=0; i<numProfiles; ++i) {
      LryaInfo("TimeProfiler.PrintStats", 
