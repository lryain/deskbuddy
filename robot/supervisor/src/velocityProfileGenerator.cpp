...
#include <math.h>
#include "lrya/common/constantsAndMacros.h"
...
namespace Lrya {
                                              ...
                                              float timeStep)
  {
    ...
    LryaAssert(maxVel_ >= endVel_, "");
    LryaAssert(accel_ > 0, "");
    LryaAssert(timeStep_ > 0, "");
...
#if(DEBUG_VPG)
      LryaDebug( "VPG", "new V_max: %f (d = %f)", maxReachableVel_, d);
#endif
    }
...
#if(DEBUG_VPG)
    LryaDebug( "VPG", "startVel %f, startPos %f, maxSpeed %f, accel %f", startVel_, startPos_, maxVel_, accel_);
    LryaDebug( "VPG", "endVel %f, endPos %f, timestep %f", endVel_, endPos_, timeStep_);
    LryaDebug( "VPG", "deltaVel %f, maxReachableVel %f, totalDist %f, decelDistToTarget %f, dir %f", deltaVelPerTimeStepStart_, maxReachableVel_, totalDistToTarget_, decelDistToTarget_, direction);
#endif
  }
...
*/
                                                            ...
                                                            float timeStep)
  {
    ...
    LryaConditionalWarnAndReturnValue((acc_start_duration + acc_end_duration <= duration),
    ...
    if (((dist > 0) == (distWhenVelMid0 > 0)) && (fabsf(distWhenVelMid0) > fabsf(dist))) {
      ...
      LryaInfo("VPG.StartProfile_fixedDuration.EndPosReachedDuringStartingAcc", "");
      return false;
    }
    ...
    if (accStartCrosses0) {
      ...
      LryaConditionalWarnAndReturnValue((discr >=  0), false, "VPG.StartProfile_fixedDuration.NegativeDiscriminant", "A = %f, B = %f, C = %f", A, B, C);

      float sqrtDiscr = sqrtf(discr);
      vm = (-B + sqrtDiscr) / (2*A);

      LryaConditionalWarnAndReturnValue(vm <= 0, false, "VPG.StartProfile_fixedDuration.NegativeVm", "vm > 0  (A = %f, B = %f, C = %f)", A, B, C);
    ...
    LryaConditionalWarnAndReturnValue(!((fabsf(vs) > vel_max) || (fabsf(vm) > vel_max)), false, "VPG.StartProfile_fixedDuration.VelExceedsMax", "vs = %f, vm = %f, vel_max = %f", vs, vm, vel_max);
    ...
    {
      LryaWarn( "VPG.StartProfile_fixedDuration.AccelMaxExceeded", "acc_start = %f, acc_end = %f, acc_max = %f",
...
#if(DEBUG_VPG)
    LryaDebug( "VPG", "startVel %f, startPos %f, endVel %f, endPos %f", startVel_, startPos_, endVel_, endPos_);
    LryaDebug( "VPG", "ts %f, tm %f, te %f, total duration %f", ts, tm, te, duration);
    LryaDebug( "VPG", "deltaVelStart %f, deltaVelEnd %f, maxReachableVel %f", deltaVelPerTimeStepStart_, deltaVelPerTimeStepEnd_, maxReachableVel_);
    LryaDebug( "VPG", "totalDist %f, decelDistToTarget %f", totalDistToTarget_, decelDistToTarget_);
#endif
  ...
  float VelocityProfileGenerator::Step(float &currVel, float &currPos)
  {
    ...
    if (noTargetMode_) {
      LryaAssert(maxReachableVel_ == endVel_, "");
...
#if(DEBUG_VPG)
    LryaDebug( "VPG.Step", "currVel %f, currPos %f, currDistToTarget %f, isDecel %d", currVel_, currPos_, currDistToTarget, isDecel_);
#endif
