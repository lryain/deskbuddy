...
namespace Lrya {
  namespace Vector {
    namespace ProxSensors {
      ...
      {
        LryaConditionalErrorAndReturnValue(ind < HAL::CLIFF_COUNT, 0, "ProxSensors.GetCliffValue.InvalidIndex", "Index %d is not valid", ind);
      ...
      void UpdateCliff()
      {
        ...
        if (_enableCliffDetect &&
          ...
          if(stoppingBecauseCliff || stoppingBecauseWhite)
          {
            ...
            LryaInfo("ProxSensors.UpdateCliff.StoppingDueToCliff", 
      ...
      void EnableCliffDetector(bool enable) {
        LryaInfo("ProxSensors.EnableCliffDetector", "%d", enable);
        _enableCliffDetect = enable;
      }

      void EnableStopOnCliff(bool enable) {
        LryaInfo("ProxSensors.EnableStopOnCliff", "%d", enable);
        _stopOnCliff = enable;
      }

      void EnableStopOnWhite(bool enable)
      {
        LryaInfo("ProxSensors.EnableStopOnWhite", "%d", enable);
        _stopOnWhite = enable;
      }

      void SetWhiteDetectThreshold(u32 ind, u16 level)
      {
        LryaConditionalError(ind < HAL::CLIFF_COUNT, "ProxSensors.SetWhiteDetectThreshold.InvalidIndex", "Index %d is not valid", ind);
        LryaInfo("ProxSensors.SetWhiteDetectThreshold.NewLevel", "cliff sensor %d, threshold %d", ind, level);
        _whiteDetectThreshold[ind] = level;
      }

      void SetCliffDetectThreshold(u32 ind, u16 level)
      {
        LryaConditionalError(ind < HAL::CLIFF_COUNT, "ProxSensors.SetCliffDetectThreshold.InvalidIndex", "Index %d is not valid", ind);
        if (level > CLIFF_SENSOR_THRESHOLD_MAX) {
          LryaWarn("ProxSensors.SetCliffDetectThreshold.TooHigh", "cliff sensor %d, threshold %d", ind, level);
        } else if (level < CLIFF_SENSOR_THRESHOLD_MIN) {
          LryaWarn("ProxSensors.SetCliffDetectThreshold.TooLow", "cliff sensor %d, threshold %d", ind, level);
        }else {
          LryaDebug("ProxSensors.SetCliffDetectThreshold.NewLevel", "cliff sensor %d, threshold %d", ind, level);
