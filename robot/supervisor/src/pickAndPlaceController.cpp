    ...
    namespace PickAndPlaceController {
                                         ...
                                         const BlockStatus blockStatus)
      {
        ...
        if(!RobotInterface::SendMessage(msg)) {
          LryaWarn("PAP.SendPickAndPlaceResultMessage.SendFailed", ""); 
        }
      }
      ...
      void SendMovingLiftPostDockMessage()
      {
        ...
        msg.action = action_;
        if(!RobotInterface::SendMessage(msg)) {
          LryaWarn("PAP.SendMovingLiftPostDockMessage.SendFailed", ""); 
        }
      }
      ...
      void SendChargerMountCompleteMessage(const bool success)
      {
        ...
        msg.didSucceed = success;
        if(!RobotInterface::SendMessage(msg)) {
          LryaWarn("PAP.SendChargerMountCompleteMessage.SendFailed", ""); 
        }
      }

      void SendCliffAlignCompleteMessage(const CliffAlignResult result)
      {
        LryaInfo("PAP.SendCliffAlignCompleteMessage.Result", "%s", EnumToString(result));
        CliffAlignComplete msg;
        msg.timestamp = HAL::GetTimeStamp();
        msg.result = result;
        if(!RobotInterface::SendMessage(msg)) {
          LryaWarn("PAP.SendCliffAlignCompleteMessage.SendFailed", "");
        }
      }
      ...
      static void StartBackingOut()
      {
        ...
        switch(action_)
        {
          ...
          default:
          {
            LryaError( "PAP.StartBackingOut.InvalidAction", "%hhu", action_);
          }
        }

        const f32 backoutTime_sec = backoutDist_mm / BACKOUT_SPEED_MMPS;

        LryaInfo( "PAP.StartBackingOut.Dist", "Starting %.1fmm backout (%.2fsec duration)", backoutDist_mm, backoutTime_sec);
      ...
      Result Update()
      {
        ...
        switch(mode_)
        {
          ...
          case SET_LIFT_PREDOCK:
          {
            ...
            LryaDebug( "PAP", "SETTING LIFT PREDOCK (action %hhu)", action_);
#endif
            ...
            switch(action_) {
              ...
              case DockAction::DA_CLIFF_ALIGN_TO_WHITE:
                ...
                break;
              default:
                LryaError( "PAP.SET_LIFT_PREDOCK.InvalidAction", "%hhu", action_);
                Reset();
                break;
            }
            break;
            ...
            if (!DockingController::IsBusy())
            {
              if (DockingController::DidLastDockSucceed())
              {
                ...
                if (action_ == DockAction::DA_ALIGN ||
                  ...
                  #if(DEBUG_PAP_CONTROLLER)
                  LryaDebug( "PAP", "SET_LIFT_POSTDOCK\n");
                  #endif
                  mode_ = SET_LIFT_POSTDOCK;
                }
              } else {
                // Docking failed for some reason. Probably couldn't see block anymore.
                LryaDebug( "PAP.DOCKING.DockingFailed", "");
                ...
                switch(action_)
                {
                  ...
                  default:
                    LryaError( "PAP.DOCKING.InvalidAction", "%hhu", action_);
                } // switch(action_)
          ...
          {
#if(DEBUG_PAP_CONTROLLER)
            LryaDebug( "PAP", "SETTING LIFT POSTDOCK\n");
#endif
            ...
            switch(action_) {
              ...
              case DockAction::DA_POP_A_WHEELIE:
                ...
                break;
              default:
                LryaError( "PAP.SET_LIFT_POSTDOCK.InvalidAction", "%hhu", action_);
                Reset();
                break;
            }
            break;
          ...
          case MOVING_LIFT_POSTDOCK:
          {
            if (LiftController::IsInPosition() ||
              ...
              switch(action_)
              {
                ...
                default:
                  LryaError( "PAP.MOVING_LIFT_POSTDOCK.InvalidAction", "%hhu", action_);
              } // switch(action_)
          ...
          {
            if (HAL::GetTimeStamp() > transitionTime_) {
              LryaInfo("PAP.BACKUP_ON_CHARGER.Timeout", "");
              ...
              LryaInfo("PAP.BACKUP_ON_CHARGER.Cliff", "");
              SendChargerMountCompleteMessage(false);
              Reset();
              ...
              if (tiltedOnChargerStartTime_ == 0) {
                ...
                LryaInfo( "PAP.BACKUP_ON_CHARGER.Tilted", "");
              ...
              LryaInfo("PAP.BACKUP_ON_CHARGER.Success", "");
              ...
              if (isBlackBL && !cliffHasSeenBlackBL_) {
                ...
                LryaInfo("PAP.BACKUP_ON_CHARGER.SawBlackBL", "");
              }
              if (isBlackBR && !cliffHasSeenBlackBR_) {
                cliffHasSeenBlackBR_ = true;
                LryaInfo("PAP.BACKUP_ON_CHARGER.SawBlackBR", "");
              }
              if (!robotHasStartedPitching_ &&
                ...
                LryaInfo("PAP.BACKUP_ON_CHARGER.StartedPitching", "pitch: %.2f deg", RAD_TO_DEG(IMUFilter::GetPitch()));
              }
            ...
            switch(cliffAlignState_) {
              ...
              case ALIGNING:
              {
                ...
                if (fabsf((cliffAlignStartAngle_ - Localization::GetCurrPose_angle()).ToFloat()) > CLIFF_ALIGN_MAX_ANGLE_RAD) {
                  LryaInfo("PAP.CLIFF_ALIGN_TO_WHITE.TurnedTooMuch", "");
                ...
                if (HAL::GetTimeStamp() - cliffAlignStartTime_ms_ > CLIFF_ALIGN_TIMEOUT_MS) {
                  LryaInfo("PAP.CLIFF_ALIGN_TO_WHITE.Timeout", "");
                ...
                if(NEAR(gyroZ_radps, 0, CLIFF_ALIGN_ZERO_GYRO_TOL)) {
                  ...
                  if(cliffAlignNearZeroGyroCount_ > CLIFF_ALIGN_MAX_COUNT_ZERO_GYRO) {
                    LryaInfo("PAP.CLIFF_ALIGN_TO_WHITE.NoGyroMovement", "");
          ...
          {
            Reset();
            LryaError( "PAP.Update.InvalidAction", "%hhu", action_);
            break;
          }
        }
      ...
      {
#if(DEBUG_PAP_CONTROLLER)
        LryaDebug( "PAP.DockToBlock.Action", "%hhu", action);
#endif
      ...
      {
        LryaDebug( "PAP.SetRollActionParams", "liftHeight: %f, speed: %f, accel: %f, duration %d, backupDist %f",
