...
//      /// --------------------------------------------------------------------------------------------------
      ...
      {
        LryaInfo( "IMUFilter.EnableBraceWhenFalling", "%d", enable);
      ...
      void UpdateGyroBiasFilter(const float rawGyro[3])
      {
        ...
        if (!isMotionDetected_) {
          ...
          LryaInfoPeriodic(12000, "IMUFilter.Bias", "%f %f %f (deg/sec)",
          ...
          if (!IsBiasFilterComplete()) {
            ...
            if (IsBiasFilterComplete()) {
              ...
              const f32 gyro_bias_deg[] = {RAD_TO_DEG_F32(gyro_bias_filt[0]),
                                           RAD_TO_DEG_F32(gyro_bias_filt[1]),
                                           RAD_TO_DEG_F32(gyro_bias_filt[2])};
              LryaInfo("IMUFilter.GyroCalibrated", "%f %f %f (deg/sec)",
                       gyro_bias_deg[0],
                       gyro_bias_deg[1],
                       gyro_bias_deg[2]);
        ...
        if (IsPickedUp()) {
          ...
          if (!AreMotorsMoving()) {
            ...
            if (potentialPickupCnt_ > PICKUP_COUNT_WHILE_MOTIONLESS || accelBasedMotionDetected) {
              LryaInfo( "IMUFilter.PDWhileStationary", "acc (%f, %f, %f), gyro (%f, %f, %f), cliff %d",
            ...
            if (CheckPickupWhileMoving() || cliffBasedPickupDetect || gyroZBasedMotionDetect) {
              if (++potentialPickupCnt_ > PICKUP_COUNT_WHILE_MOVING) {
                SetPickupDetect(true);
                LryaInfo( "IMUFilter.PickupDetected", "accX %f, accY %f, accZ %f, cliff %d, gyroZ %d",
        ...
        if (!AreMotorsMoving() && !pickedUp_) {
          ...
          if (fabsf(gyro_robot_frame_filt[2]) > PEAK_GYRO_THRESHOLD) {
            ...
            if ((_peakGyroMaxTime > _peakGyroStartTime) && (_peakGyroMaxTime - _peakGyroStartTime < MAX_GYRO_PEAK_DURATION_MS)) {
              LryaInfo( "IMUFilter.PokeDetected.Gyro", "");
              _peakGyroStartTime = currTime;
      ...
      void UpdatePitch()
      {
        ...
        //LryaDebugPeriodic(50, "RobotPitch", "%f deg (motion %d, gyro %f)", RAD_TO_DEG_F32(pitch_), MotionDetected(), gyro_robot_frame_filt[1]);
      }
...
//        LryaDebugPeriodic(250, "IMUFilter.UpdateRoll",
...
//                         isMoving);
        ...
        if (isRecording_) {
          ...
          if (++recordDataIdx_ == IMU_CHUNK_SIZE) {
            ...
            if (imuChunkMsg_.chunkId == imuChunkMsg_.totalNumChunks) {
              LryaDebug( "IMUFilter.IMURecording.Complete", "time %dms", HAL::GetTimeStamp());
              isRecording_ = false;
            }
          }
#else
          ...
          if (sentIMUDataMsgs_ == totalIMUDataMsgsToSend_) {
            LryaDebug( "IMUFilter.IMURecording.CompleteRaw", "time %dms", HAL::GetTimeStamp());
            isRecording_ = false;
      ...
      {
        LryaDebug( "IMUFilter.IMURecording.Start", "time = %dms", HAL::GetTimeStamp());
        isRecording_ = true;
#if(RECORD_AND_SEND_MODE == RECORD_AND_SEND_FILT_DATA)
        recordDataIdx_ = 0;
        imuChunkMsg_.seqId++;
