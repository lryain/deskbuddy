  ...
  namespace Vector {
    namespace Localization {
      ...
      namespace {
        ...
        // private members
        ::Lrya::Embedded::Pose2d currMatPose;
      ...
      Result InterpolatePose(int pose1_idx, int pose2_idx, f32 targetTime, int poseResult_idx)
      {
        ...
        if (p1->t > p2->t) {
          LryaWarn( "Localization.InterpolatePose.PosesOutOfOrder", "pose2 is older than pose1");
          return RESULT_FAIL;
        }

        if (targetTime < p1->t || targetTime > p2->t) {
          LryaWarn( "Localization.InterpolatePose.TargetTimeOOR", "targetTime is outside expected range");
          return RESULT_FAIL;
        }
                                    ...
                                    const f32 angle)
      {
        ...
        if (t == 0) {
          ...
          LryaInfo( "Localization.UpdatePoseWithKeyFrame.SettingPose", "x= %f, y= %f, angle= %f", x, y, angle);
          SetCurrPose(x, y, angle);
          return RESULT_OK;
        } else if (GetHistIdx(t, i) == RESULT_FAIL) {
          LryaWarn( "Localization.UpdatePoseWithKeyFrame.TimeNotFound", "Couldn't find timestamp %d in history (oldest(%d) %d, newest(%d) %d)",
        ...
        if (lastKeyframeUpdate_ > t) {
          ...
          LryaInfo( "Localization.UpdatePoseWithKeyFrame.IgnoreOldKeyframe", "Ignoring keyframe %d at time %d\n", frameID, t);
          #endif
          return RESULT_OK;
        }
      ...
      Result GetHistPoseAtIndex(u16 idx, Lrya::Embedded::Pose2d& p) {
        if (idx >= POSE_HISTORY_SIZE) {
          return RESULT_FAIL;
        }
      ...
      Result GetHistPoseAtTime(TimeStamp_t t, Lrya::Embedded::Pose2d& p)
      {
        // Check that there are actually poses in history
        if (hSize_ <= 0) {
          LryaWarn( "Localization.GetHistPoseAtTime.NoHistory", "");
          return RESULT_FAIL;
        }
        ...
        if (hist_[hStart_].t > t) {
          LryaWarn( "Localization.GetHistPoseAtTime.TimeTooOld", "History starts at time %d, pose requested at time %d. Returning oldest pose.\n",
                   hist_[hStart_].t, t);
        ...
        if (hist_[hEnd_].t < t) {
          LryaWarn( "Localization.GetHistPoseAtTime.TimeTooNew", "History ends at time %d, pose requested at time %d. Returning newest pose.\n",
                   hist_[hEnd_].t, t);
...
//          if (FLT_NEAR(lDist, rDist)) {
      ...
      void ConvertToDriveCenterPose(const Lrya::Embedded::Pose2d &robotOriginPose, Lrya::Embedded::Pose2d &driveCenterPose)
      {
      ...
      void ConvertToOriginPose(const Lrya::Embedded::Pose2d &driveCenterPose, Lrya::Embedded::Pose2d &robotOriginPose)
      {
