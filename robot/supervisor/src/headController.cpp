...
namespace HeadController {
    ...
    void OnMotorCalibrated()
    {
      ...
      LryaInfo("HeadController.Calibrated",
          ...
          case HCS_WAIT_FOR_STOP:
            ...
            if (!IsMoving()) {
              ...
              if (HAL::GetTimeStamp() - lastHeadMovedTime_ms > HEAD_STOP_TIME) {
                ...
                LryaInfo( "HeadController.CalibratedWhileApplyingPower", "");
    ...
    void PoseAndSpeedFilterUpdate()
    {
      ...
      LryaDebug( "HeadController", "HEAD FILT: speed %f, speedFilt %f, currentAngle %f, currHalPos %f, prevPos %f, pwr %f",
                                  ...
                                  f32 speed_rad_per_sec, f32 accel_rad_per_sec2, bool useVPG)
    {
      ...
      if (inPosition_ &&
        ...
        #if(DEBUG_HEAD_CONTROLLER)
        LryaDebug( "HeadController", "Already at desired angle %f degrees", RAD_TO_DEG_F32(angle));
        #endif
...
#if(DEBUG_HEAD_CONTROLLER)
      LryaDebug( "HeadController", "(fixedDuration): SetDesiredAngle %f rads (duration %f)", desiredAngle_.ToFloat(), duration_seconds);
#endif
      ...
      if (!inPosition_) {
        ...
        if (FLT_NEAR(angleError_,0.f)) {
          #if(DEBUG_HEAD_CONTROLLER)
          LryaDebug( "HeadController", "(fixedDuration): Already at desired position");
          #endif
          HAL::MotorSetPower(MotorID::MOTOR_HEAD,0);
          return;
        }
      ...
      if (duration_seconds > 0) {
        ...
        if (!res) {
          LryaInfo("HeadController.SetDesiredAngle.VPGFixedDurationFailed", "startVel %f, startPos %f, acc_start_frac %f, acc_end_frac %f, endPos %f, duration %f. Trying VPG without fixed duration.",
                   startRadSpeed,
                   startRad,
                   acc_start_frac,
                   acc_end_frac,
...
#if(DEBUG_HEAD_CONTROLLER)
      LryaDebug( "HeadController", "VPG (fixedDuration): startVel %f, startPos %f, acc_start_frac %f, acc_end_frac %f, endPos %f, duration %f",
    ...
    bool MotorBurnoutProtection() {
      ...
      if (potentialBurnoutStartTime_ms_ == 0) {
        ...
        if (IsInPosition() || IMUFilter::IsBeingHeld() || ProxSensors::IsAnyCliffDetected()) {
          ...
          LryaWarn( "HeadController.MotorBurnoutProtection", "Recalibrating (power = %f)", power_);
    ...
    void Brace() {
      LryaInfo("HeadController.Brace", "");
      HAL::MotorSetPower(MotorID::MOTOR_HEAD, BRACING_POWER);
      bracing_ = true;
    }

    void Unbrace() {
      LryaInfo("HeadController.Unbrace", "");
...
#         if(DEBUG_HEAD_CONTROLLER)
            LryaDebug( "HeadController", " HEAD ANGLE REACHED (%f rad)\n", GetAngleRad() );
#         endif
          }
        } else {
          lastInPositionTime_ms_ = 0;
...
#       endif
    ...
    void SetGains(const f32 kp, const f32 ki, const f32 kd, const f32 maxIntegralError)
    {
      ...
      LryaInfo( "HeadController.SetGains", "New head gains: kp = %f, ki = %f, kd = %f, maxSum = %f",
