...
// #pragma mark --- Messages Method Implementations ---
      ...
      void ProcessMessage(RobotInterface::EngineToRobot& msg)
      {
        switch(msg.tag)
        {
          ...
          default:
            LryaWarn( "Messages.ProcessBadTag_EngineToRobot.Recvd", "Received message with bad tag %x", msg.tag);
        }
      } // ProcessMessage()
...
// #pragma --- Message Dispatch Functions ---
      ...
      {
        LryaInfo( "Messages.Process_syncRobot.Recvd", "");
        ...
        LryaInfo("watchdog_reset_count", "%d", HAL::GetWatchdogResetCounter());
      } // ProcessRobotInit()
      ...
      void Process_calmPowerMode(const RobotInterface::CalmPowerMode& msg)
      {
        ...
        LryaInfo("Messages.Process_calmPowerMode.enable", "enable: %d", msg.enable);
      ...
      void Process_absLocalizationUpdate(const RobotInterface::AbsoluteLocalizationUpdate& msg)
      {
        ...
        /*
        LryaInfo( "Messages.Process_absLocalizationUpdate.Recvd", "Result %d, currTime=%d, updated frame time=%d: (%.3f,%.3f) at %.1f degrees (frame = %d)\n",
              res,
              HAL::GetTimeStamp(),
              msg.timestamp,
      ...
      void Update()
      {
        ...
        if (!syncRobotAckSent_) {
          if (syncRobotReceived_ &&
            ...
            LryaInfo("Messages.Update.GyroCalibrated", "%f %f %f",
        ...
        {
          Lrya::Vector::RobotInterface::EngineToRobot msgBuf;

          // Copy into structured memory
          memcpy(msgBuf.GetBuffer(), pktBuffer_, dataLen);
          if (!msgBuf.IsValid())
          {
            LryaWarn( "Receiver.ReceiveData.Invalid", "Receiver got %02x[%d] invalid", pktBuffer_[0], dataLen);
          }
          else if (msgBuf.Size() != dataLen)
          {
            LryaWarn( "Receiver.ReceiveData.SizeError", "Parsed message size error %d != %d", dataLen, msgBuf.Size());
          }
          else
          {
            Lrya::Vector::Messages::ProcessMessage(msgBuf);
          }
        }
      ...
      void Process_executePath(const RobotInterface::ExecutePath& msg) {
        ...
        LryaInfo( "Messages.Process_executePath.Result", "id=%d result=%d", msg.pathID, result);
      }

      void Process_dockWithObject(const DockWithObject& msg)
      {
        LryaInfo( "Messages.Process_dockWithObject.Recvd", "action %hhu, dockMethod %hhu, doLiftLoadCheck %d, backUpWhileLiftingCube %d, speed %f, accel %f, decel %f",
      ...
      {
        //LryaInfo( "Messages.Process_placeObjectOnGround.Recvd", "");
      ...
      void Process_drive(const RobotInterface::DriveWheels& msg) {
        ...
        if (PathFollower::IsTraversingPath()) {
          LryaWarn( "Messages.Process_drive.IgnoringBecauseAlreadyOnPath", "");
          return;
        }

        //LryaInfo( "Messages.Process_drive.Executing", "left=%f mm/s, right=%f mm/s", msg.lwheel_speed_mmps, msg.rwheel_speed_mmps);
      ...
      void Process_liftHeight(const RobotInterface::SetLiftHeight& msg) {
        //LryaInfo( "Messages.Process_liftHeight.Recvd", "height %f, maxSpeed %f, duration %f", msg.height_mm, msg.max_speed_rad_per_sec, msg.duration_sec);
      ...
      void Process_setLiftAngle(const RobotInterface::SetLiftAngle& msg) {
        // LryaInfo( "Messages.Process_liftAngle.Recvd", 
      ...
      void Process_headAngle(const RobotInterface::SetHeadAngle& msg) {
        //LryaInfo( "Messages.Process_headAngle.Recvd", "angle %f, maxSpeed %f, duration %f", msg.angle_rad, msg.max_speed_rad_per_sec, msg.duration_sec);
        if (msg.duration_sec > 0) {
      ...
      void Process_turnInPlaceAtSpeed(const RobotInterface::TurnInPlaceAtSpeed& msg) {
        //LryaInfo( "Messages.Process_turnInPlaceAtSpeed.Recvd", "speed %f rad/s, accel %f rad/s2", msg.speed_rad_per_sec, msg.accel_rad_per_sec2);
      ...
      void Process_setControllerGains(const RobotInterface::ControllerGains& msg) {
        switch (msg.controller)
        {
          ...
          default:
          {
            LryaWarn( "Messages.Process_setControllerGains.InvalidController", "controller: %hhu",  msg.controller);
          }
        }
      }
      ...
      void Process_enableMotorPower(const RobotInterface::EnableMotorPower& msg)
      {
        switch(msg.motorID) {
          ...
          default:
          {
            LryaWarn( "Messages.enableMotorPower.UnhandledMotorID", "%hhu", msg.motorID);
            break;
          }
        }
      }
      ...
      {
        LryaInfo("Messages.Process_robotStoppedAck", "");
