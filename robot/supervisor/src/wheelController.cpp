...
namespace Lrya {
  ...
  namespace WheelController {
    ...
    void SetGains(const f32 kp, const f32 ki, const f32 maxIntegralError) {
      LryaInfo( "WheelController.SetGains", "New gains: kp=%f, ki=%f, maxSum=%f", kp, ki, maxIntegralError);
      Kp_ = kp;
      Ki_ = ki;
...
#     endif
    ...
    void Run()
    {
      ...
      if(!skipActiveControl) {
        ...
        LryaDebug( "WheelController", "speeds: %f (L), %f (R)   (Curr: %d, %d)",
                filterWheelSpeedL_, filterWheelSpeedR_,
                measuredWheelSpeedL_, measuredWheelSpeedR_);
        LryaDebug( "WheelController", "desired speeds: %f (L), %f (R)",
...
#if(DEBUG_WHEEL_CONTROLLER)
        LryaDebug( "WheelController", "error: %f (L), %f (R)   error_sum: %f (L), %f (R)", errorL, errorR, error_sumL_, error_sumR_);
#endif
...
#if(DEBUG_WHEEL_CONTROLLER)
      LryaDebug( "WheelController", "power: %f (L), %f (R)\n", power_l_, power_r_);
#endif
    ...
    void SetDesiredWheelSpeeds(f32 leftws, f32 rightws)
    {
      if (enable_) {
        ...
        LryaDebug("WheelController.SetDesiredWheelSpeeds.Disabled", 
