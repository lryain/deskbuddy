...
#include "lrya/cozmo/robot/logEvent.h"
#include "lrya/cozmo/robot/DAS.h"
...
static int triggerFakeHwClock() {
  ...
  if (fifo == NULL) {
    LryaError("CozmoBot.triggerFakeHwClock",
  ...
  namespace Vector {
    namespace Robot {
      ...
      void Reboot() {
        ...
        LryaInfo("CozmoBot.Reboot.Rebooting", "res: %d", res);
        #else
        // Reboot not support in sim
        LryaWarn("CozmoBot.Reboot.WouldRebootButSim", "");
        #endif
      }
      ...
      void SendPrepForShutdown(ShutdownReason reason)
      {
        ...
        DASMSG_SET(s2, lowBattOnStart_ ? "LowBattOnStart" : "", "Battery was low when mate-robot started");
        DASMSG_SET(s3, hotBattOnStart_ ? "HotBattOnStart" : "", "Battery was overheating when mate-robot started");
        DASMSG_SEND();
      }
      ...
      void CheckForOverheatingBatteryShutdown()
      {
        ...
        if (HAL::BatteryIsOverheated() || hotBattOnStart_) {
          if (shutdownTime_ms == 0) {
            LryaInfo("CozmoBot.CheckForOverheatingBattery.PrepForShutdown", "");
        ...
        if ((shutdownTime_ms > 0) && (now_ms > shutdownTime_ms)) {
          LryaInfo("CozmoBot.CheckForOverheatingBattery.HALShutdown","");
          HAL::Shutdown();
        }
      }
      ...
      { 
        ...
        if (lowBattOnStart_ && HAL::BatteryIsOnCharger()) {
          LryaInfo("CozmoBot.CheckForCriticalBattery.LowBattOnStartCancelled", "");
          lowBattOnStart_ = false;
        }

        if (HAL::IsShutdownImminent() || lowBattOnStart_) {
          if (shutdownTime_ms == 0) {
            LryaInfo("CozmoBot.CheckForCriticalBattery.PrepForShutdown", "");
            ...
            LryaInfo("CozmoBot.CheckForCriticalBattery.HALShutdown","");
            HAL::Shutdown();
          }
          ...
          LryaInfo("CozmoBot.CheckForCriticalBattery.RebootingInstead", "");
          shutdownTime_ms = 0;
          Reboot();
        }
      }  
      ...
      void CheckForShutdown()
      {
        ...
        switch(state)
        {
          case NONE:
            if(buttonIsPressed)
            {
              ...
              else if(curTime_ms - timeMark_ms > SHUTDOWN_TIME_MS)
              {
                ...
                LryaWarn("CozmoBot.CheckForButtonHeld.PossiblyShuttingDown", "Syncing");
                timeMark_ms = curTime_ms;
                sync();
#else
                timeMark_ms = curTime_ms;
                ...
                LryaInfo("CozmoBot.CheckForButtonHeld.Shutdown", "Sending PrepForShutdown");
          ...
          case START:
            ...
            {
              LryaInfo("CozmoBot.CheckForButtonHeld.Sync", "");
              #ifdef MATEOS
              sync();
              #endif
              state = SYNC;
          ...
          case SYNC:
            ...
            {
              LryaWarn("CozmoBot.CheckForButtonHeld.HALShutdown","");
              HAL::Shutdown();
            }
            break;
        }
      ...
      void CheckForGyroCalibShutdown()
      {
        ...
        switch(state) {
          ...
          case PREP:
          {
            if(IMUFilter::IsBiasFilterComplete()) {
              ...
              if (timeDif_ms > GYRO_NOT_CALIB_PREP_FOR_SHUTDOWN_MS) {
                // Send a shutdown message to anim/engine
                LryaInfo("CozmoBot.CheckForGyroCalibShutdown.Shutdown", "Sending PrepForShutdown");
          ...
          case SHUTDOWN:
          {
            ...
            {
              LryaWarn("CozmoBot.CheckForGyroCalibShutdown.HALShutdown","");
              HAL::Shutdown();
            }
            break;
          }
      ...
      Result step_MainExecution()
      {
        ...
        if (lastCycleStartTime_usec_ != 0) {
          ...
          tracepoint(lrya_ust, mate_robot_robot_loop_period, timeBetweenCycles);
...
/*
        ...
        if (++cnt == (200 * interval_seconds)) {
          ...
          LryaInfo( "CozmoBot.TicsPerSec", "%d", numTicsPerSec);
...
*/    
        ...
        if (HAL::RadioIsConnected() && !wasConnected_) {
          LryaInfo("CozmoBot.Radio.Connected", "");
          wasConnected_ = true;
          ...
          LryaInfo( "CozmoBot.Radio.Disconnected", "");
          Messages::ResetInit();
          PathFollower::Init();
        ...
        if (nextMainCycleTimeErrorReportTime_usec_ < cycleEndTime) {
          ...
          if (reportTooLate || reportTooLong) {
            LryaWarn( "CozmoBot.MainCycleTimeError", 
