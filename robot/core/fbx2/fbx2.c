...
int main(int argc, char *argv[]) {
	...
	for(i=0; i<2; i++) {
        ...
        printf("2. INITIALIZE SPI SCREENS!\n");
	...
	*gpioSet = RESETMASK; usleep(5); // high
	...
	VC_RECT_T                  rect;
	uint16_t                  *pixelBuf;
        printf("3. star bcm_host_init...\n");

	bcm_host_init();
        printf("4. done bcm_host_init...\n");
        printf("5. start vc_dispmanx_display_open...\n");

	if(!(display = vc_dispmanx_display_open(0))) {
		err(6, "Can't open primary display");
	}
        printf("5. done vc_dispmanx_display_open!\n");
        printf("6. start vc_dispmanx_display_get_info!\n");

	if(vc_dispmanx_display_get_info(display, &info)) {
		err(7, "Can't get primary display information");
	}
        printf("6. done vc_dispmanx_display_get_info!\n");
	...
	// scaling plus 8/8/8 -> 5/6/5 dithering.
        printf("7. start vc_dispmanx_resource_create!\n");
