...
#if defined(LRYA_PLATFORM_ANDROID)
...
class ExternalOnlyConsoleChannel : public Lrya::Util::IConsoleChannel
{
public:
  ...
  virtual ~ExternalOnlyConsoleChannel()
  {
    ...
    Lrya::Util::SafeDeleteArray( _tempBuffer );
  }
...
static int GetProcessStatus(struct mg_connection *conn, void *cbdata)
{
  ...
  {
    struct mg_context* ctx = mg_get_context(conn);
    Lrya::Vector::WebService::WebService* that = static_cast<Lrya::Vector::WebService::WebService*>(mg_get_user_data(ctx));

    std::string remainder = query.substr(5);
    std::vector<std::string> args;

    args.push_back("/bin/sh");
    args.push_back("/lrya/bin/mate-getprocessstatus.sh");
...
void WebService::Start(Lrya::Util::Data::DataPlatform* platform, const Json::Value& config)
{
  if (platform == nullptr) {
    return;
  }
...
//https://lryainc.atlassian.net/browse/VIC-1554
...
//#endif
  ...
  _consoleVarsUIHTMLTemplate = Lrya::Util::StringFromContentsOfFile(consoleVarsTemplate);
...
{
  LRYA_CPU_PROFILE("WebService::Update");
        ...
        case RT_ConsoleVarGet:
          {
            ...
            if (!key.empty()) {
              const Lrya::Util::IConsoleVariable* consoleVar = consoleSystem.FindVariable(key.c_str());
              if (consoleVar) {
        ...
        case RT_ConsoleVarSet:
          {
            ...
            while (!key.empty()) {
              if (key.substr(0, 4) == "key=") {
                ...
                Lrya::Util::IConsoleVariable* consoleVar = consoleSystem.FindVariable(key.c_str());
                if (consoleVar) {
        ...
        case RT_ConsoleVarList:
          {
            ...
            for (Lrya::Util::ConsoleSystem::VariableDatabase::const_iterator it = varDatabase.begin();
                 it != varDatabase.end(); ++it)
            {
              std::string label = it->second->GetID();
              if (keyLen == 0 || Lrya::Util::StringCaseInsensitiveEquals(label.substr(0, keyLen), key))
              {
        ...
        case RT_ConsoleFuncList:
          {
            ...
            for (Lrya::Util::ConsoleSystem::FunctionDatabase::const_iterator it = funcDatabase.begin();
        ...
        case RT_ConsoleFuncCall:
          {
            ...
            Lrya::Util::IConsoleFunction* consoleFunc = consoleSystem.FindFunction(func.c_str());
            if (consoleFunc) {
...
{
  LRYA_CPU_PROFILE("GenerateConsoleVarsUI");
...
#endif // LRYA_NO_WEBSERVER_ENABLED
