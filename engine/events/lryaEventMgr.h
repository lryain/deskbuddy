...
#ifndef __Lrya_Cozmo_Basestation_Events_LryaEventMgr_H__
#define __Lrya_Cozmo_Basestation_Events_LryaEventMgr_H__
...
// Base class for the LryaEventMgr specialized variations
template<typename DataType, typename SignalStruct>
class LryaEventMgrBase : private Util::noncopyable
{
public:
  virtual ~LryaEventMgrBase() { }
...
using EventHandlerSignal = Signal::Signal<void(const LryaEvent<DataType>&)>;

// Shorthand for the function signature we accept as event handlers
template<typename DataType>
using SubscriberFunction = std::function<void(const LryaEvent<DataType>&)>;
  

// Original templated LryaEventMgr type (with a default type for the signal structure)
template<typename DataType, typename SignalStruct = EventHandlerSignal<DataType> >
class LryaEventMgr : public LryaEventMgrBase<DataType, SignalStruct>
{
public:
  using EventDataType = LryaEvent<DataType>;
...
template<typename DataType>
class LryaEventMgr<DataType, MailboxSignalMap<DataType> > : public LryaEventMgrBase<DataType, MailboxSignalMap<DataType> >
{
public:
...
}; // class LryaEventMgr (Mailbox specialization)

} // namespace Vector
} // namespace Lrya

#endif //  __Lrya_Cozmo_Basestation_Events_LryaEventMgr_H__
