...
public:
  ...
  using CanConstruct = Lrya::Util::is_explicitly_constructible<T, From*>;
  ...
  template <typename T>
  using CanBeCLADEvent = Lrya::Util::is_explicitly_constructible<CLADEvent, T>;

  template <typename T>
  using CanBeEngineToGame = Lrya::Util::is_explicitly_constructible<MessageEngineToGame, T>;
