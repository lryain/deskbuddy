...
namespace Vector {
  ...
  const std::string AnimationTransfer::kCacheFaceAnimsDir(Lrya::Util::FileUtils::FullFilePath({"assets", "faceAnimations"}));
  
  AnimationTransfer::AnimationTransfer(Lrya::Vector::IExternalInterface* externalInterface, Lrya::Util::Data::DataPlatform* dataPlatform)
  {
  ...
  {
    std::string full_path = _dataPlatform->pathToResource(Lrya::Util::Data::Scope::Cache, kCacheAnimFileName);
    ...
    {
      std::string face_anim_dir = _dataPlatform->pathToResource(Lrya::Util::Data::Scope::Cache,
                                                                kCacheFaceAnimsDir);
      std::string full_path_face_imgs = Lrya::Util::FileUtils::FullFilePath({face_anim_dir, _lastFaceAnimDir});
      if (Lrya::Util::FileUtils::DirectoryExists(full_path_face_imgs))
      {
        Lrya::Util::FileUtils::RemoveDirectory(full_path_face_imgs);
      }
  ...
  void AnimationTransfer::HandleGameEvents(const LryaEvent<ExternalInterface::MessageGameToEngine>& event)
  {
    if( event.GetData().GetTag() == ExternalInterface::MessageGameToEngineTag::TransferFile )
    {
      const Lrya::Vector::ExternalInterface::TransferFile& msg = event.GetData().Get_TransferFile();
      ...
      else if( msg.fileType == ExternalInterface::FileType::FaceImg)
      {
        ...
        if( found_char != std::string::npos)
        {
          ...
          if( _lastFaceAnimDir != currDir )
          {
            ...
            std::string full_path_face_imgs = Lrya::Util::FileUtils::FullFilePath({face_anim_dir, _lastFaceAnimDir});
            Lrya::Util::FileUtils::CreateDirectory(full_path_face_imgs);
          }
        }
        if( !_lastFaceAnimDir.empty() )
        {
          std::string face_anim_dir = _dataPlatform->pathToResource(Util::Data::Scope::Cache,kCacheFaceAnimsDir);
          std::string full_path_face_imgs = Lrya::Util::FileUtils::FullFilePath({face_anim_dir, _lastFaceAnimDir,msg.filename});
