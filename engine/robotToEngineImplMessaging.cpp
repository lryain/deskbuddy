...
#include "engine/lryaEventUtil.h"
...
{
  using localHandlerType = void(RobotToEngineImplMessaging::*)(const LryaEvent<RobotInterface::RobotToEngine>&);
  ...
  using localHandlerTypeWithRoboRef = void(RobotToEngineImplMessaging::*)(const LryaEvent<RobotInterface::RobotToEngine>&, Robot* const);
  ...
  GetSignalHandles().push_back(messageHandler->Subscribe(RobotInterface::RobotToEngineTag::state,
                                                     [robot](const LryaEvent<RobotInterface::RobotToEngine>& message){
                                                       LRYA_CPU_PROFILE("RobotTag::state");
  ...
  GetSignalHandles().push_back(messageHandler->Subscribe(RobotInterface::RobotToEngineTag::chargerMountCompleted,
                                                         [robot](const LryaEvent<RobotInterface::RobotToEngine>& message){
                                                           LRYA_CPU_PROFILE("RobotTag::chargerMountCompleted");
  ...
  GetSignalHandles().push_back(messageHandler->Subscribe(RobotInterface::RobotToEngineTag::imuTemperature,
                                                     [robot](const LryaEvent<RobotInterface::RobotToEngine>& message){
                                                       LRYA_CPU_PROFILE("RobotTag::imuTemperature");
  ...
  if (robot->HasExternalInterface())
  {
    ...
    auto helper = MakeLryaEventUtil(*robot->GetExternalInterface(), *robot, GetSignalHandles());
    helper.SubscribeGameToEngine<MessageGameToEngineTag::RequestRobotSettings>();
  }
}

void RobotToEngineImplMessaging::HandleMotorCalibration(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
  LRYA_CPU_PROFILE("Robot::HandleMotorCalibration");
...
void RobotToEngineImplMessaging::HandleMotorAutoEnabled(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
  LRYA_CPU_PROFILE("Robot::HandleMotorAutoEnabled");
...
void RobotToEngineImplMessaging::HandlePickAndPlaceResult(const LryaEvent<RobotInterface::RobotToEngine>& message,
                                                          Robot* const robot)
{
  LRYA_CPU_PROFILE("Robot::HandlePickAndPlaceResult");
...
void RobotToEngineImplMessaging::HandleDockingStatus(const LryaEvent<RobotInterface::RobotToEngine>& message)
{
  LRYA_CPU_PROFILE("Robot::HandleDockingStatus");
...
void RobotToEngineImplMessaging::HandleFallingEvent(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
...
void RobotToEngineImplMessaging::HandleFallImpactEvent(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
...
void RobotToEngineImplMessaging::HandleGoalPose(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
  LRYA_CPU_PROFILE("Robot::HandleGoalPose");

  const GoalPose& payload = message.GetData().Get_goalPose();
  Lrya::Pose3d p(payload.pose.angle, Z_AXIS_3D(),
  ...
  if (payload.followingMarkerNormal) {
    robot->GetContext()->GetVizManager()->DrawPreDockPose(100, p, ::Lrya::NamedColors::RED);
  } else {
    robot->GetContext()->GetVizManager()->DrawPreDockPose(100, p, ::Lrya::NamedColors::GREEN);
  }
}

void RobotToEngineImplMessaging::HandleRobotStopped(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
  LRYA_CPU_PROFILE("Robot::HandleRobotStopped");
...
void RobotToEngineImplMessaging::HandlePotentialCliffEvent(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
  LRYA_CPU_PROFILE("Robot::HandlePotentialCliffEvent");
  ...
  if (robot->GetIsCliffReactionDisabled()){
    ...
    //       solution was used for Cozmo (see COZMO-15326 and https://github.com/lrya/cozmo-one/pull/6467)
...
void RobotToEngineImplMessaging::HandleCliffEvent(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
  LRYA_CPU_PROFILE("Robot::HandleCliffEvent");
...
void RobotToEngineImplMessaging::HandleImuData(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
  LRYA_CPU_PROFILE("Robot::HandleImuData");
  ...
  if (payload.seqId != _imuSeqID) {
    ...
    std::string imuLogsDir = robot->GetContextDataPlatform()->pathToResource(Util::Data::Scope::Cache, LryaUtil::kP_IMU_LOGS_DIR);
...
void RobotToEngineImplMessaging::HandleImuRawData(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
  LRYA_CPU_PROFILE("Robot::HandleImuRawData");
...
void RobotToEngineImplMessaging::HandleSyncRobotAck(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
  LRYA_CPU_PROFILE("Robot::HandleSyncRobotAck");
...
void RobotToEngineImplMessaging::HandleMicDirection(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
  const auto & payload = message.GetData().Get_micDirection();
  robot->GetMicComponent().GetMicDirectionHistory().AddMicSample(payload);
}

void RobotToEngineImplMessaging::HandleMicDataState(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
  const auto & payload = message.GetData().Get_micDataState();
  robot->GetMicComponent().SetBufferFullness(payload.rawBufferFullness);
}

void RobotToEngineImplMessaging::HandleDisplayedFaceImage(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
...
void RobotToEngineImplMessaging::HandleStreamCameraImages(const LryaEvent<RobotInterface::RobotToEngine>& message,
                                                          Robot* const robot)
{
...
void RobotToEngineImplMessaging::HandleRobotPoked(const LryaEvent<RobotInterface::RobotToEngine>& message, Robot* const robot)
{
  LRYA_CPU_PROFILE("Robot::HandleRobotPoked");
