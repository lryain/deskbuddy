...
namespace Vector {
  ...
  static void SetCladVarUnionFromConsoleVar(Lrya::Vector::ConsoleVarUnion& outVarValue, const Lrya::Util::IConsoleVariable* consoleVar)
  {
  ...
  static void SetCladVarFromConsoleVar(ExternalInterface::DebugConsoleVar& outDebugConsoleVar, const Lrya::Util::IConsoleVariable* consoleVar)
  {
  ...
  static void SetCladVarFromConsoleFunc(ExternalInterface::DebugConsoleVar& outDebugConsoleVar, const Lrya::Util::IConsoleFunction* consoleFunc)
  {
  ...
  void DebugConsoleManager::SendAllDebugConsoleVars()
  {
    ...
    for (const auto& entry : varDatabase)
    {
      ...
      {
        DEV_ASSERT(messageSize < Lrya::Comms::MsgPacket::MAX_SIZE, "DebugConsoleManager.VarDatabaseOverMaxSize");
    ...
    for (const auto& entry : funcDatabase)
    {
      ...
      if (messageSize >= kMaxFlushSize)
      {
        DEV_ASSERT(messageSize < Lrya::Comms::MsgPacket::MAX_SIZE, "DebugConsoleManager.FuncDatabaseOverMaxSize");
        FlushBuffer(std::move(dataVals), _externalInterface);
        dataVals.clear();
        messageSize = 0;
      }
  ...
  void SendVerifyDebugConsoleVarMessage(IExternalInterface* externalInterface,
                                        ...
                                        const Lrya::Util::IConsoleVariable* consoleVar,
                                        bool success)
  {
  ...
  void DebugConsoleManager::HandleEvent(const LryaEvent<ExternalInterface::MessageGameToEngine>& event)
  {
    ...
    switch (eventData.GetTag())
    {
      ...
      {
        const Lrya::Vector::ExternalInterface::GetDebugConsoleVarMessage& msg = eventData.Get_GetDebugConsoleVarMessage();
        Lrya::Util::IConsoleVariable* consoleVar = Lrya::Util::ConsoleSystem::Instance().FindVariable(msg.varName.c_str());
        if (consoleVar)
        {
      ...
      {
        const Lrya::Vector::ExternalInterface::RunDebugConsoleFuncMessage& msg = eventData.Get_RunDebugConsoleFuncMessage();
        Lrya::Util::IConsoleFunction* consoleFunc = Lrya::Util::ConsoleSystem::Instance().FindFunction(msg.funcName.c_str());
        if( consoleFunc )
        {
          enum { kBufferSize = 512 };
          char buffer[kBufferSize];
          const uint32_t res = NativeLryaUtilConsoleCallFunction( msg.funcName.c_str(), msg.funcArgs.c_str(), kBufferSize, buffer);
      ...
      {
        const Lrya::Vector::ExternalInterface::SetDebugConsoleVarMessage& msg = eventData.Get_SetDebugConsoleVarMessage();
      ...
      {
        const Lrya::Vector::ExternalInterface::SetAnimDebugConsoleVarMessage& msg = eventData.Get_SetAnimDebugConsoleVarMessage();
