...
public:
  ...
  LRYA_VISIBLE static void CreateInstance(const std::string& loggingBaseDirectory, const std::string& appRunId);
  LRYA_VISIBLE static DevLoggingSystem* GetInstance();
  LRYA_VISIBLE static Lrya::Util::ILoggerProvider* GetInstancePrintProvider();
  LRYA_VISIBLE static void DestroyInstance();
