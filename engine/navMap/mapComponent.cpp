...
{
  LRYA_CPU_PROFILE("MapComponent::UpdateRobotPoseInMemoryMap");
...
namespace {
  ...
  const size_t kMaxBufferSize = Lrya::Comms::MsgPacket::MAX_SIZE;
...
void MapComponent::SendDASInfoAboutMap(const PoseOriginID_t& mapOriginID) const
{
  if ( mapOriginID != PoseOriginList::UnknownOriginID )
  {
    ...
    if ( LRYA_VERIFY( (it != _navMaps.end()) && (it->second.map != nullptr),
                                      ...
                                      Pose3d& refinedCliffPose) const
{
  ...
  if(lineIdx < linesInCartes.size()) {
    ...
    */
    ...
    auto projCliffCenter = p1 + lineUnitVec * Lrya::DotProduct(lineUnitVec, newCliffCenter - p1);
    auto projYAxis = p1 + lineUnitVec * Lrya::DotProduct(lineUnitVec, (Pose2d)newCliffPose * Y_AXIS_2D() - p1); // on the edgeLine
