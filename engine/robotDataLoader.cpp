...
static Lrya::Vector::ThreadedPrintStressTester stressTester;
...
void RobotDataLoader::LoadNonConfigData()
{
  ...
  Lrya::Util::SetThreadName(pthread_self(), "RbtDataLoader");

  LRYA_CPU_TICK_ONE_TIME("RobotDataLoader::LoadNonConfigData");
  ...
  {
    LRYA_CPU_PROFILE("RobotDataLoader::CollectFiles");
    CollectAnimFiles();
  }

  {
    LRYA_CPU_PROFILE("RobotDataLoader::LoadBehaviors");
    LoadBehaviors();
  }

  {
    LRYA_CPU_PROFILE("RobotDataLoader::LoadWeatherResponseMaps");
    LoadWeatherResponseMaps();
  }

  {
    LRYA_CPU_PROFILE("RobotDataLoader::LoadWeatherRemaps");
    LoadWeatherRemaps();
  }

  {
    LRYA_CPU_PROFILE("RobotDataLoader::LoadWeatherConditionTTSMap");
    LoadWeatherConditionTTSMap();
  }

  {
    LRYA_CPU_PROFILE("RobotDataLoader::LoadVariableSnapshotJsonMap");
    LoadVariableSnapshotJsonMap();
  }
  
  {
    LRYA_CPU_PROFILE("RobotDataLoader::LoadCubeSpinnerConfig");
    LoadCubeSpinnerConfig();
  }

  {
    LRYA_CPU_PROFILE("RobotDataLoader::LoadUserDefinedBehaviorTreeConfig");
    LoadUserDefinedBehaviorTreeConfig();
  }

  {
    LRYA_CPU_PROFILE("RobotDataLoader::LoadSpritePaths");
    LoadSpritePaths();
    _spriteCache = std::make_unique<Vision::SpriteCache>(_spritePaths.get());
  }

  {
    LRYA_CPU_PROFILE("RobotDataLoader::LoadSpriteSequences");
  ...
  {
    {
      LRYA_CPU_PROFILE("RobotDataLoader::LoadAnimationGroups");
      LoadAnimationGroups();
    }

    {
      LRYA_CPU_PROFILE("RobotDataLoader::LoadCubeLightAnimations");
      LoadCubeLightAnimations();
    }

    {
      LRYA_CPU_PROFILE("RobotDataLoader::LoadCubeAnimationTriggerMap");
      LoadCubeAnimationTriggerMap();
    }
    {
      LRYA_CPU_PROFILE("RobotDataLoader::LoadEmotionEvents");
      LoadEmotionEvents();
    }

    {
      LRYA_CPU_PROFILE("RobotDataLoader::LoadDasBlacklistedAnimations");
      LoadDasBlacklistedAnimations();
    }

    {
      LRYA_CPU_PROFILE("RobotDataLoader::LoadAnimationTriggerMap");
      LoadAnimationTriggerMap();
    }

    {
      LRYA_CPU_PROFILE("RobotDataLoader::LoadAnimationWhitelist");
...
void RobotDataLoader::LoadAnimationWhitelist()
{
  ...
  if(!success)
  {
    ...
    for( const auto& clipName : whitelistConfig[kDriveOffChargerAnimsKey] ) {
      if( LRYA_VERIFY( clipName.isString(),
  ...
  for (const auto& filename : responseJSONFiles)
  {
        ...
        responseJSON.isArray())
    {
      for(const auto& pair : responseJSON){
        if(LRYA_VERIFY(pair.isMember(kAPIValueKey) && pair.isMember(kCladTypeKey),
...
void RobotDataLoader::LoadRobotConfigs()
{
  ...
  LRYA_CPU_TICK_ONE_TIME("RobotDataLoader::LoadRobotConfigs");
  // mood config
  {
  ...
  // deskmate behavior systems config
  {
    static const std::string jsonFilename = "config/engine/behaviorComponent/deskmate_behavior_config.json";
    const bool success = _platform->readAsJson(Util::Data::Scope::Resources, jsonFilename, _deskmateFreeplayBehaviorConfig);
    if (!success)
    {
      LOG_ERROR("RobotDataLoader.BehaviorSystemJsonFailed",
                "Behavior Json config file %s not found or failed to parse",
                jsonFilename.c_str());
      _deskmateFreeplayBehaviorConfig.clear();
    }
  }
  ...
  // A/B testing definition
  {
    ...
    const std::string fileContents{Util::FileUtils::ReadFile(filename)};
    _context->GetExperiments()->GetLryaLab().Load(fileContents);
  }
