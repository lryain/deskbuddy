  ...
  , _animationTransferHandler(new AnimationTransfer(_uiMsgHandler.get(), dataPlatform))
{
  ...
  Lrya::Util::CpuThreadProfiler::SetChromeTracingFile(
      dataPlatform->pathToResource(Util::Data::Scope::Cache, "mate-engine-tracing.json").c_str());
  ...
  auto helper = MakeLryaEventUtil(*_context->GetExternalInterface(), *this, _signalHandles);
  ...
  auto handler = [this] (const std::vector<Util::LryaLab::AssignmentDef>& assignments) {
    _context->GetExperiments()->UpdateLabAssignments(assignments);
  };
  _signalHandles.emplace_back(_context->GetExperiments()->GetLryaLab()
...
Result CozmoEngine::Init(const Json::Value& config) {
  ...
  if(!_config.isMember(LryaUtil::kP_ADVERTISING_HOST_IP)) {
    PRINT_NAMED_ERROR("CozmoEngine.Init", "No AdvertisingHostIP defined in Json config.");
    return RESULT_FAIL;
  }

  if(!_config.isMember(LryaUtil::kP_UI_ADVERTISING_PORT)) {
...
# endif
  ...
  // data: Unused
  Lrya::Util::sInfo("cozmo_engine.init.build_configuration", {},
#if defined(NDEBUG)
                    "RELEASE");
#else
                    "DEBUG");
...
{
  LRYA_CPU_PROFILE("CozmoEngine::Update");
...
{
#if LRYA_DEV_CHEATS
  if(Lrya::Util::gLoggerProvider != nullptr) {
    Lrya::Util::MultiLoggerProvider* multiLoggerProvider = dynamic_cast<Lrya::Util::MultiLoggerProvider*>(Lrya::Util::gLoggerProvider);
    if (multiLoggerProvider != nullptr) {
      const std::vector<Lrya::Util::ILoggerProvider*>& loggers = multiLoggerProvider->GetProviders();
...
Util::LryaLab::AssignmentStatus CozmoEngine::ActivateExperiment(
  const Util::LryaLab::ActivateExperimentRequest& request, std::string& outVariationKey)
{
