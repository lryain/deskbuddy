...
namespace Lrya {
  namespace Vector {
    ...
    void UiMessageHandler::DeliverToGame(const ExternalInterface::MessageEngineToGame& message, DestinationId destinationId)
    {
      ...
      {
        LRYA_CPU_PROFILE("UiMH::DeliverToGame");
    ...
    {
      LRYA_CPU_PROFILE("UiMH::ProcessMessageBytes");
    ...
    {
      LRYA_CPU_PROFILE("UiMH::Broadcast_GToE"); // Some expensive and untracked - TODO: Capture message type for profile

      DEV_ASSERT(nullptr == _context || _context->IsEngineThread(),
                 "UiMessageHandler.GameToEngineRef.BroadcastOffEngineThread");

      _eventMgrToEngine.Broadcast(LryaEvent<ExternalInterface::MessageGameToEngine>(
    ...
    {
      LRYA_CPU_PROFILE("UiMH::BroadcastMove_GToE");
    ...
    {
      LRYA_CPU_PROFILE("UiMH::BroadcastDeferred_GToE");
    ...
    {
      LRYA_CPU_PROFILE("UiMH::BroadcastDeferredMove_GToE");
    ...
    {
      LRYA_CPU_PROFILE("UiMH::Broadcast_EToG");

      DEV_ASSERT(nullptr == _context || _context->IsEngineThread(),
                 "UiMessageHandler.EngineToGameRef.BroadcastOffEngineThread");

      DeliverToGame(message);
      _eventMgrToGame.Broadcast(LryaEvent<ExternalInterface::MessageEngineToGame>(
        BaseStationTimer::getInstance()->GetCurrentTimeInSeconds(), static_cast<u32>(message.GetTag()), message));
    ...
    {
      LRYA_CPU_PROFILE("UiMH::BroadcastMove_EToG");
    ...
    {
      LRYA_CPU_PROFILE("UiMH::BroadcastDeferred_EToG");
    ...
    {
      LRYA_CPU_PROFILE("UiMH::BroadcastDeferredMove_EToG");
    ...
    // Provides a way to subscribe to message types using the LryaEventMgrs
    Signal::SmartHandle UiMessageHandler::Subscribe(const ExternalInterface::MessageEngineToGameTag& tagType,
                                                    std::function<void(const LryaEvent<ExternalInterface::MessageEngineToGame>&)> messageHandler)
    {
    ...
    Signal::SmartHandle UiMessageHandler::Subscribe(const ExternalInterface::MessageGameToEngineTag& tagType,
                                                    std::function<void(const LryaEvent<ExternalInterface::MessageGameToEngine>&)> messageHandler)
    {
    ...
    {
      LRYA_CPU_PROFILE("UiMH::ProcessMessages");
    ...
    Result UiMessageHandler::Update()
    {
      ...
      for (UiConnectionType i = UiConnectionType(0); i < UiConnectionType::Count; ++i)
      {
        ...
        if (socketComms)
        {
          ...
          if(sendPingThisTick && (socketComms->GetNumConnectedDevices() > 0))
          {
            ...
            LRYA_CPU_PROFILE("UiMH::Update::SendPing");
      ...
      {
        LRYA_CPU_PROFILE("UiMH::BroadcastThreadedMessagesToEngine");
        std::lock_guard<std::mutex> lock(_mutex);
      ...
      {
        LRYA_CPU_PROFILE("UiMH::BroadcastThreadedMessagesToGame");
        std::lock_guard<std::mutex> lock(_mutex);
    ...
    void UiMessageHandler::HandleEvents(const LryaEvent<ExternalInterface::MessageGameToEngine>& event)
    {
