...
#include "util/lryaLab/lryaLabDef.h"
...
#pragma mark --- CozmoAPI Methods ---
  ...
  CONSOLE_VAR_ENUM(u8, kCozmoEngine_Logging,       LRYA_CPU_CONSOLEVARGROUP, 0, Util::CpuProfiler::CpuProfilerLogging());
#endif
...
LRYA_CPU_PROFILER_ENABLED_ONLY(const float kMaxDesiredEngineDuration = 60.0f); // Above this warn etc.
...
bool CozmoAPI::Update(const BaseStationTime_t currentTime_nanosec)
{
  ...
  LRYA_CPU_TICK("CozmoEngine", kMaxDesiredEngineDuration, Util::CpuProfiler::CpuProfilerLoggingTime(kCozmoEngine_Logging));
...
{
  using namespace Lrya::Util::LryaLab;
  ...
  return Lrya::Util::numeric_cast<uint32_t>(bytesPacked);
}
