...
namespace {
  ...
  void InitFeatureOverrides( Util::Data::DataPlatform* platform )
  {
    ...
    sOverrideSaveFilename = platform->pathToResource( Lrya::Util::Data::Scope::Cache, fileName );
...
void CozmoFeatureGate::Init(const CozmoContext* context, const std::string& jsonContents)
{
  ...
  if( context != nullptr ) {
    ...
    if( gi != nullptr ) {
      auto handleFeatureFlagRequest = [this, gi](const LryaEvent<external_interface::GatewayWrapper>& msg) {
