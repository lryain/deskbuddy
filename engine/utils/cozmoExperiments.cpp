...
{
  _lab.Enable(!Util::LryaLab::ShouldABTestingBeDisabled());
  ...
  auto labOpRunner = [this] (const Util::LryaLab::LabOperation& op) {
    op(&_lab);
  };
  ...
  Util::LryaLab::InitializeABInterface(labOpRunner, userIdAccessor);
}

Util::LryaLab::AssignmentStatus CozmoExperiments::ActivateExperiment(
...
void CozmoExperiments::WriteLabAssignmentsToRobot(const std::vector<Util::LryaLab::AssignmentDef>& assignments)
{
  Robot* robot = _context->GetRobotManager()->GetRobot();
  if (robot != nullptr)
  {
                                                      ...
                                                      const NVStorage::NVResult res, u32 serialNumber)
{
  ...
  using namespace Util::LryaLab;
  LryaLab& lab = GetLryaLab();
...
void CozmoExperiments::UpdateLabAssignments(const std::vector<Util::LryaLab::AssignmentDef>& assignments)
{
