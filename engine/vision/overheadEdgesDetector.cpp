...
Result OverheadEdgesDetector::Detect(Lrya::Vision::ImageCache &imageCache, const VisionPoseData &crntPoseData,
                                           ...
                                           VisionProcessingResult &currentResult)
{
  ...
  Lrya::Rectangle<s32> bbox(groundInImage);
  ...
  bool isLiftBotInCamera = true;
  {
    ...
    static const Lrya::Vec3f offsetTopFrontPoint{LIFT_FRONT_WRT_WRIST_JOINT, 0.f, LIFT_XBAR_HEIGHT_WRT_WRIST_JOINT};
    static const Lrya::Vec3f offsetBotBackPoint{LIFT_BACK_WRT_WRIST_JOINT, 0.f,
    ...
    const Lrya::Vec3f liftTopWrtCamera = liftPoseWrtCamera * offsetTopFrontPoint;
    Lrya::Point2f liftTopCameraPoint;
    ...
    const Lrya::Vec3f liftBotWrtCamera = liftPoseWrtCamera * offsetBotBackPoint;
    Lrya::Point2f liftBotCameraPoint;
  ...
  if (kDebugRenderBboxVsLift) {
    _vizManager->DrawCameraOval(Lrya::Point2f{120, planeTopY}, 3, 3, NamedColors::WHITE);
    _vizManager->DrawCameraOval(Lrya::Point2f{120, planeBotY}, 3, 3, NamedColors::WHITE);
  }
  ...
  if (DRAW_OVERHEAD_IMAGE_EDGES_DEBUG) {
    ...
    static const Lrya::Point2f dispOffset(-roi.GetDist(), roi.GetWidthFar() * 0.5f);
    ...
    for (const auto &chain : candidateChains.GetVector()) {
      if (chain.points.size() >= _kMinChainLength) {
        for (s32 i = 1; i < chain.points.size(); ++i) {
          Lrya::Point2f startPoint(chain.points[i - 1].position);
          startPoint.y() = -startPoint.y();
          startPoint += dispOffset;
          Lrya::Point2f endPoint(chain.points[i].position);
    ...
    for (const auto &chain : candidateChains.GetVector()) {
      for (s32 i = 0; i < chain.points.size(); ++i) {
        const Lrya::Point2f &groundPoint = chain.points[i].position;
        Point3f temp = H * Lrya::Point3f(groundPoint.x(), groundPoint.y(), 1.f);
    ...
    for (const auto &chain : edgeFrame.chains.GetVector()) {
      ...
      for (const auto &point : chain.points) {
        ...
        if (pointWrtOrigin.GetWithRespectTo(crntPoseData.cameraPose, pointWrtCamera)) {
          Lrya::Point2f pointInCameraView;
