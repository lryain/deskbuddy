...
#   define GET_JSON_PARAMETER(__json__, __fieldName__, __variable__) \
  ...
  // =============================== neuralnets start ===============================
  #if defined(LRYA_NEURALNETS_USE_TFLITE)
...
# define DEBUG_IMAGE_HISTOGRAM 0
  ...
  if(DEBUG_IMAGE_HISTOGRAM)
  {
    ...
    histImg.DrawText(Lrya::Point2f((s32)hist.GetCounts().size()/3, 12),
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Vision::Image BlackOutRects(const Vision::Image& img, const std::vector<Lrya::Rectangle<s32>>& rects)
{
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Result VisionSystem::DetectFaces(Vision::ImageCache& imageCache, std::vector<Lrya::Rectangle<s32>>& detectionRects,
...
//                                 std::vector<Lrya::Rectangle<s32>>& detections)
// {
//   const Vision::Image& grayImage = imageCache.GetGray();
...
Result VisionSystem::DetectMarkers(Vision::ImageCache& imageCache,
                                  ...
                                  std::vector<Lrya::Rectangle<s32>>& detectionRects,
...
void VisionSystem::CheckForNeuralNetResults()
{
  for(const auto& neuralNetRunnerEntry : _neuralNetRunners)
  {
    ...
    if(resultReady)
    {
      ...
      if(LRYA_VERIFY(success, "VisionSystem.CheckForNeuralNetResults.NoModeForNetworkName", "Name: %s",
                     networkName.c_str()))
      {
...
Result VisionSystem::Update(const VisionPoseData& poseData, Vision::ImageCache& imageCache)
{
  ...
  LRYA_VERIFY(RESULT_OK == lastResult, "VisionSystem.Update.FailedCLAHE", "ApplyCLAHE supposedly has no failure mode");
  ...
  for(const auto& mode : GetVisionModesUsingNeuralNets())
  {
    if(IsModeEnabled(mode))
    {
      ...
      if(LRYA_VERIFY(success, "VisionSystem.Update.NoNetworkForMode", "%s", EnumToString(mode)))
      {
