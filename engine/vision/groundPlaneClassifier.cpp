...
void ClassifyImage(const RawPixelsClassifier& clf, const Lrya::Vector::IFeaturesExtractor& extractor,
  ...
  , _profiler("GroundPlaneClassifier")
{
  ...
  const std::string fullpath = context->GetDataPlatform()->pathToResource(Lrya::Util::Data::Scope::Resources,
                                                                          path);
                                     ...
                                     std::list<OverheadEdgeFrame>& outEdges)
{
  ...
  if(DEBUG_DISPLAY_IMAGES)
  {
    ...
    for (const auto &chain : candidateChains.GetVector())
    {
      ...
      Lrya::Point2f startPoint(chain.points[0].position);
...
{
  const std::string positivePath = Lrya::Util::FileUtils::FullFilePath({path, "positivePixels.txt"});
  const std::string negativePath = Lrya::Util::FileUtils::FullFilePath({path, "negativePixels.txt"});
  ...
  // Error on whole training set
  {
    ...
    PRINT_CH_DEBUG("VisionSystem", "GroundPlaneClassifier.Train.ErrorLevel",
                   "Error after training is: %f", Lrya::calculateError(responsesMat, trainingLabels));
  }
  _initialized = true;
