...
#ifndef __Lrya_Cozmo_RawPixelsClassifier_H__
#define __Lrya_Cozmo_RawPixelsClassifier_H__
...
public:
  ...
  explicit RawPixelsClassifier(const CozmoContext *context, Lrya::Vision::Profiler* profiler = nullptr);
   ...
   */
  virtual std::vector<uchar> PredictClass(const Lrya::Array2d<FeatureType>& features) const;
...
protected:
  ...
  Lrya::Vision::Profiler* _profiler;
...
protected:
  ...
  cv::Ptr<Lrya::WeightedLogisticRegression> _logisticRegressor;
};
...
public:
  explicit DTRawPixelsClassifier(const Json::Value& config, const CozmoContext *context,
                                 Lrya::Vision::Profiler* profiler = nullptr);
  DTRawPixelsClassifier(const CozmoContext *context,
                        Lrya::Vision::Profiler* profiler = nullptr);

  uchar PredictClass(const std::vector<FeatureType>& values) const override;
  using RawPixelsClassifier::PredictClass;
  std::vector<uchar> PredictClass(const Lrya::Array2d<FeatureType>& features) const override;
