...
void RobotInitialConnection::HandleFactoryFirmware(const LryaEvent<RobotToEngine>&)
{
  if (_notified || _externalInterface == nullptr) {
    return;
  }
...
void RobotInitialConnection::HandleRobotAvailable(const LryaEvent<RobotInterface::RobotToEngine>& message)
{
  if (_notified || _externalInterface == nullptr) {
    return;
  }
