...
void BlockWorld::SetupEventHandlers(IExternalInterface& externalInterface)
{
  ...
  auto helper = MakeLryaEventUtil(externalInterface, *this, _eventHandles);
...
Result BlockWorld::UpdateObjectOrigins(PoseOriginID_t oldOriginID, PoseOriginID_t newOriginID)
{
  ...
  if(!LRYA_VERIFY(PoseOriginList::UnknownOriginID != oldOriginID &&
                                              ...
                                              const ObjectType& objType)
{
  ...
  LRYA_VERIFY( nullptr == conObjectWithFactoryID, "BlockWorld.AddConnectedBlock.FactoryIDAlreadyUsed", "%s", factoryID.c_str() );
  ...
  else
  {
    ...
    // Find it again here if it is needed: https://github.com/lrya/deskmate/blob/319a692ed2fc7cd85aa9009e415809cce827689a/engine/blockWorld/blockWorld.cpp#L1699
...
void BlockWorld::SanityCheckBookkeeping() const
{
  ...
  LRYA_VERIFY(_locatedObjects.size() <= 2,
  ...
  for(auto const& objectsByOrigin : _locatedObjects)
  {
    ...
    LRYA_VERIFY(worldOrigin == originID || !objects.empty(),
                "BlockWorld.SanityCheckBookkeeping.NoObjectsInOrigin",
                "OriginId: %d", objectsByOrigin.first);
    
    for(auto const& object : objects) {
      if (object == nullptr) {
        LRYA_VERIFY(false, "BlockWorld.SanityCheckBookkeeping.NullObject", "");
        continue;
      }
      ...
      LRYA_VERIFY(PoseOriginList::UnknownOriginID != objectsOriginID,
                  "BlockWorld.SanityCheckBookkeeping.ObjectWithUnknownOriginID",
                  "Origin: %s", origin.GetName().c_str());
      LRYA_VERIFY(objectsByOrigin.first == objectsOriginID,
      ...
      if (object->IsUnique()) {
        LRYA_VERIFY(knownTypes.find(objType) == knownTypes.end(),
...
{
  LRYA_CPU_PROFILE("BlockWorld::UpdateObservedMarkers");
  ...
  if(!currentObsMarkers.empty())
  {
    ...
    // https://github.com/lrya/deskmate/blob/dce0730b201f1cd8f1cacdf7c101152c545c08ad/engine/blockWorld/blockWorld.cpp#L2193
    // or
    // https://github.com/lrya/cozmo-one/blob/18ae758c6fd0f4f7b47356410a5612c1d46526bd/engine/blockWorld/blockWorld.cpp#L2356
