...
void MoodManager::ReadMoodConfig(const Json::Value& inJson)
{
  ...
  if (nullptr != _robot) {
    if( _robot->HasExternalInterface() ) {
      auto helper = MakeLryaEventUtil(*_robot->GetExternalInterface(), *this, _signalHandles);
...
void MoodManager::LoadAudioParameterMap(const Json::Value& inJson)
{
  ...
  if( LRYA_VERIFY( ! inJson.isArray(), "MoodManager.LoadAudioParameterMap.MissingKey",
                   "No audio parameter map specified, or it isn't a list" ) ) {

    for( auto mapIt = inJson.begin(); mapIt != inJson.end(); ++mapIt ) {
      EmotionType emo;
      if( LRYA_VERIFY( EmotionTypeFromString(mapIt.key().asCString(), emo),
        ...
        if( LRYA_VERIFY( AudioMetaData::GameParameter::ParameterTypeFromString( mapIt->asCString(), audioParam ),
...
void MoodManager::LoadAudioSimpleMoodMap(const Json::Value& inJson)
{
  ...
  LRYA_VERIFY( AudioMetaData::GameParameter::ParameterTypeFromString( audioParamStr, _simpleMoodAudioParameter ),
  ...
  for( auto mapIt = mapJson.begin(); mapIt != mapJson.end(); ++mapIt ) {
    ...
    if( LRYA_VERIFY( SimpleMoodTypeFromString( mapIt.key().asCString(), simpleMood ),
                     "MoodManager.LoadAudioSimpleMoodMap.InvalidSimpleMood",
                     "event map key '%s' is not a valid simple mood",
                     mapIt.key().asCString() ) ) {
      if( LRYA_VERIFY( mapIt->isNumeric(),
...
{
  LRYA_CPU_PROFILE("MoodManager::Update");
  ...
  if( LRYA_DEV_CHEATS && dependentComps.HasComponent<ContextWrapper>() ) {
...
{
  const Lrya::Vector::ExternalInterface::MoodMessageUnion& moodMessage = msg.MoodMessageUnion;
  switch (moodMessage.GetTag())
  {
    ...
    {
      const Lrya::Vector::ExternalInterface::SetEmotion& msg = moodMessage.Get_SetEmotion();
      SetEmotion(msg.emotionType, msg.newVal);
      break;
    }
    case ExternalInterface::MoodMessageUnionTag::AddToEmotion:
    {
      const Lrya::Vector::ExternalInterface::AddToEmotion& msg = moodMessage.Get_AddToEmotion();
    ...
    {
      const Lrya::Vector::ExternalInterface::TriggerEmotionEvent& msg = moodMessage.Get_TriggerEmotionEvent();
...
{
  LRYA_CPU_PROFILE("MoodManager::SendEmotionsToGame"); // ~1ms per tick (iPhone 5S in Release) - could send every N ticks?
...
void MoodManager::SendEmotionsToAudio(Audio::EngineRobotAudioClient& audioClient)
{
  ...
  if( nullptr != _robot ) {
    ...
    const bool isPrDemo = featureGate->IsFeatureEnabled(Lrya::Vector::FeatureType::PRDemo);
    if(isPrDemo) {
      return;
    }
  }
...
void MoodManager::TriggerEmotionEvent(const std::string& eventName, float currentTimeInSeconds)
{
  ...
  if (emotionEvent)
  {
    ...
    if( LRYA_DEV_CHEATS && kMoodManager_WebVizPeriod_s >= 0.0f && nullptr != _robot ) {
    ...
    // do some verification that the above numbers are still valid
    {
      ...
      for( const auto& simpleMood : moodList )  {
        ...
        const float conf = simpleMoods[SimpleMoodTypeToString(simpleMood)]["Confident"].isDouble()
                             ...
                             : 0.0f;
        LRYA_VERIFY( (GetSimpleMood( stim, conf ) == simpleMood)
