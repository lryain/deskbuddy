...
void SettingsCommManager::InitDependent(Robot* robot, const RobotCompMap& dependentComponents)
{
  ...
  _signalHandles.push_back(messageHandler->Subscribe(RobotInterface::RobotToEngineTag::reportCloudConnectivity, [this](const LryaEvent<RobotInterface::RobotToEngine>& event) {
    ...
    response->set_code(static_cast<::Lrya::Vector::external_interface::CheckCloudResponse_ConnectionCode>(protoVersionOfCodeEnum));
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SettingsCommManager::HandleEvents(const LryaEvent<external_interface::GatewayWrapper>& event)
{
