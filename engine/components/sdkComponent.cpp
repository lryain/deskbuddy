...
void SDKComponent::InitDependent(Vector::Robot* robot, const RobotCompMap& dependentComps)
{
  ...
  if(context != nullptr && context->GetExternalInterface() != nullptr)
  {
    ...
    auto helper = MakeLryaEventUtil(*context->GetExternalInterface(), *this, _signalHandles);
  ...
  _signalHandles.push_back(messageHandler->Subscribe(RobotInterface::RobotToEngineTag::audioStreamStatusEvent, [this](const LryaEvent<RobotInterface::RobotToEngine>& event) {
...
void SDKComponent::HandleMessage(const ExternalInterface::UnexpectedMovement& msg)
{
  ...
  LRYA_VERIFY(gi != nullptr, "SDKComponent.HandleMessage.NullGatewayInterface", "");
...
void SDKComponent::IsImageStreamingEnabledRequest(
  const LryaEvent<external_interface::GatewayWrapper>& event)
{
  auto* gi = _robot->GetGatewayInterface();
  if (gi == nullptr) {
    return;
...
void SDKComponent::OnSendAudioModeRequest(const LryaEvent<external_interface::GatewayWrapper>& event)
{
...
void SDKComponent::HandleProtoMessage(const LryaEvent<external_interface::GatewayWrapper>& event)
{
...
{
  LRYA_VERIFY(_sdkWantsControl, "SDKComponent::SDKControlLevel", "_sdkWantsControl not set when accessing _sdkControlLevel");
...
void SDKComponent::HandleAudioStreamPrepareRequest(const LryaEvent<external_interface::GatewayWrapper>& event)
{
  u32 rate, volume;
...
void SDKComponent::HandleAudioStreamChunkRequest(const LryaEvent<external_interface::GatewayWrapper>& event)
{
  ...
  if (!LRYA_VERIFY(msg.audio_chunk_size <= 1024,
  ...
  if (!LRYA_VERIFY(samplesStr.length() >= msg.audio_chunk_size,
...
void SDKComponent::HandleAudioStreamCompleteRequest(const LryaEvent<external_interface::GatewayWrapper>& event)
{
...
void SDKComponent::HandleAudioStreamCancelRequest(const LryaEvent<external_interface::GatewayWrapper>& event)
{
...
void SDKComponent::SetMasterVolume(const LryaEvent<external_interface::GatewayWrapper>& event)
{
  auto* gi = _robot->GetGatewayInterface();
...
void SDKComponent::SayText(const LryaEvent<external_interface::GatewayWrapper>& event)
{
...
void SDKComponent::EnableCameraAutoExposure(bool enable)
{
  ...
  camera_msg.enableAutoExposure = enable;
...
void SDKComponent::SetCameraSettings(const LryaEvent<external_interface::GatewayWrapper>& event)
{
  using namespace external_interface;
  ...
  if (request.enable_auto_exposure()) {
    EnableCameraAutoExposure();
  } else {
    //validate exposure params
...
void SDKComponent::SetEyeColor(const LryaEvent<external_interface::GatewayWrapper>& event)
{
...
void SDKComponent::ListAnimationTriggers(const LryaEvent<external_interface::GatewayWrapper>& event)
{
  auto* gi = _robot->GetGatewayInterface();
  if (gi == nullptr) return;
  ...
  // Send "EndOfListAnimationTriggersResponses". mate-gateway recipient depends upon it.
