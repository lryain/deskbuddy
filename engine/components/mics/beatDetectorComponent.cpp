...
void BeatDetectorComponent::InitDependent(Vector::Robot* robot, const RobotCompMap& dependentComps)
{
  ...
  _signalHandle = messageHandler->Subscribe(RobotInterface::RobotToEngineTag::beatDetectorState, [this](const LryaEvent<RobotInterface::RobotToEngine>& event) {
...
bool BeatDetectorComponent::UnregisterOnBeatCallback(const int callbackId)
{
  ...
  const bool success = LRYA_VERIFY(numErased == 1,
