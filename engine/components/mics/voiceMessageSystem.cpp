...
void VoiceMessageSystem::Initialize( Robot* robot )
{
...

  {
    ...
    AddSignalHandle( messageHandler->Subscribe( RobotToEngineTag::audioPlaybackEnd, [this]( const LryaEvent<RobotToEngine>& event )
    {
      const AudioPlaybackEnd& audioEvent = event.GetData().Get_audioPlaybackEnd();
      OnPlaybackComplete( audioEvent.path );
    } ) );

    AddSignalHandle( messageHandler->Subscribe( RobotToEngineTag::micRecordingComplete, [this]( const LryaEvent<RobotToEngine>& event )
    {
