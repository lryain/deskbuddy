...
void BatteryComponent::NotifyOfRobotState(const RobotState& msg)
{
  ...
  // This is actually handled in mate-robot, but is recorded here for viz purposes.
...
void BatteryComponent::UpdateSuggestedChargerTime(bool wasLowBattery, bool wasCharging)
{
  ...
  if( IsCharging() ) {
    ...
    if( isLowBattery && !countdownStarted ) {
      ...
      LRYA_VERIFY( _timeRemovedFromCharger_s != 0.0f, "BatteryComponent.UpdateSuggestedChargerTime.UnexpectedOffChargerTime",
      ...
      _suggestedChargeEndTime_s = Lrya::Util::Min( _suggestedChargeEndTime_s, currTime_s + kRequiredChargeTime_s);
    }
...
float BatteryComponent::GetSuggestedChargerTime() const
{
  ...
  if( _suggestedChargeEndTime_s == 0.0f ) {
    ...
    if( !IsCharging()) {
      ...
      return Lrya::Util::Clamp(fixedTime, 0.0f, kRequiredChargeTime_s);
    } else {
      return Lrya::Util::Max( _suggestedChargeEndTime_s - currTime_s, 0.0f );
    }
  }
}
