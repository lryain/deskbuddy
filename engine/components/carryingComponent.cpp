...
Result CarryingComponent::PlaceObjectOnGround()
{
  ...
  return _robot->SendRobotMessage<Lrya::Vector::PlaceObjectOnGround>(0, 0, 0,
...
{
  return _robot->SendMessage(RobotInterface::EngineToRobot(Lrya::Vector::CarryStateUpdate(state)));
}
...
void CarryingComponent::UnSetCarryingObject()
{
  ...
  if (IsCarryingObject())
  {
    ...
    } else if ( !carriedObject->GetPose().GetParent().IsRoot() )
    {
      ...
      // instead of LRYA_VERIFY (which would be my ideal choice if delete took care of also detaching)
