...
void CubeCommsComponent::InitDependent(Vector::Robot* robot, const RobotCompMap& dependentComps)
{
  ...
  if (LRYA_VERIFY(_robot->HasGatewayInterface(),
  ...
  if (LRYA_VERIFY(platform != nullptr,
...
void CubeCommsComponent::HandleGameEvents(const LryaEvent<ExternalInterface::MessageGameToEngine>& event)
{
...
void CubeCommsComponent::HandleAppEvents(const LryaEvent<external_interface::GatewayWrapper>& event)
{
