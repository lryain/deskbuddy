...
{
  _robot = robot;
  if(LRYA_VERIFY(_robot->HasExternalInterface(),
...
void CubeInteractionTracker::UpdateIsHeldEstimate(const RobotCompMap& dependentComps)
{
  ...
  if(_targetStatus.movedFarThisFrame){
    _targetStatus.probabilityIsHeld = Lrya::Util::Max(kTargetMovedFarHeldConfidence, _targetStatus.probabilityIsHeld);
  }
  else{
    ...
    _targetStatus.probabilityIsHeld = Lrya::Util::Clamp(_targetStatus.probabilityIsHeld + probHeldDelta, 0.0f, 1.0f);
  }
...
bool CubeInteractionTracker::SelectTarget(const RobotCompMap& dependentComps)
{
  ...
  if(ccc.IsConnectedToCube()){
    ...
    LRYA_VERIFY(nullptr != _targetStatus.activeObject,
...
// ^ ^ ^ Target Tracking Methods ^ ^ ^
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void CubeInteractionTracker::HandleEngineEvents(const LryaEvent<ExternalInterface::MessageEngineToGame>& event)
{
