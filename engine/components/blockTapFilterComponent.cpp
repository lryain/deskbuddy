...
CONSOLE_VAR(Lrya::TimeStamp_t, kTapWaitOffset_ms, "TapFilter.WaitOffsetTime", 75);
CONSOLE_VAR(Lrya::TimeStamp_t, kDoubleTapTime_ms, "TapFilter.DoubleTapTime", 500);
CONSOLE_VAR(Lrya::TimeStamp_t, kIgnoreMoveTimeAfterDoubleTap_ms, "TapFilter.IgnoreMoveTimeAfterDoubleTap", 500);
...
{
  LRYA_CPU_PROFILE("BlockTapFilterComponent::Update");
...
void BlockTapFilterComponent::HandleObjectTapped(const ExternalInterface::ObjectTapped& payload)
{
  ...
  const Lrya::TimeStamp_t engineTime = BaseStationTimer::getInstance()->GetCurrentTimeStamp();
