...
void AnimationComponent::InitDependent(Vector::Robot* robot, const RobotCompMap& dependentComps)
{
  ...
  if (extInterface != nullptr) {
    ...
    auto helper = MakeLryaEventUtil(*extInterface, *this, GetSignalHandles());
  ...
  using localHandlerType = void(AnimationComponent::*)(const LryaEvent<RobotInterface::RobotToEngine>&);
                                                                 ...
                                                                 int recursionCount) const
{
  ...
  if( !LRYA_VERIFY( recursionCount < kMaxAnimGroupRecursionDepth,
                                                       ...
                                                       const std::string& lockFaceAtEndOfAnimTag)
{
  ...
  for(const auto& remap : remaps){
    ...
    if(!LRYA_VERIFY(spritePathMap.IsValidAssetID(remap.second),
...
Result AnimationComponent::DisplayFaceImageBinary(const Vision::Image& img, u32 duration_ms, bool interruptRunning)
{
  ...
  if (!LRYA_VERIFY(imageIsValidSize, 
...
{
  if( LRYA_VERIFY( _numKeepFaceAliveDisableLocks > 0,
...
void AnimationComponent::HandleAnimAdded(const LryaEvent<RobotInterface::RobotToEngine>& message)
{
...
void AnimationComponent::HandleAnimStarted(const LryaEvent<RobotInterface::RobotToEngine>& message)
{
...
void AnimationComponent::HandleAnimEnded(const LryaEvent<RobotInterface::RobotToEngine>& message)
{
...
void AnimationComponent::HandleAnimationEvent(const LryaEvent<RobotInterface::RobotToEngine>& message)
{
...
void AnimationComponent::HandleAnimState(const LryaEvent<RobotInterface::RobotToEngine>& message)
{
