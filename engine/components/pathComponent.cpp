...
void PathComponent::InitDependent(Vector::Robot* robot, const RobotCompMap& dependentComps)
{
  ...
  auto eventLambda = [this](const LryaEvent<RobotInterface::RobotToEngine>& event)
  {
      ...
      if( IsWaitingToCancelPath() ) {
        if( payload.pathID == _lastCanceledPathID ) {
          ...
          if( _driveToPoseStatus == ERobotDriveToPoseStatus::WaitingToCancelPath ) {
            ...
            LRYA_VERIFY(_driveToPoseStatus == ERobotDriveToPoseStatus::WaitingToCancelPathAndSetFailure,
    ...
    switch(payload.eventType)
    {
      case PathEventType::PATH_STARTED:
      {
        ...
        if( !LRYA_VERIFY(ERobotDriveToPoseStatus::WaitingToBeginPath == _driveToPoseStatus ||
        ...
        LRYA_VERIFY(payload.pathID == _lastSentPathID,
      ...
      case PathEventType::PATH_INTERRUPTED:
      {
        ...
        LRYA_VERIFY(payload.pathID == _lastRecvdPathID,
...
void PathComponent::HandlePossibleOriginChanges()
{
  ...
  LRYA_VERIFY(_currPlanParams->commonOriginID != PoseOriginList::UnknownOriginID,
              "PathComponent.HandlePossibleOriginChanges.NullParamsOrigin", "");

  LRYA_VERIFY(_robot->GetPoseOriginList().ContainsOriginID(_currPlanParams->commonOriginID),
...
void PathComponent::RejiggerTargetsAndReplan()
{
  ...
  if( LRYA_VERIFY(rejiggerSuccess, "PathComponent.Update.Rejigger",
...
{
  LRYA_VERIFY(_hasCustomMotionProfile, "PathComponent.GetCustomMotionProfile.NoCustomProfile",
