...
void JdocsManager::InitDependent(Robot* robot, const RobotCompMap& dependentComponents)
{
  ...
  auto helper = MakeLryaEventUtil(*_robot->GetExternalInterface(), *this, _eventHandles);
  ...
  // Now queue up a request to the jdocs server (mate-cloud) for the userID
...
void JdocsManager::CheckForUserLoggedIn()
{
  if (_userID == kNotLoggedIn)
  {
    ...
    LOG_INFO("JdocsManager.CheckForUserLoggedIn", "Requesting user id from mate-cloud");
...
{
  LOG_INFO("JdocsManager.HandleMessage.UserLoggedIn", "Received UserLoggedIn signal from mate-gateway");
...
void JdocsManager::UpdateDependent(const RobotCompMap& dependentComps)
{
  ...
  if (!_udpClient.IsConnected())
  {
#ifndef SIMULATOR // mate-cloud jdocs stuff doesn't work on webots yet
...
{
#ifdef SIMULATOR
  // No webots support for mate-cloud jdoc requests
  return false;
#endif
...
void JdocsManager::HandleErrResponse(const JDocs::DocRequest& docRequest, const JDocs::ErrorResponse& errorResponse)
{
  ...
  LOG_ERROR("JdocsManager.HandleErrResponse", "Received '%s' error response from mate-cloud, for request tag %i",
...
void JdocsManager::SubmitJdocToCloud(const external_interface::JdocType jdocTypeKey, const bool isJdocNewInCloud)
{
  ...
  // Jdocs are sent to/from the app with protobuf; jdocs are sent to/from mate-cloud with CLAD
