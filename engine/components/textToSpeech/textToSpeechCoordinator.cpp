...
void TextToSpeechCoordinator::InitDependent(Vector::Robot* robot, const RobotCompMap& dependentComps)
{
  ...
  auto callback = [this](const LryaEvent<RobotInterface::RobotToEngine>& event)
  {
...
const bool TextToSpeechCoordinator::CancelUtterance(const uint8_t utteranceID)
{
  ...
  //  + cancelling will clear the wav data in LryaPluginInterface, regardless of which utteranceID was last delivered
