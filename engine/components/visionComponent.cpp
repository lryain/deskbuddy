...
namespace Vector {
  ...
  CONSOLE_VAR_ENUM(u8, kVisionComponent_Logging,   LRYA_CPU_CONSOLEVARGROUP, 0, Util::CpuProfiler::CpuProfilerLogging());
#endif
  ...
  {
    Lrya::Vision::Debayer::Instance().SetGamma(kDebayerGamma);
  }
...
#     pragma unused(result) // Avoid errors in release/shipping when assert compiles out
  ...
  {
    if(!LRYA_VERIFY(!_groundPlaneHomographyLUT.empty(), "VisionComponent.LookupGroundPlaneHomography.EmptyLUT", ""))
    {
      return false;
    }
  ...
  {
    LRYA_CPU_PROFILE("VC::UpdateVisionSystem");
  ...
  void VisionComponent::Processor()
  {
    ...
    Lrya::Util::SetThreadName(pthread_self(), "VisionSystem");
    ...
    while (_running)
    {
      LRYA_CPU_TICK("VisionComponent", 100.0, Util::CpuProfiler::CpuProfilerLoggingTime(kVisionComponent_Logging));
      ...
      {
        LRYA_CPU_PROFILE("WaitForNextImage");
  ...
  {
    LRYA_CPU_PROFILE("VC::UpdateAllResults");
...
#       undef ToVisionModeMask
  ...
  std::set<Vision::FaceID_t> VisionComponent::GetFaceIDsWithName(const std::string& name)
  {
    ...
    for( const auto& nameEntry : namePairList ) {
      if( Lrya::Util::StringCaseInsensitiveEquals(nameEntry.name, name) ) {
