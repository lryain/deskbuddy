...
#   define DEFINE_HANDLER(__actionTag__, __g2eTag__, __numRetries__) \
    ...
    //
    // For all other messages, just use an LryaEventUtil object:
    //
...
void RobotEventHandler::HandleMessage(const ExternalInterface::DrawPoseMarker& msg)
{
  ...
  else
  {
    if(robot->GetCarryingComponent().IsCarryingObject()) {
      ...
      robot->GetContext()->GetVizManager()->DrawPoseMarker(0, objectFootprint, ::Lrya::NamedColors::GREEN);
    }
  }
}
...
public:
  virtual void Invoke(Robot& robot, const LryaEvent<external_interface::GatewayWrapper>& event) const = 0;
...
{
public:
  using GatewayMessageConverterFcn = std::function<const MessageType(const LryaEvent<external_interface::GatewayWrapper>&)>;
  ...
  virtual void Invoke(Robot& robot, const LryaEvent<external_interface::GatewayWrapper>& event) const
  {
...
#   define ADD_GATEWAY_HANDLER(__gatewayTag__, __requestType__, __extractionFunction__) \
      result[external_interface::GatewayWrapperTag::__gatewayTag__] = \
        std::make_unique<GatewayActionRunner<external_interface::__requestType__> >( \
          []( const LryaEvent<external_interface::GatewayWrapper>& event ) { return event.GetData().__extractionFunction__(); } \
        )
...
template<>
void RobotEventHandler::HandleMessage(const LryaEvent<external_interface::GatewayWrapper>& event)
{
  Robot* robot = _context->GetRobotManager()->GetRobot();
  if (nullptr == robot) {
