...
MessageHandler::~MessageHandler()
{
  #if LRYA_PROFILE_ENGINE_SOCKET_BUFFER_STATS
...
{
  LRYA_CPU_PROFILE("MessageHandler::ProcessMessages");
...
{
  LRYA_CPU_PROFILE("Broadcast_R2E");

  u32 type = static_cast<u32>(message.GetTag());
  _eventMgr.Broadcast(LryaEvent<RobotInterface::RobotToEngine>(BaseStationTimer::getInstance()->GetCurrentTimeInSeconds(), type, message));
}
...
void MessageHandler::Broadcast(RobotInterface::RobotToEngine&& message)
{
  ...
  u32 type = static_cast<u32>(message.GetTag());
  _eventMgr.Broadcast(LryaEvent<RobotInterface::RobotToEngine>(BaseStationTimer::getInstance()->GetCurrentTimeInSeconds(), type, std::move(message)));
}
