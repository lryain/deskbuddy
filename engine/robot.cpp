...
//  Copyright (c) 2013 Lrya, Inc. All rights reserved.
//
...
#define ENABLE_DRAWING LRYA_DEV_CHEATS
...
static void AddAnimation(ConsoleFunctionContextRef context)
{
  if (_thisRobot != nullptr) {
    ...
    if (animFile) {
      const std::string animationFolder = _thisRobot->GetContextDataPlatform()->pathToResource(Lrya::Util::Data::Scope::Resources, "/assets/animations/");
...
{
  LRYA_CPU_PROFILE("Robot::UpdateFullRobotState");
...
Result Robot::Update()
{
  ...
  LRYA_CPU_PROFILE_START(prof_UpdateVis, "UpdateVisualization");
  ...
  */
  ...
  LRYA_CPU_PROFILE_STOP(prof_UpdateVis);
...
void Robot::SetPose(const Pose3d &newPose)
{
  ...
  if (!LRYA_VERIFY(newPose.HasSameRootAs(GetWorldOrigin()),
                               ...
                               ObservableObject* existingObject)
{
  ...
  if (!existingObject->GetPose().HasSameRootAs(origOrigin))
  {
    ...
    if (LRYA_VERIFY(RESULT_OK == result, "Robot.LocalizeToObject.RejiggerFailed", ""))
    {
...
{
  LRYA_CPU_PROFILE("Robot::SetPoseOnCharger");
...
RobotState Robot::GetDefaultRobotState()
{
  ...
  const RobotState state(1, //uint32_t timestamp, (Robot does not report at t=0
                         ...
                         kDefaultPose, //const Lrya::Vector::RobotPose &pose,
                         0.f, //float lwheel_speed_mmps,
                         0.f, //float rwheel_speed_mmps,
                         0.f, //float headAngle
                         0.f, //float liftAngle,
                         AccelData(), //const Lrya::Vector::AccelData &accel,
                         GyroData(), //const Lrya::Vector::GyroData &gyro,
                         std::move(defaultImuDataFrames), // std::array<Lrya::Vector::IMUDataFrame, 6> imuData,
                         ...
                         ProxSensorDataRaw(), //const Lrya::Cozmo::ProxSensorDataRaw &proxData,
...
{
  IDependencyManagedComponent<Lrya::Vector::RobotComponentID>* explicitUpcast = aiComponent;
...
bool Robot::UpdateGyroCalibChecks(Result& res)
{
     ...
     !_syncRobotAcked)
  {
    ...
    const std::string imgPath = GetContextDataPlatform()->pathToResource(Lrya::Util::Data::Scope::Resources,
...
{
  if (!Lrya::Util::Locale::IsValidLocaleString(locale))
  {
  ...
  if (!localeComponent.SetLocale(locale)) {
    ...
    localeComponent.SetLocale(Lrya::Util::Locale::kDefaultLocale.ToString());
  }
