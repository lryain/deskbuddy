  ...
  , _context( robot.GetContext() )
{
  ...
  auto triggerWordCallback = [this]( const LryaEvent<RobotInterface::RobotToEngine>& event ){
  ...
  if( robot.HasExternalInterface() ){
    ...
    auto onEvent = [this](const LryaEvent<MessageGameToEngine>& event) {
...
void UserIntentComponent::SetUserIntentPending(UserIntentTag userIntent, const UserIntentSource& source)
{
  ...
  uint8_t buffer = Lrya::Util::numeric_cast<uint8_t>( userIntent );
  UserIntent intent;
