  ...
  : IDependencyManagedComponent( this, BCComponentID::BehaviorsBootLoader )
{
  ...
  if( LRYA_VERIFY(!config.empty(), "BehaviorsBootLoader.Ctor.InvalidConfig", "Empty config") ) {
...
# endif
  ...
  {
    auto startSelfTestLambda = [this, robot](const LryaEvent<RobotInterface::RobotToEngine>& event)
      {
  ...
  if( ei != nullptr ) {
    ...
    auto onOnboardingStage = [this](const LryaEvent<ExternalInterface::MessageEngineToGame>& onboardingStageEvt){
    ...
    auto selfTestEndLambda = [this](const LryaEvent<ExternalInterface::MessageEngineToGame>& selfTestEnd){
      _selfTestEnded = true;
    };
...
# endif
  ...
  if( !fileContents.empty() && reader.parse( fileContents, onboardingStateJSON ) ) {
    if( LRYA_VERIFY( onboardingStateJSON[BehaviorOnboardingCoordinator::kOnboardingStageKey].isString(), "BehaviorsBootLoader.InitOnboarding.InvalidKey", "" ) )
    {
      const auto& stageStr = onboardingStateJSON[BehaviorOnboardingCoordinator::kOnboardingStageKey].asString();
      LRYA_VERIFY( OnboardingStagesFromString(stageStr, _stage),
...
{
  LRYA_VERIFY( _bootBehavior != nullptr, "BehaviorsBootLoader.GetBootBehavior.BootNotInitialized", "Requested behavior before initialization");
