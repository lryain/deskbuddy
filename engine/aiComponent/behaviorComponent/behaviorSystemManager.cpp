...
void BehaviorSystemManager::UpdateDependent(const BCCompMap& dependentComps)
{
  ...
  auto& bei = dependentComps.GetComponent<BehaviorExternalInterface>();
  LRYA_CPU_PROFILE("BehaviorSystemManager::Update");
...
bool BehaviorSystemManager::Delegate(IBehavior* delegator, IBehavior* delegated)
{
  ...
  if (!LRYA_VERIFY(delegator == _behaviorStack->GetTopOfStack(),
                  "BehaviorSystemManager.Delegate.DelegatorNotOnTopOfStack",
                  "")) {
    return false;
  }

  if (!LRYA_VERIFY(delegated != nullptr,
...

  {
    ...
    if (!LRYA_VERIFY(_behaviorStack->IsValidDelegation(delegator, delegated),
...
{
  if (!LRYA_VERIFY(_behaviorStack->IsInStack(delegator),
  ...
  if (LRYA_VERIFY(!IsControlDelegated(delegator),
