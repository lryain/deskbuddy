...
{
  LRYA_VERIFY(_behaviorStack.empty(),
  ...
  LRYA_VERIFY(baseOfStack->WantsToBeActivated(),
...
{
  if(LRYA_VERIFY(_externalInterface != nullptr,
...
, indexInStack(indexInStack)
{
  if (behavior != nullptr) {
    ...
    for (auto& delegate: delegates) {
      if (LRYA_VERIFY(delegate != nullptr,
                                                                         ...
                                                                         std::set<IBehavior*>& linkedBehaviors)
{
  ...
  BOUNDED_WHILE(1000, !rawBehaviors.empty()){
    ...
    if (res.second) {
      // Lrya dev cheats - make sure no one erases behavior set passed in existing behaviors
      if (LRYA_DEV_CHEATS) {
        std::set<IBehavior*> dupSet = rawBehaviors;
        (*bIter)->GetLinkedActivatableScopeBehaviors(dupSet);
        for (auto& rBehavior : rawBehaviors) {
          LRYA_VERIFY(dupSet.find(rBehavior) != dupSet.end(),
