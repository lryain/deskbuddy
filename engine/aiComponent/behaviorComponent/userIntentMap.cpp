  ...
  , _unmatchedUserIntent{ USER_INTENT(unmatched_intent) }
{
  ...
  LRYA_VERIFY(config[kUserIntentMapKey].size() > 0,
  ...
  auto featureGateOK = [ctx](const Json::Value& jsonGroup, const std::string& intentStr) {
    ...
    if( !featureName.empty() && (ctx != nullptr) ) {
      ...
      LRYA_VERIFY( FeatureTypeFromString( featureName, feature ),
  ...
  for( const auto& mapping : config[kUserIntentMapKey] ) {
    ...
    LRYA_VERIFY( UserIntentTagFromString( userIntentStr, intentTag ),
      ...
      if( !intentName.empty() ) {
        ...
        if( !subs.isNull() ) {
          for( const auto& fromStr : subs.getMemberNames() ) {
            ...
            if( LRYA_VERIFY( !to.isNull(), "UserIntentMap.Ctor.InvalidConfig.MissingSubstitution", "Missing value") ) {
        ...
        LRYA_VERIFY( container.find(intentName) == container.end(),
  ...
  LRYA_VERIFY( UserIntentTagFromString(unmatchedString, _unmatchedUserIntent),
    ...
    for( const auto& varName : paramsList.getMemberNames() ) {
      ...
      if( it != subs.end() ) {
        if( !it->to.empty() ) {
          ...
          if( !LRYA_VERIFY( paramsList[it->to].isNull(),
        ...
        if( it->isNumeric ) {
          ...
          try {
            ...
            if( str.find(".") != std::string::npos ) {
              paramsList[*varNameP] = static_cast<int64_t>(std::stof(str));
            } else {
              paramsList[*varNameP] = static_cast<int64_t>(std::stoll(str));
            }
          } catch( std::exception ) {
