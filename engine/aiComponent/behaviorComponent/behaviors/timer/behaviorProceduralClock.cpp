...
const Vision::SpriteBoxKeyFrame kTensLeftOfColonKeyFrame(
  Vision::SpriteBox(
    ...
    Lrya::Vision::LayerName::Layer_6,
    Lrya::Vision::SpriteRenderMethod::EyeColor,
    0
  ),
  0.0f,
  kClockEmptyGridSpriteID,
  Lrya::Vision::SpriteSeqEndType::Clear,
  0
);
...
{
  if(LRYA_VERIFY(_lifetimeParams.targetFaceID.IsValid(),
...
void BehaviorProceduralClock::DisplayClock()
{
  ...
  if(_instanceParams.shouldPlayAudioOnClockUpdates){
    ...
    for(const auto& triggerTime_ms : _lifetimeParams.audioTickTimes){
      ...
      audioMessage.gameObject = Lrya::AudioMetaData::GameObjectType::Animation;
