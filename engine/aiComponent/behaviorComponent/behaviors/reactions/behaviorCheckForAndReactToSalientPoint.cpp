...
, _dVars(new DynamicVariables)
{
  ...
  if(LRYA_VERIFY(config.isMember(JsonKeys::ReactionsKey),
                 "BehaviorCheckForAndReactToSalientPoint.Constructor.MissingAnimTriggers", ""))
  {
    ...
    if(LRYA_VERIFY(animTriggers.isArray(),
                   "BehaviorCheckForAndReactToSalientPoint.Constructor.AnimTriggersNotArray", ""))
    {
      for(const auto& triggerDefinition : animTriggers)
      {
        ...
        if(LRYA_VERIFY(triggerDefinition.isMember(JsonKeys::SalientPointTypeKey) &&
                       ...
                       "BehaviorCheckForAndReactToSalientPoint.Constructor.BadTriggerDefinition", ""))
        {
          ...
          if(LRYA_VERIFY(Vision::SalientPointTypeFromString(salientPointTypeString, salientPointType),
                         ...
                         "%s", salientPointTypeString.c_str()))
          {
            ...
            if(hasAnimTrigger)
            {
              ...
              LRYA_VERIFY(AnimationTriggerFromString(animString, newEntry.animTrigger),
  ...
  if(LRYA_VERIFY(config.isMember(JsonKeys::VisionModeKey),
                 "BehaviorCheckForAndReactToSalientPoint.Constructor.MissingVisionMode", ""))
  {
    ...
    LRYA_VERIFY(VisionModeFromString(visionModeString, _iConfig->visionMode),
  ...
  if(config.isMember(JsonKeys::WaitAnimTriggerKey))
  {
    ...
    if(LRYA_VERIFY(AnimationTriggerFromString(animString, animTrigger),
...
void BehaviorCheckForAndReactToSalientPoint::InitBehavior()
{
  ...
  for(auto & entryPair : _iConfig->reactions)
  {
    ...
    if(!entry.behaviorString.empty())
    {
      ...
      LRYA_VERIFY(entry.behavior != nullptr,
