...
: Lrya::Vector::ICozmoBehavior(config)
{
  SubscribeToTags({{
  ...
  LRYA_VERIFY(_iConfig.onTreadsTimeCondition != nullptr,
...
void BehaviorConfirmHabitat::TransitionToLocalizeCharger()
{
  ...
  if( LRYA_VERIFY( _iConfig.searchForChargerBehavior->WantsToBeActivated(),
...
void BehaviorConfirmHabitat::TransitionToSeekLineFromCharger()
{
  ...
  const ObservableObject* charger = GetChargerIfObserved();
  if(!LRYA_VERIFY(charger != nullptr,"ConfirmHabitat.TransitionToSeekLineFromCharger.ChargerIsNull","")) {
    _dVars._phase = ConfirmHabitatPhase::RandomWalk;
    return;
  }
