...
void BehaviorInitialHeldInPalmReaction::InitBehavior()
{
  ...
  LRYA_VERIFY( _iConfig.joltInPalmReaction != nullptr, debugStr.c_str(),
  ...
  for(const auto& name : _iConfig.interruptingBehaviorNames){
    ...
    if (LRYA_VERIFY(behavior != nullptr, debugStr.c_str(),
  ...
  LRYA_VERIFY(_iConfig.animSelectorBehavior != nullptr, debugStr.c_str(),
...
void BehaviorInitialHeldInPalmReaction::OnBehaviorActivated() 
{
  ...
  if( !LRYA_VERIFY(_iConfig.animSelectorBehavior->WantsToBeActivated(),
