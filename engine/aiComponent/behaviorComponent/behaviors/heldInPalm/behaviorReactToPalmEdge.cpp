...
BehaviorReactToPalmEdge::InstanceConfig::InstanceConfig(const Json::Value& config, const std::string& debugName)
{
  ...
  LRYA_VERIFY(Util::IsFltGTZero(cliffBackupDist_mm), (debugName + ".InvalidCliffBackupDistance").c_str(),
  ...
  LRYA_VERIFY(Util::IsFltGTZero(cliffBackupSpeed_mmps), (debugName + ".InvalidCliffBackupSpeed").c_str(),
  ...
  if (JsonTools::GetValueOptional(config, kAnimOnActionFailureKey, animOnTurnFailureName)) {
    ...
    LRYA_VERIFY(animOnActionFailure != AnimationTrigger::Count,
...
void BehaviorReactToPalmEdge::TransitionToStuckOnPalmEdge()
{
  ...
  if (LRYA_VERIFY(_iConfig.askForHelpBehavior->WantsToBeActivated(),
