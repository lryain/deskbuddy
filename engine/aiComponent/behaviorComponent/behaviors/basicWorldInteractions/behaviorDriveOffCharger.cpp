...
: ICozmoBehavior(config)
{
  ...
  if( LRYA_VERIFY( config[kDriveDirectionCuesKey].isArray() && (config[kDriveDirectionCuesKey].size() > 0),
                   ...
                   "Direction list is not a valid list") )
  {
    for( const auto& directionStr : config[kDriveDirectionCuesKey] ) {
      if( LRYA_VERIFY( directionStr.isString(), "BehaviorDriveOffCharger.Ctor.InvalidDirStr", "" ) ) {
...
{
  if( LRYA_VERIFY( _iConfig.driveRandomlyBehavior->WantsToBeActivated(),
...
BehaviorDriveOffCharger::DriveDirection BehaviorDriveOffCharger::ParseDriveDirection( const std::string& driveDirectionStr ) const
{
  ...
  LRYA_VERIFY( false, "BehaviorDriveOffCharger.ParseDriveDirection.Unknown", "Unknown direction %s", driveDirectionStr.c_str() );
  return DriveDirection::Straight;
}
