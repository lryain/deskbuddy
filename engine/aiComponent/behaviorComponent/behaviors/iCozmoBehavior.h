...
protected:
  ...
  // found (and performs an LRYA_VERIFY internally)
...
private:
  ...
  std::vector< std::unique_ptr<Lrya::Util::IConsoleVariable> > _tunableParams;
                                                              ...
                                                              std::shared_ptr<T>& outPtr) const
{
  ...
  if( LRYA_VERIFY(behavior != nullptr,
     ...
     LRYA_VERIFY(behavior->GetClass() == requiredClass,
       ...
       if( LRYA_VERIFY(outPtr != nullptr, "ICozmoBehavior.FindAnonymousBehaviorByNameAndDowncast.CastFailed",
                       "BehaviorName: %s requiredClass: %s",
                       behaviorName.c_str(),
                       GetClassString(requiredClass).c_str()) ) {
         return true;
...
void ICozmoBehavior::MakeMemberTunable(T& param, const std::string& name, const char* category)
{
  ...
  Lrya::Util::StringReplace( uniqueName, "@", "" );
  ...
  for( const auto& var : _tunableParams ) {
    if( !LRYA_VERIFY( var->GetID() != uniqueName,
