 ...
 * Created: 2018-02-27
 *
 * Description: deskmate plays the game snake. his solver is greedy, so it eventually fails
 *
...
#include "engine/aiComponent/behaviorComponent/behaviors/deskmate/snakeGame.h"
#include "engine/aiComponent/behaviorComponent/behaviors/deskmate/snakeGameSolver.h"
...
{
  if( !LRYA_VERIFY(_dVars.game != nullptr,
                   "",
                   "") )
  {
    return;
  ...
  if( !LRYA_VERIFY( (width == kScalingFactor*(2+game.GetWidth())) && (height == kScalingFactor*(2+game.GetHeight())) ,
                    "",
                    "" ) )
  {
    return;
...
void BehaviorSnakeGame::AnimateDirection( uint8_t directionInt )
{
  ...
  {
    float degrees = 0.0f;
    // note: this is inverted... screen right is deskmate left
    if( direction == SnakeGame::Direction::LEFT ) {
      degrees = -10.f;
    } else {
      degrees = 10.f;
