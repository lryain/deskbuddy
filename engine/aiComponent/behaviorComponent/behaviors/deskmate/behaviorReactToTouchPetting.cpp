...
, _isPressedPrevTick(false)
{
  ...
  LRYA_VERIFY(JsonTools::GetVectorOptional(config, kAnimGroupNamesKey, tmp),"BehaviorReactToTouchPetting.Ctor.MissingVectorOfAnimTriggers","");
  for(const auto& t : tmp) {
    _animPettingResponse.push_back( AnimationTriggerFromString(t) );
  }
  
  tmp.clear();
  LRYA_VERIFY(JsonTools::GetVectorOptional(config, kAnimGroupNamesGetoutKey, tmp),"BehaviorReactToTouchPetting.Ctor.MissingVectorOfAnimTriggersGetout","");
  for(const auto& t : tmp) {
