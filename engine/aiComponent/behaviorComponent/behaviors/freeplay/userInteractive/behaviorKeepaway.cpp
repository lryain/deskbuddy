...
// Distance past nominalPounceDist that we allow deskmate to creep. Helps ensure he eventually pounces.
...
void BehaviorKeepaway::BehaviorUpdate()
{
  ...
  if(KeepawayState::Searching == _dVars.state){
    ...
    if(!_dVars.deskmateGotLastPoint && PitchIndicatesPounceSuccess()){
      _dVars.deskmateGotLastPoint = true;
    }
  }
}
...
void BehaviorKeepaway::TransitionToPouncing()
{
  ...
  _dVars.deskmateGotLastPoint = false;
...
void BehaviorKeepaway::TransitionToReacting()
{
  ...
  float probExit = 0.0f;
  if(_dVars.deskmateGotLastPoint){
    ...
    _dVars.frustrationExcitementScale = Lrya::Util::Min(newVal, max);
    probExit = Lrya::Util::Max(0.0f, _dVars.frustrationExcitementScale - 1.0f);
    ...
    _dVars.frustrationExcitementScale = Lrya::Util::Max( newVal, min);
    probExit = Lrya::Util::Max(0.0f, 1.0f - _dVars.frustrationExcitementScale);
  ...
  if(exitRoll < probExit){
    ...
    AnimationTrigger winResponseAnim = _dVars.deskmateGotLastPoint ?
    ...
    if(_dVars.deskmateGotLastPoint){
      if(GetRNG().RandDblInRange(_iConfig.minProbToReact, 1.0f) < _dVars.probReactToHit){
        ...
        _dVars.probReactToHit = Lrya::Util::Min(_dVars.probReactToHit + _iConfig.probReactIncrement,
      ...
      if(GetRNG().RandDblInRange(_iConfig.minProbToReact, 1.0f) < _dVars.probReactToMiss){
        ...
        _dVars.probReactToMiss = Lrya::Util::Min(_dVars.probReactToMiss + _iConfig.probReactIncrement,
                                                 _iConfig.probReactMax);
        DelegateIfInControl(new TriggerAnimationAction(AnimationTrigger::CubePounceGetReady),
                            &BehaviorKeepaway::TransitionToStalking);
      }
