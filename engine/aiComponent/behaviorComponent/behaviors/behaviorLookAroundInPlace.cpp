...
void BehaviorLookAroundInPlace::LoadConfig(const Json::Value& config)
{
  ...
  if( config.isMember("motionProfile") ) {
    ...
    LRYA_VERIFY( !config.isMember("sx_BodyTurnSpeed_degPerSec"),
                 debugName.c_str(),
                 "Using motion profile, shouldn't set body turn speed in JSON" );
    LRYA_VERIFY( !config.isMember("sxt_HeadTurnSpeed_degPerSec"),
                 debugName.c_str(),
                 "Using motion profile, shouldn't set sxt head turn speed in JSON" );
    LRYA_VERIFY( !config.isMember("sxh_HeadTurnSpeed_degPerSec"),
                 debugName.c_str(),
  ...
  if( kVizConeOfFocus ) {
    ...
    if( hasConeOfFocus ) {
      ...
      robotInfo.GetContext()->GetVizManager()->DrawSegment("BehaviorLookInPlace.FocusCone",
                                                           center, coneCenter,
                                                           Lrya::NamedColors::WHITE, false, zOffset_mm);
      robotInfo.GetContext()->GetVizManager()->DrawSegment("BehaviorLookInPlace.FocusCone",
                                                           center, coneLeft,
                                                           Lrya::NamedColors::YELLOW, false, zOffset_mm);
      robotInfo.GetContext()->GetVizManager()->DrawSegment("BehaviorLookInPlace.FocusCone",
                                                           center, coneRight,
                                                           Lrya::NamedColors::YELLOW, false, zOffset_mm);
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
IAction* BehaviorLookAroundInPlace::CreatePauseAction(const Lrya::Vector::AnimationTrigger &trigger,
