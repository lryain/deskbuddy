 ...
 * Created: 2017-10-16
 *
 * Description: Root behavior to handle the state machine for the high level AI of deskmate (similar to Cozmo's
 *              freeplay activities)
 *
  ...
  : InternalStatesBehavior( config, CreateCustomConditions() )
{
  ...
  if( config[kPostBehaviorSuggestionResumeOverrides].isObject() ) {
    ...
    for( const auto& suggestionName : resumeOverrides.getMemberNames() ) {
      ...
      if( LRYA_VERIFY( PostBehaviorSuggestionsFromString( suggestionName, suggestion ),
                       "BehaviorHighLevelAI.Ctor.PBSInvalid",
                       "Post behavior suggestion '%s' not valid",
                       suggestionName.c_str() )
         && LRYA_VERIFY( suggestion != PostBehaviorSuggestions::Invalid,
                         ...
                         "The 'Invalid' clad value was specified" ) )
      {
        ...
        if( LRYA_VERIFY( state != InvalidStateID,
