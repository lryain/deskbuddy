 ...
 : ICozmoBehavior(config)
{
  ...
  if( LRYA_VERIFY(!responseData.isNull(), "BehaviorAlexa.Ctor.MissingResponseData", "Could not find key 'responseData'" ) ) {
...
void BehaviorAlexa::LoadConfig( const Json::Value& data )
{
  ...
  for( const auto& uxStateStr : data.getMemberNames() ) {
    ...
    if( LRYA_VERIFY( AlexaUXStateFromString( uxStateStr, uxState),
                     ...
                     uxStateStr.c_str() ) )
    {
      ...
      if( LRYA_VERIFY( !stateInfo["transitions"].isNull(),
                        ...
                        uxStateStr.c_str() ) )
      {
        ...
        for( const auto& nextUXStateStr : transitions.getMemberNames() ) {
          ...
          if( LRYA_VERIFY( AlexaUXStateFromString( nextUXStateStr, nextUXState),
                          "BehaviorAlexa.LoadConfig.InvalidNextUXState",
                          "'%s' is not a valid AlexaUXState",
                          nextUXStateStr.c_str() ) )
          {
            if( LRYA_VERIFY( transitions[nextUXStateStr].isString(),
                             ...
                             nextUXStateStr.c_str() ) )
            {
              ...
              if( LRYA_VERIFY( AnimationTriggerFromString( transAnim, transTrigger ),
      ...
      // load loops
      if( LRYA_VERIFY( stateInfo["loop"].isString(),
                      ...
                      uxStateStr.c_str() ) )
      {
        ...
        if( LRYA_VERIFY( AnimationTriggerFromString( loopAnim, loopTrigger ),
