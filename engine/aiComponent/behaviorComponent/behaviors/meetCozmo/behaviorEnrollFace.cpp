...
, _dVars(new DynamicVariables)
{
  ...
  if( config.isMember(kFaceSelectionPenaltiesKey) ) {
    ...
    LRYA_VERIFY(parsedOK, "BehaviorEnrollFace.InvalidFaceSelectionConfig",
...
void BehaviorEnrollFace::CheckForIntentData()
{
  ...
  if( uic.IsUserIntentPending( USER_INTENT(meet_deskmate) ) ) {
    // activate the intent
    if( AreScanningLightsEnabled() ) {
      uic.ActivateUserIntent( USER_INTENT(meet_deskmate), GetDebugLabel(), false );
    }
    else {
      // let our config decide what to do with intent feedback
      ActivateUserIntentHelper( USER_INTENT(meet_deskmate) );
    }
  }

  UserIntentPtr intentData = uic.GetUserIntentIfActive(USER_INTENT(meet_deskmate));
  if( intentData != nullptr ) {
    const auto& meetVictor = intentData->intent.Get_meet_deskmate();
...
bool BehaviorEnrollFace::WantsToBeActivatedBehavior() const
{
  auto& uic = GetBehaviorComp<UserIntentComponent>();
  UserIntent intent;
  const bool pendingIntent = uic.IsUserIntentPending(USER_INTENT(meet_deskmate), intent);
  if(pendingIntent)
  {
    const bool intentHasName = !intent.Get_meet_deskmate().username.empty();
    if(intentHasName)
    {
...
Result BehaviorEnrollFace::InitEnrollmentSettings()
{
  ...
  else
  {
    ...
    if(!_dVars->enrollingSpecificID
       && LRYA_VERIFY(!_dVars->persistent.isManualReEnroll,
                      "BehaviorEnrollFace.InitEnrollmentSettings.ManualReEnrollWithNoSaveID", ""))
    {
      ...
      {
        if(LRYA_VERIFY(faceIDsWithName.size() == 1,
  ...
  switch(_dVars->persistent.state)
  {
    ...
    case State::LookingForFace:
    {
      ...
      if(IsSeeingWrongFace(wrongID, wrongName, maxScore))
      {
        ...
        const bool wrongFaceExists = LRYA_VERIFY(face != nullptr,
        ...
        else
        {
          ...
          DASMSG(behavior_meet_deskmate_force_enrolling_named_face, "behavior.meet_deskmate.force_enrolling_named_face",
    ...
    case State::Enrolling:
    {
      ...
      if(GetBEI().GetFaceWorld().IsFaceEnrollmentComplete())
      {
        ...
        {
          // Sanity checks
          LRYA_VERIFY(!_dVars->faceName.empty(), "BehaviorEnrollFace.BehaviorUpdate.AboutToAssignWithEmptyName", "");
          ...
          LRYA_VERIFY( (isReenrollment && idsWithName.size()==1) || (!isReenrollment && idsWithName.empty()),
      ...
      else
      {
        ...
        if(lostEnrollee)
        {
          DASMSG(behavior_meet_deskmate_lost_enrollee, "behavior.meet_deskmate.lost_enrollee",
      ...
      if( finishedScanning )
      {
        ...
        // das msg
        {
          ...
          DASMSG(behavior_meet_deskmate_scan_end, "behavior.meet_deskmate.scan_end", "Face scanning ended in meet deskmate");
          ...
          DASMSG(behavior_meet_deskmate_scan_faces, "behavior.meet_deskmate.scan_faces", "Info about # of faces seen when scanning, sent at scan_end");
...
void BehaviorEnrollFace::OnBehaviorDeactivated()
{
  ...
  auto& uic = GetBehaviorComp<UserIntentComponent>();
  if( uic.IsUserIntentActive( USER_INTENT(meet_deskmate) ) ) {
    DeactivateUserIntentHelper( USER_INTENT(meet_deskmate) );
  }
  ...
  {
    DASMSG(behavior_meet_deskmate_end, "behavior.meet_deskmate.end", "Meet deskmate completed");
    DASMSG_SET(s1,
...
bool BehaviorEnrollFace::HasTimedOut() const
{
  ...
  if(hasTimedOut)
  {
    DASMSG(behavior_meet_deskmate_has_timed_out, "behavior.meet_deskmate.has_timed_out",
           "We reached the global timeout for meet deskmate");
...
void BehaviorEnrollFace::TransitionToAlreadyKnowYouPrompt()
{
  ...
  LRYA_VERIFY(_dVars->faceID != Vision::UnknownFaceID,
  ...
  LRYA_VERIFY(_iConfig->alreadyKnowYouPromptBehavior->WantsToBeActivated(),
...
void BehaviorEnrollFace::TransitionToAlreadyKnowYouHandler()
{
  ...
  if(uic.IsUserIntentPending(kAffirmativeIntent))
  {
    ...
    LRYA_VERIFY(_dVars->saveID != Vision::UnknownFaceID,
...
void BehaviorEnrollFace::TransitionToStartEnrollment()
{
  ...
  {
    DASMSG(behavior_meet_deskmate_scan_start, "behavior.meet_deskmate.scan_start", "Face scanning started in meet deskmate");
    DASMSG_SEND();
  }
...
void BehaviorEnrollFace::TransitionToSayingIKnowThatName()
{
  ...
  if(_dVars->sayName) {
    ...
    DelegateIfInControl(actConfusedAnim, [this](ActionResult result) {
      ...
      LRYA_VERIFY( _iConfig->ttsBehavior->WantsToBeActivated(), "BehaviorEnrollFace.TransitionToSayingIKnowThatName.NoTTS","");
...
void BehaviorEnrollFace::TransitionToWrongFace( FaceID_t faceID, const std::string& faceName )
{
  ...
  DASMSG(behavior_meet_deskmate_wrong_face, "behavior.meet_deskmate.wrong_face",
  ...
  switch(static_cast<SayWrongNameMode>(kEnrollFace_SayWrongNameMode))
  {
    ...
    case SayWrongNameMode::Long:
      if((faceName == _dVars->faceName)               // "wrong" name matches enrolling name
         ...
         && LRYA_VERIFY( faceID != _dVars->saveID,    // saveID should not also match "wrong" ID
  ...
  DelegateIfInControl(action, [this,text=std::move(text),faceName,faceID](ActionResult result) {
    ...
    if(LRYA_VERIFY(it != _dVars->persistent.wrongFaceStats.end(), "BehaviorEnrollFace.TransitionToWrongFace.MissingStats",
    ...
    else
    {
      ...
      LRYA_VERIFY( _iConfig->ttsBehavior->WantsToBeActivated(), "BehaviorEnrollFace.TransitionToWrongFace.NoTTS","");
      DelegateIfInControl(_iConfig->ttsBehavior.get(), [this]() {
        if(kEnrollFace_FailOnWrongFace)
        {
                                                               ...
                                                               bool playScanningGetOut)
{
  ...
  if(GetBEI().GetFaceWorld().HasAnyFaces())
  {
    ...
    else
    {
      ...
      if(!LRYA_VERIFY(nullptr != face, "BehaviorEnrollFace.CreateTurnTowardsFaceAction.NullBestFace",
