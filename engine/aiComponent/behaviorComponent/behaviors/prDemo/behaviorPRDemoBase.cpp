...
void BehaviorPRDemoBase::InitBehavior()
{
  ...
  LRYA_VERIFY(_iConfig.sleepingBehavior != nullptr, "BehaviorPRDemoBase.CouldntGetSleepingBehavior", "");

  _iConfig.wakeUpBehavior = BC.FindBehaviorByID(BEHAVIOR_ID(SleepingWakeUp));
  LRYA_VERIFY(_iConfig.wakeUpBehavior != nullptr, "BehaviorPRDemoBase.CouldntGetWakeupBehavior", "");

  // TODO:(bn) at least read this last one from config?
  _iConfig.mainBehavior = BC.FindBehaviorByID(BEHAVIOR_ID(ModeSelector));
  LRYA_VERIFY(_iConfig.mainBehavior != nullptr, "BehaviorPRDemoBase.CouldntGetMainBehavior", "");
}
