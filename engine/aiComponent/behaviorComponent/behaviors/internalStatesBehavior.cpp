...
// NOTE: current deskmate lights are in a wonky order
  ...
  , _currDebugLights(kLightsOff)
{
  ...
  if( LRYA_VERIFY( stateIt != _stateNameToID.end(),
...
{
  if( LRYA_VERIFY( GetStateID(state._name) != InvalidStateID,
                   "InternalStatesBehavior.AddState.InvalidName",
                   "State has invalid name '%s'",
                   state._name.c_str()) &&
      LRYA_VERIFY( _states->find(GetStateID(state._name)) == _states->end(),
...
void InternalStatesBehavior::OnBehaviorActivated()
{
  ...
  if( LRYA_VERIFY( _currState != InvalidStateID,
...
InternalStatesBehavior::State::State(const Json::Value& config)
{
  ...
  LRYA_VERIFY(cancelSelf != !_behaviorName.empty(),
  ...
  if( !activateIntent.empty() ) {
    LRYA_VERIFY( UserIntentTagFromString( activateIntent, _activateIntent ),
...
void InternalStatesBehavior::State::OnActivated(BehaviorExternalInterface& bei, bool isResuming)
{
  ...
  if( !isResuming || adjustedTimeNeverSet ) {
    ...
    if( LRYA_VERIFY( pausedFor_s >= 0.0f,
...
{
  if( LRYA_VERIFY( _stateNameToID.find(stateName) == _stateNameToID.end(),
...
void InternalStatesBehavior::AddConsoleVarTransitions( const char* uniqueVarName, const char* category )
{
  if( LRYA_DEV_CHEATS ) {
    ...
    auto* cfunc = new Lrya::Util::IConsoleFunction( uniqueVarName, std::move(func), category, "const char* stateName" );
...
InternalStatesBehavior::StateID InternalStatesBehavior::GetStateID(const std::string& stateName) const
{
  ...
  if( LRYA_VERIFY( it != _stateNameToID.end(),
...
{
  if( LRYA_VERIFY( config[key].isString(),
...
IBEIConditionPtr InternalStatesBehavior::ParseTransitionStrategy(const Json::Value& transitionConfig)
{
  ...
  if( LRYA_VERIFY( strategyConfig.isObject(), "InternalStatesBehavior.ParseTransitionStrategy.NoCondition",
