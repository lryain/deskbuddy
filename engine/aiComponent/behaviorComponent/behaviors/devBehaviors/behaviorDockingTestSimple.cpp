...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void BehaviorDockingTestSimple::HandleDockingStatus(const LryaEvent<RobotInterface::RobotToEngine>& message)
{
  const DockingStatus& payload = message.GetData().Get_dockingStatus();
  switch(payload.status)
  {
    case(Lrya::Vector::Status::STATUS_BACKING_UP):
    {
      _dVars.numDockingRetries++;
      break;
    }
    case(Lrya::Vector::Status::STATUS_DOING_HANNS_MANEUVER):
    {
      _dVars.didHM = true;
      break;
    }
