...
: ICozmoBehavior(config)
{
  ...
  if( !config[kTracksToLock].isNull() ) {
    for( const auto& trackStr : config[kTracksToLock] ) {
      if( LRYA_VERIFY(trackStr.isString(), "BehaviorAnimSequence.Ctor.TrackNotString", "Must be array of track strings" ) ) {
        AnimTrackFlag flag = AnimTrackFlag::NO_TRACKS;
        if( LRYA_VERIFY( AnimTrackFlagFromString(trackStr.asString(), flag),
