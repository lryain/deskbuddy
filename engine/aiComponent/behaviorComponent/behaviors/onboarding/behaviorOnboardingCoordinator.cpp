...
void BehaviorOnboardingCoordinator::SaveToDisk(OnboardingStages stage) const
{
  ...
  if (stage == OnboardingStages::Complete) {
    ...
    // TODO: error: conversion from 'std::__1::chrono::duration<long long, std::__1::ratio<1, 1> >::rep' (aka 'long long') to 'Json::Value' is ambiguous [Semantic Issue]
    toSave[kOnboardingTimeKey] =  static_cast<int64_t>(std::chrono::duration_cast<std::chrono::seconds>(tse).count()); // write out as seconds since the epoch
  }
  const std::string filename = _iConfig.saveFolder + kOnboardingFilename;
