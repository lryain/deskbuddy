 ...
 : ICozmoBehavior(config)
{
  ...
  LRYA_VERIFY( RecentOccurrenceTracker::ParseConfig(config, _iConfig.numberOfTimes, _iConfig.amountOfSeconds),
...
{
  if( LRYA_VERIFY( _iConfig.reason != AttentionTransferReason::Invalid,
...
bool BehaviorAttentionTransferIfNeeded::WantsToBeActivatedBehavior() const
{
  ...
  const bool featureEnabled = featureGate->IsFeatureEnabled(Lrya::Vector::FeatureType::AttentionTransfer);
