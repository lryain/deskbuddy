  ...
  : ICozmoBehavior(config)
{
  ...
  // passes in
  LRYA_VERIFY(config.get(kInterruptBehaviorKey, shouldInterruptActiveBehavior).asBool() == shouldInterruptActiveBehavior,
...
void IBehaviorDispatcher::BehaviorUpdate()
{
  ...
  if( ! LRYA_VERIFY( GetBEI().HasDelegationComponent(),
