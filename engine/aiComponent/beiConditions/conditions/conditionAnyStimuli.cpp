  ...
  : IBEICondition(config)
{
  ...
  LRYA_VERIFY( !_conditions.empty(), "ConditionAnyStimuli.Ctor.NoStimuli", "At least one type must be true");
  LRYA_VERIFY( config.getMemberNames().size() == 1 + kSupportedTypes.size(),
  ...
  : IBEICondition( IBEICondition::GenerateBaseConditionConfig(BEIConditionType::AnyStimuli) )
{
  for( const auto& type : types ) {
    if( LRYA_VERIFY( std::find(kSupportedTypes.begin(), kSupportedTypes.end(), type) != kSupportedTypes.end(),
