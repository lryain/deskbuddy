  ...
  , _type( type )
{
  ...
  LRYA_VERIFY( (_type == BEIConditionType::UserIntentPending) || (_type == BEIConditionType::UserIntentActive),
...
void IConditionUserIntent::AddUserIntent( const Json::Value& json )
{
  ...
  if( LRYA_VERIFY( intent.SetFromJSON( json ),
...
{
  const bool validFunc = LRYA_VERIFY( func != nullptr,
  ...
  const bool notInvalid = LRYA_VERIFY( tag != USER_INTENT(INVALID),
