...
#include "lib/util/source/lrya/util/logging/logging.h"
  ...
  : IBEICondition( config )
{
  ...
  LRYA_VERIFY( maxDepth <= 2, "ConditionCompound.Ctor.MaxDepthExceeded", "Depth=%d", maxDepth );
}
...
int ConditionCompound::CreateNode( std::unique_ptr<Node>& node, const Json::Value& config, const bool isRoot = false )
{
  ...
  if( !isRoot && BEIConditionFactory::IsValidCondition(config) ) { // this is a leaf
    ...
    LRYA_VERIFY( !childConfig.isArray(), "ConditionCompound.CreateNode.TooMany", "Too many children for a NOT condition" );
    ...
    LRYA_VERIFY( childrenConfig->isArray() && (childrenConfig->size() > 1),
...
bool ConditionCompound::EvaluateNode( const std::unique_ptr<Node>& node, const std::vector<bool>& conditionValues ) const
{
  ...
  switch( node->type ) {
    ...
    {
      if( LRYA_VERIFY( node->conditionIndex < conditionValues.size(),
