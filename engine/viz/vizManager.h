  ...
  enum class MessageVizTag : uint8_t;
    ...
    public:
      ...
      Handle_t DrawRobot(const u32 robotID,
                         const Pose3d &pose,
                         const ColorRGBA& color = ::Lrya::NamedColors::DEFAULT);
      ...
      // Declaration for message handling specializations. See LryaEventUtil.h
      template <typename T>
      ...
      const Point<3,T>& from, const Point<3,T>& to, const ColorRGBA& color, bool clearPrevious, float zOffset)
    {
      SendMessage(VizInterface::MessageViz(VizInterface::LineSegment
        {identifier,
         color.AsRGBA(),
         { {Lrya::Util::numeric_cast<float>(MM_TO_M(from.x())),
            Lrya::Util::numeric_cast<float>(MM_TO_M(from.y())),
            Lrya::Util::numeric_cast<float>(MM_TO_M(from.z()+zOffset))}
         },
         { {Lrya::Util::numeric_cast<float>(MM_TO_M(to.x())),
            Lrya::Util::numeric_cast<float>(MM_TO_M(to.y())),
            Lrya::Util::numeric_cast<float>(MM_TO_M(to.z()+zOffset))}
         },
         clearPrevious
        })
      );
