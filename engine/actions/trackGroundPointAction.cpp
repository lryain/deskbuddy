...
void TrackGroundPointAction::GetRequiredVisionModes(std::set<VisionModeRequest>& requests) const
{
  switch(_salientTag)
  {
    ...
    default:
      LRYA_VERIFY(false, "TrackGroundPointAction.Constructor.NoVisionModeForTag",
...
ActionResult TrackGroundPointAction::InitInternal()
{
  ...
  // Subscribe to the right message based on the saliency tag specified at construction
  {
    ...
    auto handler = [this](const LryaEvent<ExternalInterface::MessageEngineToGame>& event)
    {
                                                                  ...
                                                                  const Point2f& observedGroundPt)
{
  ...
  LRYA_VERIFY(success, "TrackGroundPointAction.GetGroundWrtCurrentRobot.GetWrtFailed", "");
