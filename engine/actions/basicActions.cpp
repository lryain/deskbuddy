    ...
    ActionResult TurnInPlaceAction::Init()
    {
      ...
      if(!_inPosition) {
        ...
        if(_moveEyes)
        {
          ...
          angleDiff_rad = Lrya::Util::Clamp(angleDiff_rad, DEG_TO_RAD(-89.f), DEG_TO_RAD(89.f));
      ...
      auto actionStartedLambda = [this](const LryaEvent<RobotInterface::RobotToEngine>& event)
      {
...
#pragma mark ---- SearchForNearbyObjectAction ----
    ...
    void SearchForNearbyObjectAction::OnRobotSet()
    {
      if(GetRobot().HasExternalInterface()){
        ...
        [this](const LryaEvent<ExternalInterface::MessageEngineToGame>& event){
...
#pragma mark ---- DriveStraightAction ----
    ...
    {
      LRYA_VERIFY(!HasStarted(), "DriveStraightAction.SetCanMoveOnCharger.ActionAlreadyStarted", "[%d]", GetTag());
...
#pragma mark ---- CalibrateMotorAction ----
    ...
    ActionResult CalibrateMotorAction::Init()
    {
      ...
      {
        using namespace ExternalInterface;
        auto helper = MakeLryaEventUtil(*(GetRobot().GetExternalInterface()), *this, _signalHandles);
...
#pragma mark ---- WaitForImagesAction ----
    ...
    WaitForImagesAction::~WaitForImagesAction()
    {
      ...
      if(LRYA_DEV_CHEATS && nullptr != _saveParams)
      {
    ...
    ActionResult WaitForImagesAction::Init()
    {
      ...
      auto imageProcLambda = [this](const LryaEvent<ExternalInterface::MessageEngineToGame>& msg)
      {
