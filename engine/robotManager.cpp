...
void RobotManager::Init(const Json::Value& config)
{
  ...
  Lrya::Util::Time::PushTimedStep("RobotManager::Init");
  _robotMessageHandler->Init(config, this, _context);
  Lrya::Util::Time::PopTimedStep(); // RobotManager::Init

  Lrya::Util::Time::PrintTimedSteps();
  Lrya::Util::Time::ClearSteps();
...
void RobotManager::Shutdown(ShutdownReason reason)
{
  ...
  if(_robot != nullptr)
  {
    ...
    if(reason != ShutdownReason::SHUTDOWN_UNKNOWN)
    {
      ...
      // Send fault code
      // Fault code handler will kill mate-dasMgr and do other stuff as necessary
...
void RobotManager::RemoveRobot(bool robotRejectedConnection)
{
  if(_robot != nullptr) {
    ...
    Lrya::Util::sSetGlobal(DPHYS, nullptr);
    Lrya::Util::sSetGlobal(DGROUP, nullptr);
  } else {
...
{
  LRYA_CPU_PROFILE("RobotManager::UpdateRobot");
...
{
  LRYA_CPU_PROFILE("RobotManager::UpdateRobotConnection");
