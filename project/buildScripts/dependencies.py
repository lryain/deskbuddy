...
# These are the Lrya modules/packages:
...
import lryabuild.util
import lryabuild.deptool
...
ASSET_VALIDATION_TRIGGERS = ["deskmate-animation-assets", "deskmate-audio-assets"]

def get_lrya_deps_cache_directory():
   lrya_deps_cache_dir = os.path.join(os.path.expanduser("~"), ".lrya", "deps-cache")
   lryabuild.util.File.mkdir_p(lrya_deps_cache_dir)
   return lrya_deps_cache_dir


def get_lrya_sha256_cache_directory():
   lrya_sha256_cache_dir = os.path.join(get_lrya_deps_cache_directory(), "sha256")
   lryabuild.util.File.mkdir_p(lrya_sha256_cache_dir)
   return lrya_sha256_cache_dir


def get_lrya_svn_cache_directory(url = None):
   lrya_svn_cache_dir = os.path.join(get_lrya_deps_cache_directory(), "svn")
   if url:
      url = url.replace('/', '-')
      lrya_svn_cache_dir = os.path.join(lrya_svn_cache_dir, url)
   lryabuild.util.File.mkdir_p(lrya_svn_cache_dir)
   return lrya_svn_cache_dir

def get_lrya_file_cache_directory(url = None):
   lrya_file_cache_dir = os.path.join(get_lrya_deps_cache_directory(), "files")
   if url:
      url = url.replace('/', '-')
      lrya_file_cache_dir = os.path.join(lrya_file_cache_dir, url)
   lryabuild.util.File.mkdir_p(lrya_file_cache_dir)
   return lrya_file_cache_dir

def get_lrya_svn_cache_tarball(url, rev):
   lrya_svn_cache_dir = get_lrya_svn_cache_directory(url)
   tarball = os.path.join(lrya_svn_cache_dir, "r" + str(rev) + ".tgz")
   return tarball

def extract_svn_cache_tarball_to_directory(url, rev, loc):
   tarball = get_lrya_svn_cache_tarball(url, rev)
   if not os.path.isfile(tarball):
      return False
   lryabuild.util.File.rm_rf(loc)
   lryabuild.util.File.mkdir_p(loc)
   ptool = "tar"
...
def save_svn_cache_tarball_from_directory(url, rev, loc, cred):
   ...
   lryabuild.util.File.rm_rf(tarball)
   lryabuild.util.File.rm_rf(tmp_tarball)
...
def svn_package(svn_dict):
    ...
    # Old code below: Retrieves and extracts SVN assets using Lrya servers
    """
...
def files_package(files):
    ...
    for file in files:
        ...
        if sha256 != download_hash:
           lryabuild.util.File.rm_rf(tmpfile)
...
def deptool_package(deptool_dict):
   ...
   for dep in deps:
      ...
      if lryabuild.deptool.is_valid_dep_dir_for_version_and_sha256(dst, required_version, sha256_checksum):
         continue
      if os.path.islink(dst):
         os.unlink(dst)
      else:
         lryabuild.util.File.rm_rf(dst)
      src = lryabuild.deptool.find_or_install_dep(project, dep, required_version, url_prefix, sha256_checksum)
      if not src:
...
def teamcity_package(tc_dict):
    cache_dir = get_lrya_sha256_cache_directory()
    downloaded_builds = []
    teamcity=True
    tool = "curl"
    ptool = "tar"
    ...
    for build in builds:
        ...
        lryabuild.util.File.rm_rf(unpack_path)
        lryabuild.util.File.mkdir_p(unpack_path)
        ...
        if sha:
           ...
           lryabuild.util.File.rm_rf(dist)
...
def extract_dependencies(version_file, location=EXTERNALS_DIR, validate_assets=True):
    ...
    lryabuild.util.File.mkdir_p(location)
