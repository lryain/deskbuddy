...
pushd ${TOPLEVEL}/_build/mateos/$1/bin > /dev/null
for i in `ls *.full`; do
    i=${i%.*}
    echo Processing ${i}...
    ...
    mkdir -p ${TOPLEVEL}/_build/mateos/$1/symbols/$i/$VER
    mv $i.sym ${TOPLEVEL}/_build/mateos/$1/symbols/$i/$VER/
done
popd > /dev/null # bin

echo "Making symbols for the lib folder"
pushd ${TOPLEVEL}/_build/mateos/$1/lib > /dev/null
for i in `ls *.full`; do
    i=${i%.*}
    echo Processing ${i}...
    ${TOPLEVEL}/tools/crash-tools/osx/dump_syms $i.full . > $i.sym || ${TOPLEVEL}/tools/crash-tools/osx/dump_syms $i > $i.sym
