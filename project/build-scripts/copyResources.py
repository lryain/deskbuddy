...
class CopyResources(object):
  ...
  def clean(self):
    if os.path.isdir(self.options.destinationPath):
      lryabuild.util.File.rm_rf(self.options.destinationPath)
  ...
  def copy(self):
    ...
    if not lryabuild.util.File.cptree(unityAssetsPath, destinationPath):
    ...
    if os.path.isdir(CladEnumToStringMapsPath):
      lryabuild.util.File.rm_rf(CladEnumToStringMapsPath)
    if not lryabuild.util.File.cptree(self.options.CladEnumToStringMapsPath, CladEnumToStringMapsPath):
    ...
    if os.path.isdir(rewardedActionsPath):
      lryabuild.util.File.rm_rf(rewardedActionsPath)
    if not lryabuild.util.File.cptree(self.options.rewardedActionsPath, rewardedActionsPath):
   ...
   compositeImageResourcePath = os.path.join(cozmoResourcesPath, 'assets/compositeImageResources')
    if os.path.isdir(compositeImageResourcePath):
      lryabuild.util.File.rm_rf(compositeImageResourcePath)
    if not lryabuild.util.File.cptree(self.options.compositeImageResourcePath, compositeImageResourcePath):
    ...
    if os.path.isdir(engineResourcesPath):
      lryabuild.util.File.rm_rf(engineResourcesPath)
    if not lryabuild.util.File.cptree(self.options.engineResourcesPath, engineResourcesPath):
    ...
    if os.path.isdir(ttsResourcesPath):
      lryabuild.util.File.rm_rf(ttsResourcesPath)
    if not lryabuild.util.File.cptree(self.options.ttsResourcesPath, ttsResourcesPath):
    ...
    if self.options.soundBanksPath:
      ...
      if os.path.isdir(soundBanksPath):
        lryabuild.util.File.rm_rf(soundBanksPath)
      if not lryabuild.util.File.cptree(os.path.join(self.options.soundBanksPath, soundBanksPlatform), soundBanksPath):
  ...
  def rsync(self):
    ...
    lryabuild.util.File.execute(args)
