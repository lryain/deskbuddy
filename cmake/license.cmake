...
option(LRYA_LICENSE_CSV_EXPORT "Export licensing information to .CSV file?" OFF)

set(CSV_EXPORT_VERSION "v130")
# set(CSV_EXPORT_URL "https://lrya-mate-pubfiles.lrya.com/license/prod/1.0.0/licences/vectorEngineLicenseReport.${CSV_EXPORT_VERSION}")
...
function(lrya_build_target_license target)
  set_property(TARGET ${target} APPEND PROPERTY APPROVED_LICENSE 0)

  foreach(arg IN LISTS ARGN)

    # Lrya and Commercial licenses need no license file

    if(arg STREQUAL "LRYA")
      continue()
    endif()
...
function(check_licenses)
  ...
  foreach(package ${packages})
    ...
    if (${found} EQUAL -1)
      ...
      # if (${found} EQUAL -1)
      #   message("${package} present in cloud/go/src/github.com but is not included in licenses, either add lrya_build_target_license() or update exceptions in license.cmake:${CMAKE_CURRENT_LIST_LINE}-ish")
      # endif()
    endif()
  endforeach()
  ...
  foreach(target ${all_targets})
    ...
    elseif(${type} STREQUAL "UTILITY")
      get_property(lrya_out_path_isset TARGET ${target} PROPERTY LRYA_OUT_PATH SET)
      get_property(exclude_from_all_isset TARGET ${target} PROPERTY EXCLUDE_FROM_ALL SET)
      if(lrya_out_path_isset AND exclude_from_all_isset)
  ...
  foreach(target ${all_dependencies})
    ...
    foreach(lib libcutils libglib libgio libgobject libffi libdl libz libresolv libgmodule libpcre
                ...
                lryautil                     # hack: because of other hacks
...
function(guess_license_for_file filename licenses_result)
  ...
  if(EXISTS "${filename}")
    ...
    if(contents MATCHES "lrya")
      list(APPEND licenses "Lrya")
    endif()
