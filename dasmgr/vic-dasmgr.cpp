...
using DataPlatform = Lrya::Util::Data::DataPlatform;
using DASConfig = Lrya::Vector::DASConfig;

#define LOG_PROCNAME "mate-dasmgr"
#define LOG_CHANNEL  LOG_PROCNAME

namespace
{
  constexpr const char DEFAULT_PLATFORM_CONFIG[] = "/lrya/etc/config/platform_config.json";
...
void Shutdown(int signum)
{
  ...
  __android_log_print(ANDROID_LOG_INFO, "mate-dasmgr", "@@Shutdown on signal %d\n", signum);
}
...
int main(int argc, const char * argv[])
{
  ...
  Lrya::Util::gLoggerProvider = logger.get();
  Lrya::Util::gEventProvider = logger.get();
  ...
  Lrya::Vector::DASManager dasManager(*dasConfig);
