/**
* File: deskmate/dasmgr/dasManager.cpp
*
...
// Imported types
using LogLevel = Lrya::Util::LogLevel;
...
// See also: util/logging/deskmateLogger_mateos.cpp
//
//static constexpr const char * EVENT_FORMAT = "@%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s";
//

// Convert android log timestamp to Lrya DAS equivalent
...
// Convert android log level to Lrya DAS equivalent
...
{
  using FileUtils = Lrya::Util::FileUtils;
...
std::string DASManager::ConvertLogEntryToJson(const AndroidLogEntry & logEntry)
{
  ...
  // Lrya::Util::StringSplit ignores trailing separator so don't use it
  std::vector<std::string> values;
  const char * pos = logEntry.message+1; // (skip leading event marker)
  while (1) {
    const char * end = strchr(pos, Lrya::Util::DAS::FIELD_MARKER);
    if (end == nullptr) {
  ...
  if (values.size() < Lrya::Util::DAS::FIELD_COUNT) {
    LOG_ERROR("DASManager.ConvertLogEntry", "Unable to parse %s from %s (%zu != %d)",
              logEntry.message, logEntry.tag, values.size(), Lrya::Util::DAS::FIELD_COUNT);
    return "";
  }
...
void DASManager::ProcessLogEntry(const AndroidLogEntry & logEntry)
{
  ...
  if (*message != Lrya::Util::DAS::EVENT_MARKER) {
    return;
  } else if (message[1] == Lrya::Util::DAS::EVENT_MARKER) {
...
void DASManager::LoadGlobalState()
{
  ...
  _feature_run_id = Lrya::Util::GetUUIDString();

  // Get persistent values from OS
  {
    auto * osState = Lrya::Vector::OSState::getInstance();
    DEV_ASSERT(osState != nullptr, "DASManager.LoadGlobalState.InvalidOSState");
    if (osState->HasValidEMR()) {
      _robot_id = Lrya::Util::StringToLower(osState->GetSerialNumberAsString());
    } else {
