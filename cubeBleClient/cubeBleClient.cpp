...
{
  if (LRYA_VERIFY(_cubeConnectionState == CubeConnectionState::UnconnectedIdle,
...
{
  bool result = false;
  if (LRYA_VERIFY(_cubeConnectionState == CubeConnectionState::Connected && !_currentCube.empty(),
...
{
  bool result = false;
  if (LRYA_VERIFY(_cubeConnectionState == CubeConnectionState::UnconnectedIdle && _currentCube.empty(),
...
bool CubeBleClient::RequestDisconnectFromCube()
{
  const bool connectedOrPending = ((_cubeConnectionState == CubeConnectionState::Connected) ||
                                  ...
                                  !_currentCube.empty();
  bool result = false;
  if (LRYA_VERIFY(connectedOrPending,
