 ...
 * Created: 3/28/2018
 *
 * Description: ble Client for lryabluetoothd. This is essentially a wrapper around
...
#include "lrya-ble/common/gatt_constants.h"
...
void BleClient::FlashCube()
{
  ...
  while (nBytesRemaining > 0) {
    ...
    SendMessage(_connectionId, Lrya::kCubeOTATarget_128_BIT_UUID, true, packet);
    offset += chunkLength;
                                           ...
                                           const std::vector<Lrya::BluetoothDaemon::GattDbRecord>& records)
{
  ...
  if (connected) {
    ...
    ReadCharacteristic(_connectionId, Lrya::kCubeAppVersion_128_BIT_UUID);
                                           ...
                                           const std::vector<uint8_t>& data) 
{
  ...
  const bool isAppVersion = Util::StringCaseInsensitiveEquals(characteristic_uuid,
                                                              Lrya::kCubeAppVersion_128_BIT_UUID);
                                 ...
                                 const std::vector<uint8_t>& value)
{
  ...
  if (Util::StringCaseInsensitiveEquals(characteristic_uuid, Lrya::kCubeAppVersion_128_BIT_UUID)) {
    ...
    _pendingFirmwareCheckOrUpdate = false;
  } else if (Util::StringCaseInsensitiveEquals(characteristic_uuid, Lrya::kCubeAppRead_128_BIT_UUID)) {
