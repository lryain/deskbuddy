...
class Connection:
    def __init__(self, serial):
        config_file = str(Path.home() / ".lrya_vector" / "sdk_config.ini")
...
def is_local():
    return os.environ.get('LRYA_ROBOT_SERIAL', '') == "Local"

def live_robot_only(fn):
    return pytest.mark.skipif(is_local(), reason="mate-cloud is not available on webots")(fn)

@pytest.fixture(scope="module")
def vector_connection():
    serial = os.environ.get('LRYA_ROBOT_SERIAL', None)
    if serial is None:
        sys.exit("Please set 'LRYA_ROBOT_SERIAL' environment variable with 'export LRYA_ROBOT_SERIAL=<your robot's serial number>'. To run with webots set your serial number to 'Local'")
