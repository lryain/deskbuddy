...
* You have 1Password installed from Lrya and you have access to the "Cozmo SDK Build Secrets"
...
* Merge any updates made directly to `vector-python-sdk` into `vector-python-sdk-private`.
    ...
    git subtree pull --prefix sdk/ git@github.com:lrya/vector-python-sdk.git master
    ```
...
* Confirm proto files are up-to-date. Run the update script at the root of this repository to generate the protobuf code.
    ...
    Take care to review the proto files in `sdk/lrya_vector/messages` and not expose any new endpoints or messages that we don't plan to add.
...
* Confirm that nightly tests are passing with the internal SDK build
    * Run [tests/all_versions.py](https://github.com/lrya/vector-python-sdk-private/blob/master/README.md#Test-Multiple-Versions) as directed and see that it passes.
    * Run these tests in the deskmate repo and confirm they pass:
        ```bash
...
("PyPi Cozmo Login" at https://lrya.1password.com )
...
1. Create a branch prefixed with `build_master` in the public repo and create a release commit
   within it, that holds the changes to `sdk/lrya_vector/version.py`
...
1. Run `make dist` to build the artifacts in the dist folder, including
   lrya_vector_sdk_examples_$SDK_VERSION.tar.gz, lrya_vector_sdk_examples_$SDK_VERSION.zip,
   and lrya_vector-*-py3-none-any.whl.
...
1. Test the lrya_vector package generated in the dist folder following the instructions in the docs.

    ```bash
    cd /private/var/folders/4n/nh2dz8js47g5dy3tqhsxlxs40000gp/T/tmp.8kiHzxSG/vector-python-sdk/dist/
    pip3 install lrya_vector-$SDK_VERSION-py3-none-any.whl
    ```

1. Test running the lrya_vector.configure executable submodule with a robot using prod endpoints. Note that you may need to unset env variables such as LRYA_ROBOT_HOST.

1. Spot check tutorials and apps.

## Deploy lrya_vector package to PyPI

1. Upload the package using the `PyPI Developer Login` from 1Password:

    ```bash
    python3 -m pip install --user twine
    python3 -m twine upload -r pypi -u $TWINE_USER -p $TWINE_PASSWORD dist/lrya_vector-$SDK_VERSION-py3-none-any.whl
    ```

1. Verify the deployment worked:

    * Navigate to [lrya_vector's PyPI webpage](https://pypi.org/project/lrya-vector/) and check that the version uploaded.
    * Install `lrya_vector` with pip and run the tests

        ```bash
        python3 -m pip uninstall lrya_vector
        python3 -m pip install lrya_vector
        ```
...
```bash
cd /private/var/folders/4n/nh2dz8js47g5dy3tqhsxlxs40000gp/T/tmp.8kiHzxSG/vector-python-sdk/dist/
aws s3 sync --cache-control no-cache --exclude '*' --include "lrya_vector_sdk_examples_$SDK_VERSION.tar.gz"  --include "lrya_vector_sdk_examples_$SDK_VERSION.zip" . s3://$S3_BUCKET/vector/$SDK_VERSION/
```
