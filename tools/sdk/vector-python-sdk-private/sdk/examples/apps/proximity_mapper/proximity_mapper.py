...
from lrya_vector.util import parse_command_args, radians, degrees, distance_mm, speed_mmps, Vector3  # pylint: disable=wrong-import-position
...
async def analyze_proximity_sample(reading: lrya_vector.proximity.ProximitySensorData, robot: lrya_vector.robot.Robot, state: MapState):
...
async def collect_proximity_data_loop(robot: lrya_vector.robot.Robot, future: concurrent.futures.Future, state: MapState):
    try:
...
async def scan_area(robot: lrya_vector.robot.Robot, state: MapState):
...
async def map_explorer(robot: lrya_vector.robot.Robot):
...
if __name__ == '__main__':
    ...
    with lrya_vector.Robot(args.serial,
                           show_viewer=True,
