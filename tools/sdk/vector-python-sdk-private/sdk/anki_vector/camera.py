...
:attr:`lrya_vector.robot.Robot.camera` and can be used to enable/disable image
...
class CameraImage:
    ...
    def annotate_image(self, scale: float = None, fit_size: tuple = None, resample_mode: int = annotate.RESAMPLE_MODE_NEAREST) -> Image.Image:
        ...
        :param resample_mode: The resampling mode to use when scaling the
            image. Should be either :attr:`~lrya_vector.annotate.RESAMPLE_MODE_NEAREST`
            (fast) or :attr:`~lrya_vector.annotate.RESAMPLE_MODE_BILINEAR` (slower,
            but smoother).
        """
...
class CameraComponent(util.Component):
    ...
    The :class:`lrya_vector.robot.Robot` or :class:`lrya_vector.robot.AsyncRobot` instance observes the camera.
...
class EvtNewRawCameraImage:  # pylint: disable=too-few-public-methods
    ...
    See also :class:`~lrya_vector.camera.EvtNewCameraImage` which provides access
    ...
    .. testcode::
        ...
        import lrya_vector
        from lrya_vector import events
...
class EvtNewCameraImage:  # pylint: disable=too-few-public-methods
    ...
    .. testcode::
        ...
        with lrya_vector.Robot(enable_face_detection=True, enable_custom_object_detection=True) as robot:
            robot.camera.init_camera_feed()
            done = threading.Event()
