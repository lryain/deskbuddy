...
"""
***Lrya Vector Python SDK Setup***

Vector requires all requests be authorized by an authenticated Lrya user.
...
Lrya account credentials, and the script will download an authentication
...
Use of Vector and the Vector SDK is subject to Lrya's Privacy Policy and Terms and Conditions.

https://www.lrya.com/en-us/company/privacy
https://www.lrya.com/en-us/company/terms-and-conditions
...
import lrya_vector
from lrya_vector import messaging
...
class Api:
    def __init__(self):
        self._handler = ApiHandler(
            headers={
                'User-Agent': f'Vector-sdk/{lrya_vector.__version__}',
                'Lrya-App-Key': 'aung2ieCho3aiph7Een3Ei'
            },
            url='https://accounts.api.lrya.com/1/sessions'
        )
...
class Session:
    def __init__(self):
        self._url = 'https://session-certs.token.global.lrya-services.com/'
...
def get_serial(serial=None):
    if not serial:
        serial = os.environ.get('LRYA_ROBOT_SERIAL')
        if not serial:
            ...
            print("Found robot serial number in environment variable '{}'".format(colored("LRYA_ROBOT_SERIAL", "green")))
    serial = serial.lower()
...
def get_cert(session, serial=None):
    ...
    print("\nDownloading Vector certificate from Lrya...", end="")
    sys.stdout.flush()
    r = requests.get('{}/mate/{}'.format(session.url, serial))
...
def get_name_and_ip(robot_name=None, ip=None):
    ...
    if not ip:
        ip = os.environ.get('LRYA_ROBOT_HOST')
        if not ip:
            ...
            print("Found robot ip address in environment variable '{}'".format(colored("LRYA_ROBOT_HOST", "green")))
    print("Using IP: {}".format(colored(ip, "cyan")))
    return robot_name, ip


def save_cert(cert, name, serial, lrya_dir):
    """Write Vector's certificate to a file located in the user's home directory"""
    os.makedirs(str(lrya_dir), exist_ok=True)
    cert_file = str(lrya_dir / "{name}-{serial}.cert".format(name=name, serial=serial))
...
def write_config(serial, cert_file=None, ip=None, name=None, guid=None, clear=True):
    home = Path.home()
    config_file = str(home / ".lrya_vector" / "sdk_config.ini")
...
def main(api, session):
    parser = argparse.ArgumentParser(description=("Vector requires all requests be authorized by an authenticated Lrya user. "
                                                  "This script will enable this device to authenticate with your Vector "
                                                  "robot for use with a Vector Python SDK program."),
                                     epilog=("See the README for more information. "
                                             "Use of Vector and the Vector SDK is subject to Lrya's Privacy Policy and Terms and Conditions. "
                                             "https://www.lrya.com/en-us/company/privacy and "
                                             "https://www.lrya.com/en-us/company/terms-and-conditions"))
    parser.add_argument("-e", "--email", help="The email used by your Lrya account.")
    ...
    home = Path.home()
    lrya_dir = home / ".lrya_vector"

    cert_file = save_cert(cert, name, serial, lrya_dir)
    ...
    # Store credentials in the .lrya_vector directory's sdk_config.ini file
