...
def parse_command_args(parser: argparse.ArgumentParser = None):
    ...
    is specified, we next attempt to read the robot serial number from environment variable LRYA_ROBOT_SERIAL.
    If LRYA_ROBOT_SERIAL is specified, the value will be used as the robot's serial number.
    ...
    .. code-block:: python
        ...
        args = lrya_vector.util.parse_command_args(parser)
    ...
    if parser is None:
        parser = argparse.ArgumentParser()
    parser.add_argument("-s", "--serial", nargs='?', default=os.environ.get('LRYA_ROBOT_SERIAL', None))
...
def setup_basic_logging(custom_handler: logging.Handler = None,
    ...
    if handler is None:
        ...
        class LogCleanup(logging.Filter):  # pylint: disable=too-few-public-methods
            def filter(self, record):
                # Drop 'lrya_vector' from log messages
    ...
    vector_logger = logging.getLogger('lrya_vector')
...
def get_class_logger(module: str, obj: object) -> logging.Logger:
    ...
    .. testcode::
        ...
        logger = lrya_vector.util.get_class_logger("module_name", "object_name")
...
class Angle:
    ...
    @property
    def abs_value(self):
        """:class:`lrya_vector.util.Angle`: The absolute value of the angle.
...
class Quaternion:
    ...
    def to_matrix(self, pos_x: float = 0.0, pos_y: float = 0.0, pos_z: float = 0.0):
        ...
        Returns:
            :class:`lrya_vector.util.Matrix44`: A matrix representing this Quaternion's
...
class Pose:
    ...
    .. testcode::
        ...
        with lrya_vector.Robot() as robot:
            pose = Pose(x=50, y=0, z=0, angle_z=lrya_vector.util.Angle(degrees=0))
    ...
    def define_pose_relative_this(self, new_pose):
        ...
        :param lrya_vector.util.Pose new_pose: The pose which origin is being changed.

        Returns:
            A :class:`lrya_vector.util.Pose` object for which the origin was this pose's origin.
        """
...
def distance_mm(distance_mm: float):  # pylint: disable=redefined-outer-name
    """Returns an :class:`lrya_vector.util.Distance` instance set to the specified number of millimeters."""
    return Distance(distance_mm=distance_mm)


def distance_inches(distance_inches: float):  # pylint: disable=redefined-outer-name
    """Returns an :class:`lrya_vector.util.Distance` instance set to the specified number of inches."""
...
def speed_mmps(speed_mmps: float):  # pylint: disable=redefined-outer-name
    """:class:`lrya_vector.util.Speed` instance set to the specified millimeters per second speed."""
...
def read_configuration(serial: str, name: str, logger: logging.Logger) -> dict:
    ...
    if not sections:
        raise VectorConfigurationException('Could not find the sdk configuration file. Please run `python3 -m lrya_vector.configure` to set up your Vector for SDK usage.')
