...
class Robot:
    ...
    .. testcode::
        ...
        with lrya_vector.Robot() as robot:
            # Run your commands
    ...
    :param estimate_facial_expression: Turn estimating facial expression on/off. Enabling :code:`estimate_facial_expression`
                                       returns a facial expression, the expression values and the :class:`lrya_vector.util.ImageRect`
    ...
    def __init__(self,
        ...
        if self._name is None or self._ip is None or self._cert_file is None or self._guid is None:
            raise ValueError("The Robot object requires a serial and for Vector to be logged in (using the app then running the lrya_vector.configure executable submodule).\n"
        ...
        #: :class:`lrya_vector.connection.Connection`: The active connection to the robot.
    ...
    def conn(self) -> Connection:
        """A reference to the :class:`~lrya_vector.connection.Connection` instance."""
        return self._conn

    @property
    def events(self) -> events.EventHandler:
        """A reference to the :class:`~lrya_vector.events.EventHandler` instance."""
        return self._events

    @property
    def anim(self) -> animation.AnimationComponent:
        """A reference to the :class:`~lrya_vector.animation.AnimationComponent` instance."""
    ...
    def behavior(self) -> behavior.BehaviorComponent:
        """A reference to the :class:`~lrya_vector.behavior.BehaviorComponent` instance."""
        return self._behavior

    @property
    def camera(self) -> camera.CameraComponent:
        """The :class:`~lrya_vector.camera.CameraComponent` instance used to control Vector's camera feed.
    ...
    def faces(self) -> faces.FaceComponent:
        """A reference to the :class:`~lrya_vector.faces.FaceComponent` instance."""
    ...
    def motors(self) -> motors.MotorComponent:
        """A reference to the :class:`~lrya_vector.motors.MotorComponent` instance."""
    ...
    def nav_map(self) -> nav_map.NavMapComponent:
        """A reference to the :class:`~lrya_vector.nav_map.NavMapComponent` instance."""
    ...
    def screen(self) -> screen.ScreenComponent:
        """A reference to the :class:`~lrya_vector.screen.ScreenComponent` instance."""
    ...
    def photos(self) -> photos.PhotographComponent:
        """A reference to the :class:`~lrya_vector.photos.PhotographComponent` instance."""
    ...
    def proximity(self) -> proximity.ProximityComponent:
        """:class:`~lrya_vector.proximity.ProximityComponent` containing state related to object proximity detection.
    ...
    def touch(self) -> touch.TouchComponent:
        """:class:`~lrya_vector.touch.TouchComponent` containing state related to object touch detection.
    ...
    def viewer(self) -> ViewerComponent:
        """The :class:`~lrya_vector.viewer.ViewerComponent` instance used to render Vector's camera feed.
    ...
    def viewer_3d(self) -> Viewer3DComponent:
        """The :class:`~lrya_vector.viewer.Viewer3DComponent` instance used to render Vector's navigation map.
        ...
        .. testcode::
            ...
            with lrya_vector.Robot(show_3d_viewer=True, enable_nav_map_feed=True) as robot:
    ...
    def vision(self) -> vision.VisionComponent:
        """:class:`~lrya_vector.vision.VisionComponent` containing functionality related to vision based object detection.
    ...
    def world(self) -> world.World:
        """A reference to the :class:`~lrya_vector.world.World` instance, or None if the World is not yet initialized."""
    ...
    def pose(self) -> util.Pose:
        """:class:`lrya_vector.util.Pose`: The current pose (position and orientation) of Vector.
    ...
    def accel(self) -> util.Vector3:
        """:class:`lrya_vector.util.Vector3`: The current accelerometer reading (x, y, z)
    ...
    def carrying_object_id(self) -> int:
        ...
        .. testcode::
            ...
            import lrya_vector
            from lrya_vector.util import degrees
    ...
    def status(self) -> status.RobotStatus:
        ...
        :class:`RobotStatus <lrya_vector.status.RobotStatus>` class documentation.
    ...
    def enable_audio_feed(self) -> bool:
        ...
        .. code-block:: python
            ...
            with lrya_vector.Robot(enable_audio_feed=True) as robot:
                time.sleep(5)
    ...
    def connect(self, timeout: int = 10) -> None:
        ...
        :param timeout: The time to allow for a connection before a
            :class:`lrya_vector.exceptions.VectorTimeoutException` is raised.
        """
...
class AsyncRobot(Robot):
    ...
    .. testcode::
        ...
        # Create the robot connection
        with lrya_vector.AsyncRobot() as robot:
    ...
    .. testcode::
        ...
        async def callback(robot, event_type, event):
            ...
            await asyncio.wrap_future(robot.behavior.set_head_angle(lrya_vector.util.degrees(40)))

        if __name__ == "__main__":
            args = lrya_vector.util.parse_command_args()
            with lrya_vector.AsyncRobot(serial=args.serial, enable_face_detection=True) as robot:
                robot.behavior.set_head_angle(lrya_vector.util.degrees(40))
                robot.events.subscribe(callback, lrya_vector.events.Events.robot_observed_face)
