...
Objects can generate events which can be subscribed to from the lrya_vector.events
...
# TODO Curious why events like the following aren't listed? At least some do seem to be supported in other parts of lrya_vector.
...
class EvtObjectObserved():  # pylint: disable=too-few-public-methods
    ...
    .. testcode::
        ...
        import lrya_vector
        from lrya_vector.events import Events
        ...
        with lrya_vector.Robot(default_logging=False,
                               show_viewer=True,
                               show_3d_viewer=True,
    ...
    :param image_rect: An :class:`lrya_vector.util.ImageRect`: defining where the object is within Vector's camera view
    :param pose: The :class:`lrya_vector.util.Pose`: defining the position and rotation of the object
    """
...
class ObservableObject(util.Component):
    ...
    def time_since_last_seen(self) -> float:
        ...
        .. testcode::
            ...
            with lrya_vector.Robot(enable_face_detection=True) as robot:
...
class LightCube(ObservableObject):
    ...
    to localize himself, taking note of the :class:`lrya_vector.util.Pose` of the objects.

    You can subscribe to cube events including :class:`lrya_vector.events.Events.object_tapped`,
    :class:`lrya_vector.events.Events.object_appeared`, and :class:`lrya_vector.events.Events.object_disappeared`.
        ...
        .. testcode::
            ...
            with lrya_vector.Robot() as robot:
                ...
                if robot.world.connected_light_cube:
                    ...
                    cube.set_light_corners(lrya_vector.lights.blue_light,
                                           lrya_vector.lights.green_light,
                                           lrya_vector.lights.red_light,
                                           lrya_vector.lights.white_light)
                    time.sleep(3)
        ...
        .. testcode::
            ...
            with lrya_vector.Robot() as robot:
                ...
                if robot.world.connected_light_cube:
                    ...
                    cube.set_lights(lrya_vector.lights.yellow_light)
                    time.sleep(3)
...
class Charger(ObservableObject):
    ...
    We get an :class:`lrya_vector.objects.EvtObjectObserved` message when the
...
class CustomObjectArchetype():
    ...
    :meth:`~lrya_vector.world.World.define_custom_box`,
    :meth:`~lrya_vector.world.World.define_custom_cube`, or
    :meth:`~lrya_vector.world.World.define_custom_wall`.
    """
    ...
    def custom_type(self) -> protocol.CustomType:
        ...
        .. testcode::
            ...
            import lrya_vector
            with lrya_vector.Robot(enable_custom_object_detection=True) as robot:
...
class CustomObject(ObservableObject):
    ...
    def object_id(self) -> int:
        ...
        .. testcode::
            ...
            import lrya_vector
            from lrya_vector.objects import CustomObjectMarkers, CustomObjectTypes
...
class CustomObjectTypes():  # pylint: disable=too-few-public-methods
    ...
    :meth:`lrya_vector.world.World.define_custom_box`,
    :meth:`lrya_vector.world.World.define_custom_cube`, and
    :meth:`lrya_vector.world.World.define_custom_wall`
...
class FixedCustomObject(util.Component):
    ...
    To create these use :meth:`~lrya_vector.world.World.create_custom_fixed_object`

    .. testcode::

        import lrya_vector
        from lrya_vector.util import degrees, Pose
        import time
