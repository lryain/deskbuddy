...
class Connection:
    ...
    :class:`~lrya_vector.robot.Robot`, and instead talk to aiogrpc more directly.
    ...
    .. code-block:: python
        ...
        conn = lrya_vector.connection.Connection("Vector-XXXX", "XX.XX.XX.XX:443", "/path/to/file.cert", "<guid>")
        conn.connect()
        # Run your commands
        async def play_animation():
            # Run your commands
            anim = lrya_vector.messaging.protocol.Animation(name="anim_pounce_success_02")
            anim_request = lrya_vector.messaging.protocol.PlayAnimationRequest(animation=anim)
    ...
    def grpc_interface(self) -> client.ExternalInterfaceStub:
        ...
        This may be used to directly call grpc messages bypassing :class:`lrya_vector.Robot`
    ...
    def requires_behavior_control(self) -> bool:
        ...
        .. code-block:: python
            ...
            with lrya_vector.Robot(behavior_control_level=None) as robot:
    ...
    def control_lost_event(self) -> asyncio.Event:
        ...
        .. testcode::
            ...
            async def auto_reconnect(conn: lrya_vector.connection.Connection):
    ...
    def control_granted_event(self) -> asyncio.Event:
        ...
        .. testcode::
            ...
            async def wait_for_control(conn: lrya_vector.connection.Connection):
...
def on_connection_thread(log_messaging: bool = True, requires_control: bool = True, is_cancellable_behavior=False) -> Callable[[Coroutine[util.Component, Any, None]], Any]:
    ...
    .. code-block:: python
        ...
        class MyComponent(lrya_vector.util.Component):
    ...
    :returns: A decorator which has 3 possible returns based on context: the result of the decorated function,
        ...
        These contexts are: when the robot is a :class:`~lrya_vector.robot.Robot`,
        when the robot is an :class:`~lrya_vector.robot.AsyncRobot`, and when
    ...
    def _on_connection_thread_decorator(func: Coroutine) -> Any:
        ...
        async def log_handler(conn: Connection, func: Coroutine, logger: logging.Logger, *args: List[Any], **kwargs: Dict[str, Any]) -> Coroutine:
            """Wrap the provided coroutine to better express exceptions as specific :class:`lrya_vector.exceptions.VectorException`s, and
