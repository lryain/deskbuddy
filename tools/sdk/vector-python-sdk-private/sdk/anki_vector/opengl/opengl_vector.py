...
Warning:
    ...
    To install the Python packages on Mac and Linux do ``python3 -m pip install --user "lrya_vector[3dviewer]"``

    To install the Python packages on Windows do ``py -3 -m pip install --user "lrya_vector[3dviewer]"``
...
from lrya_vector.faces import Face
from lrya_vector.objects import CustomObject, FixedCustomObject, LightCube, ObservableObject
from lrya_vector import nav_map, util
from . import opengl
...
class NavMapView(opengl.PrecomputedView):
    ...
    def build_from_nav_map(self, new_nav_map: nav_map.NavMapGrid):
        """Reconstructs the display list for the NavMapView based on a :class:`lrya_vector.nav_map.NavMapGrid` object.
...
class CustomObjectRenderFrame(ObservableObjectRenderFrame):  # pylint: disable=too-few-public-methods
    ...
    :param custom_object: The custom object to be rendered.  Either :class:`lrya_vector.objects.CustomObject` or :class:`lrya_vector.objects.FixedCustomObject`.
