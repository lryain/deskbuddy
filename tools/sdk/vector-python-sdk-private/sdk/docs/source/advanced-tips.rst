...
when you connect as follows::
    ...
    with lrya_vector.Robot(name="Vector-A1B2") as robot:
...
Alternatively, you can manually make changes to your SDK setup. To assist in this migration, the ``lrya_vector.configure``
...
Then from your terminal run::
    ...
    python3 -m lrya_vector.configure -u "<your_new_ip>" -s "<your_robot_serial_number>"
...
to the Robot constructor::
  ...
  with lrya_vector.Robot("00e20142") as robot:
    robot.anim.play_animation_trigger("GreetAfterLongTime")


Alternatively, you can pass a ``--serial`` flag on the command
line, and ``lrya_vector.util.parse_command_args`` will parse out
the serial number::
...
^^^^^^^^^^^^^^^^^^^^^
Set LRYA_ROBOT_SERIAL
^^^^^^^^^^^^^^^^^^^^^

In order to avoid entering Vector's serial number for each program run,
you can create environment variable ``LRYA_ROBOT_SERIAL``
and set it to Vector's serial number::

    export LRYA_ROBOT_SERIAL=00e20100


^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Set LRYA_ROBOT_HOST and VECTOR_ROBOT_NAME
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

When running the ``lrya_vector.configure`` executable submodule, you must provide Vector's ip and name.
To avoid typing these in, you can instead create environment variables
LRYA_ROBOT_HOST and VECTOR_ROBOT_NAME. Then ``lrya_vector.configure`` will automatically pick
up those settings::

    export LRYA_ROBOT_HOST="192.168.42.42"
...
* Control can be reserved from the command prompt: ``python3 -m lrya_vector.reserve_control``  Vector will remain still between SDK scripts until the script/process is exited.
...
* A single Python file can explicitly reserve control using the ``ReserveBehaviorControl`` object.  Consult the ``lrya_vector.connection`` `documentation <https://developer.lrya.com/vector/docs/generated/lrya_vector.connection.html>`_ for more information.
