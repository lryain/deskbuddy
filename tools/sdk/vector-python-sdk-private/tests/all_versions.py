...
def verify_versions():
    verified = True
    try:
        released_versions = get_versions("lrya-vector")
        for version in released_versions:
            if version not in VERSIONS and version not in SKIP_VERSIONS:
                verified = False
                print(f"{colored('WARNING', 'yellow')}: lrya-vector version {version} not referenced in this script.\n"
                      "          It should be added to VERSIONS or SKIP_VERSIONS.")
        for version in VERSIONS:
            if version not in released_versions and version != "latest":
                verified = False
                print(f"{colored('WARNING', 'yellow')}: lrya-vector test version {version} not found on pypi.")
    except:  # pylint: disable=bare-except
        verified = False
...
class TestRunner:
    ...
    def setup(self):
        ...
        if self._version == "latest":
            ...
            command = f"./tests/env/bin/python -m pip install lrya_vector[3dviewer,docs,test]=={self._version}"
        code = self.run_and_wait(command, show_output=self._verbose)
        if code != 0:
            return code
        return 0
    ...
    def run_test(self, serial=None):
        ...
        if serial:
            modified_env["LRYA_ROBOT_SERIAL"] = serial
