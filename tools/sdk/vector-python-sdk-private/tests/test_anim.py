...
def test_bad_play_animation(robot):
    with pytest.raises(lrya_vector.exceptions.VectorException) as excinfo:
...
def test_anim_list_property_cached():
    ...
    with lrya_vector.Robot(serial=serial, **behavior_control_param) as robot:
...
def test_anim_list_property_cached_async():
    with lrya_vector.AsyncRobot(serial=serial, **behavior_control_param) as robot:
        start_time = time.time()
        anim_list = robot.anim.anim_list
        assert len(anim_list) > 1000
        elapsed_time = time.time() - start_time
...
def test_anim_trigger_list_property_cached():
    with lrya_vector.Robot(serial=serial, behavior_control_level=None) as robot:
        start_time = time.time()
        anim_trigger_list = robot.anim.anim_trigger_list
...
def test_anim_trigger_list_property_cached_async():
    with lrya_vector.AsyncRobot(serial=serial, behavior_control_level=None) as robot:
        start_time = time.time()
        anim_trigger_list = robot.anim.anim_trigger_list
        assert len(anim_trigger_list) > 300
        elapsed_time = time.time() - start_time
