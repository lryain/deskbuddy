...
try:
    ...
    sys.exit("\n\nThis script requires you to install the Vector SDK.\n"
             ...
             "Then try again".format(
                 ...
                 configure="python3 -m lrya_vector.configure",
             ))


serial = os.environ.get("LRYA_ROBOT_SERIAL", default="Local")
...
def version_lesser_than(version):
    return version_check(version, lrya_vector.version.__version__)
...
robots_with_control_params = [
    pytest.param(lrya_vector.Robot(serial=serial, **cache_animation_lists), id="sync"),
    pytest.param(lrya_vector.AsyncRobot(serial=serial, **cache_animation_lists), id="async")
]
robots_without_control_params = [
    pytest.param(lrya_vector.Robot(serial=serial, **cache_animation_lists, **behavior_control_param), id="sync_no_control"),
    pytest.param(lrya_vector.AsyncRobot(serial=serial, **cache_animation_lists, **behavior_control_param), id="async_no_control")
]
all_robots_params = [
    *robots_with_control_params,
    *robots_without_control_params,
...
def version_greater_than(version):
    return version_check(lrya_vector.version.__version__, version)
