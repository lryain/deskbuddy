...
- Copies proto from the "deskmate-proto" submodule to "sdk/lrya_vector/messages" (default).
            ...
            for line in lines:
                if not skip_edit:
                    ...
                    if "// App only" in line:
                        continue
                if not skip_path:
                    line = re.sub(r'import "([^\/]*).proto"', 'import "lrya_vector/messaging/\\1.proto"', line)
                sources.write(line)
    print(" DONE")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Update the proto files from the a given commit of the deskmate-proto repo."
            "To make changes to the proto files, you need to modify them in the deskmate repo and push to the deskmate-proto subtree."
        ))
    parser.add_argument("-d", "--dir", help="Custom dir from which to grab the proto files.", default=str(working_dir / "deskmate-proto"))
    parser.add_argument("-o", "--output", help="Custom output location.", default=str(working_dir / "sdk/lrya_vector/messaging"))
