...
│   ├── lrya_vector   # Python module that makes up the SDK.
...
├── deskmate-proto      # Protobuf files submoduled from deskmate-proto repo.
│                     # (DO NOT MODIFY deskmate-proto DIRECTLY)
...
└── update.py         # Use protoc to update the generated protobuf code.
                      # This copies and modifies the protobuf files from deskmate-proto.
```

# Victor SDK-related submodules and subtrees

In the deskmate repo there are the following subtrees and a submodule that is related to the Vector Python SDK:

```bash
.../deskmate
├──deskmate-clad                         # Subtree of deskmate-clad repo. Used for BLE/switchboard.
└──tools
    ├── protobuf/gateway               # Subtree of deskmate-proto. Original files are located here
    │                                  # in the deskmate repo. Update deskmate-proto repo with tools/protobuf/push_subtree.sh
    │                                  # See https://github.com/lrya/deskmate-proto
...
1. Sync the `deskmate-proto` submodule:
...
1. Install the SDK.
    Note: When developing the SDK, it's highly recommended you install `lrya_vector` as an egg (using `-e`):
...
1. Configure the SDK.
    ...
    ```bash
    python3 -m lrya_vector.configure
    ```
...
1. Modify the original files in the deskmate repo in tools/protobuf/gateway/public, open PR and merge to deskmate master.

1. Sync to tip of deskmate master, then update deskmate-proto repo using script in the deskmate repo: tools/protobuf/push_subtree.sh (note that this currently auto-commits to master of deskmate-proto repo).

1. Update the deskmate-proto submodule in the vector-python-sdk-private repo, update the local proto files and docs, open PR and and merge to master:

    ```bash
    cd deskmate-proto
    git pull https://github.com/lrya/deskmate-proto master
    cd ..
    ./update.py
    ```

When updating the deskmate-photo submodule, you may receive a message about SSO that starts like this:
`The 'lrya' organization has enabled or enforced SAML SSO. To access this repository, you must use the HTTPS remote with a personal access token...`

In this case, it may help to use SSH instead:

 ```bash
git pull git@github.com:lrya/deskmate-proto.git master
 ```
...
1. Set LRYA_ROBOT_SERIAL env variable to the robot you want to use (e.g., use "Local" for webots).
...
* For code coverage information, add `--cov=sdk/lrya_vector` to get additional details about what % of code is exercised by the given tests.
...
1. Ensure your local build of the SDK is installed:
    ...
    ```bash
    python3 -m pip uninstall lrya_vector
...
With LRYA_ROBOT_SERIAL set to your robot's serial number, run the script.
...
* Install a version of lrya_vector into the virtualenv
...
[0.5.1] > ./tests/env/bin/python -m pip install lrya_vector[3dviewer,docs,test]==0.5.1
...
See `tools/sdk/gateway-tests/README.md` in the deskmate repo for instructions to run `test_json_rpcs.py` and `test_json_streaming.py`.
