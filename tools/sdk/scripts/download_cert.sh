...
function usage() {
    ...
    echo "  -s [SERIAL_NUMBER]  Serial number of the robot whose cert to download (LRYA_ROBOT_SERIAL)"
...
if [ -z "${OUTPUT_FILE}" ]; then
    OUTPUT_FILE="/tmp/${LRYA_ROBOT_SERIAL}.cert"
fi

echo "> Downloading file to ${OUTPUT_FILE}"
curl https://session-certs.token.global.lrya-services.com/mate/${LRYA_ROBOT_SERIAL} > ${OUTPUT_FILE}
