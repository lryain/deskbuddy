...
from lrya_vector.messaging import protocol  # pylint: disable=wrong-import-position
from lrya_vector.messaging import client  # pylint: disable=wrong-import-position
...
MESSAGES_TO_TEST = [
    ...
    (Interface.PlayAnimation,
     protocol.PlayAnimationRequest(animation=protocol.Animation(name='anim_blackjack_deskmatewin_01'), loops=1),
    ...
    (Interface.DisplayFaceImageRGB,
     protocol.DisplayFaceImageRGBRequest(face_data=bytes(lrya_vector.color.Color(rgb=[255, 0, 0]).rgb565_bytepair * 17664), duration_ms=1000, interrupt_running=True),
    ...
    (Interface.SetCubeLights,
     protocol.SetCubeLightsRequest(
         object_id=1,
         on_color=[lrya_vector.color.green.int_color] * 4,
         off_color=[lrya_vector.color.blue.int_color] * 4,
         on_period_ms=[1000] * 4,
...
def main():
    ...
    logger = logging.getLogger('lrya_vector')
    ...
    with lrya_vector.Robot(args.serial, default_logging=False, cache_animation_list=False) as robot:
