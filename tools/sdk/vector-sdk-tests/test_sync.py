...
from lrya_vector.events import Events  # pylint: disable=wrong-import-position
...
def main():
    ...
    with lrya_vector.Robot(args.serial) as robot:
        ...
        robot.anim.play_animation("anim_blackjack_deskmatewin_01")
    ...
    robot = lrya_vector.Robot(args.serial)
    robot.events.subscribe_by_name(test_subscriber, event_name='test1')
    try:
        robot.connect()
        robot.anim.play_animation("anim_blackjack_deskmatewin_01")
    ...
    with lrya_vector.AsyncRobot(args.serial) as robot:
        robot.events.subscribe(test_subscriber, Events.robot_state)
        robot.anim.play_animation("anim_blackjack_deskmatewin_01").result()
        robot.motors.set_wheel_motors(-100.0, 100.0).result()

    time.sleep(2)

    print("------ Asynchronous Robot using try finally ------")
    robot = lrya_vector.AsyncRobot(args.serial)
    robot.events.subscribe(test_subscriber, Events.robot_state)
    try:
        robot.connect()
        robot.anim.play_animation("anim_blackjack_deskmatewin_01").result()
