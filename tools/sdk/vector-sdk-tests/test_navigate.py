...
def main():
    ...
    with lrya_vector.Robot(args.serial) as robot:
        ...
        pose = lrya_vector.util.Pose(x=50, y=0, z=0, angle_z=lrya_vector.util.Angle(degrees=0))
        robot.behavior.go_to_pose(pose)
        # Permit enough time to pass to reach required pose
        time.sleep(5)

        pose = lrya_vector.util.Pose(x=0, y=50, z=0, angle_z=lrya_vector.util.Angle(degrees=90))
