...
Description: This script allows sound designers to use locally generated sound banks in local Victor project builds.
             ...
             EXTERNALS/local-audio-assets directory. After replacing the audio assets build-deskmate.sh must be ran to
             copy the assets to the projects deliverables. Likewise, you must run deploy-assets.py to copy new assets
             to robot. For convenience this script has arguments to update audio CLAD files, build deskmate and deploy
             changes.
"""
...
__deskmate_external_path = path.join(__project_root_path, 'EXTERNALS', 'local-audio-assets')
...
__deskmate_bank_list_file_name = 'deskmate-banks-list.json'
...
def main(args):
    ...
    # Bank list file path
    bank_list_filepath = path.join(args.audio_project_repo_dir, __audio_metadata_dir_name, __deskmate_bank_list_file_name)
    ...
    if path.isdir(__deskmate_external_path):
        Logger.info('Delete old audio assets: \'{0}\''.format(__deskmate_external_path))
        shutil.rmtree(__deskmate_external_path)

    # Create Directory structure
    tmp_dir = path.join(__deskmate_external_path, 'tmp')
    tmp_dev_mac_dir = path.join(tmp_dir, 'dev_mac')
    tmp_linux_dir = path.join(tmp_dir, 'deskmate_linux')
    dest_metadata_dir = path.join(__deskmate_external_path, 'metadata')
    dest_dev_mac_dir = path.join(__deskmate_external_path, 'deskmate_robot', 'dev_mac')
    dest_linux_dir = path.join(__deskmate_external_path, 'deskmate_robot', 'deskmate_linux')
    ...
    if args.update_clad:
        ...
        result = UpdateAudioAssets.main(['update-alt-workspace', __deskmate_external_path])
        if result != os.EX_OK:
        ...
        result = UpdateAudioAssets.main(['generate', '--soundBankDir', __deskmate_external_path])
        if result != os.EX_OK:
    ...
    if args.build_deploy:
        # Force build project
        Logger.info('Run build-deskmate.sh')
        deskmate_project_dir = path.join('project', 'deskmate')
        deskmate_build_script = path.join(deskmate_project_dir, 'build-deskmate.sh')
        build_commands = [deskmate_build_script, '-f', '-a', '-DUSE_LOCAL_AUDIO_ASSETS=ON']
        is_mac_build = False;
        ...
        if result != os.EX_OK:
            Logger.error('\'{0}\' Failed'.format(deskmate_build_script))
            return os.EX_SOFTWARE

        if not is_mac_build:
            # Deploy bins
            deskmate_project_script_dir = path.join(deskmate_project_dir, 'scripts')
            Logger.info('Stop deskmate process')
            deskmate_stop_script = path.join(deskmate_project_script_dir, 'deskmate_stop.sh')
            result = subprocess.call([deskmate_stop_script], cwd=__project_root_path)
            if result != os.EX_OK:
                Logger.error('\'{0}\' Failed'.format(deskmate_stop_script))
                return os.EX_SOFTWARE

            Logger.info('Run deskmate_deploy.sh')
            deskmate_deploy_script = path.join(deskmate_project_script_dir, 'deskmate_deploy.sh')
            build_commands = [deskmate_deploy_script]
            if args.configuration:
                build_commands.append('-c')
                build_commands.append(args.configuration)
...
def parse_args(argv=[], print_usage=False):
    ...
    parser.add_argument('--platform', '-p', dest='platform', metavar='platform',
                        help='Build target platform {mac, mateos}')
