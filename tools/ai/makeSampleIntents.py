...
# - - - - - - - - - - - - - - - - - - - - - -
def GetSamplesForDataType(paramName, dataType, entities, lryaEntities):
...
# - - - - - - - - - - - - - - - - - - - - - -
def MakeSamples( intents, entities, lryaEntities ):
  ...
  for intent in intents:
    ...
    for param in intent["params"]:
      paramSamples.append( GetSamplesForDataType( param["name"], param["dataType"], entities, lryaEntities ) )
    if len( paramSamples ) == 0:
      sample = {}
...
def main():
  ...
  lryaEntities = GetEntityFiles( entitiesDir, False )
  lryaEntities = map(lambda x: os.path.splitext(x)[0], lryaEntities)
  lryaEntities.sort()

  sampleList = MakeSamples( intents, entities, lryaEntities )
