...
def display(stdscr):
  robotIP = os.environ['LRYA_ROBOT_HOST']
  if robotIP is None:
    print('Robot IP must be set!     export LRYA_ROBOT_HOST="x.x.x.x"')
    sys.exit(1)

  path = subprocess.check_output(['git','rev-parse','--show-toplevel']).strip().decode('utf-8')
  command = path + '/project/deskmate/scripts/deskmate_log.sh'
