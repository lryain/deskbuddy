...
: ${LRYA_BUILD_BASE_LANG:="en-US"}
: ${LRYA_BUILD_TRANSLATED_LANGS:="de-DE fr-FR ja-JP"}
...
function smartling_download()
{
    ...
    ${SMARTLING_HOME}/download.sh \
        -t ${TRANS_TYPE} \
        ${SMARTLING_OPTS} -l "${LRYA_BUILD_TRANSLATED_LANGS}" \
        -u "$1" \
        "${OUTPUT_DIR}"
}

# Use base file names to determine which translated files to download
BASE_FILES=(`ls ${LOCALIZATION_DIR}/${LRYA_BUILD_BASE_LANG}/*.json`)
...
for lang in ${LRYA_BUILD_TRANSLATED_LANGS}; do
