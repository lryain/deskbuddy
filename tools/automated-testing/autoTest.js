...
class AutoScript {
  ...
  async run(handler) {
    ...
    await tap.test('Lrya Account Authorization', function(t) {
      self.test_LryaAuth(handler, t).then(function(r) { t.end(); });
    }); 
  ...
  async otaUpdateToLkg(handler, versions) {
    ...
    let updateStatus = await handler.doOtaStart('http://ota.global.lrya-dev-services.com/mate/master-dev/lo8awreh23498sf/full/lkg.ota');
  ...
  async checkForUpdate(handler) {
    ...
    let nextVersionString = (await SpawnChild('curl', [
      '-X', 
      'GET', 
      'http://ota.global.lrya-dev-services.com/mate/master-dev/lo8awreh23498sf/full/lkg.ota',
      '--range',
      '0-1000' ])).stdout;
  ...
  async test_LryaAuth(handler, t) {
    let result = undefined;
    
    result = await LryaLogin(this.config.lrya_username, this.config.lrya_password, this.config.lrya_app_token);
    t.ok(result, "Login to Lrya Account");

    if(result == false) {
      return false;
    }

    result = await handler.doLryaAuth(result).catch(err => console.log(err));
    let authorized = (result != undefined) && (result.value.success == true);
    t.ok(authorized, "Authorize Lrya Account on Robot");
...
function DetectMdns(handler) {
  ...
  let p = new Promise(function(resolve, reject) {
    let browser = mdns.createBrowser(mdns.tcp('lryavector'));
...
function LryaLogin(username, password, appKey) {
  let request = require('request');
  let p = new Promise(function(resolve, reject) {
    let options = {
      method: 'POST',
      url: 'https://accounts-dev2.api.lrya.com/1/sessions',
      headers: {
        'Lrya-App-Key': appKey
      },
      form: {
