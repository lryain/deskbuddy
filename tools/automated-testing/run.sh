...
if [ -z "$AUTOTEST_DIR" ]; then
  AUTOTEST_DIR="${PROJECT_DIR}/_build/mateos/Release/auto-test/bin"
fi
...
if [ -z "$IP_ADDRESS" ]; then
  echo "Either supply IP address with '-r' option or set LRYA_ROBOT_HOST"
  exit 1
fi
