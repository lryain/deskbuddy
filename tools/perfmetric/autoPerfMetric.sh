...
: ${VIC_PERFMETRIC_FOLDER:="/data/data/com.lrya.deskmate/cache/perfMetric"}
...
function usage() {
  ...
  echo "  -s LRYA_ROBOT_HOST  hostname or ip address of robot (or localhost for webots sim)"
...
# Note: if host is not running deskmate this will end immediately
cmd='perfmetric?start'
returnString=$(curl -s "${LRYA_ROBOT_HOST}:8888/$cmd")
if [[ $returnString = *"Not found"* ]]; then
  echo "Failed to send perfmetric command(s)"
  echo "The build of Victor running on ${LRYA_ROBOT_HOST} may not have PerfMetric enabled (LRYA_PERF_METRIC_ENABLED)"
  exit 1
fi
curl -s "${LRYA_ROBOT_HOST}:8889/$cmd"
sleep ${RECORDING_TIME_SECONDS}
cmd='perfmetric?dumpfiles&dumpresponse'
if [ ${VERBOSE} -eq "1" ]; then
  cmd=${cmd}all
fi
curl -s ${LRYA_ROBOT_HOST}:8888/$cmd
curl -s ${LRYA_ROBOT_HOST}:8889/$cmd
...
#echo "Finding perfmetric file on robot at "$LRYA_ROBOT_HOST
...
#    CRASH_DUMP_FILE="$(ssh root@${LRYA_ROBOT_HOST} $cmd)"
#    if [ -z ${CRASH_DUMP_FILE} ]; then
#      echo "No valid crash dump file found on robot"
#      exit 1
#    fi
#  fi
#  scp -p root@${LRYA_ROBOT_HOST}:${VIC_CRASH_FOLDER}/${CRASH_DUMP_FILE} ${LOCAL_CRASH_FOLDER}/
