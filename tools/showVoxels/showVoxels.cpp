...
#include "lrya/common/robot/utilities.h"
...
namespace Lrya
{
  ...
  static int initSDL(const int windowWidth, const int windowHeight)
  {
    if( SDL_Init(SDL_INIT_VIDEO | SDL_INIT_TIMER) < 0 ) {
      Lrya::CoreTechPrint("SDL_Init failed. SDL Error: %s\n", SDL_GetError());
      return -1;
    }
    ...
    if(!sdlWindow) {
      Lrya::CoreTechPrint("SDL_CreateWindow failed. SDL Error: %s\n", SDL_GetError());
      return -2;
    }

    sdlGlContext = SDL_GL_CreateContext(sdlWindow);
    if(!sdlGlContext) {
      Lrya::CoreTechPrint("SDL_GL_CreateContext failed. SDL Error: %s\n", SDL_GetError());
      return -3;
    }
    
    // vsync on
    if( SDL_GL_SetSwapInterval(1) < 0 ) {
      Lrya::CoreTechPrint( "SDL_GL_SetSwapInterval failed. SDL Error: %s\n", SDL_GetError());
      return -4;
    }
    
    voxelMutex = SDL_CreateMutex();
    if (!voxelMutex) {
      Lrya::CoreTechPrint( "SDL_CreateMutex failed. SDL Error: %s\n", SDL_GetError());
      return -5;
    }
  ...
  static int reshapeGL(const int windowWidth, const int windowHeight)
  {
    ...
    if(error != GL_NO_ERROR) {
      Lrya::CoreTechPrint( "Error with glGetError: %s\n", gluErrorString(error) );
      return -1;
    }
  ...
  int gameMain(f64 numSecondsToRun)
  {
    ...
    const f64 startTime = Lrya::Embedded::GetTimeF64();
    
    bool isRunning = true;
    while(isRunning) {
      if(numSecondsToRun > 0 && (Lrya::Embedded::GetTimeF64() - startTime) > numSecondsToRun) {
        isRunning = false;
        continue;
      }
      ...
      while( SDL_PollEvent( &e ) != 0 ) {
        //User requests quit
        if( e.type == SDL_QUIT ) {
          //Lrya::CoreTechPrint("quit event\n");
          isRunning = false;
          break;
        } else if( e.type == SDL_KEYDOWN) {
          Lrya::CoreTechPrint("keydown event\n");
        } else if( e.type == SDL_MOUSEMOTION) {
          //Lrya::CoreTechPrint("SDL_MOUSEMOTION event 0x%x (%d,%d)\n", buttonState, x, y);
          ...
          //Lrya::CoreTechPrint("SDL_MOUSEBUTTONDOWN event 0x%x (%d,%d)\n", buttonState, x, y);
          SDL_PumpEvents();
          int x = 0;
          int y = 0;
          ...
          //Lrya::CoreTechPrint("SDL_MOUSEBUTTONUP event 0x%x (%d,%d)\n", buttonState, x, y);
          lastMouseX = -1;
          lastMouseY = -1;
        } else if( e.type == SDL_MOUSEWHEEL) {
          //Lrya::CoreTechPrint("SDL_MOUSEWHEEL event\n");
          cameraZ += mouseScrollSpeed * e.wheel.y;
          cameraZ = MIN(0, cameraZ);
        }
        else {
          //Lrya::CoreTechPrint("Unknown event 0x%x\n", e.type);
        }
      } // while( SDL_PollEvent( &e ) != 0 )
