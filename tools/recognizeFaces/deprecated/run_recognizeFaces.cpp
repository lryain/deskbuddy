...
/*std::vector<std::vector<cv::Mat_<u8> > > GetTrainingImages()
{
  ...
  const char *baseDirectory = "/Users/pbarnum/Documents/Lrya/products-cozmo-large-files/faceRecognition/lryaPeople/";
...
/*  std::vector<std::vector<cv::Mat_<u8> > > trainingImages = GetTrainingImages();
  ...
  */
  ...
  while(isRunning) {
    ...
    if(ABS(Lrya::Embedded::GetTimeF64() - trainingStartTime) > numSecondsToTrain) {
      trainingOn = false;
    }
    ...
    if((pressedKey & 0xFF) == 'q') {
      ...
      trainingStartTime = Lrya::Embedded::GetTimeF64();
      trainingOn = true;
