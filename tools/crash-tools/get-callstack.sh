...
: ${VERBOSE:="0"}
: ${CRASH_DUMP_FILE:=""}
: ${VIC_CRASH_FOLDER:="/data/data/com.lrya.deskmate/cache/crashDumps"}
...
function usage() {
  ...
  echo "  -c LRYA_BUILD_TYPE  build configuration {Debug,Release}"
...
if [ $PULL_CRASH_FROM_ROBOT -eq "1" ]; then
  if [ -z ${CRASH_DUMP_FILE} ]; then
    echo "Searching for latest crash dump file on robot at "$LRYA_ROBOT_HOST
...
if [ $CALLSTACK_WITH_SYMBOLS -eq "1" ]; then
  ...
  SRC_FOLDER=${TOPLEVEL}/_build/mateos/${LRYA_BUILD_TYPE}
