...
def find_lrya_cmake_exe(version):
    d = toolget.get_lrya_tool_dist_directory(CMAKE)
...
def install_cmake(version):
    ...
    cmake_downloads_path = toolget.get_lrya_tool_downloads_directory(CMAKE)
    cmake_dist_path = toolget.get_lrya_tool_dist_directory(CMAKE)
...
def find_cmake(required_ver, cmake_exe=None):
    ...
    cmake_exe = find_lrya_cmake_exe(required_ver)
    if get_cmake_version_from_command(cmake_exe) == required_ver:
        return cmake_exe
...
def find_or_install_cmake(required_ver, cmake_exe=None):
    ...
    return find_lrya_cmake_exe(required_ver)
