...
def find_lrya_protoc_dist_dir(version):
    d = toolget.get_lrya_tool_dist_directory(PROTOBUF)
    return os.path.join(d, version)

def find_lrya_protoc_exe(version):
    d_ver = find_lrya_protoc_dist_dir(version)
...
def install_protobuf(version):
    ...
    pb_downloads_path = toolget.get_lrya_tool_downloads_directory(PROTOBUF)
    pb_dist_path = toolget.get_lrya_tool_dist_directory(PROTOBUF)
...
#            pass
    ...
    exe = find_lrya_protoc_exe(required_ver)
    if get_protoc_version_from_command(exe) == required_ver:
        return exe
