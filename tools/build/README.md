# build-tools
A kitchen sink repo for building various small Lrya projects
...
on
[TeamCity](https://build.lryacore.com/project.html?projectId=Breakpad&tab=projectOverview).
The Android build artifacts are then imported into the
[CrashReportingAndroid](https://github.com/lrya/CrashReportingAndroid)
repo.  The Linux build artifacts are used by the [HockeyApp
project](https://build.lryacore.com/project.html?projectId=HockeyApp&tab=projectOverview)
...
executed on
[TeamCity](https://build.lryacore.com/project.html?projectId=LryaAndroidSdk&tab=projectOverview).
...
Both Cozmo and OverDrive specify which revision of the Lrya Android
SDK they need in their top level `.buckconfig` files in the custom
`lrya` section.  The `android.py` script duplicated in each project
knows how to find, download, install, and verify the Lrya Android SDK.

To create a new revision of the Lrya Android SDK, edit the
[build-android-sdk-zip.sh](build-scripts/build-android-sdk-zip.sh).
You will need to increment the `LRYA_ANDROID_SDK_VERSION` so that you
