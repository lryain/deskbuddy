...
LRYA_ANDROID_SDK_NAME=android-sdk-${REPO_OS_OVERRIDE}-lrya-${LRYA_ANDROID_SDK_VERSION}
LRYA_ANDROID_SDK_DIR=${BUILD_DIR}/${LRYA_ANDROID_SDK_NAME}

rm -rf ${BUILD_DIR}
mkdir -p ${LRYA_ANDROID_SDK_DIR}

TOOLS_ZIP_NAME=tools_${TOOLS_VERSION}-${REPO_OS_OVERRIDE}.zip
TOOLS_ZIP_PATH=${LRYA_ANDROID_SDK_DIR}/${TOOLS_ZIP_NAME}
...
unzip -q -d ${LRYA_ANDROID_SDK_DIR} ${TOOLS_ZIP_PATH}
rm ${TOOLS_ZIP_PATH}
...
set timeout -1
spawn ${LRYA_ANDROID_SDK_DIR}/tools/bin/sdkmanager ${PACKAGES}
expect "Accept? (y/N):"
send "y\r"
expect eof
EOS

mkdir -p ${LRYA_ANDROID_SDK_DIR}/lrya/bin
...
cp -pv `./bin/buck targets --show_output buck | awk '{print $2;}'` \
   ${LRYA_ANDROID_SDK_DIR}/lrya/bin/buck
popd
popd


# Write out package info
echo "Pkg.Desc = Android SDK" > ${LRYA_ANDROID_SDK_DIR}/lrya/source.properties
echo "Pkg.Revision = ${LRYA_ANDROID_SDK_VERSION}" >> ${LRYA_ANDROID_SDK_DIR}/lrya/source.properties

LRYA_ANDROID_SDK_DIST_ZIP=${DIST_DIR}/${LRYA_ANDROID_SDK_NAME}.zip

mkdir ${DIST_DIR}
pushd ${BUILD_DIR}
echo "Making ${LRYA_ANDROID_SDK_DIST_ZIP}...."
zip -r ${LRYA_ANDROID_SDK_DIST_ZIP} ${LRYA_ANDROID_SDK_NAME}
popd
...
SHA1SUM=`shasum -a 1 ${LRYA_ANDROID_SDK_DIST_ZIP} | awk '{print $1;}'`
SIZE=`stat $STATARG $STATFMT ${LRYA_ANDROID_SDK_DIST_ZIP}`

cat <<EOF
sdk_info =
{
  "$LRYA_ANDROID_SDK_VERSION" : {"size": $SIZE, "sha1": "$SHA1SUM"},
}
EOF

rm -rf ${LRYA_ANDROID_SDK_DIR}
