...
if (NOT ${GO_VERSION_MATCH} STREQUAL ${LRYA_GO_VERSION})
  message(FATAL_ERROR "Detected version of Go is ${GO_VERSION_MATCH}, but we want ${LRYA_GO_VERSION}\n"
...
file(WRITE ${__GO_VERSION_TMP} ${LRYA_GO_VERSION})
...
macro(__lrya_setup_go_environment target_basedir)
  ...
  if (LRYA_DEV_CHEATS EQUAL 0)
    list(APPEND __go_build_tags "shipping")
  endif()

  if (MATEOS)
    # set mateos flags for `go build`
    list(APPEND __go_compile_env "GOOS=linux")
    list(APPEND __go_compile_env "GOARCH=arm")
    list(APPEND __go_compile_env "GOARM=7")
    list(APPEND __go_compile_env "CC=${MATEOS_C_COMPILER}")
    list(APPEND __go_compile_env "CXX=${MATEOS_CXX_COMPILER}")
...
macro(__lrya_setup_go_build target_name)
  ...
  if (MATEOS)
    ...
    set(__go_platform_ldflags "-r /lrya/lib -compressdwarf=false")
  elseif (MACOSX)
...
macro(__lrya_run_go_build extra_deps)
  add_custom_command(
...
macro(__lrya_run_go_test package_name extra_deps)
  add_custom_command(
    OUTPUT ${__gobuild_out}
    COMMAND ${CMAKE_COMMAND} -E env ${__go_compile_env} ${__cppflags_env} ${__ldflags_env} ${__cxxflags_env}
...
macro(__lrya_build_go_fake_target target_name)
  ...
  lrya_build_target_license(${target_name}_fake_dep "LRYA")
endmacro()
...
macro(lrya_build_go_mateos_strip output output_full)
  add_custom_command(
    OUTPUT ${output_full}
    DEPENDS ${output}
...
# usage: lrya_build_go_c_library(mytarget generated_header_variable "path/to/source/directory"
#                                "path/to/GOPATH/directory" ${LRYA_SRCLIST_DIR})
macro(lrya_build_go_c_library target_name gensrc_var srclist_dir extra_deps)
  ...
  __lrya_setup_go_environment(${_ab_GO_DIR})
  ...
  __lrya_build_go_fake_target(${target_name})

  set_target_properties(${target_name} PROPERTIES GO_CLINK_FOLDERS "")

  __lrya_setup_go_build(${target_name})
  __lrya_run_go_build("${extra_deps}")
...
# usage: lrya_build_go_executable(mytarget "path/to/source/directory" "path/to/GOPATH/directory" ${LRYA_SRCLIST_DIR})

macro(lrya_build_go_executable target_name srclist_dir extra_deps)
  lrya_build_source_list(${target_name} ${srclist_dir})
  ...
  #
  # On mateos, generate additional commands to strip executable.
  ...
  if (MATEOS)
    ...
    lrya_build_go_mateos_strip(${__gobuild_out} ${__gobuild_out_full})
  endif()
...
macro(lrya_go_add_c_library target_name c_target)
...
macro(lrya_go_add_include_dir target_name include_dir)
  target_include_directories(${target_name}_fake_dep PUBLIC ${include_dir})
endmacro()

macro(lrya_go_set_ldflags target_name flags)
  set_target_properties(${target_name} PROPERTIES GO_LDFLAGS "${flags}")
endmacro()

macro(lrya_go_set_cgo_cppflags target_name flags)
  set_target_properties(${target_name} PROPERTIES CGO_CPPFLAGS "${flags}")
endmacro()

macro(lrya_go_set_cgo_ldflags target_name flags)
...
macro(lrya_go_add_test_dir dir_name srclist_dir created_targets_var extra_deps)
  ...
  foreach(i ${__gotest_packages})
    lrya_go_add_test(${i} ${srclist_dir} "${extra_deps}")
  endforeach(i)
  set(${created_targets_var} ${__gotest_added_targets})
endmacro()

# add test executable for a given package
macro(lrya_go_add_test package_name srclist_dir extra_deps)
  ...
  lrya_build_go_test_exe(${package_name} "gotest_${__gotest_unslashed}" "${extra_deps}")
  list(APPEND __gotest_added_targets "gotest_${__gotest_unslashed}")

endmacro()

# set up test executable build
macro(lrya_build_go_test_exe package_name target_name extra_deps)
  ...
  __lrya_setup_go_environment(${_ab_GO_DIR})
  __lrya_setup_go_build(${target_name})
  __lrya_run_go_test(${package_name} "${extra_deps}")
  # test executables don't need license
  lrya_build_target_license(${target_name} "LRYA")
