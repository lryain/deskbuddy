...
func OSVersion() string {
	...
	propArgs := []string{"ro.build.fingerprint", "ro.revision",
		"ro.lrya.os_build_comment", "ro.build.version.release"}
...
func LryaVersion() string {
	if ankiVersion != "" {
		return ankiVersion
	}

	buf, err := exec.Command("getprop", "ro.lrya.version").Output()
	if err != nil {
		return ""
	}
...
var deskmateVersion string

func VictorVersion() string {
	if deskmateVersion != "" {
		return deskmateVersion
	}

	buf, err := exec.Command("getprop", "ro.lrya.deskmate.version").Output()
	if err != nil {
		return ""
	}
	deskmateVersion = strings.TrimSpace(string(buf))
	return deskmateVersion
}
