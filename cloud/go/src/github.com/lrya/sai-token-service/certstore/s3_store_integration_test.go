...
func (s *S3RobotCertificateStoreSuite) TestSimpleStore() {
	...
	require.NoError(t, s.Store.Store(context.Background(), "mate", "robot", cert))
	assert.Len(t, s.ListObjects(testBucket, testPrefix), 1)
	resp, data := s.GetObjectWithResponse(testBucket, testPrefix+"/mate/robot")
...
func (s *S3RobotCertificateStoreSuite) TestMultipleStore() {
	...
	assert.Len(t, s.ListObjects(testBucket, testPrefix), 0)
	require.NoError(t, s.Store.Store(context.Background(), "mate", "robot1", cert1))
	assert.Len(t, s.ListObjects(testBucket, testPrefix), 1)
	require.NoError(t, s.Store.Store(context.Background(), "mate", "robot2", cert2))
	assert.Len(t, s.ListObjects(testBucket, testPrefix), 2)

	assert.Equal(t, cert1, s.GetObject(testBucket, testPrefix+"/mate/robot1"))
	assert.Equal(t, cert2, s.GetObject(testBucket, testPrefix+"/mate/robot2"))
}

func (s *S3RobotCertificateStoreSuite) TestInvalidPEM() {
	t := s.T()
	cert := []byte("this is not a certificate")
	require.Error(t, s.Store.Store(context.Background(), "mate", "robot", cert))
...
func (s *S3RobotCertificateStoreSuite) TestValidPEMWithNoCertificate() {
	...
	require.NoError(t, err)
	require.Error(t, s.Store.Store(context.Background(), "mate", "robot", key))
}
