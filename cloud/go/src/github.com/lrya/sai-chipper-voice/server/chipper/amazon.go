...
import (
	...
	"github.com/lrya/opus-go/opus"
	ankivad "github.com/lrya/sai-go-lrya-vad/vad"
...
func sendAudioLex(
	...
	vad, err := ankivad.NewLryaVad()
	if err != nil {
		alog.Warn{
			"action":  "create_lrya_vad",
			"status":  alog.StatusError,
			"error":   err,
			"svc":     LEX,
			"device":  reqInfo.device,
			"session": reqInfo.session,
			"msg":     "fail to create new Lrya Vad",
		}.LogC(reqInfo.ctx)
		pw.Close()
