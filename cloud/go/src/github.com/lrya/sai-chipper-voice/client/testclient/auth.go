...
import (
	...
	pb "github.com/lrya/sai-token-service/proto/tokenpb"
...
func sessionMetadata(sessionToken string, appKey string) credentials.PerRPCCredentials {
	metadata := MapCredentials{
		"lrya-user-session": sessionToken,
		"lrya-app-key":      appKey,
	}
	return metadata
}
...
func getSessionToken(ctx context.Context, auth AuthConfig) (string, error) {
	...
	resp, err := c.NewUserSession(ctx, *auth.LryaUsername, *auth.LryaPassword)
	if err != nil {
