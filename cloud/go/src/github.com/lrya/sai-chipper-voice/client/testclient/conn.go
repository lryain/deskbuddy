...
func (authCfg *AuthConfig) CommandInitializ(cmd *cli.Cmd) {
	authCfg.AccountsEnv = svc.StringOpt(cmd, "accounts-env", "", "env for lrya account (prod/dev/beta)")
	authCfg.LryaUsername = svc.StringOpt(cmd, "lrya-user-name", "", "lrya account username")
	authCfg.LryaPassword = svc.StringOpt(cmd, "lrya-password", "", "lrya account password")
