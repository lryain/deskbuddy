...
1. Get  Dialogflow service accounts credentials from the team vault:
   - download `deskmate-dev-en-us-e54e8e28b8f5.json` and `deskmate-games-dev-en-us-ba82892ccc47.json` to the chipper repo dir
...
3. Set up your AWS credentials for the main Lrya account in `./aws/credentials`
...
2. Build the docker container:
   - make sure you have access to Lrya's Docker Hub
   - `docker pull lrya/busybox` if you haven't done so
   - add these two lines to your local `Dockerfile`, **do not commit** this updated file to git.
     - `ADD deskmate-dev-en-us-e54e8e28b8f5.json /tmp/deskmate-dev-en-us-e54e8e28b8f5.json`
     - `ADD deskmate-games-dev-en-us-ba82892ccc47.json /tmp/deskmate-games-dev-en-us-ba82892ccc47.json`
   - run `mkdir -p bin` in the chipper repo directory
   - run `make docker`

3. **could be obselete** Run `docker run -v /etc/ssl/certs:/etc/ssl/certs --env-file [directory]/chipper_local_env_var.list -it -p 10000:10000 lrya/sai-chipper-voice:[branch-name]`
