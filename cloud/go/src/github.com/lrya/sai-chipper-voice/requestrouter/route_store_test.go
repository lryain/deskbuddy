...
func (s *RouteStoreSuite) TestStoreVersion() {
	...
	fwVer := CreateVersion("dialogflow", "en", "vc", normFw, "deskmate-test", dfVer, "df-creds", "", "", true)
...
func (s *RouteStoreSuite) TestGetEnabledVersion() {
	...
	for _, tc := range testCases {
		...
		if tc.svc == "df" {
			fwVer = CreateVersion(tc.svc, "en", "vc", tc.fw, "mate-test", tc.ver, "creds", "", "", tc.enable)
		} else {
			fwVer = CreateVersion(tc.svc, "en", "vc", tc.fw, "", "", "", "mate-test", tc.ver, tc.enable)
		}
		t.Log(fwVer)
		assert.NotNil(t, fwVer)
