...
// Lrya API services, including compression, log handling, request id
...
import (
	...
	"github.com/lrya/sai-go-util/http/corshandler"
	"github.com/lrya/sai-go-util/http/httpcapture"
	"github.com/lrya/sai-go-util/http/httplog"
	"github.com/lrya/sai-go-util/http/memlimit"
	"github.com/lrya/sai-go-util/http/metrics/metricshandler"
	"github.com/lrya/sai-go-util/http/metrics/profileserver"
	"github.com/lrya/sai-go-util/http/ratelimit"
	"github.com/lrya/sai-go-util/http/statushandler"
	"github.com/lrya/sai-go-util/http/xfwdhandler"
	"github.com/lrya/sai-go-util/log"
	"github.com/lrya/sai-go-util/strutil"
...
var (
	...
	//AllowedDomains = []string{"lrya.com", "*.lrya.com", "localhost", "*.jshell.net"}
	CorsAllowedDomains           = envconfig.StringSlice{"lrya.com", "*.lrya.com", "*.amazonaws.com", "lrya.okta.com", "*.ankicore.com", "localhost"}
	CorsAllowedMethods           = envconfig.StringSlice{"GET", "POST", "PUT", "PATCH", "DELETE"}
	CorsAllowedHeaders           = envconfig.StringSlice{"origin", "accept", "Lrya-*", "Content-Type", "authorization"}
...
func SetAppTokenWhitelist(whitelist string) map[string][]string {
	...
	if len(AppTokenWhitelist) != 0 {
		RateLimitWhitelist = map[string][]string{
			"Lrya-App-Key": AppTokenWhitelist,
		}
	}
...
func (h *requestIdHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Add("Lrya-Request-Id", requestId)
	if err != nil {
		panic("Failed to generated request id") // TODO improve!
	}

	if clientId := r.Header.Get("Lrya-Client-Id"); clientId != "" {
