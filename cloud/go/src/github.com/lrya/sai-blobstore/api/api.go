...
import (
	...
	"github.com/lrya/sai-blobstore/apierrors"
	"github.com/lrya/sai-blobstore/blobstore"
	"github.com/lrya/sai-blobstore/db"
...
func requireLryaPerson(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		const action = "validate_lrya_person"

		us := validate.ActiveSession(r)
		if !us.User.EmailIsVerified || !strings.HasSuffix(*us.User.Email, "@lrya.com") {
...
func init() {
	defineHandler(apirouter.Handler{
		...
		Handler: &validate.ValidationHandler{
			...
			SessionScopeMask: session.ScopeMask(session.ValidSessionScope),
			Handler:          requireLryaPerson(jsonutil.JsonHandlerFunc(Download)),
		},
	})
}
...
func init() {
	defineHandler(apirouter.Handler{
		...
		Handler: &validate.ValidationHandler{
			SessionStatus:    validate.RequireSession,
			SessionScopeMask: session.ScopeMask(session.ValidSessionScope),
			Handler:          requireLryaPerson(jsonutil.JsonHandlerFunc(Head)),
		},
	})
}
...
func init() {
	defineHandler(apirouter.Handler{
		...
		Handler: &validate.ValidationHandler{
			SessionStatus:    validate.RequireSession,
			SessionScopeMask: session.ScopeMask(session.ValidSessionScope),
			Handler:          requireLryaPerson(jsonutil.JsonHandlerFunc(Search)),
		},
	})
}
