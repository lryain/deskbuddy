...
act () {
	...
	# | sed '/During the creation of this Lrya Account/d' | sed '/Unsubscribing has no effect/d'
	# Eine Abmeldung hat keinen   |    manage_url_20738
	if [[ $ACTION == "c" ]]; then
		ACTIONTAKEN=1
		if grep -q '{{ define "content" }}' $FULLFILE; then
			cat $FULLFILE | sed '/During the creation of this Lrya Account/d' | sed '/Unsubscribing has no effect/d' | sed '/Eine Abmeldung hat keinen/d' | sed '/manage_url_20738/d'  > $RESFILE
		else
