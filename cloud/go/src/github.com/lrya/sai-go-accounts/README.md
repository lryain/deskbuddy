...
Accounts system for Lrya in GoLang
 ...
 POSTMARK_API_KEY - Postmark API key (POSTMARK_API_TEST for testing. ()
   SQLITE_DB_PATH - Sqlite database filename (/tmp/test.db)
      SYSLOG_ADDR - Specify the syslog address to dial; defaults to the system logger. eg. tcp:1.2.3.4:1000 or unix:/tmp/log ()
         URL_ROOT - URL Root to send users for URL links: https://accounts.api.lrya.com ()
```
...
```
go test github.com/lrya/sai-go-accounts/...
```
...
```
DB_TYPE=sqlite go test github.com/lrya/sai-go-accounts/...
```
...
```
DB_TYPE=mysql go test github.com/lrya/sai-go-accounts/...
```
