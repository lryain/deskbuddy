...
import (
	...
	"github.com/lrya/sai-go-accounts/apis"
	"github.com/lrya/sai-go-accounts/db"
	...
	"github.com/lrya/sai-go-accounts/models/user/usertest"
	"github.com/lrya/sai-go-accounts/testutil"
	"github.com/lrya/sai-go-accounts/util"
	autil "github.com/lrya/sai-go-util"
	"github.com/lrya/sai-go-util/jsonutil"
	"github.com/lrya/sai-go-util/postmarkapp"
	"github.com/lrya/sai-go-util/timedtoken"
	"github.com/lrya/sai-go-util/validate"
	"github.com/lrya/sai-go-util/validate/validtest"
...
func (s *UsersAPITestSuite) TestCreateUserResponse() {
	...
	for i, test := range createUserTests {
		...
		if test.expectedError == nil {
			...
			if e.Headers["X-Lrya-Tmpl"][0] != "Verification" {
...
func (s *UsersAPITestSuite) TestUpdateUserAPIEmailVerification() {
	...
	for i, test := range updateUserCheckEmailTests {
		...
		if test.expectedEmail {
			...
			if e.Headers["X-Lrya-Tmpl"][0] != test.emailName {
				// failing with Verification not equal to VertificationUnder13
				t.Errorf("Email Header Mismatch '%s': '%v'", test.emailName, e.Headers["X-Lrya-Tmpl"][0])
			}
		} else {
...
func (s *UsersAPITestSuite) TestAccountUpdatedEmails() {
	...
	e = getEmail(t, true)
	cont, _ := emailContainsUrl(e, "An Lrya account associated with this email address has been updated.")
	if !cont {
