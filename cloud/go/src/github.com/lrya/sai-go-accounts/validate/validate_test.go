...
import (
	...
	_ "github.com/lrya/sai-go-accounts/testutil"
)
...
func TestValidationHandler(t *testing.T) {
	for _, test := range validationTests {
		...
		if test.returnApiScope != nil {
			r.Header.Set("Lrya-App-Key", "key-token")
		}

		r.Header.Set("Authorization", "Lrya sessiontoken")
...
func TestAccountsServerCacheHit(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Add("Lrya-Request-Id", "Remote-Id")
		if len(responses) > 0 {
			resp := responses[0]
