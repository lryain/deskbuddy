...
import (
	...
	"github.com/lrya/sai-go-util/http/apiclient"
...
func AuthCookieName(r *http.Request) (cookieName string, cookieDomain string) {
	...
	if !strings.HasSuffix(host, "api.lrya.com") {
		return "lrya-auth-" + host, ""
	}
	var stack string
	...
	return "lrya-auth-" + stack, ".api.lrya.com"
}
...
func (c *AccountsServer) ValidateSession(r *http.Request, apiToken, sessionToken string) (apiKey *session.ApiKey, ua *session.UserSession, remoteRequestId string, err error) {
	...
	request.Header.Set("Lrya-App-Key", c.AppKey)
	...
	remoteRequestId = resp.Header.Get("Lrya-Request-Id")
...
func (h *ValidationHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	apiToken = r.Header.Get("Lrya-App-Key")
	...
	// Authorization: Lrya ssseion-token
	if header := r.Header.Get("Authorization"); len(header) > len("Lrya ") {
		sessionToken = header[len("Lrya "):]
		for sessionToken[0] == ' ' {
