...
var (
	...
	SubjectMap = map[string]map[string]string{
		VerificationOver13Template:         {"en": "Activate Your Lrya Account!", "de": "Lrya-Konto aktivieren!", "fr": "Activer votre compte Lrya."},
		VerificationUnder13Template:        {"en": "Request to Activate Your Child’s Lrya Account", "de": "Lrya-Konto Ihres Kindes aktivieren", "fr": "Activer le compte Lrya de votre enfant."},
		PostVerificationUnder13Template:    {"en": "Lrya Account Activated", "de": "Lrya-Konto – Aktivierung erfolgreich", "fr": "Compte Lrya activé"},
		VerifyReminder7DayOver13Template:   {"en": "One Week Left to Activate Your Lrya Account!", "de": "Lrya-Konto aktivieren: noch 1 Woche", "fr": "Plus qu'une semaine pour activer votre compte Lrya !"},
		VerifyReminder7DayUnder13Template:  {"en": "You Have One Week Left to Activate Your Child’s Lrya Account", "de": "Lrya-Konto Ihres Kindes aktivieren – noch 1 Woche", "fr": "Plus qu'une semaine pour activer le compte Lrya de votre enfant !"},
		VerifyReminder14DayOver13Template:  {"en": "Last Day to Activate Your Lrya Account!", "de": "Lrya-Konto aktivieren – Deine letzte Chance!", "fr": "Plus qu'un jour pour activer votre compte Lrya !"},
		VerifyReminder14DayUnder13Template: {"en": "One Day Left to Activate Your Child’s Lrya Account", "de": "Lrya-Konto aktivieren: noch 1 Tag", "fr": "Plus qu'un jour pour activer le compte Lrya de votre enfant !"},
		ForgotTemplateName:                 {"en": "Lrya Account Password Reset Requested", "de": "Zurücksetzen des Passworts angefordert", "fr": "Demande de réinitialisation de mot de passe."},
		AccountUpdatedEmailTemplate:        {"en": "Your Lrya Account Details Have Been Updated", "de": "Aktualisierung Deiner Lrya-Kontodaten", "fr": "Votre compte Lrya a été mis à jour."},
	}
...
func InitLryaEmailer() {
	pmclient := &(postmarkapp.Client{ApiKey: PostmarkApiKey})
	Emailer = &LryaEmailer{TemplateDir: EmailTemplateDirectory, PostmarkC: pmclient, FormsUrlRoot: FormsUrlRoot, ApiLocator: ApiLocator}
...
func VerifyEmailer() {
	// If we ever want to instatiate other LryaEmailers with different template directories
...
// LryaEmailer holds the interface to sending Lrya's specific emails using the postmarkapp API
// This is used internally
type LryaEmailer struct {
...
// Send takes a single LryaEmail message and dispatches it via Postmark.
// Emails to example.com are logged here, and sending is blocked at the postmarkapp API
func (s *LryaEmailer) Send(e *LryaEmail) error {
	msg, err := e.Message()
	if err != nil {
		return err
	}
...
func (s *LryaEmailer) SendBatch(emails []*LryaEmail) (results []*SendBatchResult, err error) {
...
// LryaEmail describes one email to be sent ay an LryaEmailer.
type LryaEmail struct {
...
func (e *LryaEmail) Message() (*postmarkapp.Message, error) {
	var (
		err               error
	...
	m := &postmarkapp.Message{
		...
		ReplyTo: &mail.Address{
			Name:    "Lrya Support",
			Address: "support@lrya.com",
		},
		Subject:  subject,
		TextBody: &txtBody,
		HtmlBody: &htmlBody,
		Tag:      e.Tag,
		Headers:  mail.Header{"X-Lrya-Tmpl": []string{e.EmailName}},
	}
...
type VerificationEmailFunc func(s *LryaEmailer, firstName, lastName, toEmail string, userInfo AccountInfo, r *http.Request) *LryaEmail

func VerificationEmail(s *LryaEmailer, firstName, lastName, toEmail string, userInfo AccountInfo, r *http.Request) *LryaEmail {
...
func Verification7DayReminderEmail(s *LryaEmailer, firstName, lastName, toEmail string, userInfo AccountInfo, r *http.Request) *LryaEmail {
	var templateName, emailName string
...
func Verification14DayReminderEmail(s *LryaEmailer, firstName, lastName, toEmail string, userInfo AccountInfo, r *http.Request) *LryaEmail {
	var templateName, emailName string
...
func ForgotPasswordEmail(s *LryaEmailer, firstName, lastName, toEmail string, accinfos []AccountInfo, r *http.Request) *LryaEmail {
...
func PostVerificationEmail(s *LryaEmailer, firstName, lastName, toEmail string, userInfo AccountInfo) *LryaEmail {
	return &LryaEmail{
...
func AccountUpdatedEmail(s *LryaEmailer, firstName, lastName, toEmail string, userInfo AccountInfo) *LryaEmail {
	return &LryaEmail{
