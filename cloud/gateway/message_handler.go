...
func (service *rpcService) checkConnectionID(id string) bool {
	...
	if IsOnRobot {
		...
		if !ok {
			log.Println("Failed to receive ConnectionID response from mate-switchboard")
			return false
		}
...
func (service *rpcService) GetUpdateStatus() (*extint.CheckUpdateStatusResponse, error) {
	...
	// If /run/update-engine/done exists that means that /lrya/bin/update-engine
...
func (service *rpcService) StartUpdateEngine(
	...
	// /lrya/bin/update-engine is running.  By the way, /lrya/bin/update-engine will NOT
...
// CheckCloudConnection is used to verify Vector's connection to the Lrya Cloud
...
func (service *rpcService) NavMapFeed(in *extint.NavMapFeedRequest, stream extint.ExternalInterface_NavMapFeedServer) error {
	...
	// Every frame the engine can send up to: (Lrya::Comms::MsgPacket::MAX_SIZE-3)/sizeof(QuadInfoVector::value_type) quads.
