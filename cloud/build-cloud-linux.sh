...
function usage() {
    ...
    echo "  -n [TARGET_NAME]        name for the target (executable), e.g.: 'mate-cloud' or 'logcollectorcli'"
}
...
export GOPATH=${SCRIPT_PATH}/cloud/go:${SCRIPT_PATH}/generated/cladgo:${SCRIPT_PATH}/generated/go:${SCRIPT_PATH}/deskmate-clad/tools/message-buffers/support/go
export CGO_FLAGS=-g
export CGO_CPPFLAGS=-I\ ${SCRIPT_PATH}/lib/util/source/lrya/util/..\ -I\ /usr/include/opus\ -I\ ${SCRIPT_PATH}
...
if [ $COMMAND = "test" ]; then
    ...
    go ${COMMAND} \
        ...
        -ldflags -X\ \'lrya/voice.ChipperSecret=${CHIPPER_SECRET}\'\ \
        ./${TARGET_DIR}
else
    go ${COMMAND} \
        -tags shipping \
