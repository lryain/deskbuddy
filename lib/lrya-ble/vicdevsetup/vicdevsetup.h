...
class VicDevSetup : public Lrya::BluetoothDaemon::IPCClient {
 public:
  VicDevSetup(struct ev_loop* loop)
      : IPCClient(loop)
      , connection_id_(-1)
      , heartbeat_count_(1)
      , robot_name_(Lrya::GetRobotName())
 ...
 private:
  ...
  void SendMessageToConnectedCentral(const std::vector<uint8_t>& value,
                                     const std::string& char_uuid = Lrya::kAppReadCharacteristicUUID);
  ...
  Lrya::ExecCommandCallback send_output_callback_;
};
