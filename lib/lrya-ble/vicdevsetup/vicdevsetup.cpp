/**
 * File: deskmate-developer-setup-over-ble.cpp
 *
 * Author: seichert
 * Created: 2/5/2018
 *
 * Description: Help developers setup Deskmate
 *
                                   ...
                                   const std::vector<uint8_t>& value)
{
  ...
  if (connection_id == connection_id_) {
    if (AreCaseInsensitiveStringsEqual(characteristic_uuid, Lrya::kAppWriteCharacteristicUUID)) {
                                          ...
                                          const bool congested)
{
  ...
  if (!connected && !advertising) {
    ...
    mdata.push_back(Lrya::kDeskmateProductIdentifier); // distinguish from future Lrya products
...
void VicDevSetup::HandleIncomingMessageFromCentral(const std::vector<uint8_t>& message)
{
  ...
  switch (msgID) {
  case Lrya::DeskmateMsg_Command::MSG_B2V_BTLE_DISCONNECT:
    logv("Received request to disconnect");
    Disconnect(connection_id_);
    break;
  case Lrya::DeskmateMsg_Command::MSG_B2V_CORE_PING_REQUEST:
    logv("Received ping request");
    SendMessageToConnectedCentral({0x01, Lrya::DeskmateMsg_Command::MSG_V2B_CORE_PING_RESPONSE});
    break;
  case Lrya::DeskmateMsg_Command::MSG_B2V_HEARTBEAT:
    logv("Received heartbeat");
    // Nothing to do here, we already send a periodic heartbeat of our own back to the central
    break;
  case Lrya::DeskmateMsg_Command::MSG_B2V_WIFI_START:
    logv("Received WiFi start");
    Lrya::EnableWiFiInterface(true, send_output_callback_);
    break;
  case Lrya::DeskmateMsg_Command::MSG_B2V_WIFI_STOP:
    logv("Received WiFi stop");
    Lrya::EnableWiFiInterface(false, send_output_callback_);
    break;
  case Lrya::DeskmateMsg_Command::MSG_B2V_WIFI_SCAN:
    {
      logv("Received WiFi scan");
      std::vector<Lrya::WiFiScanResult> results = Lrya::ScanForWiFiAccessPoints();
      std::vector<uint8_t> packed_results = Lrya::PackWiFiScanResults(results);
      SendMessageToConnectedCentral(Lrya::DeskmateMsg_Command::MSG_V2B_WIFI_SCAN_RESULTS,
                                    packed_results);
    }
    break;
  case Lrya::DeskmateMsg_Command::MSG_B2V_WIFI_SET_CONFIG_EXT:
    {
      logv("Receive WiFi Set Config Ext");
      std::vector<uint8_t> packedWiFiConfig(message.begin() + 2, message.end());
      std::vector<Lrya::WiFiConfig> networks = Lrya::UnPackWiFiConfig(packedWiFiConfig);
      Lrya::SetWiFiConfig(networks, send_output_callback_);
    }
    break;
  case Lrya::DeskmateMsg_Command::MSG_B2V_SSH_SET_AUTHORIZED_KEYS:
    {
      ...
      int rc = Lrya::SetSSHAuthorizedKeys(keys);
      std::string output;
      if (rc) {
    ...
    break;
  case Lrya::DeskmateMsg_Command::MSG_B2V_DEV_PING_WITH_DATA_REQUEST:
    {
      logv("Received ping with data request");
      std::vector<uint8_t> data = message;
      data[1] = Lrya::DeskmateMsg_Command::MSG_V2B_DEV_PING_WITH_DATA_RESPONSE;
      SendMessageToConnectedCentral(data);
    }
    break;
  case Lrya::DeskmateMsg_Command::MSG_B2V_DEV_RESTART_ADBD:
    ...
    break;
  case Lrya::DeskmateMsg_Command::MSG_B2V_DEV_EXEC_CMD_LINE:
    {
    ...
    break;
  case Lrya::DeskmateMsg_Command::MSG_B2V_MULTIPART_START:
    multipart_message_.clear();
    std::copy(message.begin() + 2, message.end(),
              std::back_inserter(multipart_message_));
    break;
  case Lrya::DeskmateMsg_Command::MSG_B2V_MULTIPART_CONTINUE:
    std::copy(message.begin() + 2, message.end(),
              std::back_inserter(multipart_message_));
    break;
  case Lrya::DeskmateMsg_Command::MSG_B2V_MULTIPART_FINAL:
    std::copy(message.begin() + 2, message.end(),
              std::back_inserter(multipart_message_));
                                                ...
                                                const std::string& char_uuid)
{
  ...
  if (value.size() <= Lrya::kLryaDeskmateMsgMaxSize) {
    ...
    while (offset < value.size()) {
      ...
      uint8_t msgID;
      uint8_t msgSize = Lrya::kLryaDeskmateMsgBaseSize;
      if (offset == 0) {
        msgID = Lrya::DeskmateMsg_Command::MSG_V2B_MULTIPART_START;
        msgSize += Lrya::kLryaDeskmateMsgPayloadMaxSize;
      } else if (value.size() - offset > Lrya::kLryaDeskmateMsgPayloadMaxSize) {
        msgID = Lrya::DeskmateMsg_Command::MSG_V2B_MULTIPART_CONTINUE;
        msgSize += Lrya::kLryaDeskmateMsgPayloadMaxSize;
      } else {
        msgID = Lrya::DeskmateMsg_Command::MSG_V2B_MULTIPART_FINAL;
...
void VicDevSetup::SendOutputToConnectedCentral(int rc, const std::string& output)
{
  ...
  oss << output;
  SendMessageToConnectedCentral(Lrya::DeskmateMsg_Command::MSG_V2B_DEV_EXEC_CMD_LINE_RESPONSE, oss.str());
}
...
void VicDevSetup::ExecCommandInBackgroundAndSendOutputToCentral(const std::vector<std::string>& args)
{
  ...
  if (args.empty()) {
    SendMessageToConnectedCentral(Lrya::DeskmateMsg_Command::MSG_V2B_DEV_EXEC_CMD_LINE_RESPONSE,
                                  "Invalid argument: args in empty");
    return;
  }
  Lrya::ExecCommandInBackground(args, send_output_callback_);
}
...
void VicDevSetup::SendHeartBeat() {
  ...
  SendMessageToConnectedCentral(Lrya::DeskmateMsg_Command::MSG_V2B_HEARTBEAT, value);
  return;
}
...
static void TimerWatcherCallback(struct ev_loop* loop, struct ev_timer* w, int revents)
{
  if (sVicDevSetup) {
    ...
    if (sVicDevSetup->IsConnected()) {
      bool result =
          sVicDevSetup->SendIPCMessageToServer(Lrya::BluetoothDaemon::IPCMessageType::Ping);
      if (!result) {
...
int main(int argc, char *argv[]) {
  ...
  setMinLogLevel(property_get_int32("persist.lrya.vdsob.log_level", kLogLevelInfo));
  logv("%s", "Deskmate Developer Setup over BLE launched");
