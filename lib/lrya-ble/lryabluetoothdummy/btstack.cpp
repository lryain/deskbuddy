 ...
 * Created: 1/10/2018
 *
 * Description: Bluetooth Stack functions for Lrya Bluetooth Daemon
 *
...
typedef struct BluetoothGattConnectionInfo {
  Lrya::Util::CodeTimer::TimePoint start;
  ...
  BluetoothGattConnectionInfo(const std::string &address)
      ...
      , start(Lrya::Util::CodeTimer::Start()) {}
} BluetoothGattConnectionInfo;
  ...
  for (auto & service : connection.services) {
    for (auto & characteristic : service.characteristics) {
      if (characteristic.properties & kGattCharacteristicPropNotify) {
        ...
        for (auto & descriptor : characteristic.descriptors) {
          if (bt_uuid_string_equals(Lrya::kCCCDescriptorUUID, descriptor.uuid)) {
...
static void LogConnectionEstablishmentDuration(const BluetoothGattConnectionInfo& info)
{
  logi("connection to %s established in %d milliseconds",
       ...
       Lrya::Util::CodeTimer::MillisecondsElapsed(info.start));
}
...
bool SetAdapterName(const std::string& name) {
  logv("Lrya::BluetoothStack::SetAdapterName('%s')", name.c_str());
  std::lock_guard<std::mutex> lock(sBtStackCallbackMutex);
...
bool StartAdvertisement(const Lrya::BLEAdvertiseSettings& settings)
{
  const Lrya::BLEAdvertiseData& advertisement = settings.GetAdvertisement();
  ...
  const Lrya::BLEAdvertiseData& scanResponse = settings.GetScanResponse();
