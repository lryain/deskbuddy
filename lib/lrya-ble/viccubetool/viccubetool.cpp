...
void VicCubeTool::ConnectRetryTimerCallback(ev::timer& w, int revents) {
  ...
  if (IsConnected()) {
    bool result = SendIPCMessageToServer(Lrya::BluetoothDaemon::IPCMessageType::Ping);
    if (!result) {
...
void VicCubeTool::ScanForCubes() {
  ...
  scanning_ = true;
  StartScan(Lrya::kCubeService_128_BIT_UUID);
  if (!stop_scan_timer_) {
...
void VicCubeTool::OnScanResults(int error,
                          const std::vector<Lrya::BluetoothDaemon::ScanResultRecord>& records)
{
  if (!scanning_) {
    return;
  }
...
void VicCubeTool::ConnectToCube() {
  ...
  connection_start_time_ = Lrya::Util::CodeTimer::Start();
...
void VicCubeTool::OnOutboundConnectionChange(const std::string& address,
                                             ...
                                             const std::vector<Lrya::BluetoothDaemon::GattDbRecord>& records) {
  if (address != address_) {
    return;
  }
  ...
  if (!connected && cube_connect_timer_) {
    std::cout << "Unexpected disconnect "
              << Lrya::Util::CodeTimer::MillisecondsElapsed(connection_start_time_)
  ...
  ReadCharacteristic(connection_id_, Lrya::kModelNumberString_128_BIT_UUID);
}
                                   ...
                                   const std::vector<uint8_t>& value)
{
  ...
  if (AreCaseInsensitiveStringsEqual(characteristic_uuid,Lrya::kCubeAppVersion_128_BIT_UUID)) {
    if (!value.empty()) {
  ...
  } else if (AreCaseInsensitiveStringsEqual(characteristic_uuid,Lrya::kCubeAppRead_128_BIT_UUID)) {
                                             ...
                                             const std::vector<uint8_t>& data)
{
  ...
  if (AreCaseInsensitiveStringsEqual(characteristic_uuid, Lrya::kModelNumberString_128_BIT_UUID)) {
...
void VicCubeTool::FlashCubeDVT1(const std::string& pathToFirmware) {
  ...
  if (!Lrya::ReadFileIntoVector(pathToFirmware, firmware)) {
  ...
  while (offset < firmware.size()) {
    ...
    SendMessage(connection_id_, Lrya::kCubeOTATarget_128_BIT_UUID, true, packet.GetStdVector());
    offset += chunk_length;
  }
  Lrya::ByteVector final_packet({(uint8_t) 0xff, (uint8_t) 0xff});
  final_packet.push_back_le((uint16_t) firmware.size());
  final_packet.push_back_le(Lrya::Crc32(firmware));
  SendMessage(connection_id_, Lrya::kCubeOTATarget_128_BIT_UUID, true, final_packet.GetStdVector());
}
