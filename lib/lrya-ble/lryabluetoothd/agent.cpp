...
static Lrya::BluetoothDaemon::Agent* sAgent = nullptr;
...
Agent::Agent(struct ev_loop* loop)
    : IPCServer(loop)
{
  Lrya::BluetoothStack::Callbacks cbs = {0};
  ...
  Lrya::BluetoothStack::SetCallbacks(&cbs);
}
...
void Agent::PeripheralWriteCallback(int conn_id, int trans_id, int attr_handle, int offset,
  ...
  if (attr_handle == app_read_characteristic_handle_) {
    ...
    OnReceiveMessage(conn_id, Lrya::kAppWriteCharacteristicUUID, value);
                        ...
                        const std::vector<uint8_t>& value)
{
  if (inbound_connection_id_ == connection_id) {
    if (connected_) {
      ...
      if (AreCaseInsensitiveStringsEqual(characteristic_uuid,Lrya::kAppReadCharacteristicUUID)) {
...
void Agent::StartAdvertising(const int sockfd, const BLEAdvertiseSettings& settings)
{
  ...
  if (BluetoothStack::StartGattService(&bluetooth_gatt_service_)) {
    logv("Lrya BLE Peripheral Service is started.");
  } else {
  ...
  advertising_ = BluetoothStack::StartAdvertisement(ble_advertise_settings_);
  if (advertising_) {
    logv("Started advertising Lrya BLE peripheral service");
  } else {
    loge("Failed to start advertising Lrya BLE peripheral service");
  }
}
...
{
  bluetooth_gatt_service_ = BluetoothGattService(Lrya::kLryaSingleMessageService_128_BIT_UUID);

  BluetoothGattCharacteristic
      appReadCharacteristic(Lrya::kAppReadCharacteristicUUID,
                            (kGattCharacteristicPropNotify | kGattCharacteristicPropRead),
                            kGattPermRead);

  BluetoothGattDescriptor appReadCCCDescriptor(Lrya::kCCCDescriptorUUID, (kGattPermRead | kGattPermWrite));
  ...
  BluetoothGattCharacteristic
      appWriteCharacteristic(Lrya::kAppWriteCharacteristicUUID,
  ...
  if (!BluetoothStack::AddGattService(&bluetooth_gatt_service_)) {
    loge("Failed to add Lrya BLE peripheral service");
    return false;
  }

  logv("Lrya BLE peripheral service added.");

  for (auto const& cit : bluetooth_gatt_service_.characteristics) {
    if (AreCaseInsensitiveStringsEqual(cit.uuid, Lrya::kAppWriteCharacteristicUUID)) {
      app_write_characteristic_handle_ = cit.char_handle;
    } else if (AreCaseInsensitiveStringsEqual(cit.uuid, Lrya::kAppReadCharacteristicUUID)) {
      app_read_characteristic_handle_ = cit.char_handle;
      for (auto const& dit: cit.descriptors) {
        if (AreCaseInsensitiveStringsEqual(dit.uuid,Lrya::kCCCDescriptorUUID)) {
...
bool StartBLEAgent() {
  sAgent = new Lrya::BluetoothDaemon::Agent(ev_default_loop(EVBACKEND_SELECT));
