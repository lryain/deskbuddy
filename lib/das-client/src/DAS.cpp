...
//  Copyright (c) 2013 Lrya. All rights reserved.
//
...
static Lrya::Das::DasLocalAppender* sLocalAppender =
  Lrya::Das::DasLocalAppenderFactory::CreateAppender(DASLogMode_Both);

static std::unique_ptr<Lrya::Das::DasAppender> sRemoteAppender = nullptr;
static std::mutex sRemoteAppenderMutex;
static std::unique_ptr<Lrya::Das::DasGameLogAppender> sGameLogAppender = nullptr;
...
// Local Log Filtering
static Lrya::DAS::DASFilter sDASLevelOverride;
static std::mutex sDASLevelOverrideMutex;

// Thread tagging
static std::atomic<Lrya::Das::ThreadId_t> sDASThreadMax {0};
...
static Lrya::Das::ThreadId_t _DASThreadId() {
  Lrya::Das::ThreadId_t* p = (Lrya::Das::ThreadId_t*)pthread_getspecific(sDASThreadIdKey);
  if(nullptr == p) {
    p = new Lrya::Das::ThreadId_t {++sDASThreadMax};
...
static void _freeDASThreadId(void* arg) {
  Lrya::Das::ThreadId_t* p = (Lrya::Das::ThreadId_t*) arg;
  delete p;
}
...
DASLogLevel DASLogLevelNameToEnum(const std::string& logLevelName) {
  ...
  if (LryaUtil::StringCaseInsensitiveEquals(logLevelName, "debug")) {
    logLevel = DASLogLevel_Debug;
  } else if (LryaUtil::StringCaseInsensitiveEquals(logLevelName, "info")) {
    logLevel = DASLogLevel_Info;
  } else if (LryaUtil::StringCaseInsensitiveEquals(logLevelName, "event")) {
    logLevel = DASLogLevel_Event;
  } else if (LryaUtil::StringCaseInsensitiveEquals(logLevelName, "warn")) {
    logLevel = DASLogLevel_Warn;
  } else if (LryaUtil::StringCaseInsensitiveEquals(logLevelName, "error")) {
...
void DASConfigure(const char* configurationJsonFilePath,
  ...
  if (nullptr != configurationJsonFilePath) {
    // load configuration
    std::string config = LryaUtil::StringFromContentsOfFile(configurationJsonFilePath);
    Json::Value root;
    Json::Reader reader;
...
{
  Lrya::Das::DasLocalAppender* appender;
  appender = Lrya::Das::DasLocalAppenderFactory::CreateAppender(logMode);

  Lrya::Das::DasLocalAppender* oldAppender = sLocalAppender;
  ...
  std::map< std::string, std::string > & data)
{
  ...
  data[Lrya::Das::kMessageLevelGlobalKey] = logLevel;
  Lrya::Das::ThreadId_t threadId {_DASThreadId()};

  DASGlobals* globals = _getThreadLocalCopyOfDASGlobals();
  if (globals->end() == globals->find(Lrya::Das::kTimeStampGlobalKey)
    && data.end() == data.find(Lrya::Das::kTimeStampGlobalKey)) {
    ...
    data[Lrya::Das::kTimeStampGlobalKey] = ts;
  }

  {
    if (level >= _DAS_GetLevel(eventName, sLocalMinLogLevel)) {
      std::string trimmedEventValue = eventValue;
      LryaUtil::StringTrimWhitespaceFromEnd(trimmedEventValue);
...
void _DAS_SetGlobal(const char* key, const char* value) {
  ...
  if (0 == strcmp(Lrya::Das::kGameIdGlobalKey, key)) {
    if (nullptr != value && '\0' != value[0]) {
      std::lock_guard<std::mutex> lg(sGameLogAppenderMutex);
      sGameLogAppender.reset(new Lrya::Das::DasGameLogAppender(sGameLogDir, value));
    } else {
      _DAS_DestroyGameLogAppender();
    }
  }
  ...
  (void) LryaUtil::StoreStringMapInFile(pathToThisRunDasGlobalsFile, sDASGlobals);
}
...
void _DAS_GetGlobalsForLastRun(std::map<std::string, std::string>& dasGlobals) {
  dasGlobals.clear();
  dasGlobals = LryaUtil::JsonFileToStringMap(pathToLastRunDasGlobalsFile);
}
...
size_t _DAS_GetGlobalsForLastRunAsJsonString(char* buffer, size_t len) {
  ...
  std::string dasGlobalsFromLastRunAsJson =
    LryaUtil::StringFromContentsOfFile(pathToLastRunDasGlobalsFile);
...
void _DAS_ReportCrashForLastAppRun(const char* apprun) {
  ...
  auto search = dasGlobalsFromLastRun.find(Lrya::Das::kApplicationRunGlobalKey);
  ...
  std::string logLevel;
  getDASLogLevelName(level, logLevel);
  dasGlobalsFromLastRun[Lrya::Das::kMessageLevelGlobalKey] = logLevel;

  // Use a sequence number high enough that it will be after all of the DAS events that
  // were reported during the apprun
  dasGlobalsFromLastRun[Lrya::Das::kSequenceGlobalKey] = "999999999";
  ...
  std::string ts = std::to_string(milliSecondsSince1970.count());
  dasGlobalsFromLastRun[Lrya::Das::kTimeStampGlobalKey] = ts;
  {
    std::lock_guard<std::mutex> lock(sRemoteAppenderMutex);
    if (!sRemoteAppender) {
      return;
...
Lrya::Das::DasGameLogAppender* _DAS_GetGameLogAppender() {
                                ...
                                std::string& outGlobalsAndData)
{
  ...
  static const std::unordered_set<std::string> excludedKeys =
    {
      Lrya::Das::kApplicationVersionGlobalKey,
      Lrya::Das::kApplicationRunGlobalKey,
      Lrya::Das::kMessageVersionGlobalKey,
      Lrya::Das::kMessageLevelGlobalKey,
      Lrya::Das::kPhoneTypeGlobalKey,
      Lrya::Das::kPlatformGlobalKey,
      Lrya::Das::kProductGlobalKey,
      Lrya::Das::kTimeStampGlobalKey,
      Lrya::Das::kUnitIdGlobalKey
    };

  std::map<std::string,std::string> dict;
  for (const auto& kv : *globals) {
    if (!excludedKeys.count(kv.first) && strcmp(kv.first.c_str(), Lrya::Das::kUserIdGlobalKey)) {
