...
class Lrya::DAS::DASFilter::DasFilterImpl {

#pragma mark FilterNode
  class FilterNode {
  private:
    std::unordered_map<std::string, std::unique_ptr<FilterNode>> _children;
//     std::unordered_map<std::string, FilterNode> _children;
  ...
  public:
    ...
    DASLogLevel GetMinLogLevel(typename std::vector<std::string>::const_iterator& logger_cur,
                               typename std::vector<std::string>::const_iterator& logger_end) const
    {
    ...
    void SetMinLogLevel(typename std::vector<std::string>::const_iterator& logger_cur,
                        typename std::vector<std::string>::const_iterator& logger_end,
      ...
      if (logger_cur == logger_end) {
        ...
        if (it == _children.end()) {
          ...
          auto result = _children.emplace(std::piecewise_construct,
                            ...
                            std::forward_as_tuple(std::make_unique<FilterNode>(_logLevel)));
