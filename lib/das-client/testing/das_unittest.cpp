...
TEST(DAS, SetGlobalGameId) {
  ...
  Lrya::Das::DasGameLogAppender* appender = _DAS_GetGameLogAppender();
  EXPECT_EQ(nullptr, appender);

  std::string gameId = "ABC-123-DEF-456";
  DAS_SetGlobal(Lrya::Das::kGameIdGlobalKey, gameId.c_str());

  appender = _DAS_GetGameLogAppender();
  EXPECT_TRUE(nullptr != appender);

  DAS_SetGlobal(Lrya::Das::kGameIdGlobalKey, nullptr);
  appender = _DAS_GetGameLogAppender();
  EXPECT_EQ(nullptr, appender);
