...
protected:
  virtual void SetUp() {
    ...
    appender_ = new Lrya::Das::DasLogFileAppender(testDirectory);
  }
...
protected:
  Lrya::Das::DasLogFileAppender* appender_;
...
TEST_F(DasLogFileAppenderTest, GetCurrentLogfile_firstTime) {
  ...
  ASSERT_TRUE(LryaUtil::FileExistsAtPath(currentLogFilePath)) << "Should have created an empty log file at the returned path";
}
...
TEST_F(DasLogFileAppenderTest, WriteData_firstTime) {
  ...
  ASSERT_TRUE(LryaUtil::FileExistsAtPath(currentLogFile)) << "File should exist at current log path";

  appender_->Flush();

  std::string logFileData = LryaUtil::StringFromContentsOfFile(currentLogFile);
...
TEST_F(DasLogFileAppenderTest, Rollover_oldFile) {
  ...
  ASSERT_TRUE(LryaUtil::FileExistsAtPath(completedLogFilePath)) << "Should have moved the old file to a .das file (not in progress)";

  std::string logFileData = LryaUtil::StringFromContentsOfFile(completedLogFilePath);
...
TEST_F(DasLogFileAppenderTest, Rollover_newFile) {
  ...
  ASSERT_TRUE(LryaUtil::FileExistsAtPath(currentLogFilePath)) << "Our new log file should exist";
}
...
TEST_F(DasLogFileAppenderTest, Rollover_write) {
  ...
  appender_->Flush();
  std::string logFileData = LryaUtil::StringFromContentsOfFile(appender_->CurrentLogFilePath());
...
TEST_F(DasLogFileAppenderTest, Rollover_limitNumFiles) {
  for (int i = 0 ; i < Lrya::Das::kDasDefaultMaxLogFiles + 1; i++) {
    appender_->WriteDataToCurrentLogfile(testMessage_);
    appender_->RolloverCurrentLogFile();
  }
  appender_->Flush();
...
TEST_F(DasLogFileAppenderTest, ConsumeLogFiles_oneFinishedLog) {
  ...
  appender_->ConsumeLogFiles(std::bind([&](const std::string& logFilePath, bool *stop) {
        ...
        processedLog = LryaUtil::StringFromContentsOfFile(logFilePath);
  ...
  ASSERT_FALSE(LryaUtil::FileExistsAtPath(processedLogPath)) << "Should have deleted the file we consumed";
}
...
TEST_F(DasLogFileAppenderTest, ConsumeLogFiles_oneFinishedLogNoDelete) {
  ...
  ASSERT_EQ(1, numLogFilesProcessed) << "Should have only consumed one log file";
  ASSERT_EQ(testMessage_, processedLog) << "Logged data should have been consumed";
  ASSERT_TRUE(LryaUtil::FileExistsAtPath(processedLogPath)) << "Should NOT have deleted the file we consumed";
}
...
TEST_F(DasLogFileAppenderTest, WriteToLog_autoRollover) {
  ...
  std::string completedLogFilePath = ExpectedPath(1, false, true);
  ASSERT_TRUE(LryaUtil::FileExistsAtPath(completedLogFilePath)) << "Should have moved the old file to a .das file (not in progress)";
  std::string verificationData = LryaUtil::StringFromContentsOfFile(completedLogFilePath);
  ...
  ASSERT_TRUE(LryaUtil::FileExistsAtPath(inProgressLogFilePath)) << "Should have logged the second bind of data to the new file (in progress)";
  verificationData = LryaUtil::StringFromContentsOfFile(inProgressLogFilePath);
...
TEST_F(DasLogFileAppenderTest, Init_resumeInProgress) {
  ...
  std::string dataInFile = LryaUtil::StringFromContentsOfFile(appender_->CurrentLogFilePath());
