...
//   std::auto_ptr<unsigned char> dst(new unsigned char[dst_bound]);
  std::unique_ptr<unsigned char[]> dst(std::make_unique<unsigned char[]>(dst_bound));
  stream.next_out = dst.get();
  stream.avail_out = static_cast<unsigned int>(dst_bound);
  ...
  if (curl) {
    ...
    const std::string & escapedBody = curl_easy_escape(curl, encodedBody.c_str(), static_cast<int>(encodedBody.size()));
    ...
    postBody.append("&MessageAttribute.3.Value.StringValue=application%2Fvnd.lrya%2Bjson%3B+format%3Dnormal%3B+product%3Dvic");
