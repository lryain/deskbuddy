...
#ifndef __LryaAudio_AudioEngineController_H__
#define __LryaAudio_AudioEngineController_H__
...
struct AudioScene;
namespace PlugIns {
  class LryaPluginInterface;
}

class AUDIOENGINE_EXPORT AudioEngineController : private Lrya::Util::noncopyable
{
public:
  ...
  // Plugin Access
  // Can be nullptr if not initialized
  PlugIns::LryaPluginInterface* const GetPluginInterface() const { return _pluginInterface.get(); }
...
private:
  ...
  std::unique_ptr< PlugIns::LryaPluginInterface > _pluginInterface;
...
//   std::unordered_map< std::string, AudioScene > _audioScenes;
  std::unordered_map<std::string, std::shared_ptr<AudioScene>> _audioScenes;
...
#endif // __LryaAudio_AudioEngineController_H__
