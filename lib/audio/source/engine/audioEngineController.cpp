...
{
  _pluginInterface.reset( new PlugIns::LryaPluginInterface() );
}
                                                      ...
                                                      AudioCallbackContext* callbackContext )
{
  ...
  if ( _isInitialized ) {
    ...
    if ( kInvalidAudioPlayingId != playingId &&
      ...
      Lrya::Util::SafeDelete( callbackContext );
    }
...
//   _audioScenes[scene.SceneName] = std::move( scene );
  _audioScenes[scene.SceneName] = std::make_shared<Lrya::AudioEngine::AudioScene>(scene);
}
...
void AudioEngineController::ClearGarbageCollector()
{
  std::for_each(_callbackGarbageCollector.begin(),
                ...
                [](AudioCallbackContext* aContext){ Lrya::Util::SafeDelete( aContext ); } );
