...
SoundbankBundleInfo::SoundbankBundleInfo( const Json::Value& jsonData )
{
  ...
  if ( jsonData.isMember(TYPE_KEY) ) {
    ...
    if ( Lrya::Util::StringCaseInsensitiveEquals( typeValStr, "lazy" ) ) {
      Type = SoundbankBundleInfo::BankType::LazyLoad;
    }
    else if ( Lrya::Util::StringCaseInsensitiveEquals( typeValStr, "debug" ) ) {
...
{
  out_list.clear();
  const std::string jsonStr = Lrya::Util::StringFromContentsOfFile( filePath );
  if ( jsonStr.empty() ) {
