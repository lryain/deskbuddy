...
AKRESULT AudioMultipleFileLocation<OPEN_POLICY>::CheckFileExists(const AkOSChar * in_fileName)
{
  ...
  for (typename AkListBareLight<FilePath>::Iterator it = m_Locations.Begin(); it != m_Locations.End(); ++it) {
    ...
    const std::string fullFilePath = Lrya::Util::FileUtils::FullFilePath({ (*it)->szPath, in_fileName });
  ...
  if (m_AssetManager) {
    ...
    const std::string fullFilePath = Lrya::Util::FileUtils::FullFilePath(std::move(pathList));
  ...
  AkFileDesc&         out_fileDesc)       // Returned file descriptor.
{
  ...
  const std::string fullFilePath = Lrya::Util::FileUtils::FullFilePath({ m_WriteFilePath, in_pszFileName });
  if (fullFilePath.length() >= AK_MAX_PATH) {
  ...
  AkFileDesc&         out_fileDesc)       // Returned file descriptor.
{
  ...
  for (typename AkListBareLight<FilePath>::Iterator it = m_Locations.Begin(); it != m_Locations.End(); ++it) {
    // Create full file path
    const std::string fullFilePath = Lrya::Util::FileUtils::FullFilePath({ (*it)->szPath, out_FileInPath });
    if (fullFilePath.length() >= AK_MAX_PATH) {
      AKASSERT( !"File name string too large" );
      return AK_Fail;
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
template<class OPEN_POLICY>
void AudioMultipleFileLocation<OPEN_POLICY>::SetPathResolver( Lrya::AudioEngine::AudioFilePathResolverFunc pathResolver )
{
...
void AudioMultipleFileLocation<OPEN_POLICY>::RemoveMissingZipFiles()
{
  std::vector<std::string> pathsToRemove;
  {
    ...
    for (const ZipReaderItem* p : m_ZipReaders) {
      std::vector<std::string> parts = Lrya::AudioEngine::ZipReader::SplitPath(p->path);
      if (!parts.empty()) {
...
void AudioMultipleFileLocation<OPEN_POLICY>::SetWriteBasePath( const std::string& writePath )
{
  ...
  if (!m_WriteFilePath.empty() &&
      !Lrya::Util::FileUtils::CreateDirectory(m_WriteFilePath)) {
