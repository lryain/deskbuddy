 ...
 *              Lrya’s RAMS system where assets may or may not be available. Using a build server generated metadata
 *              file “SoundbankBundleInfo.json” to search and load available sound banks
 *              (See lrya-audio/build-scripts/bundle_soundbank_products.py).
 *
 * TODO: Implement RAMS
 *
...
void SoundbankLoader::LoadSoundbankBundleMetadata()
{
  ...
  const std::string pathToSoundConfig = Lrya::Util::FileUtils::FullFilePath({ _assetPath, filename });
...
std::vector<std::string> SoundbankLoader::GetAvailableSoundbankZipFiles(const BankInfoList& infoList) const
{
  ...
  for ( auto const& it : infoList ) {
    ...
    const std::string filePath = Lrya::Util::FileUtils::FullFilePath({ _assetPath, filename });
    if ( Lrya::Util::FileUtils::FileExists( filePath ) ) {
