...
// Convert between Lrya's AudioRTPCValueType & Wwise's RTPCValue_type
...
{
  Terminate();
  Lrya::Util::SafeDelete( _fileLocationResolver );
}
                                                 ...
                                                 const std::vector<AudioAuxBusValue>& auxSendValues )
{
  ...
  if ( _isInitialized )
  {
    ...
    Lrya::Util::SafeDeleteArray( akSendValues );
...
const std::string WwiseComponent::GetLanguageLocaleString( AudioLocaleType locale )
{
  ...
  static const std::unordered_map<AudioLocaleType, std::string, Lrya::Util::EnumHasher> localeMap {
