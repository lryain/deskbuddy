...
void MusicConductor::UpdateTick()
{
  ...
  if ( Lrya::Util::IsFltGEZero( _currentStateExpirationTimeStamp_ms ) ) {
    // Check if minimum duration has passed
    if ( Lrya::Util::Time::UniversalTime::GetCurrentTimeInMilliseconds() < _currentStateExpirationTimeStamp_ms ) {
      return;
    }
  }
...
void MusicConductor::UpdateMusicState()
{
  ...
  if ( _pendingStateMinDuration_ms != kZeroDuration ) {
    _currentStateExpirationTimeStamp_ms = Lrya::Util::Time::UniversalTime::GetCurrentTimeInMilliseconds() + _pendingStateMinDuration_ms;
  }
  else {
    _currentStateExpirationTimeStamp_ms = 0.0;
  }
