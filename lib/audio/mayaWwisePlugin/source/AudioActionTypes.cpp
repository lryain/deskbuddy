...
namespace {
// Need static RNG
Lrya::Util::RandomGenerator kRng;
}
...
{
  // Do nothing if the audio node (x:LryaAudioNode) doesn't exist
  int nodeExists;
  MString existCmd("objExists \"x:LryaAudioNode\"");
  ...
  static const std::string cmdNode = " \"x:LryaAudioNode\"";
...
//  MGlobal::displayWarning("EventActionCapture::ValidateComputedResults!!");
  ...
  for ( auto& anActionMapIt : actionMap ) {
    ...
    for ( auto& anActionIt : anActionMapIt.second ) {
      if ( Lrya::Util::IsNear( anActionIt.probability, kInvalidAtt ) ) {
        validProb = false;
        break;
      }
    }
