...
```bash
  #!/usr/bin/env bash
  deskmate-branch-cherrypick.sh [picklist]
```
...
```bash
  #!/usr/bin/env bash
  deskmate-branch-update-spreadsheet.sh
```
...
`lrya-branch-update-spreadsheet.py` requires a secret application key to access the Google Sheets API.
...
When you run `lrya-branch-update-spreadsheet.py`, it will attempt to launch a browser window to complete authentication.
...
set of credentials. When you run `lrya-branch-update-spreadsheet.py`, it will attempt to use whatever client secret
