...
class SourceFileAnnotator(object):
    ...
    def __init__(self, config):
        ...
        symfs_dir = config['symfs'] # Lrya, from command-line arguments symfs_dir = 'binary_cache'
        if not os.path.isdir(symfs_dir):
            symfs_dir = None
        kallsyms = config['symfs']+'/kallsyms' # Lrya, from command-line arguments kallsyms = 'binary_cache/kallsyms'
        ...
        # Lrya, from command-line argumments config['annotate_dest_dir'] = 'annotated_files'
...
# Lrya, override symbol cache and destination directory via command-line arguments
...
"""Directories to output annotated files.""")
    ...
    annotator.annotate()
    # Lrya, override destination directory
