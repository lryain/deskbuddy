...
if [ -z $VersionFileName ]; then
  VersionFileName=$TOPLEVEL/source/lrya/basestation/version.h
fi


# find user source and parse it
VersionSource=$TOPLEVEL/source/lrya/basestation/VERSION
...
if [ "$oldVersion" != "$newVersion" -o $? -ne 0 ];then
  echo "[lrya-util] version file updated"
  generateVersion $VersionFileName $Major $Minor $Release $Revision $Commit $DasUser
  exit 0
fi


echo "[lrya-util] version file ok"
exit 0
