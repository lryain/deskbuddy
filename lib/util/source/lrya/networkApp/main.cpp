...
//  Copyright (c) 2015 Lrya. All rights reserved.
//
...
class DummyNetTransportDataReceiver : public Lrya::Util::INetTransportDataReceiver
{
public:
  ...
  virtual void ReceiveData(const uint8_t* buffer, uint32_t size, const Lrya::Util::TransportAddress& sourceAddress) override
  {
    ...
    if (size > 0)
    {
      if (gConfigSettings._printMessagesReceived)
      {
        ...
        Lrya::Util::LogVerboseMessage("AppRecv", "", buffer, size, nullptr);
      }
    }
  }
...
int main(int argc, const char * argv[])
{
  ...
  printfLoggerProvider.SetMinLogLevel(Lrya::Util::ILoggerProvider::LOG_LEVEL_DEBUG);
