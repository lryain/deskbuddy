...
namespace {
  Lrya::AudioUtil::AudioCaptureSystem::RequestCapturePermissionCallback     _requestCapturePermissionCallback;
...
AudioCaptureSystem::PermissionState AudioCaptureSystem::GetPermissionState(bool isRepeatRequest) const
{
  ...
  Util::JClassHandle captureClass{envWrapper->FindClassInProject("com/lrya/audioUtil/AudioCaptureSystem"), env};
  if (nullptr == captureClass)
  {
    PRINT_NAMED_ERROR("AudioCaptureSystem.GetPermissionState.ClassNotFound",
                      "Unable to find com.lrya.audioUtil.AudioCaptureSystem");
    return PermissionState::DeniedAllowRetry;
  }
...
void JNICALL
Java_com_lrya_audioUtil_AudioCaptureSystem_NativeRequestCapturePermissionCallback(JNIEnv* env, jobject clazz, jboolean permissionGranted)
{
  std::lock_guard<std::mutex> lock(_requestPermCallbackLock);
  if (_requestCapturePermissionCallback)
  {
    _requestCapturePermissionCallback();
    _requestCapturePermissionCallback = Lrya::AudioUtil::AudioCaptureSystem::RequestCapturePermissionCallback{};
  }
}
