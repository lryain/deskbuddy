...
#include "platform/lrya-trace/tracing.h"
...
#if !defined(LRYA_BREADCRUMBS)
#define LRYA_BREADCRUMBS 0
#endif
...
// Helper for use with LRYA_VERIFY macro. Always returns true.
bool sVerifySucceededReturnTrue(const char* file, int line);

// Helper for use with LRYA_VERIFY macro. Always returns false.
__attribute__((__used__))
...
// This function is enabled for build configurations with LRYA_DEVELOPER_CODE=1.
// This function is a no-op for build configurations with LRYA_DEVELOPER_CODE=0.
//
void sDebugBreak();

// Lrya::Util::sDebugBreakOnError()
...
#define PRINT_NAMED_ERROR(name, format, ...) do { \
  if(LRYATRACE_ENABLED) { \
    ...
    ::Lrya::Util::sErrorF(name, {}, "%s", PRINT_LOG_VAR_logString);     \
  } else { \
    ::Lrya::Util::sErrorF(name, {}, format, ##__VA_ARGS__); \
  } \
  ::Lrya::Util::DropBreadcrumb(false, __FILE__, __LINE__);       \
  ::Lrya::Util::sSetErrG(); \
  if (::Lrya::Util::_errBreakOnError) { \
    ::Lrya::Util::sDebugBreakOnError(); \
  } \
} while(0)
...
#define PRINT_NAMED_WARNING(name, format, ...) do { \
  if(LRYATRACE_ENABLED) { \
    ...
    ::Lrya::Util::sWarningF(name, {}, "%s", PRINT_LOG_VAR_logString);\
  } else { \
    ::Lrya::Util::sWarningF(name, {}, format, ##__VA_ARGS__);\
  } \
  ::Lrya::Util::DropBreadcrumb(false, __FILE__, __LINE__); \
} while(0)
...
#define PRINT_NAMED_INFO(name, format, ...) do { \
  if(LRYATRACE_ENABLED) { \
    ...
    ::Lrya::Util::sChanneledInfoF(name, name, {}, "%s", PRINT_LOG_VAR_logString); \
  } else { \
    ::Lrya::Util::sChanneledInfoF(name, name, {}, format, ##__VA_ARGS__); \
  } \
  ::Lrya::Util::DropBreadcrumb(false, __FILE__, __LINE__);              \
} while(0)
...
#define PRINT_NAMED_DEBUG(name, format, ...) do { \
  if(LRYATRACE_ENABLED) { \
    ...
    ::Lrya::Util::sChanneledDebugF(name, name, {}, "%s", PRINT_LOG_VAR_logString); \
  } else { \
    ::Lrya::Util::sChanneledDebugF(name, name, {}, format, ##__VA_ARGS__); \
  } \
  ::Lrya::Util::DropBreadcrumb(false, __FILE__, __LINE__);              \
} while(0)
#else
...
//
// LRYA_VERIFY(expr, name, format, args...)
...
//
// If the conditional expression (expr) is true, LRYA_VERIFY returns true.
// If the conditional expression (expr) is false, LRYA_VERIFY logs an error message, dumps the callstack,
// and returns false.
//
...
// Example 1:
// Use
//   if (LRYA_VERIFY(x == y, "VerifyXY", "%p != %p", x, y)) {
//     /* do stuff */
//   }
// in place of
//   if (x == y) {
...
//
#define LRYA_VERIFY(expr, name, format, ...) \
 (expr ? ::Lrya::Util::sVerifySucceededReturnTrue(__FILE__, __LINE__) : (::Lrya::Util::sVerifyFailedReturnFalse(__FILE__, __LINE__, name, "VERIFY(%s): " format, #expr, ##__VA_ARGS__) && false))
...
#define PRINT_CH_INFO(channel, name, format, ...) do { \
  if(LRYATRACE_ENABLED) { \
    ...
    ::Lrya::Util::sChanneledInfoF(channel, name, {}, "%s", PRINT_LOG_VAR_logString); \
  } else { \
    ::Lrya::Util::sChanneledInfoF(channel, name, {}, format, ##__VA_ARGS__); \
  } \
  ::Lrya::Util::DropBreadcrumb(false, __FILE__, __LINE__);  \
} while(0)
...
#define PRINT_CH_DEBUG(channel, name, format, ...) do { \
  if(LRYATRACE_ENABLED) {                                               \
    ...
    ::Lrya::Util::sChanneledDebugF(channel, name, {}, "%s", PRINT_LOG_VAR_logString); \
  } else { \
    ::Lrya::Util::sChanneledDebugF(channel, name, {}, format, ##__VA_ARGS__); \
  } \
  ::Lrya::Util::DropBreadcrumb(false, __FILE__, __LINE__); \
} while(0)
#else
...
{ static u16 cnt = period;                                                 \
  if (++cnt >= period) {                                                   \
    ::Lrya::Util::DropBreadcrumb(false, __FILE__, __LINE__);                 \
    ::Lrya::Util::func(channel, name, {}, format, ##__VA_ARGS__);          \
    cnt = 0;                                                               \
  }                                                                        \
}
...
#define PRINT_STREAM_ERROR(name, args) do{         \
      ...
      ::Lrya::Util::DropBreadcrumb(false, __FILE__, __LINE__); \
      ::Lrya::Util::sError(name, {}, ss.str().c_str()); \
    } while(0)
...
#define PRINT_STREAM_WARNING(name, args) do{       \
      ...
      ::Lrya::Util::DropBreadcrumb(false, __FILE__, __LINE__); \
      ::Lrya::Util::sWarning(name, {}, ss.str().c_str()); \
    } while(0)
...
#define PRINT_STREAM_INFO(name, args) do{          \
      ...
      ::Lrya::Util::DropBreadcrumb(false, __FILE__, __LINE__); \
      ::Lrya::Util::sChanneledInfo(name, name, {}, ss.str().c_str()); \
    } while(0)
...
#define PRINT_STREAM_DEBUG(name, args) do {         \
      ...
      ::Lrya::Util::DropBreadcrumb(false, __FILE__, __LINE__); \
      ::Lrya::Util::sChanneledDebug(name, name, {}, ss.str().c_str()); \
    } while(0)
#else
...
#define ASSERT_NAMED(expr, name) do {                       \
  ::Lrya::Util::DropBreadcrumb(expr ? true : false, __FILE__, __LINE__); \
  if (!(expr)) {                                            \
    PRINT_NAMED_ERROR(name, "Assertion Failed: %s", #expr); \
    Lrya::Util::sDumpCallstack("AssertCallstack");          \
    Lrya::Util::sLogFlush();                                \
    DEBUG_ABORT;                                            \
  }                                                         \
} while(0)
...
#define ASSERT_NAMED_EVENT_AND_RETURN_FALSE_IF_FAIL(exp, name, format, ...) do { \
  ::Lrya::Util::DropBreadcrumb(exp ? true : false, __FILE__, __LINE__);         \
  if(!(exp)) {                                                                \
...
//
// Code blocks that are only used for developer assertions should be guarded with #if LRYA_DEV_ASSERT_ENABLED.
// Variables that are only used for developer assertions should be guarded with DEV_ASSERT_ONLY.

#ifndef LRYA_DEV_ASSERT_ENABLED
  #define LRYA_DEV_ASSERT_ENABLED LRYA_DEVELOPER_CODE
#endif

#if LRYA_DEV_ASSERT_ENABLED

#define DEV_ASSERT_MSG(expr, name, format, ...) do { \
  if (!(expr)) { \
    PRINT_NAMED_ERROR(name, "ASSERT(%s): " format, #expr, ##__VA_ARGS__); \
    Lrya::Util::sDumpCallstack("ASSERT"); \
    Lrya::Util::sLogFlush(); \
    Lrya::Util::sAbort(); \
  } \
} while (0)
