...
// These events are used to indicate start/stop of association with an Lrya profile ID.
// If event name or event parameters change, DASManager must be updated to match.
// Names are defined as a macro, not a constexpr, so they can be expanded by doxygen.
//
...
#define DASMSG(ezRef, eventName, documentation) { Lrya::Util::DasMsg __DAS_msg(eventName);
#define DASMSG_SET(dasEntry, value, comment) __DAS_msg.dasEntry = Lrya::Util::DasItem(value); \
                                             /* define an empty struct (to detect if this item has been set twice) */ \
                                             struct _already_set_##dasEntry {};
#define DASMSG_SEND()         Lrya::Util::sLogInfo(__DAS_msg); }
#define DASMSG_SEND_WARNING() Lrya::Util::sLogWarning(__DAS_msg); }
#define DASMSG_SEND_ERROR()   Lrya::Util::sLogError(__DAS_msg); }
#define DASMSG_SEND_DEBUG()   Lrya::Util::sLogDebug(__DAS_msg); }
...
#define DASMSG_ESCAPE(str) Lrya::Util::DAS::Escape(str)
