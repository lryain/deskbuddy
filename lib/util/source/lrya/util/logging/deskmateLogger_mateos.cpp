/**
* File: util/logging/deskmateLogger_mateos.cpp
*
  ...
  const KVPairVector & keyvals)
{
  ...
  // Note that key names must be an EXACT MATCH for values declared by Lrya::Util::DAS namespace.
  //
  ...
  static_assert(Lrya::Util::DAS::EVENT_MARKER == '@', "DAS event marker does not match declarations");
  static_assert(Lrya::Util::DAS::FIELD_MARKER == '\x1F', "DAS field marker does not match declarations");
  static_assert(Lrya::Util::DAS::FIELD_COUNT == 9, "DAS field count does not match declarations");
  ...
  __android_log_print(prio, _tag.c_str(), "@%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%llu",
                      name, str1, str2, str3, str4, int1, int2, int3, int4, static_cast<unsigned long long>(uptime_ms));
  ...
  const char * strval)
{
...
#if LRYA_REPORT_ERRORS_WITH_STRVAL_TO_DAS
  ...
  // in shipping builds.
  std::string escaped = Lrya::Util::DAS::Escape(strval);
...
#endif // LRYA_REPORT_ERRORS_WITH_STRVAL_TO_DAS
  kv.insert(std::end(kv), std::begin(keyvals), std::end(keyvals));
  LogEvent(prio, "log.error", kv);
#else
  Log(prio, name, keyvals, strval);
#endif // LRYA_REPORT_ERRORS_TO_DAS
}

//
// Map Lrya log level to android log priority
//
...
void VictorLogger::LogEvent(LogLevel level, const DasMsg & dasMsg)
{
  ...
  const auto uptime_ms = Lrya::Util::DAS::UptimeMS();

  __android_log_print(prio, _tag.c_str(), "@%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%s\x1F%llu",
                      dasMsg.event.c_str(), dasMsg.s1.c_str(), dasMsg.s2.c_str(), dasMsg.s3.c_str(), dasMsg.s4.c_str(),
                      dasMsg.i1.c_str(), dasMsg.i2.c_str(), dasMsg.i3.c_str(), dasMsg.i4.c_str(), static_cast<unsigned long long>(uptime_ms));
