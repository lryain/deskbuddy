...
#include "util/console/consoleSystem.h"
  ...
  #define CONSOLE_VAR( type, name, path, default ) \
    type name = default; \
    namespace { \
      static Lrya::Util::ConsoleVar<type> cvar_##name( name, #name, path, false ); \
    }
  ...
  #define CONSOLE_VAR_RANGED( type, name, path, default, minVal, maxVal ) \
    type name = default; \
    namespace { \
      static Lrya::Util::ConsoleVar<type> cvar_##name( name, #name, path, minVal, maxVal, false ); \
    }

  #define CONSOLE_VAR_ENUM( type, name, path, default, values ) \
    type name = default; \
    namespace { \
      static Lrya::Util::ConsoleVar<type> cvar_##name( name, #name, path, values, false ); \
    }

  #define CONSOLE_FUNC( name, path, args... ) \
    namespace { \
      Lrya::Util::IConsoleFunction cfunc_##name( #name, &name, path, #args ); \
    }
