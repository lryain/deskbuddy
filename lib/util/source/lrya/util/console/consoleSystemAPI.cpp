...
{
  Lrya::Util::ConsoleSystem::Instance().Register( *var, name, category );
}
...
{
  Lrya::Util::ConsoleSystem::Instance().Register( name, function, categoryName, args );
}
...
#define CONSOLE_ARG_TYPE( __argname__, __argtype__, ... ) \
  ...
  { \
    ...
    Lrya::Util::ConsoleSystem::Instance().GetArgumentValue( *context, varname, result ); \
    return result; \
  } \
  \
