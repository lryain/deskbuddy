...
void NativeLryaUtilConsoleLoadVarsWithContext(ConsoleFunctionContextRef context);
void NativeLryaUtilConsoleSaveVarsWithContext(ConsoleFunctionContextRef context);
void NativeLryaUtilConsoleDeleteVarsWithContext(ConsoleFunctionContextRef context);
...
void ConsoleSystem::FinishInitialization(const char* iniPath)
{
  if (!_isInitializationComplete)
  {
    ...
    NativeLryaUtilConsoleLoadVars();

    LOG_INFO("ConsoleSystem.FinishInitialization",
             "NativeLryaUtilAreDevFeaturesEnabled() = %d",
             NativeLryaUtilAreDevFeaturesEnabled() ? 1 : 0);
  }
}
...
uint32_t NativeLryaUtilConsoleGetVarCount()
{
  const Lrya::Util::ConsoleSystem& consoleSystem = Lrya::Util::ConsoleSystem::Instance();
  const Lrya::Util::ConsoleSystem::VariableDatabase& varDatabase = consoleSystem.GetVariableDatabase();
  return Lrya::Util::numeric_cast<uint32_t>(varDatabase.size());
}


uint32_t NativeLryaUtilConsoleGetFunctionCount()
{
  const Lrya::Util::ConsoleSystem& consoleSystem = Lrya::Util::ConsoleSystem::Instance();
  const Lrya::Util::ConsoleSystem::FunctionDatabase& funcDatabase = consoleSystem.GetFunctionDatabase();
  return Lrya::Util::numeric_cast<uint32_t>(funcDatabase.size());
}


uint32_t NativeLryaUtilConsoleGetMaxVarNameLen()
{
  const Lrya::Util::ConsoleSystem& consoleSystem = Lrya::Util::ConsoleSystem::Instance();
  const Lrya::Util::ConsoleSystem::VariableDatabase& varDatabase = consoleSystem.GetVariableDatabase();
  ...
  {
    const Lrya::Util::IConsoleVariable* consoleVar = entry.second;

    const uint32_t nameLength = Lrya::Util::numeric_cast<uint32_t>(strlen(consoleVar->GetID().c_str()));
    maxStringLength = Lrya::Util::Max(nameLength, maxStringLength);
  }
...
uint32_t NativeLryaUtilConsoleGetMaxFunctionNameLen()
{
  const Lrya::Util::ConsoleSystem& consoleSystem = Lrya::Util::ConsoleSystem::Instance();
  const Lrya::Util::ConsoleSystem::FunctionDatabase& funcDatabase = consoleSystem.GetFunctionDatabase();
  ...
  {
    const Lrya::Util::IConsoleFunction* consoleFunc = entry.second;

    const uint32_t nameLength = Lrya::Util::numeric_cast<uint32_t>(strlen(consoleFunc->GetID().c_str()));
    maxStringLength = Lrya::Util::Max(nameLength, maxStringLength);
  }
...
uint32_t NativeLryaUtilConsoleGetMaxCategoryNameLen()
{
  const Lrya::Util::ConsoleSystem& consoleSystem = Lrya::Util::ConsoleSystem::Instance();
  ...
  // Test all console vars
  {
    ...
    for (const auto& entry : varDatabase)
    {
      ...
      const uint32_t categoryLength = Lrya::Util::numeric_cast<uint32_t>(strlen(consoleVar->GetCategory().c_str()));
      maxStringLength = Lrya::Util::Max(categoryLength, maxStringLength);
    }
  }
  // Test all console functions
  {
    ...
    for (const auto& entry : funcDatabase)
    {
      ...
      const uint32_t categoryLength = Lrya::Util::numeric_cast<uint32_t>(strlen(consoleFunc->GetCategory()));
      maxStringLength = Lrya::Util::Max(categoryLength, maxStringLength);
    }
  }
...
uint32_t NativeLryaUtilConsoleGetMaxFunctionSignatureLen()
{
  const Lrya::Util::ConsoleSystem& consoleSystem = Lrya::Util::ConsoleSystem::Instance();
  const Lrya::Util::ConsoleSystem::FunctionDatabase& funcDatabase = consoleSystem.GetFunctionDatabase();
  ...
  {
    const Lrya::Util::IConsoleFunction* consoleFunc = entry.second;
    const uint32_t sigLength = Lrya::Util::numeric_cast<uint32_t>(strlen(consoleFunc->GetSignature().c_str()));
    maxStringLength = Lrya::Util::Max(sigLength, maxStringLength);
  }

  return maxStringLength;
}

uint8_t NativeLryaUtilConsoleGetVar(int varIndex,
                                    ...
                                    uint8_t* outIsToggleable, uint8_t* outIsIntType, uint8_t* outIsSigned)
{
  ...
  const Lrya::Util::StringID variableId = consoleSystem.GetVariableIds()[varIndex];

  const auto& found = varDatabase.find(variableId);
  if( found != varDatabase.end()) {
    const Lrya::Util::IConsoleVariable* consoleVar = found->second;

    // name
    *outNameLength = Lrya::Util::numeric_cast<int>(consoleVar->GetID().size() + 1);
    *outName = consoleVar->GetID().c_str();

    // category
    *outCategoryNameLength = Lrya::Util::numeric_cast<int>(consoleVar->GetCategory().size() + 1);
...
uint8_t NativeLryaUtilConsoleGetFunction(int functIndex,
                                         int* outNameLength,      const char** outName,
                                         int* outCategoryLength,  const char** outCategoryName,
                                         int* outSignatureLength, const char** outSignature)
{
  const Lrya::Util::ConsoleSystem& consoleSystem = Lrya::Util::ConsoleSystem::Instance();
  const Lrya::Util::ConsoleSystem::FunctionDatabase& functDatabase = consoleSystem.GetFunctionDatabase();

  assert(functIndex < consoleSystem.GetFunctionIds().size());
  const Lrya::Util::StringID functionId = consoleSystem.GetFunctionIds()[functIndex];

  const auto& found = functDatabase.find(functionId);
  if ( found != functDatabase.end())
  {
    const Lrya::Util::IConsoleFunction* consoleFunct = found->second;

    // name
    *outNameLength = Lrya::Util::numeric_cast<int>(consoleFunct->GetID().size() + 1);
    *outName = consoleFunct->GetID().c_str();

    // category
    *outCategoryLength = Lrya::Util::numeric_cast<int>(strlen(consoleFunct->GetCategory()) + 1);
    *outCategoryName = consoleFunct->GetCategory();

    // signature
    *outSignatureLength = Lrya::Util::numeric_cast<int>(consoleFunct->GetSignature().size() + 1);
...
uint32_t NativeLryaUtilConsoleGetVarValueString(const char* varName, char* outBuffer, uint32_t outBufferLen)
{
  const Lrya::Util::IConsoleVariable* consoleVar = Lrya::Util::ConsoleSystem::Instance().FindVariable(varName);
  return LryaCopyStringIntoOutBuffer(consoleVar ? consoleVar->ToString().c_str() : "", outBuffer, outBufferLen);
}


void NativeLryaUtilConsoleToggleValue(const char* varName)
{
  Lrya::Util::IConsoleVariable* consoleVar = Lrya::Util::ConsoleSystem::Instance().FindVariable(varName);
  if (consoleVar)
  {
  ...
  {
    LOG_WARNING("NativeLryaUtilConsoleToggleValue", "No var named '%s'!", varName);
  }
}


void NativeLryaUtilConsoleResetValueToDefault(const char* varName)
{
  Lrya::Util::IConsoleVariable* consoleVar = Lrya::Util::ConsoleSystem::Instance().FindVariable(varName);
  if (consoleVar)
  {
  ...
  {
    LOG_WARNING("NativeLryaUtilConsoleResetValueToDefault", "No var named '%s'!", varName);
  }
}


uint32_t NativeLryaUtilConsoleIsDefaultValue(const char* varName)
{
  Lrya::Util::IConsoleVariable* consoleVar = Lrya::Util::ConsoleSystem::Instance().FindVariable(varName);
  if (consoleVar)
  {
  ...
  {
    LOG_WARNING("NativeLryaUtilConsoleIsDefaultValue", "No var named '%s'!", varName);
    return 0;
  }
}


void NativeLryaUtilConsoleResetAllToDefault()
{
  const Lrya::Util::ConsoleSystem& consoleSystem = Lrya::Util::ConsoleSystem::Instance();
  const Lrya::Util::ConsoleSystem::VariableDatabase& varDatabase = consoleSystem.GetVariableDatabase();
  ...
  {
    Lrya::Util::IConsoleVariable* consoleVar = entry.second;
    consoleVar->ResetToDefault();
  }
}


void NativeLryaUtilConsoleSetValueWithString(const char* varName, const char* inString)
{
  Lrya::Util::IConsoleVariable* consoleVar = Lrya::Util::ConsoleSystem::Instance().FindVariable(varName);
  if (consoleVar)
  {
    ...
    if (!parsedOK)
    {
      LOG_WARNING("NativeLryaUtilConsoleSetValueWithString", "Error parsing '%s' into var '%s': was = '%s', now = '%s'", inString, varName, oldValue.c_str(), consoleVar->ToString().c_str());
    }
  }
  else
  {
    LOG_WARNING("NativeLryaUtilConsoleSetValueWithString", "No var named '%s'!", varName);
  }
}


double NativeLryaUtilConsoleGetVarValueAsDouble(const char* varName)
{
  const Lrya::Util::IConsoleVariable* consoleVar = Lrya::Util::ConsoleSystem::Instance().FindVariable(varName);
...
int64_t NativeLryaUtilConsoleGetVarValueAsInt64(const char* varName)
{
  const Lrya::Util::IConsoleVariable* consoleVar = Lrya::Util::ConsoleSystem::Instance().FindVariable(varName);
...
uint64_t NativeLryaUtilConsoleGetVarValueAsUInt64(const char* varName)
{
  const Lrya::Util::IConsoleVariable* consoleVar = Lrya::Util::ConsoleSystem::Instance().FindVariable(varName);
...
class MenuConsoleChannel : public Lrya::Util::IConsoleChannel
{
public:
...
uint32_t NativeLryaUtilConsoleCallFunction(const char* funcName, const char* funcArgs, uint32_t outTextLength, char* outText)
{
  Lrya::Util::ConsoleSystem& consoleSystem = Lrya::Util::ConsoleSystem::Instance();
  Lrya::Util::IConsoleFunction* consoleFunc = consoleSystem.FindFunction(funcName);
  if (consoleFunc)
  {
  ...
  {
    LOG_WARNING("NativeLryaUtilConsoleCallFunction", "No func named '%s'!", funcName);
    return 0;
  }
}
...
void NativeLryaUtilConsoleLoadVarsWithContext(ConsoleFunctionContextRef context)
{
  ...
  if (inputFile)
  {
    char tempBuffer[Lrya::Util::TEXT_PARSING_MAX_LENGTH];
...
void NativeLryaUtilConsoleLoadVars()
{
  NativeLryaUtilConsoleLoadVarsWithContext( nullptr );
}
...
{
  bool operator()(const Lrya::Util::IConsoleVariable* a, const Lrya::Util::IConsoleVariable* b) const
  {
...
void NativeLryaUtilConsoleSaveVarsWithContext(ConsoleFunctionContextRef context)
{
  ...
  if (outputFile)
  {
    ...
    std::vector<const Lrya::Util::IConsoleVariable*> sortedVars;
    for (const auto& entry : varDatabase)
    {
      const Lrya::Util::IConsoleVariable* consoleVar = entry.second;
    ...
    std::string tempStr;
    for (const Lrya::Util::IConsoleVariable* consoleVar : sortedVars)
    {
...
void NativeLryaUtilConsoleSaveVars()
{
  NativeLryaUtilConsoleSaveVarsWithContext(nullptr);
}
