...
{
  return (Lrya::Util::Abs(x-y) < Lrya::Util::FLOATING_POINT_COMPARISON_TOLERANCE_FLT);
}

inline constexpr bool IsFltNear(double x, double y)
{
  return (Lrya::Util::Abs(x-y) < Lrya::Util::FLOATING_POINT_COMPARISON_TOLERANCE);
}
...
inline constexpr bool IsNear(float x, float y, float epsilon=Lrya::Util::FLOATING_POINT_COMPARISON_TOLERANCE_FLT)
{
  return (Lrya::Util::Abs(x-y) < epsilon);
}
...
{
  return IsNear(x, 0.0f, Lrya::Util::FLOATING_POINT_COMPARISON_TOLERANCE_FLT);
}

inline constexpr bool IsNearZero(double x)
{
  return IsNear(x, 0.0, Lrya::Util::FLOATING_POINT_COMPARISON_TOLERANCE);
}
...
{
  return (x > Lrya::Util::FLOATING_POINT_COMPARISON_TOLERANCE_FLT);
}
 
inline constexpr bool IsFltGTZero(double x)
{
  return (x > Lrya::Util::FLOATING_POINT_COMPARISON_TOLERANCE);
}
...
{
  return (x >= (y - Lrya::Util::FLOATING_POINT_COMPARISON_TOLERANCE_FLT));
}

inline constexpr bool IsFltGE(double x, double y)
{
  return (x >= (y - Lrya::Util::FLOATING_POINT_COMPARISON_TOLERANCE));
}
...
template<class T>
inline LRYA_NON_DEVELOPER_CONSTEXPR T Clamp(const T& val, const T& min, const T& max)
{
  LRYA_DEVELOPER_CODE_ONLY( assert(min <= max) );
...
#undef FLT_NEAR
#define FLT_NEAR(x, y)        Lrya::Util::IsFltNear(x, y)

#undef NEAR
#define NEAR(x, y, epsilon)   Lrya::Util::IsNear(x, y, epsilon)

#undef NEAR_ZERO
#define NEAR_ZERO(x)          Lrya::Util::IsNearZero(x)


// float comparisons (with epsilon tolerance)
#undef FLT_GE_ZERO
#define FLT_GE_ZERO(x)        Lrya::Util::IsFltGEZero(x)

#undef FLT_GE
#define FLT_GE(x, y)          Lrya::Util::IsFltGE(x, y)

#undef FLT_GT
#define FLT_GT(x, y)          Lrya::Util::IsFltGT(x, y)

#undef FLT_LE
#define FLT_LE(x, y)          Lrya::Util::IsFltLE(x, y)

#undef FLT_LT
#define FLT_LT(x, y)          Lrya::Util::IsFltLT(x, y)


#undef IN_RANGE
#define IN_RANGE(val, minVal, maxVal)   Lrya::Util::InRange(val, minVal, maxVal)

#undef ABS
#define ABS(a)    Lrya::Util::Abs(a)

#undef SQUARE
#define SQUARE(x) Lrya::Util::Square(x)
...
#undef MAX
//#define MAX(a, b)   Lrya::Util::Max(a, b)
#define MAX(a, b) (((a) > (b)) ? (a) : (b))

#undef MIN
//#define MIN(a, b)   Lrya::Util::Min(a, b)
#define MIN(a, b) (((a) < (b)) ? (a) : (b))

#undef CLIP
//#define CLIP(val, lo, hi)   Lrya::Util::Clamp(val, lo, hi)
...
#undef M_TO_MM
#define M_TO_MM(x)   Lrya::Util::ConvertMToMM(x)

#undef MM_TO_M
#define MM_TO_M(x)   Lrya::Util::ConvertMMToM(x)

#undef DEG_TO_RAD
#define DEG_TO_RAD(deg)      Lrya::Util::DegToRad(deg)

#undef RAD_TO_DEG
#define RAD_TO_DEG(rad)      Lrya::Util::RadToDeg(rad)
