...
#define LRYA_CPU_PROFILER_WARN_ON_TINY_SAMPLES     0   // Enable to track down functions that are too quick to be worth profiling
#define LRYA_CPU_PROFILER_WARN_ON_SKIPPED_SAMPLES  0   // Enable to track down functions that aren't being profiled
...
public:
                 ...
                 CpuProfileSampleShared& sharedData)
  {
    ...
    if (duration_ms > sMinSampleDuration_ms)
    {
      ...
      {
      #if LRYA_CPU_PROFILER_WARN_ON_SKIPPED_SAMPLES
        WarnOfSkippedProfileSample(sharedData, duration_ms);
      #endif // LRYA_CPU_PROFILER_WARN_ON_SKIPPED_SAMPLES
      }
    }
    else
    {
    #if LRYA_CPU_PROFILER_WARN_ON_TINY_SAMPLES
      WarnOfTinyProfileSample(sharedData, duration_ms);
    #endif // LRYA_CPU_PROFILER_WARN_ON_TINY_SAMPLES
    }
