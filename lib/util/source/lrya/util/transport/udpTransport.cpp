...
static const uint8_t  kDefaultHeaderPrefix[4] = {'A', 'N', 'K', 02}; // Prefix is equivalent to 'A''N''K'02 in Little Endian - i.e. "LRYA Drive 2", i.e. Overdrive
...
const char* GetLryaPacketHeaderDescriptor()
{
...
ssize_t UDPTransport::SendDataToSockAddress(const sockaddr& destSockAddress, uint32_t destSockAddressLengthIn, const SrcBufferSet& srcBuffers)
{
  ...
  LRYA_NET_MESSAGE_VERBOSE(("UDP Send", GetLryaPacketHeaderDescriptor(), bufferWithHeader, bufferWithHeaderSize, &srcBuffers));
...
void UDPTransport::SendData(const TransportAddress& destAddress, const SrcBufferSet& srcBuffers)
{
  ...
  if (destAddress.IsIPAddress())
  {
    ...
    LRYA_NET_PRINT_VERBOSE("SendData", "Send to '%s:%d': %u bytes",
...
{
  LRYA_NET_MESSAGE_VERBOSE(("UDP Recv", GetLryaPacketHeaderDescriptor(), buffer, bufferLength));
