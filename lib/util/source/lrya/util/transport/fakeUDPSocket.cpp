...
void FakeUDPPacketData::Set(const void* inBuffer, size_t inSize,
                            ...
                            const sockaddr* destSockAddress, socklen_t destSockAddressLength)
{
  _message.Set(inBuffer, Lrya::Util::numeric_cast<uint32_t>(inSize));
  _socketAddressSrc.Set(srcSockAddress, Lrya::Util::numeric_cast<uint32_t>(srcSockAddressLength));
  _socketAddressDest.Set(destSockAddress, Lrya::Util::numeric_cast<uint32_t>(destSockAddressLength));
}
  ...
  if (_isOpen)
  {
    if (_inMessageQueue.size() > 0)
    {
      ...
      // Copy the SocketAddress out
      {
        ...
        messageHeader->msg_namelen = Lrya::Util::numeric_cast<socklen_t>(bytesToWrite);
      }
...
ssize_t FakeUDPSocket::SendTo(int socketId, const void* messageData, size_t messageDataSize, int flags, const sockaddr* destSockAddress, socklen_t destSockAddressLength)
{
  ...
  if (fakeSocket != nullptr)
  {
    ...
    const socklen_t srcSockAddressLength = Lrya::Util::numeric_cast<socklen_t>(sizeof( srcSockAddress ));
