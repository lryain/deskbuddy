...
static const uint8_t k_LryaReliablePacketHeaderPrefix[3] = {'R', 'E', 01}; // i.e. "Reliable Transport Layer 1"
  
struct LryaReliablePacketHeader
{
  explicit LryaReliablePacketHeader(uint8_t messageType, ReliableSequenceId seqIdMin, ReliableSequenceId seqIdMax, ReliableSequenceId lastReceivedId)
  {
  ...
  {
    for (uint32_t i = 0; i < sizeof(k_LryaReliablePacketHeaderPrefix); ++i)
    {
      _prefix[i] = k_LryaReliablePacketHeaderPrefix[i];
    }
    _type              = messageType;
    _seqIdMin          = seqIdMin;
    _seqIdMax          = seqIdMax;
  ...
  bool HasCorrectPrefix() const
  {
    for (uint32_t i = 0; i < sizeof(k_LryaReliablePacketHeaderPrefix); ++i)
    {
      if (_prefix[i] != k_LryaReliablePacketHeaderPrefix[i])
      {
        return false;
      }
    }
  ...
  uint8_t             _prefix[sizeof(k_LryaReliablePacketHeaderPrefix)]; // 3:  0+3 = 3
...
static_assert(sizeof(LryaReliablePacketHeader) == 10, "Expected size mismatch, check layout and padding" );
...
{
  return _unreliable->MaxTotalBytesPerMessage() - sizeof(LryaReliablePacketHeader);
}
...
{
  uint8_t  headerBuffer[ sizeof(LryaReliablePacketHeader) ];
    ...
    case eRMT_MultiPartMessage:
      {
        ...
        if (pendingMultiPart.AddMessagePart(innerMessage, innerMessageSize))
        {
          ...
          LRYA_NET_MESSAGE_VERBOSE(("Multi Complete", "Message...", pendingMultiPart.GetBytes(), pendingMultiPart.GetSize()));
...
void ReliableTransport::ReceiveData(const uint8_t* buffer, unsigned int size, const TransportAddress& sourceAddress)
{
  ...
  if (size >= sizeof(LryaReliablePacketHeader))
  {
    const LryaReliablePacketHeader* reliablePacketHeader = reinterpret_cast<const LryaReliablePacketHeader*>(buffer);
    
    if (reliablePacketHeader->HasCorrectPrefix())
    {
      LRYA_NET_MESSAGE_VERBOSE(("Rel Recv", "|" RELIABLE_HEADER_DESCRIPTOR_TEXT "Message...", buffer, size));
      ...
      uint32_t innerMessageSize = size - sizeof(LryaReliablePacketHeader);
      const uint8_t* innerMessage = (innerMessageSize > 0) ? &buffer[sizeof(LryaReliablePacketHeader)] : nullptr;
      ...
      if (isReliable)
      {
        ...
        if (!anyMessagesToReceive)
        {
          ...
          else
          {
            ...
            LRYA_NET_PRINT_VERBOSE("ReliableTransport.Recv.OutOfOrder", "ReceiveData - Ignoring out of order messages %u..%u (waiting for %u)", minSeqId, maxSeqId, connectionInfo->GetNextInSequenceId() );
    ...
    else
    {
      PRINT_NAMED_WARNING("ReliableTransport.BadPrefix", "Header '%s' has wrong prefix!",
                          ConvertMessageBufferToString(buffer, sizeof(LryaReliablePacketHeader), Util::eBTTT_Ascii).c_str());
  ...
  if (!handledMessageType)
  {
    ...
    LRYA_NET_MESSAGE_VERBOSE(("Rel Unhandled", "Message...", buffer, size));
...
{
  ASSERT_AND_RETURN_VALUE_IF_FAIL(outBufferCapacity >= sizeof(LryaReliablePacketHeader), 0);

  LryaReliablePacketHeader* packetHeader = reinterpret_cast<LryaReliablePacketHeader*>(outBuffer);
  packetHeader->Set(type, seqIdMin, seqIdMax, lastReceivedId);
  
  return sizeof(LryaReliablePacketHeader);
}
...
void ReliableTransport::Update()
{
    ...
    if (_lastUpdateTime != kNetTimeStampZero)
    {
      ...
      if (timeSinceLastUpdate > 15.0 && !_reliableConnectionMap.empty())
      {
        ...
        if (timeSinceLastReport > kMinTimeBetweenSlowSleepReports_ms) {
          Lrya::Util::sChanneledInfoF("Network", "reliable_transport.update.sleep.slow",
...
#endif // #if LRYA_NET_MESSAGE_LOGGING_ENABLED
...
#if ENABLE_RUN_TIME_PROFILING
  {
    ...
    if (_lastUpdateTime != kNetTimeStampZero)
    {
      ...
      {
        Lrya::Util::sEventF("reliable_transport.update.run.slow", {{DDATA,"2"}}, "%.2f", timeSinceLastUpdate);
      }
    }
  }
...
#endif // LRYA_NET_MESSAGE_LOGGING_ENABLED
}
