...
const char* MessageErrorToString(EMessageError messageError)
{
  ...
  {
  #define LRYA_MESSAGE_ERROR(expr)  # expr,
    static const char* k_Names[eME_Count] = { LRYA_MESSAGE_ERRORS };
...
void MessageStats::Print(const char* inName) const
{
  if (_numMessages > 0)
  {
    ...
    PRINT_CH_INFO("Network", "MessageStats", "[%s][%s] %u messages, %llu bytes, avg = %2.2f bytes (%u..%u)", inName, _name, _numMessages, static_cast<unsigned long long>(_numBytes), averageBytes, _smallestMessage, _largestMessage);
    for (int i=0; i < eME_Count; ++i)
    {
      if (_errorCounts[i] > 0)
      {
