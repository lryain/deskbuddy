...
{
  LRYA_NET_PRINT_VERBOSE("ReliableConnection.Constructor", "%p '%s' Constructed _nextOutSequenceId = %u)", this, _transportAddress.ToString().c_str(), _nextOutSequenceId);
}


ReliableConnection::~ReliableConnection()
{
  LRYA_NET_PRINT_VERBOSE("ReliableConnection.Destructor", "%p '%s' Destructed _nextOutSequenceId = %u, %zu (%d..%d)", this, _transportAddress.ToString().c_str(), _nextOutSequenceId, _pendingMessageList.size(), (int)GetFirstUnackedOutId(), (int)GetLastUnackedOutId());
...
ReliableSequenceId ReliableConnection::GetNextOutSequenceNumber()
{
  ...
  LRYA_NET_PRINT_VERBOSE("ReliableConnection.GetNextOutSequenceNumber", "%p '%s' GetNextOutSequenceNumber ret = %u", this, _transportAddress.ToString().c_str(), nextSequenceId);
...
bool ReliableConnection::UpdateLastAckedMessage(ReliableSequenceId seqId)
{
  ...
  {
    LRYA_NET_PRINT_VERBOSE("ReliableConnection.UpdateLastAckedMessage", "%p '%s' UpdateLastAckedMessage(%u) (%u..%u were unacked)", this, _transportAddress.ToString().c_str(), seqId, GetFirstUnackedOutId(), GetLastUnackedOutId());
...
size_t ReliableConnection::SendUnAckedMessages(ReliableTransport* reliableTransport, size_t firstToSend)
{
  ...
  LRYA_NET_PRINT_VERBOSE("ReliableConnection.SendUnAckedMessages", "%p '%s' SendUnAckedMessages - sending %zu messages (%zu..%zu of %zu), %u bytes, id %u..%u",
  ...
  else
  {
    ...
    for (size_t i=firstToSend; i < (firstToSend + numMessagesToSend); ++i)
    {
      ...
      // Type
      const MultiSubMessageType messageType = Lrya::Util::numeric_cast<MultiSubMessageType>( (int)pendingMessage->GetType() );
      memcpy(&buffer[bufferSize], &messageType, sizeof(messageType));
      bufferSize += sizeof(messageType);
      // Size
      const MultiSubMessageSize messageSize = Lrya::Util::numeric_cast<MultiSubMessageSize>( pendingMessage->GetMessageSize() );
...
uint32_t ReliableConnection::SendUnAckedPackets(ReliableTransport* reliableTransport, uint32_t maxPacketsToSend, size_t firstToSend)
{
  ...
  do
  {
    ...
    {
      LRYA_NET_PRINT_VERBOSE("ReliableConnection.SendUnAckedPackets", "%p '%s' SendUnackedPackets packet %u: %zu messages sent from msg %zu",
...
uint32_t ReliableConnection::SendOptimalUnAckedPackets(ReliableTransport* reliableTransport, uint32_t maxPacketsToSend)
{
  ...
  if (IsPacketWorthSending(reliableTransport, currentTime, oldestMessageIdx) && (currentTime > (lastSentTimeForOldestMessage + sTimeBetweenResendsInMS)))
  {
    ...
    LRYA_NET_PRINT_VERBOSE("ReliableConnection.SendOptimalUnAckedPackets",
