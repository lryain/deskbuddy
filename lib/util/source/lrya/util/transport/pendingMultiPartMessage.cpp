...
bool PendingMultiPartMessage::AddMessagePart(const uint8_t* message, uint32_t messageSize)
{
  ...
  {
    LRYA_NET_MESSAGE_VERBOSE(("HandleMultiPartMessage Recv", "|Mu|Mu|Message...", message, messageSize));
    ...
    if (messageIndex == _nextExpectedPart)
    {
      ...
      {
        // start new
        LRYA_NET_PRINT_VERBOSE("MultiPartMessage", "new message %d of %d", messageIndex, messageCount);
      ...
      LRYA_NET_PRINT_VERBOSE("MultiPartMessage", "Message part %d of %d - adding %u bytes to %u existing", messageIndex, messageCount, messageSize, (uint32_t)_bytes.size());
...
{
  return Lrya::Util::numeric_cast<uint32_t>(_bytes.size());
}
