...
public:
  ...
  bool Load(const Lrya::Util::Data::DataPlatform* data,
...
private:
  std::unordered_map<CladEnum, std::string, Lrya::Util::EnumHasher> _cladMap;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
template<class CladEnum>
bool CladEnumToStringMap<CladEnum>::Load(const Lrya::Util::Data::DataPlatform* data,
                                         const std::string& path,
                                         const std::string& fileNameKey,
                                         bool isReverseLookupAllowed)
{
  ...
  const bool fileExists = Lrya::Util::FileUtils::FileExists(fullPath);
  LRYA_VERIFY(fileExists, "CladEnumToStringMap.Load.FileDoesNotExistAtExpectedPath",
  ...
  if(fileExists)
  {
    ...
    if( success)
    {
      ...
      for(int i = 0; i < numPairs; ++i)
      {
        ...
        if(LRYA_VERIFY(memberNames.size() == 2,
          ...
          LRYA_VERIFY(success, "CladEnumToStringMap.Load.EnumFromStringFailure",
