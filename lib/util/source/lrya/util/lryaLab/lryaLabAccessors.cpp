 ...
 * Created: 1/25/17
 *
 * Collection of functions to access derived data from LryaLabDef (POD objects) in
...
const Experiment* FindExperiment(const LryaLabDef* lab,
                                 const std::string& experimentKey)
{
  ASSERT_NAMED(nullptr != lab, "LryaLab must not be NULL");
                                                  ...
                                                  const uint8_t userHashBucket)
{
  ...
  LOG_DEBUG("LryaLab.GetExperimentVariation.bucket",
            "%s : %u",
  ...
  for (const ExperimentVariation& v : variations) {
    ...
    if (maxBucketSize > 0) {
      ...
      if (bucketSize > 0) {
        ...
        LOG_DEBUG("LryaLab.GetExperimentVariation.bucket.range",
                  "|[%2u - %2u] - %2u| : %s",
  ...
  if (!inExperiment) {
    ...
    LOG_DEBUG("LryaLab.GetExperimentVariation.bucket.assign",
              "%s : %u : %s",
                                      ...
                                      const std::string& userId)
{
  ...
  uint8_t hash = Lrya::Util::numeric_cast_clamped<uint8_t>(percentBucket);
  return hash;
}
...
AudienceTagList GetKnownAudienceTags(const LryaLabDef& labDef)
{
