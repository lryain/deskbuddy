...
class Queue {
public:
  Lrya::Util::TaskExecutor taskExecutor;
  ...
  virtual Lrya::Util::TaskHandle ScheduleCallback(std::chrono::milliseconds repeatPeriodMSec, const Block &block, const char* name)
  {
...
//    taskExecutor.WakeAfter(block, when);
  ...
  class InvalidTaskHandle : public Lrya::Util::TaskHandleContainer::ITaskHandle {
    void Invalidate() override {}
    virtual ~InvalidTaskHandle() {}
  };

  Lrya::Util::TaskHandle ScheduleCallback(std::chrono::milliseconds repeatPeriodMSec, const Block &block, const char* name) override
  {
    block(); // I guess the concept of repetition is kinda weird for the immediate queue...
    return Lrya::Util::TaskHandle(new Lrya::Util::TaskHandleContainer(*(new InvalidTaskHandle())));
  }
...
Lrya::Util::TaskHandle ScheduleCallback(Queue* queue, std::chrono::milliseconds repeatPeriodMSec, const Block& block, const char* name)
{
