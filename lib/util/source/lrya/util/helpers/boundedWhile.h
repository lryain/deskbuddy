...
#define BOUNDED_WHILE(n, exp) unsigned int MAKE_NAME=0; while(MAKE_NAME++ < (n) ? (exp) : Lrya::Util::util_PrintErrorAndCrashInDev(__FILE__, __LINE__))

#define SOFT_BOUNDED_WHILE(str, n, exp) unsigned int MAKE_NAME=0; while(MAKE_NAME++ < (n) ? (exp) : Lrya::Util::util_PrintWarning(str))
...
inline bool util_PrintErrorAndCrashInDev(const char* file, int line) {
  ...
  if( LRYA_DEVELOPER_CODE ) { // dont crash prod
