...
std::unordered_map<uint64_t, Lrya::Util::HttpAdapter*> requestInstances;
}
...
, _adapterState(IHttpAdapter::AdapterState::DOWN)
{
  ...
  if (nullptr != env) {
    JClassHandle httpClass{env->FindClass("com/lrya/util/http/HttpAdapter"), env};
    if (httpClass == nullptr) {
      PRINT_NAMED_ERROR("HttpAdapter_Android.Init.ClassNotFound", 
        "Unable to find com.lrya.util.http.HttpAdapter");
      return;
    }

    {
      // get singleton instance of adapter
      jmethodID instanceMethodID = env->GetStaticMethodID(httpClass.get(), "getInstance", "()Lcom/lrya/util/http/HttpAdapter;");
...
void JNICALL
Java_com_lrya_util_http_HttpAdapter_NativeHttpRequestCallback(JNIEnv* env, jobject thiz,
                                                                   ...
                                                                   jbyteArray responseBody)
{
  ...
  if( bytes != NULL ) {
    ...
    for (jsize i = 0; i < stringCount; i += 2) {
      Lrya::Util::JStringHandle jKey{(jstring) env->GetObjectArrayElement(responseHeaders, i), env};
      Lrya::Util::JStringHandle jValue{(jstring) env->GetObjectArrayElement(responseHeaders, i + 1), env};
...
void JNICALL
Java_com_lrya_util_http_HttpAdapter_NativeHttpRequestDownloadProgressCallback(JNIEnv* env,
                                                                  jobject thiz,
                                                                  jlong hash,
                                                                  jlong bytesWritten,
