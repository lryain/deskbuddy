...
size_t QuestStore::Pack(CLAD::SafeMessageBuffer& buffer) const
{
  ...
  for (const Lrya::Util::QuestEngine::EventPair& m : eventStats) {
    m.Pack(buffer);
  }
  buffer.Write(static_cast<uint16_t>(notices.size()));
  for (const Lrya::Util::QuestEngine::NoticePair& m : notices) {
    m.Pack(buffer);
  }
  buffer.Write(static_cast<uint16_t>(ruleTriggers.size()));
  for (const Lrya::Util::QuestEngine::TriggerLog& m : ruleTriggers) {
    m.Pack(buffer);
  }
  const size_t bytesWritten {buffer.GetBytesWritten()};
  return bytesWritten;
...
{
  buffer.ReadCompoundTypeVArray<Lrya::Util::QuestEngine::EventPair, uint16_t>(this->eventStats);
  buffer.ReadCompoundTypeVArray<Lrya::Util::QuestEngine::NoticePair, uint16_t>(this->notices);
  buffer.ReadCompoundTypeVArray<Lrya::Util::QuestEngine::TriggerLog, uint16_t>(this->ruleTriggers);
  return buffer.GetBytesRead();
}
...
size_t QuestStore::Size() const
{
  ...
  // eventStats
  result += 2; // uint_16 (array length)
  for (const Lrya::Util::QuestEngine::EventPair& m : this->eventStats) {
    result += m.Size();
  }
  // notices
  result += 2; // uint_16 (array length)
  for (const Lrya::Util::QuestEngine::NoticePair& m : this->notices) {
    result += m.Size();
  }
  // ruleTriggers
  result += 2; // uint_16 (array length)
  for (const Lrya::Util::QuestEngine::TriggerLog& m : this->ruleTriggers) {
    result += m.Size();
  }
  return result;
}
