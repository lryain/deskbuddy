...
extern "C" {
  ...
  // C++ Compiler
  #define STRINGID( name ) static const Lrya::Util::StringID STRID_##name( #name )
...
{
  template<>
  struct hash<Lrya::Util::StringID>
  {
    typedef Lrya::Util::StringID argument_type;
    typedef std::size_t result_type;

    result_type operator()( argument_type const& id ) const
    {
      return std::hash<Lrya::Util::StringTable::STRID>()( id.GetID() );
    }
  };
} // namespace std
