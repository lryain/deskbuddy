...
void ExtensionId::Apply()
{
  ...
  for(auto& extIt : _extensions)
  {
    ...
    OrderedDeepOverride(*extIt, *_ptr, Lrya::Util::kP_ID );

    // remove the extension we just resolved
    extIt->erase( Lrya::Util::kP_EXTENDS );
  }
}
...
void FindExtensions(ptree& tree, ExtensionMap& idMap, std::vector<ExtensionId*>& openIds )
{
  ...
  boost::optional<string> idMatch = tree.get_optional<string>(Lrya::Util::kP_ID);
  if ( idMatch )
  {
...
//    PrintJson(tree);
  ...
  boost::optional<string> extendMatch = tree.get_optional<string>(Lrya::Util::kP_EXTENDS);
  if ( extendMatch )
  {
...
namespace
{
  ...
  {
    const size_t extendsLen = strlen( Lrya::Util::kP_EXTENDS );
    ...
    for( const auto& entry : ptree )
    {
      ...
      size_t extendsIdx = key.find(Lrya::Util::kP_EXTENDS);
...
{
  if(tree.get<bool>(Lrya::Util::kP_PREPROCESSED, false)) {
    tree.erase(Lrya::Util::kP_PREPROCESSED);
  }
}
...
void FastPreprocess(ptree& tree)
{
  ...
  tree.put(Lrya::Util::kP_PREPROCESSED, true);
...
//    printTree.put_child(key, *child);
    ...
    Lrya::Util::PtreeTools::PrintJson(*child);
  }
  else {
...
void Preprocess_Deprecated(ptree& tree)
{
  ...
  BOUNDED_WHILE(50000, !firstPass.done()) {
    boost::optional<string> id = firstPass.topTree().get_optional<string>(Lrya::Util::kP_ID);
    if(id) {
      ptree t = firstPass.topTree();
      t.erase(Lrya::Util::kP_ID);
      ...
      idMap[*id].first.erase(Lrya::Util::kP_ID);
    }

    boost::optional<string> extends = firstPass.topTree().get_optional<string>(Lrya::Util::kP_EXTENDS);
    if(extends) {
      numRefs++;
  ...
  BOUNDED_WHILE(100, numIDRefs > 0) {
    ...
    //   Lrya::Util::PtreeTools::PrintJson((it->second.first);
    // }
    ...
    for(internal::IDMap::iterator it = idMap.begin(); it != end; ++it) {
      ...
      BOUNDED_WHILE(1000, !trav.done()) {
        boost::optional<string> extends = trav.topTree().get_optional<string>(Lrya::Util::kP_EXTENDS);
        if(extends) {
          numIDRefs++;
          ...
          if(targetIt != end) {
            ...
            if(target.second) {
              ...
              child.erase(Lrya::Util::kP_EXTENDS);
  ...
  #if DEBUG_PTREE
  {
    ...
    Lrya::Util::PtreeTools::PrintJson(tree);
  }
  #endif
