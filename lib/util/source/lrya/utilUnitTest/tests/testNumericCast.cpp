...
{
  EXPECT_EQ(expectedIsValid, Lrya::Util::IsValidNumericCast<ToType>(fromValue));
  
  TestEqual(expectedResult, Lrya::Util::numeric_cast_clamped<ToType>(fromValue));
  
  if (expectedIsValid)
  {
    TestEqual(expectedResult, Lrya::Util::numeric_cast<ToType>(fromValue));
  }
}
...
void PrintValue(float value, bool isForCodeGen)
{
  ...
  else if (Lrya::Util::Abs(value) > 10000000000000000000.0f)
  {
    printf("%e", value);
  }
  else
...
void PrintValue(double value, bool isForCodeGen)
{
  ...
  else if (Lrya::Util::Abs(value) > 10000000000000000000.0)
  {
    printf("%e", value);
  }
  else
...
void CodeGenTestNumericCastTo(const FromType* fromValues, size_t numValues)
{
  ...
  printf("    };\n");
  printf("\n");
  printf("    static_assert(Lrya::Util::array_size(fromValues) == Lrya::Util::array_size(expectedValues_%s), \"Array Sizes Differ!\");\n", toTypeName);
  printf("    static_assert(Lrya::Util::array_size(fromValues) == Lrya::Util::array_size(expectedValid_%s),  \"Array Sizes Differ!\");\n", toTypeName);
  printf("\n");
  printf("    TestNumericCastValues(fromValues, expectedValues_%s, expectedValid_%s, Lrya::Util::array_size(fromValues));\n", toTypeName, toTypeName);
  printf("  }\n");
}
...
void CodeGenTestNumericCastAll()
{
  ...
  // int16 ->
  {
    ...
    CodeGenTestNumericCastFrom(values, Lrya::Util::array_size(values));
  }
