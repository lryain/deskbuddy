...
TEST(PtreeTools, shallowMerge)
{
  ...
  Lrya::Util::PtreeTools::ShallowMerge(first, second);
...
TEST(PtreeTools, shallowMergeWithCopy)
{
  ...
  ptree result = Lrya::Util::PtreeTools::ShallowMergeWithCopy(first, second);
...
TEST(PtreeTools, deepMergeLimited)
{
  ...
  string key("5");
  ptree result = Lrya::Util::PtreeTools::DeepMergeLimited(first, second, key);
  ASSERT_EQ(result.get<int>("1"), 1);
  ASSERT_EQ(result.get<int>("2"), 10);
  ASSERT_EQ(result.get<int>("6"), 6);
  ASSERT_EQ(result.get<int>("7"), 7);
...
TEST(PtreeTools, deepMerge)
{
  ...
  Lrya::Util::PtreeTools::FastDeepOverride(first, second);
...
TEST(PtreeTools, deepMergeLimitedList)
{
  ...
  ptree result = Lrya::Util::PtreeTools::DeepMergeLimited(first, second, "simple_list");
...
TEST(PtreeTools, deepMergeList)
{
  ...
  Lrya::Util::PtreeTools::FastDeepOverride(result, second);
...
TEST(PtreeTools, ListKeys)
{
  ...
  Lrya::Util::PtreeTools::PtreeKey key;

  key.push_back(pair<string, int>("lst1", 1));
  key.push_back(pair<string, int>("list1_1", 2));

  ptree t = Lrya::Util::PtreeTools::Deprecated::GetChild_Deprecated(input, key);
  EXPECT_EQ(3.0, t.get<double>(""));

  Lrya::Util::PtreeTools::PtreeKey key2;
  key2.push_back(pair<string, int>("a", -1));
  ptree t2 = Lrya::Util::PtreeTools::Deprecated::GetChild_Deprecated(input, key2);
  EXPECT_EQ("asdf", t2.get<string>(""));

  Lrya::Util::PtreeTools::PtreeKey key3;
  key3.push_back(pair<string, int>("grp", -1));
  key3.push_back(pair<string, int>("lst", 0));
  ptree t3 = Lrya::Util::PtreeTools::Deprecated::GetChild_Deprecated(input, key3);
...
TEST(PtreeTools, ListKeysRef)
{
  ...
  Lrya::Util::PtreeTools::Deprecated::GetChild_Deprecated(input, key) = insert;

  Lrya::Util::PtreeTools::PtreeKey key2;
  key2.push_back(pair<string, int>("lst1", 0));

  Lrya::Util::PtreeTools::Deprecated::GetChild_Deprecated(input, key2).put("a", 3.5);
...
{
  using namespace Lrya::Util::PtreeTools;
...
TEST(PtreeTools, Traversal)
{
  ...
  Lrya::Util::PtreeTools::PtreeTraverser trav(input);
  ...
  while(!trav.done()) {
    ...
    // Lrya::Util::PtreeTools::PrintJson((trav.topTree());
 
    ASSERT_LT(numSteps, 30);

    EXPECT_EQ(trav.topTree(), Lrya::Util::PtreeTools::Deprecated::GetChild_Deprecated(input,trav.topKey())) << "tree mismatch!";
...
TEST(PtreeTools, PreprocessSimpleTree)
{
  ...
  // Lrya::Util::PtreeTools::PrintJson((input);

  Lrya::Util::PtreeTools::FastPreprocess(input);
  input.erase("preprocessed");

  // cout<<"result:\n";
  // Lrya::Util::PtreeTools::PrintJson((processed);
...
TEST(PtreeTools, PreprocessComplexTree)
{
  ...
  // cout<<"\n\nexpected result:\n";
  // Lrya::Util::PtreeTools::PrintJson((output);
