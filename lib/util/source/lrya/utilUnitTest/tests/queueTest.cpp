...
using Lrya::Util::Dispatch::Queue;
...
TEST(QueueTest, DISABLED_ScheduledCallbacks)
{
  ...
  Queue* queue = Lrya::Util::Dispatch::Create();
  volatile int a = 0;
  Lrya::Util::TaskHandle handle = Lrya::Util::Dispatch::ScheduleCallback(queue, std::chrono::milliseconds(500), [&a] { a++; } );
  ...
  ASSERT_EQ(a, 2);
  Lrya::Util::Dispatch::Sync(queue, [&a] { a++; } );
  std::this_thread::sleep_for(std::chrono::milliseconds(500)); // total 1750
  Lrya::Util::Dispatch::Stop(queue);
  Lrya::Util::Dispatch::Release(queue);
  ASSERT_EQ(a, 4);
}
...
TEST(QueueTest, DISABLED_RemoveCallback)
{
  ...
  Queue* queue = Lrya::Util::Dispatch::Create();
  volatile int a = 0;
  Lrya::Util::TaskHandle handle = Lrya::Util::Dispatch::ScheduleCallback(queue, std::chrono::milliseconds(250), [&a] { a++; } );
...
TEST(QueueTest, TaskHandleHeartbeat)
{
  ...
  Queue* queue = Lrya::Util::Dispatch::Create();
  Lrya::Util::TaskHandle handle = Lrya::Util::Dispatch::ScheduleCallback(queue, std::chrono::milliseconds(1000), [] {} );
  Lrya::Util::Dispatch::Stop(queue);
  Lrya::Util::Dispatch::Release(queue);
  (*handle).Invalidate();
}
...
TEST(QueueTest, DISABLED_ReverseTaskHeartbeat)
{
  ...
  public:
    ...
    ~TestClass() {
      Lrya::Util::SafeDelete(_pointer);
      _handle->Invalidate();
    }

    void Update() { std::cout << "QueueTest.ReverseTaskHeartbeat: " << *_pointer; _counter++; }
    void SetTaskHandle(Lrya::Util::TaskHandle handle) {
  ...
  Queue* queue = Lrya::Util::Dispatch::Create();
  obj->SetTaskHandle(Lrya::Util::Dispatch::ScheduleCallback(queue, std::chrono::milliseconds(100), [obj] {
    obj->Update();
  }));
  ...
  Lrya::Util::Dispatch::Sync(queue, [&obj] {
    std::this_thread::sleep_for(std::chrono::milliseconds(300));
    // now delete the object
    Lrya::Util::SafeDelete(obj);
  });
...
TEST(QueueTest, TestQueueHandle)
{
  ...
  volatile int a = 0;
  {
    ...
    Lrya::Util::TaskHandle handle = Lrya::Util::Dispatch::ScheduleCallback(
