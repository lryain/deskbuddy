...
public:
  ...
  void RampZeroToOne(Lrya::Util::LowPassFilterSimple& filter, int nSamples) {
...
TEST_F(TestLowPassFilterSimple, TestConstantInput)
{
  ...
  Lrya::Util::LowPassFilterSimple filter(kSamplePeriod_sec, kTimeConstant_sec);
...
TEST_F(TestLowPassFilterSimple, TestFilterParamChanges)
{
  ...
  // that we can.
  Lrya::Util::LowPassFilterSimple filter(kInitialSamplePeriod_sec, kInitialTimeConstant_sec);
