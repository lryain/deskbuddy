...
#define ASSERT_FILE_EXISTS(fileName) \
  ASSERT_PRED1(Lrya::Util::FileUtils::FileExists, fileName)

#define ASSERT_FILE_DOES_NOT_EXIST(fileName) \
  ASSERT_PRED1(Lrya::Util::FileUtils::FileDoesNotExist, fileName)

#define ASSERT_DIR_EXISTS(dirName) \
  ASSERT_PRED1(Lrya::Util::FileUtils::DirectoryExists, dirName)

#define ASSERT_DIR_DOES_NOT_EXIST(dirName) \
  ASSERT_PRED1(Lrya::Util::FileUtils::DirectoryDoesNotExist, dirName)
...
public:
  ...
  {
    std::string workRoot = "./";
    char* workRootChars = getenv("LRYAWORKROOT");
