...
TEST_F(FileUtilsTest, WriteDeleteFile)
{
  ...
  Lrya::Util::FileUtils::DeleteFile(fileName);
  FileShouldNotExist(fileName);
  
  Lrya::Util::FileUtils::WriteFile(fileName, "file contents");
...
TEST_F(FileUtilsTest, CreateRemoveDirectory)
{
  ...
  Lrya::Util::FileUtils::RemoveDirectory(path);
  DirectoryShouldNotExist(path);
  
  Lrya::Util::FileUtils::CreateDirectory(path);
  ...
  Lrya::Util::FileUtils::CreateDirectory(path+"/A/B/cc", false, true);
  ...
  Lrya::Util::FileUtils::CreateDirectory(path+"/A/B/cc/myfile.txt", true, true);
  DirectoryShouldExist(path+"/A/B/cc");
  ...
  Lrya::Util::FileUtils::CreateDirectory(path+"A/myfile.txt", true, false);
  DirectoryShouldExist(path+"A");

  Lrya::Util::FileUtils::RemoveDirectory(path+"A");
...
TEST_F(FileUtilsTest, ListDirectory)
{
  ...
  Lrya::Util::FileUtils::WriteFile(path + "/" + fileName, "file contents");
  FileShouldExist(path + "/" + fileName);
  
  std::vector<std::string> files = Lrya::Util::FileUtils::FilesInDirectory(path);
  ASSERT_EQ(1, files.size());
  ASSERT_EQ(fileName, files[0]);
  
  // Check that useFullPath works
  files = Lrya::Util::FileUtils::FilesInDirectory(path, true);
  ASSERT_EQ(1, files.size());
  ...
  Lrya::Util::FileUtils::DeleteFile(path + "/" + fileName);
...
TEST_F(FileUtilsTest, ListDirectoryWithExtensionFilters)
{
  ...
  foundFiles = Lrya::Util::FileUtils::FilesInDirectory(path);
  ASSERT_TRUE(FilesMatch(fileNames, foundFiles));
  
  foundFiles = Lrya::Util::FileUtils::FilesInDirectory(path, false, nullptr);
  ASSERT_TRUE(FilesMatch(fileNames, foundFiles));
  
  foundFiles = Lrya::Util::FileUtils::FilesInDirectory(path, false, "");
  ASSERT_TRUE(FilesMatch(fileNames, foundFiles));
  
  foundFiles = Lrya::Util::FileUtils::FilesInDirectory(path, false, "jpg");
  std::set<std::string> correctAnswer = {"jpgLowerExtension.jpg"};
  ASSERT_TRUE(FilesMatch(correctAnswer, foundFiles));

  foundFiles = Lrya::Util::FileUtils::FilesInDirectory(path, false, {"jpg", "JPG"});
  correctAnswer = {"jpgLowerExtension.jpg", "jpgUpperExtension.JPG"};
  ASSERT_TRUE(FilesMatch(correctAnswer, foundFiles));

  foundFiles = Lrya::Util::FileUtils::FilesInDirectory(path, false, {"jpg", "JPG", "jpeg"});
  correctAnswer = {"jpegExtension.jpeg", "jpgLowerExtension.jpg", "jpgUpperExtension.JPG"};
  ASSERT_TRUE(FilesMatch(correctAnswer, foundFiles));
  
  foundFiles = Lrya::Util::FileUtils::FilesInDirectory(path, false, {"jpg", "png", "jpeg", "JPG"});
  correctAnswer = {"jpegExtension.jpeg", "jpgLowerExtension.jpg", "jpgUpperExtension.JPG", "pngExtension.png"};
  ASSERT_TRUE(FilesMatch(correctAnswer, foundFiles));

  foundFiles = Lrya::Util::FileUtils::FilesInDirectory(path, false, {"jpg", "jpeg"});
...
TEST_F(FileUtilsTest, FileExists)
{
  ...
  Lrya::Util::FileUtils::DeleteFile(fileName);
  FileShouldNotExist(fileName);
  ASSERT_FALSE(Lrya::Util::FileUtils::FileExists(fileName)) << fileName << " should not exist";

  Lrya::Util::FileUtils::WriteFile(fileName, "file contents");
  AssertFileExistsWithContents(fileName, "file contents");
  ASSERT_TRUE(Lrya::Util::FileUtils::FileExists(fileName)) << fileName << " should exist";

  Lrya::Util::FileUtils::DeleteFile(fileName);
  FileShouldNotExist(fileName);

  Lrya::Util::FileUtils::CreateDirectory(fileName);
  DirectoryShouldExist(fileName);
  ASSERT_FALSE(Lrya::Util::FileUtils::FileExists(fileName)) << fileName << " is a directory";

  Lrya::Util::FileUtils::RemoveDirectory(fileName);
...
TEST_F(FileUtilsTest, RecurseGetFiles)
{
  ...
  Lrya::Util::FileUtils::CreateDirectory(path + subPath, false, true);
  DirectoryShouldExist(path + subPath);
  
  Lrya::Util::FileUtils::WriteFile(path + subPath + fileName, "file contents");
  
  // Find files with recursion
  std::vector<std::string> foundFiles = Lrya::Util::FileUtils::FilesInDirectory(path,true,nullptr,true);
...
# endif
  ...
  //
  std::string result = Lrya::Util::FileUtils::FullFilePath({"path", "to", "given", "file.jpg"});
  ASSERT_EQ(result, groundTruth1);
  
  const std::string path1("path"), path2("to"), path3("given"), file("file.jpg");
  result = Lrya::Util::FileUtils::FullFilePath({path1, path2, path3, file});
  ASSERT_EQ(result, groundTruth1);
  
  result = Lrya::Util::FileUtils::FullFilePath({path1, file});
  ASSERT_EQ(result, groundTruth2);
  
  result = Lrya::Util::FileUtils::FullFilePath({"file.jpg"});
  ASSERT_EQ(result, groundTruth3);
  
  result = Lrya::Util::FileUtils::FullFilePath({});
  ASSERT_TRUE(result.empty());
  
  //
  // Tests for when strings already have file separators in them
  //
  result = Lrya::Util::FileUtils::FullFilePath({"path" FILESEP, FILESEP "to", "given", FILESEP "file.jpg"});
  ASSERT_EQ(result, groundTruth1);
  
  result = Lrya::Util::FileUtils::FullFilePath({FILESEP "path" FILESEP, FILESEP "to", FILESEP "given" FILESEP, FILESEP "file.jpg"});
  ASSERT_EQ(result, groundTruth1b);
  
  //
  // Tests for when some already-concatenated subpaths are passed in
  //
  result = Lrya::Util::FileUtils::FullFilePath({"path" FILESEP "to", "given" FILESEP "file.jpg"});
  ASSERT_EQ(result, groundTruth1);
  
  result = Lrya::Util::FileUtils::FullFilePath({FILESEP "path" FILESEP "to" FILESEP, FILESEP "given" FILESEP "file.jpg"});
  ASSERT_EQ(result, groundTruth1b);
  ...
  result = Lrya::Util::FileUtils::FullFilePath({"", "path" FILESEP, "", "file.jpg"});
  ASSERT_EQ(result, groundTruth2);
...
# endif
  ...
  std::string result = Lrya::Util::FileUtils::GetFileName(path1, true, true);
  ASSERT_EQ(result, groundTruth);
  
  result = Lrya::Util::FileUtils::GetFileName(path2, true, true);
  ASSERT_EQ(result, groundTruth);
  
  result = Lrya::Util::FileUtils::GetFileName(path3, true, true);
  ASSERT_EQ(result, groundTruth);
  ...
  result = Lrya::Util::FileUtils::GetFileName(path4, true, true);
  ASSERT_EQ(result, "");
  
  // path5 does not have an extension so an empty string is returned
  result = Lrya::Util::FileUtils::GetFileName(path5, true, true);
  ASSERT_EQ(result, "");
  
  result = Lrya::Util::FileUtils::GetFileName(path5, false, true);
  ASSERT_EQ(result, groundTruth);
}
