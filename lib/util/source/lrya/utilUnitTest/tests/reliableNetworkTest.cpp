...
class TestNetTransportDataReceiver : public Lrya::Util::INetTransportDataReceiver
{
public:
  ...
  virtual void ReceiveData(const uint8_t* buffer, uint32_t size, const Lrya::Util::TransportAddress& sourceAddress) override
  {
    if (size > 0)
    {
  ...
  int GetConnectionIndex(const Lrya::Util::TransportAddress& address) const
  {
    int i = 0;
    for (const Lrya::Util::TransportAddress& existingAddress : _connections)
    {
  ...
  bool HasConnection(const Lrya::Util::TransportAddress& address) const
  {
...
private:
  ...
  std::vector<Lrya::Util::TransportAddress> _connections;
...
static void MultipleLargePacketTest()
{
  ...
  // Multiple smaller sends of k_BytesPerMessage to transmit data
  {
    ...
    {
      const uint32_t bytesLeftToSend = Lrya::Util::numeric_cast<uint32_t>(k_SizeOfHugeMessageBuffer - numBytesSent);
